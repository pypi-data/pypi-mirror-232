...
class XDisplay(Extension):
    ...
    @property
    async def autoOff(self) -> int:
        """Return the current delay before X-Display standby."""
        return await self._config["autoOff"]

    @property
    async def sensitive(self) -> int:
        """Return the current sensitivity of X-Display screen."""
        return await self._config["sensitive"]

    @property
    async def version(self) -> int:
        """Return the version of the X-Display."""
        return await 2 if self._config["bVersion2"] is True else 1

    # Screens
    async def refresh_screens(self) -> None:
        self.__init__(self._ipx, self._ext_number)
    ...
    @property
    async def screens(self) -> list[XDisplayScreen]:
        """Return X-Display screens."""
        if not self._screens:
            await self.refresh_screens()
    ...
    async def screen_status(self) -> bool:
        ...
        return await self._ipx.get_io(self.io_on_screen_id)
