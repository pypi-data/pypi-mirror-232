...
class Deal:
    ...
    def create_deal(
        ...
        Args:
            ...
            custom_deal_fields (dict, optional): Deal custom fields. Default is {}.

        Returns:
            The created deal object.

        """
        if custom_deal_fields is None:
            custom_deal_fields = {}
        url = "/deals"
        ...
        deal_fields = self.client.deal_field.get_all_deal_fields()

        for key, value in custom_deal_fields.items():
            deal_field_key = self.verify_if_key_or_name_exists(key=key, deal_fields=deal_fields)
            if deal_field_key is not None:
                payload[deal_field_key] = value
    ...
    @staticmethod
    def verify_if_key_or_name_exists(key, deal_fields):
        for deal_field in deal_fields:
            deal_field_key = deal_field.get('key')
            deal_field_name = deal_field.get('name')
            if key == deal_field_key or key == deal_field_name:
                return deal_field_key
