...
from bitcoin.core import b2x, lx, b2lx, COIN, COutPoint, CMutableTxOut, CMutableTxIn, CMutableTransaction, Hash160, CTxIn, CTxOut, CTransaction
...
class BTCScript():
    ...
    def get_fee(cls, conn_data, blocks = tannhauser['fee_blocks'], fee_mode = tannhauser['fee_estimate_mode']):
        try:
            rpc_connection = AuthServiceProxy(f"http://{conn_data['btc_rpc_user']}:{conn_data['btc_rpc_password']}@{conn_data['btc_rpc_url']}:{conn_data['btc_rpc_port']}")
    ...
    def import_address(cls, address, conn_data):
        try:
            address = str(address)
            _rpc_connection = AuthServiceProxy(f"http://{conn_data['btc_rpc_user']}:{conn_data['btc_rpc_password']}@{conn_data['btc_rpc_url']}:{conn_data['btc_rpc_port']}")
    ...
    def unlock_wallet(cls, conn_data, gap = 10):
        try:
            _passphrase = conn_data['btc_walletpassphrase']
            _rpc_connection = AuthServiceProxy(f"http://{conn_data['btc_rpc_user']}:{conn_data['btc_rpc_password']}@{conn_data['btc_rpc_url']}:{conn_data['btc_rpc_port']}")
    ...
    def get_redeem_script(self, blocks, hashed_preimage, sender_address, receiver_address, conn_data, validate = False):
        if validate:
            ...
            self.current_blockheight = BTCScript.blockcount(conn_data)
    ...
    def gen_fund_tx(self, network, sender_address, htlc_address, amount, conn_data, fee_per_byte = 1, wif = None):
        ...
        _unspent = BTCScript.unspent(self.sender_address, conn_data)
        _txouts = _unspent[1]
        ...
        # sign transaction
        _sign_raw = BTCScript.sign_transaction(_tx, conn_data)
        ...
        # decode tx
        _decode_tx_hex = BTCScript.decode_rawtransaction(_signed_tx_hex, conn_data)
        ...
        _final_sign_raw = BTCScript.sign_transaction(_final_tx, conn_data)
    ...
    def refund(self, txid, vout, amount, fee_per_byte, address, redeem_blockheight, redeem_script, privkey, conn_data):
        ...
        # decode tx
        _decode_tx_hex = BTCScript.decode_rawtransaction(self.refund_hex, conn_data)
    ...
    def claim(self, txid, vout, amount, fee_per_byte, address, redeem_blockheight, redeem_script, privkey, preimage, conn_data):
        ...
        # decode tx
        _decode_tx_hex = BTCScript.decode_rawtransaction(self.claim_hex, conn_data)
    ...
    def get_secret(self, txid, conn_data):
        try:
            self.txid = lx(txid)
            self.tx_raw = BTCScript.get_rawtransaction(txid, conn_data, True)
    ...
    def get_lock_utxos(self, address, conn_data, lock = False):
        try:
            # get utxos
            _btc_unspent = self._handle_btc.unspent(address, confirmations = 0)

            if _btc_unspent[0]:
                for _data in _btc_unspent[0]:
                    _utxo = {'txid': _data['txid'], 'vout': _data['vout'], 'balance': _data['amount'], 'confirmations': _data['confirmations']}

                    # lock utxo
                    _lock_utxo = BTCScript.lock_utxo(_utxo['txid'], _utxo['vout'], lock, conn_data)
    ...
    def unlock_utxos(self, address, conn_data, lock = True):
        try:
            # list locked utxos
            _list_lock = BTCScript.list_lock(conn_data)

            if _list_lock:
                for _data in _list_lock:
                    _txid = _data['txid']
                    _vout = _data['vout']
                    _addr_raw = BTCScript.get_transaction(_txid, conn_data, _vout)

                    if _addr_raw[0] == address:
                        _unlock_utxo = BTCScript.lock_utxo(_txid, _vout, lock, conn_data)
