...
class BTCScript():
    def __init__(self, net, localhost, path):
        ...
        if self.localhost:
            import bitcoin.rpc
            self.bitcoind = bitcoin.rpc.Proxy(path)
        else:
            pass

    def get_fee(self, blocks = tannhauser['fee_blocks'], fee_mode = tannhauser['fee_estimate_mode']):
        try:
            fee = self.bitcoind.estimatesmartfee(blocks, fee_mode)
    ...
    def getnewaddress(self, label, addr_type = 'legacy'):
        try:
            new_address = self.bitcoind.getnewaddress(label, addr_type)
            return new_address
        except Exception as ex:
            print(ex)
            return False
    ...
    def import_address(self, address):
        try:
            address = str(address)
            import_address = self.bitcoind.importaddress(address, 'htlc_address', False)
        except Exception as ex:
                print(ex)
    ...
    def get_addressinfo(self, address):
        try:
            address = str(address)
            addr_info = self.bitcoind.validateaddress(address)
        except Exception as ex:
            print(ex)
    ...
    def list_lock(self):
        try:
            _lilo = self.bitcoind.listlockunspent()
            return _lilo
        except Exception as ex:
            print(ex)
            return False
    ...
    def lock_utxo(self, txid, vout, lock):
        try:
            _lock = self.bitcoind.lockunspent(lock, [{'txid':txid, 'vout':vout}])
            return _lock
        except Exception as ex:
            print(ex)
            return False
    ...
    def unspent(self, address, confirmations = 1):
        try:
            ...
            r = self.bitcoind.listunspent(confirmations, 9999999, [_address])
    ...
    def get_privkey(self, address):
        try:
            wif = self.bitcoind.dumpprivkey(address)
        except Exception as ex:
            print(ex)
    ...
    def unlock_wallet(self, passphrase, gap = 10):
        try:
            _unlock = self.bitcoind.walletpassphrase(_passphrase, gap)
        except Exception as ex:
            print(ex)

    def blockcount(self):
        try:
            blockheight = self.bitcoind.getblockcount()
        except Exception as ex:
            print(ex)
    ...
    def send_transaction(self, tx_hex):
        try:
            broadcast = self.bitcoind.sendrawtransaction(tx_hex)
        except Exception as ex:
            print(ex)
    ...
    def sign_transaction(self, tx):
        try:
            ...
            sign = self.bitcoind.signrawtransactionwithwallet(_hex_tx)
    ...
    def decode_rawtransaction(self, tx_hex):
        try:
            decode_tx_hex = self.bitcoind.decoderawtransaction(tx_hex)
        except Exception as ex:
            print(ex)
    ...
    def get_rawtransaction(self, txid, debug = False):
        try:
            tx_raw = self.bitcoind.getrawtransaction(txid, debug)
        except Exception as ex:
            print(ex)
            tx_raw = False
    ...
    def get_lock_utxos(self, address, lock = False):
        try:
            # get utxos
            _btc_unspent = self.unspent(address, confirmations = 0)
