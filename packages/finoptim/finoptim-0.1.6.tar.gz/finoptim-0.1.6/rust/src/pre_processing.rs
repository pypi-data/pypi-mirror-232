...
impl<'a> Optimisable for CostFunction<'a> {
    ...
    fn round_res(&mut self, x: ArrayView1<f64>) -> Array1<usize> {
        ...
        let mut h = Array2::from_diag(&self.price_to_level) * self.step_size;
        h += &xp;
        ...
        for i in indices {
            xp[i] = xp[i].floor();
            let cost_floor: f64 = self.call(xp.view());
            xp[i] += 1.;
            let cost_ceiling: f64 = self.call(xp.view());
