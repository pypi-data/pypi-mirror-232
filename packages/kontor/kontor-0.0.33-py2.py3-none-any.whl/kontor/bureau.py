...
class Bureau:
    def __init__(self, working_folder_path: str):
        ...
        self.__configuration.working_directory = str(working_folder_path)
        pathlib.Path(self.__configuration.working_directory).mkdir(
            parents=True, exist_ok=True
        )

        self.__configuration.temp_directory = os.path.join(
            self.__configuration.working_directory, "temp"
        )
        pathlib.Path(self.__configuration.temp_directory).mkdir(
            parents=True, exist_ok=True
        )
        ...
        #
        log_directory = os.path.join(self.__configuration.working_directory, "logs")
    ...
    def __parse_configuration_json_file(self, configuration_filepath=None):
        ...
        if configuration_filepath is None:
            configuration_filepath = os.path.join(
                self.__configuration.working_directory, "server_configuration.json"
            )
    ...
    def __service_applicant_in_new_cubicle(self, clerk: Clerk, address):
        ...
        #
        if threading.current_thread() in self.__client_threads:
            del self.__client_threads[threading.current_thread().native_id]
    ...
    def add_user(self, username: str, password: str, allowed_procedures: list):
        if not os.path.exists(self.__configuration.working_directory):
            raise MissingWorkingDirectoryException(
                "Working directory is not set, aborting start."
            )

        if not os.path.exists(self.__configuration.temp_directory):
            raise MissingWorkingDirectoryException(
        ...
        user_db_filepath = os.path.join(
            self.__configuration.working_directory, "server_users.json"
        )
    ...
    def start(self):
        ...
        while not self.__is_bureau_shutting_down:
            try:
                ...
                clerk = Clerk(self.__configuration, client, address)
                ...
                thread.start()
                self.__client_threads[thread.native_id] = clerk
