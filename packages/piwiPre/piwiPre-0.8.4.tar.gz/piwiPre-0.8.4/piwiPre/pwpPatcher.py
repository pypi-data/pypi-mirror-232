...
class PwpPatcher:
    def __init__(self, arguments=None, standalone=True):
        ...
        self.parser.add_argument('--autotest', help="autotest, no real action",
                                 action='store_true')
        self.parser.add_argument('--push', help="push to remote",
                                 action='store_true')
        self.parser.add_argument('--sphinx', help="runs sphinx to update the doc",
                                 action='store_true')

        self.parser.add_argument('--install', help="builds the exe with PyInstaller",
                                 action='store_true')
        self.parser.add_argument('--pypi', help="uploads package to Pypi",
                                 action='store_true')
        self.parser.add_argument('--gitlab', help="uploads exe to girlab",
                                 action='store_true')
    ...
    def run(self):
        ...
        epilogue = '<https://gitlab.com/api/v4/projects/48915444/packages/generic/piwiPre'
        ...
        if self.args.install:
            res = os.system("python -m PyInstaller -y piwiPre/__main__.py --onefile --name piwiPre ")
            print(f"msg     DONE: install -> {res}")
