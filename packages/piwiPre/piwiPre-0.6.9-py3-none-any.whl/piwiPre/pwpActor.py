...
class PwpActor:
    ...
    def remote_file_or_dir_exists(self, filepath):
        if self.dryrun:
            return None
        all_files = self.remote_ls(os.path.dirname(filepath))
    ...
    def remote_rm(self, src: str):
        LOGGER.debug(f"remote rm '{src}'")
        self.remote_run(f'rm -f {src}')

    def remote_rmdir(self, src: str):
        LOGGER.debug(f"remote rmdir '{src}'")
        self.remote_run(f'rm -rf {src}')
    ...
    def connect_ssh(self, config: dict):
        ...
        if self.tmp_dir in af:
            self.remote_rmdir(self.tmp_dir)
