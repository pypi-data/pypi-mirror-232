...
class PwpJpg(PwpObject):
    ...
    @staticmethod
    def get_remote_filename(config, filename):
        """
        :param config: local configuration
    ...
    def thumbnail(self, filename, width, height, crop, config: PwpConfig, allow_clobber: bool):
        ...
        if remote:
            remote_file = self.get_remote_filename(config, filename)
            if ACTOR.remote_file_or_dir_exists(remote_file) and not allow_clobber:
