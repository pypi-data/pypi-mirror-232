...
class RHEmailMessage:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if args:
            logger.warning("Received unsupported args: %s", args, extra={"args": args})
        if kwargs:
            logger.warning(
                "Received unsupported kwargs: %s", kwargs, extra={"kwargs": kwargs}
            )
    ...
    def get_image_path(self, image_file):
        ...
        if image_file.startswith(settings.MEDIA_URL):
            ...
            directory = os.path.join(
                settings.MEDIA_ROOT, directory[len(settings.MEDIA_URL) :]  # noqa: E203
            )
    ...
    def _process_html_body(self, msg):
        ...
        for image_file in image_files:
            ...
            msg.attach_inline_image(
                filepath=self.get_image_path(image_file), cid=b64_image_file
            )
            body_html = body_html.replace(
                "cid:%s" % image_file, "cid:%s" % b64_image_file
            )
    ...
    def add_attachment_by_filename(
        self, email_message: EmailMessage, filename: str, description: str
    ):
        try:
    ...
    def add_attachment_by_content(
        self, email_message: EmailMessage, content: str, description: str
    ):
