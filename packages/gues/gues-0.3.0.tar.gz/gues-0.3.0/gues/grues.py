...
class InputData(object):
    ...
    def mcmc(self, init="empty", p="uniform", max_moves=10000, prior=None):
        ...
        likelihood, bic, nuc = self.compute_scores()
        self.old_likelihood = likelihood
        self.bic_markov_chain[0] = self.best_bic = bic
        self.nuc_markov_chain[0] = self.best_nuc = nuc
    ...
    def get_likelihood_ratio(self):
        self.expand()
        new_likelihood, new_bic, new_nuc = self.compute_scores()
        likelihood_ratio = self.old_likelihood / new_likelihood
        return likelihood_ratio, new_likelihood, new_bic, new_nuc

    def compute_scores(self, graph=None):
        if graph is None:
            graph = self.cpdag
            self.uec = self.get_uec()

        bic_score, neg_log_likelihood = GaussObsL0Pen(self.samples).full_score(graph)
        _, s, _ = svd(graph)
        # test_bic = -0.5 * neg_log_likelihood - (s**2).sum() * np.log(self.num_samps)
        # print(bic_score, test_bic)
        nuc_score = -0.5 * neg_log_likelihood - s.sum() * np.log(self.num_samps)
        return neg_log_likelihood, bic_score, nuc_score
