...
from degreedays._private import getSafeDictKeys, Immutable, XmlElement, \
    raiseWrapped
...
# Now a check for Python 3.0 or 3.1, for which gzip streams won't work.
# https://docs.python.org/3.2/whatsnew/3.2.html#gzip-and-zipfile
...
class DecompressingStream(RawIOBase):
    def __init__(self, compressedStream, minReadBuffer):
        ...
        super(DecompressingStream, self).__init__()
    def close(self):
...
class DefaultHttpRequestDispatcher(object):
    def dispatch(self, httpRequest):
        try:
            ...
            raiseWrapped(TransportError, ('Error sending HTTP request to the '
                'API servers and getting a response stream back.'))
    ...
    def onFreshResponse(self, response):
        ...
        if (isinstance(response, FailureResponse) and
            ...
            if (code.startswith('RateLimit') and
                ...
                self._lock.acquire()
                try:
                    self._cachedRateLimitFailureResponse = response
    ...
    def process(self, request):
        ...
        try:
            ...
            try:
                ...
                raiseWrapped(TransportError, ('Error parsing the response XML. '
                    'Quite likely there was some sort of transport error that '
                    'caused invalid XML to be received in response to the '
                    'request, perhaps a firewall or proxy server is intercepting '
                    'requests and returning its own responses (e.g. messages '
                    'saying that it is blocking access to apiv1.degreedays.net) '
                    'or injecting something invalid into the responses from the '
                    'Degree Days.net API servers.'))
            ...
            if responseStream is not None:
                try:
                    ...
                    if isSuccess:
                        raiseWrapped(TransportError, ('Error closing the '
                            'response stream.'))
