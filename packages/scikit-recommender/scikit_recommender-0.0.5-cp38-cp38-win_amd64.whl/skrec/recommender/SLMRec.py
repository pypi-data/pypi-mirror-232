...
Author: Zhulin Tao, Xiaohao Liu, Yewei Xia, Xiang Wang, Lifang Yang, Xianglin Huang, and Tat-Seng Chua
TMM 2022
References: https://github.com/zltao/SLMRec
            https://github.com/enoche/MMRec/blob/master/src/models/slmrec.py
...
class SLMRecConfig(Config):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        self.lr: float = lr  # [0.0001, 0.001, 0.01, 0.1]
        self.reg: float = reg  # [0.0001, 0.001, 0.01, 0.1]
        self.rec_dim: int = rec_dim
        self.layer_num: int = layer_num
        self.ssl_alpha: float = ssl_alpha  # [0.01, 0.05, 0.1, 0.5, 1.0]
        self.ssl_temp: float = ssl_temp  # [0.1, 0.2, 0.5, 1.0]
        self.dropout_rate: float = dropout_rate
        self.temp: float = temp
        self.weight_decay: float = weight_decay
        self.mm_fusion_mode: str = mm_fusion_mode
        self.adj_type: str = adj_type
        self.ssl_task: str = ssl_task
        self.init: str = init
...
class _SLMRec(nn.Module):
    def __init__(self, config: SLMRecConfig, dataset: RSDataset, device):
        ...
        self.v_feat = torch.from_numpy(dataset.img_features).type(torch.FloatTensor).to(self.device)
        self.t_feat = torch.from_numpy(dataset.txt_features).type(torch.FloatTensor).to(self.device)
    ...
    def __init_weight(self, dataset):
        ...
        self.latent_dim = self.config.rec_dim
        ...
        train_interactions = dataset.train_data.to_csr_matrix().astype(np.float32)
    ...
    def compute(self):
        ...
        if self.dataset.data_name != "kwai":
    ...
    def feature_masking(self, users_idx, items_idx, dropout=False):
        ...
        rand_range = 4 if self.dataset.data_name != "kwai" else 2
    ...
    def calculate_loss(self, users, pos):
        # multi-task loss
...
class SLMRec(AbstractRecommender):
    def __init__(self, run_config: RunConfig, model_config: Dict):
        self.config = SLMRecConfig(**model_config)
        super().__init__(run_config, self.config)
        self.num_users, self.num_items = self.dataset.num_users, self.dataset.num_items
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

        self.model: _SLMRec = _SLMRec(self.config, self.dataset, self.device).to(self.device)
