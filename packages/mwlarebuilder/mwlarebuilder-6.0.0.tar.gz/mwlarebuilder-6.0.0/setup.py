...
# Hack that prevents MwlareBuilder.compat from failing due to unmet run-time dependencies (importlib-metadata on
...
os.environ["_MWLAREBUILDER_SETUP_PY"] = "1"
...
class build_bootloader(Command):
    ...
    def bootloader_exists(self):
        ...
        from MwlareBuilder.compat import is_win, is_cygwin
        exe = 'run'
        if is_win or is_cygwin:
            exe = 'run.exe'
        exe = os.path.join(HOMEPATH, 'MwlareBuilder', 'bootloader', PLATFORM, exe)
    ...
    def compile_bootloader(self):
        ...
        additional_args = os.getenv('MWLAREBUILDER_BOOTLOADER_WAF_ARGS', '').strip().split()
    ...
    def run(self):
        if getattr(self, 'dry_run', False):
            return
        if self.bootloader_exists() and not os.environ.get("MWLAREBUILDER_COMPILE_BOOTLOADER"):
            return
        print(
...
class Wheel(bdist_wheel):
    ...
    # The folder of bootloaders from MwlareBuilder/bootloaders to include.
    ...
    def finalize_options(self):
        ...
        if not self.has_bootloaders():
            raise SystemExit(
                ...
                f"https://mwlarebuilder.readthedocs.io/en/stable/bootloader-building.html for how to compile them."
            )
    ...
    def bootloaders_dir(cls):
        """
        Locate the bootloader folder inside the MwlareBuilder package.
        """
        return f"MwlareBuilder/bootloader/{cls.PYI_PLAT_NAME}"
...
# Map MwlareBuilder platform names to their setuptools counterparts. Other OSs can be added as and when we start shipping
# wheels for them.
PLATFORMS = {
...
class bdist_macos(wheel_commands["wheel_darwin_64bit"]):
    def finalize_options(self):
        ...
        * The architecture should similarly mirror the bootloader architecture(s).
        """
        try:
            from MwlareBuilder.utils.osx import get_binary_architectures, macosx_version_min
        except ImportError:
            raise SystemExit(
                "Building wheels for macOS requires that MwlareBuilder and macholib be installed. Please run:\n"
...
class bdist_wheels(Command):
    ...
    `MwlareBuilder/bootloaders/[platform-name]`.
    """
