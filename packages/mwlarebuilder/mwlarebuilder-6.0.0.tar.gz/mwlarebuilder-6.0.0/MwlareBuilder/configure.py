...
#-----------------------------------------------------------------------------
"""
Configure MwlareBuilder for the current Python installation.
"""
...
def _get_mwlarebuilder_cache_dir():
    old_cache_dir = None
    if compat.getenv('MWLAREBUILDER_CONFIG_DIR'):
        cache_dir = compat.getenv('MWLAREBUILDER_CONFIG_DIR')
    elif compat.is_win:
    ...
    cache_dir = os.path.join(cache_dir, 'mwlarebuilder')
    # Move old cache-dir, if any, to new location.
    if old_cache_dir and not os.path.exists(cache_dir):
        old_cache_dir = os.path.join(old_cache_dir, 'mwlarebuilder')
...
def get_config(upx_dir=None):
    ...
    config['cachedir'] = _get_mwlarebuilder_cache_dir()
    ...
    if upx_available:
        ...
        elif os.environ.get("MWLAREBUILDER_FORCE_UPX", "0") != "0":
            logger.warning(
