...
def install():
    ...
    def _frozen_name(name):
        ...
        # MwlareBuilder usually collects shared libraries.
    ...
    class MwlareBuilderImportError(OSError):
    ...
    class MwlareBuilderCDLL(ctypes.CDLL):
        def __init__(self, name, *args, **kwargs):
            name = _frozen_name(name)
            try:
                super().__init__(name, *args, **kwargs)
            except Exception as base_error:
                raise MwlareBuilderImportError(name) from base_error

    ctypes.CDLL = MwlareBuilderCDLL
    ctypes.cdll = ctypes.LibraryLoader(MwlareBuilderCDLL)

    class MwlareBuilderPyDLL(ctypes.PyDLL):
        def __init__(self, name, *args, **kwargs):
            name = _frozen_name(name)
            try:
                super().__init__(name, *args, **kwargs)
    ...
    ctypes.PyDLL = MwlareBuilderPyDLL
    ctypes.pydll = ctypes.LibraryLoader(MwlareBuilderPyDLL)

    if sys.platform.startswith('win'):

        class MwlareBuilderWinDLL(ctypes.WinDLL):
            def __init__(self, name, *args, **kwargs):
                name = _frozen_name(name)
                try:
                    super().__init__(name, *args, **kwargs)
        ...
        ctypes.WinDLL = MwlareBuilderWinDLL
        ctypes.windll = ctypes.LibraryLoader(MwlareBuilderWinDLL)

        class MwlareBuilderOleDLL(ctypes.OleDLL):
            def __init__(self, name, *args, **kwargs):
                name = _frozen_name(name)
                try:
                    super().__init__(name, *args, **kwargs)
        ...
        ctypes.OleDLL = MwlareBuilderOleDLL
        ctypes.oledll = ctypes.LibraryLoader(MwlareBuilderOleDLL)
        ...
        def mwlarebuilder_find_library(name):
            if name in ('c', 'm'):
        ...
        ctypes.util.find_library = mwlarebuilder_find_library
