...
from MwlareBuilder.utils import hooks, misc
from MwlareBuilder.utils.hooks.qt import _modules_info

logger = logging.getLogger(__name__)

# Qt deployment approach
# ----------------------
# This is the core of MwlareBuilder's approach to Qt deployment. It is based on:
#
...
# <https://github.com/mwlarebuilder/mwlarebuilder/blob/fbf7948be85177dd44b41217e9f039e1d176de6b/MwlareBuilder/utils/hooks/qt.py#L248-L362>`_
...
class QtLibraryInfo:
    ...
    def collect_module(self, module_name):
        ...
        for translation_base_name in translation_base_names:
            ...
            # https://github.com/mwlarebuilder/mwlarebuilder/pull/3229#issuecomment-359479893
            # and
            # https://github.com/mwlarebuilder/mwlarebuilder/issues/2857#issuecomment-368744341.
    ...
    def collect_plugins(self, plugin_type):
        ...
        # Windows this would grab debug copies of Qt plugins, which then causes MwlareBuilder to add a dependency on the
    ...
    def collect_extra_binaries(self):
        ...
        # See the "Deployment approach" section in ``MwlareBuilder/utils/hooks/qt.py``.
    ...
    def collect_qtqml_files(self):
        ...
        #
        # https://github.com/mwlarebuilder/mwlarebuilder/pull/3229#issuecomment-359735031
        # https://github.com/mwlarebuilder/mwlarebuilder/issues/3864
        #
    ...
    def collect_qtwebengine_files(self):
        ...
        if is_macos_framework:
            ...
            # using `MwlareBuilder.utils.osx.collect_files_from_framework_bundles` helper, so we do not have to
            ...
            if not os.path.isfile(qt_conf_file):
                ...
                from MwlareBuilder.config import CONF  # workpath
