 ...
 *
 * In this file the splash screen feature as of discussed in mwlarebuilder#4354 is
 ...
 *
 * MwlareBuilder will bundle (if splash screen is enabled) all necessary resources of
 ...
 * guaranteed. MwlareBuilder will check at build time if tcl is threaded.
 */
...
pyi_splash_extract(ARCHIVE_STATUS *archive_status, SPLASH_STATUS *splash_status)
{
     ...
     *         system in MwlareBuilder and the bootloader
     */
    size_t pos;
    const TOC *ptoc;
    TOC *tmp_toc;
...
_tcl_findLibrary_Command(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *const objv[])
{
     ...
     * We don't check the version of tk, since the library packed by MwlareBuilder
 ...
 * which some files may not be included. MwlareBuilder includes at build time
