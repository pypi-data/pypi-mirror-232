...
pyi_main(int argc, char * argv[])
{
    ...
    VS("MwlareBuilder Bootloader 6.x\n");
    ...
    /* If the Python program we are about to run invokes another MwlareBuilder
     * one-file program as subprocess, this subprocess must not be fooled into
     * thinking that it is already unpacked. Therefore, MwlareBuilder deletes
    ...
    if (!pyi_arch_setup(archive_status, executable, executable)) {
        if (!pyi_arch_setup(archive_status, archivefile, executable)) {
            FATALERROR("Cannot open MwlareBuilder archive from executable (%s) or external archive (%s)\n",
     ...
     * For the splash screen function to work MwlareBuilder
