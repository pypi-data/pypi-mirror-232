...
from MwlareBuilder.utils.git import get_repo_revision
...
if os.path.exists(os.path.join(HOMEPATH, 'setup.py')):
    # MwlareBuilder is run directly from source without installation, or __version__ is called from 'setup.py'...
    if compat.getenv('MWLAREBUILDER_DO_RELEASE') == '1':
    ...
    # MwlareBuilder was installed by `python setup.py install'.
    from importlib.metadata import version
    __version__ = version('MwlareBuilder')
# Default values of paths where to put files created by MwlareBuilder. If changing these, do not forget to update the
