...
from MwlareBuilder import DEFAULT_SPECPATH, HOMEPATH
from MwlareBuilder import log as logging
from MwlareBuilder.building.templates import bundleexetmplt, bundletmplt, onedirtmplt, onefiletmplt, splashtmpl
from MwlareBuilder.compat import expand_path, is_darwin, is_win
...
class _RemovedNoEmbedManifestAction(_RemovedFlagAction):
    def __call__(self, *args, **kwargs):
        from MwlareBuilder.exceptions import RemovedExternalManifestError
...
class Preamble:
    ...
    def _add_hookutil_import(self, name):
        self.content += ['from MwlareBuilder.utils.hooks import {0}'.format(name)]
...
def __add_options(parser):
    ...
    g.add_argument(
        ...
        "platform format (ico on Windows, icns on Mac), MwlareBuilder tries to use Pillow to translate the icon into "
        "the correct format (if Pillow is installed). Use \"NONE\" to not apply any icon, thereby making the OS show "
        "some default (default: apply MwlareBuilder's icon). This option can be used multiple times.",
    )
    g.add_argument(
...
def main(
    ...
    **_kwargs
    ...
    # If cwd is the root directory of MwlareBuilder, generate the .spec file in ./appname/ subdirectory.
