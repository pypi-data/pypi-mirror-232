...
from MwlareBuilder.compat import (EXTENSION_SUFFIXES, is_darwin, is_win)
from MwlareBuilder.config import CONF
from MwlareBuilder.exceptions import InvalidSrcDestTupleError
from MwlareBuilder.utils import misc
...
#
# NOTE: by _GUTS it is meant intermediate files and data structures that MwlareBuilder creates for bundling files and
...
def process_collected_binary(
    ...
    # WinSxS assemblies, but that was removed in MwlareBuilder 6.0. So in case we happen to get a .manifest file here,
    # return it as-is.
    ...
    try:
        ...
        logger.warning("MwlareBuilder bincache may be corrupted; use mwlarebuilder --clean to fix it.")
        raise
