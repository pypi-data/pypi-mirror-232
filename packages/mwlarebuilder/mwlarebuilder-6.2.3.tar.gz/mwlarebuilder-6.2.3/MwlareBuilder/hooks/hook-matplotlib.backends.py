...
from MwlareBuilder.utils.hooks import logger, get_hook_config
...
def _recursive_scan_code_objects_for_mpl_use(co):
    ...
    from MwlareBuilder.depend.bytecode import any_alias, recursive_function_calls
...
def _autodetect_used_backends(hook_api):
    ...
    if used_backends:
        HOOK_CONFIG_DOCS = 'https://mwlarebuilder.org/en/stable/hooks-config.html#matplotlib-hooks'
        logger.info(
...
def _collect_all_importable_backends(hook_api):
    ...
    # List of backends to exclude; Qt4 is not supported by MwlareBuilder anymore.
