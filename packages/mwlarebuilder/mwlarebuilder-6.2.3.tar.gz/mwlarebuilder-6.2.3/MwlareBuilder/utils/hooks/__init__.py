...
from MwlareBuilder import HOMEPATH, compat
from MwlareBuilder import log as logging
from MwlareBuilder.depend.imphookapi import PostGraphAPI
...
#
# For example the 'wx' module needs variable 'wxpubsub'. This tells MwlareBuilder which protocol of the wx module
# should be bundled.
hook_variables = {}
...
def __exec_python_cmd(cmd, env=None, capture_stdout=True):
    ...
    """
    # 'MwlareBuilder.config' cannot be imported as other top-level modules.
    ...
    # Some functions use some MwlareBuilder code in subprocess, so add MwlareBuilder HOMEPATH to sys.path as well.
...
def exec_statement(statement: str):
    ...
    Notes:
        ...
        new :mod:`MwlareBuilder.isolated` module.
...
def get_pyextension_imports(module_name: str):
    ...
    This function could be used for 'hiddenimports' in MwlareBuilder hooks files.
    """
    import sys
    import importlib
...
def is_module_satisfies(
    ...
    In contrast to original implementation from MwlareBuilder < 6, this implementation only checks the specified
...
def collect_submodules(
    ...
    This function is intended to be used by hook scripts, not by main MwlareBuilder code.
...
def _collect_submodules(name, on_error):
    ...
    from MwlareBuilder.utils.hooks import logger
...
def collect_data_files(
    ...
    -   By default, python source files and byte-compiled modules (files with ``.py`` and ``.pyc`` suffix) are not
        ...
        that tries to extend `sys.path` with subpackage paths in a way that is incompatible with MwlareBuilder's frozen
        importer.. However, in contemporary MwlareBuilder versions, the preferred way of collecting source .py files is by
...
def copy_metadata(package_name: str, recursive: bool = False):
    ...
    This function returns a list to be assigned to the ``datas`` global variable. This list instructs MwlareBuilder to
    ...
    ``pkg_resources``) module. MwlareBuilder does not collect these metadata files by default.
    ...
    while todo:
        ...
        if src_path.is_dir():
            ...
            # and requires special handling (as of MwlareBuilder v6, we support only non-zipped eggs).
...
def collect_delvewheel_libs_directory(package_name, libdir_name=None, datas=None, binaries=None):
    ...
    # As of MwlareBuilder 5.4, this should be safe (should not result in duplication), because binary dependency
...
if compat.is_pure_conda:
    from MwlareBuilder.utils.hooks import conda as conda_support  # noqa: F401
elif compat.is_conda:
    from MwlareBuilder.utils.hooks.conda import CONDA_META_DIR as _tmp
    logger.warning(
