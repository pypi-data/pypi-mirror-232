...
from MwlareBuilder.exceptions import ExecCommandFailed

# setup.py sets this environment variable to avoid errors due to unmet run-time dependencies. The MwlareBuilder.compat
...
_setup_py_mode = os.environ.get('_MWLAREBUILDER_SETUP_PY', '0') != '0'

# MwlareBuilder requires importlib.metadata from python >= 3.10 stdlib, or equivalent importlib-metadata >= 4.6.
if _setup_py_mode:
    ...
    if sys.version_info >= (3, 10):
        ...
        try:
            ...
            from MwlareBuilder.exceptions import ImportlibMetadataError
...
strict_collect_mode = os.environ.get("MWLAREBUILDER_STRICT_COLLECT_MODE", "0") != "0"
...
# Mac OS is not considered as unix since there are many platform-specific details for Mac in MwlareBuilder.
...
# encountered will be conda packages meaning that the default non-conda behaviour is generally desired from MwlareBuilder.
...
if is_ms_app_store:
    ...
    if not os.path.exists(python_executable):
        raise SystemExit(
            'MwlareBuilder cannot locate real python executable belonging to Python from Microsoft App Store!'
        )
...
# -> all mwlarebuilder modules should use win32api from MwlareBuilder.compat to
...
if is_win:
    if _setup_py_mode:
        ...
        try:
            ...
            raise SystemExit(
                'MwlareBuilder cannot check for assembly dependencies.\n'
            ...
            if sys.flags.optimize == 2:
                raise SystemExit(
                    "pycparser, a Windows only indirect dependency of MwlareBuilder, is incompatible with "
                    "Python's \"discard docstrings\" (-OO) flag mode. For more information see:\n"
                    "    https://github.com/mwlarebuilder/mwlarebuilder/issues/6345"
                ) from e
            raise
...
# CYGWIN_NT-10.0-19042 that do not fit MwlareBuilder's OS naming scheme. Explicitly set `system` to 'Cygwin'.
    ...
    *cmdargs: str, encoding: str | None = None, raise_enoent: bool | None = None, **kwargs: int | bool | list | None
    ...
    .. DANGER::
       **Ignore this function's return value** -- unless this command's standard output contains _only_ pathnames, in
       which case this function returns the correct filesystem-encoded string expected by MwlareBuilder. In all other
...
def exec_command_stdout(
    *command_args: str, encoding: str | None = None, **kwargs: float | str | bytes | bool | list | None
    ...
    .. NOTE::
       ...
       filesystem-encoded string expected by MwlareBuilder. If this is the case, consider calling the filesystem-specific
...
# modules used by MwlareBuilder's bootstrap scripts and modules (loader/pyi*.py)
...
SPECIAL_MODULE_TYPES = {
    ...
    # MwlareBuilder handles scripts differently and not as standard Python modules.
    'Script',
}
...
def check_requirements():
    """
    Verify that all requirements to run MwlareBuilder are met.

    Fail hard if any requirement is not met.
    """
    # Fail hard if Python does not have minimum required version
    if sys.version_info < (3, 8):
        raise EnvironmentError('MwlareBuilder requires Python 3.8 or newer.')
    ...
    for name in ["enum34", "typing", "pathlib"]:
        ...
        raise SystemExit(
            ...
            f"MwlareBuilder. Please remove this package (located in {dist.locate_file('')}) using\n    {remove}\n"
            "then try again."
        )
