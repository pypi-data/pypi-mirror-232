...
from MwlareBuilder.building.utils import get_code_object, strip_paths_in_code
from MwlareBuilder.compat import BYTECODE_MAGIC, is_win, strict_collect_mode
from MwlareBuilder.loader.pyimod01_archive import PYZ_ITEM_MODULE, PYZ_ITEM_NSPKG, PYZ_ITEM_PKG


class ZlibArchiveWriter:
    """
    Writer for MwlareBuilder's PYZ (ZlibArchive) archive. The archive is used to store collected byte-compiled Python
...
class CArchiveWriter:
    """
    Writer for MwlareBuilder's CArchive (PKG) archive.
    ...
    # For cookie and TOC entry structure, see `MwlareBuilder.archive.readers.CArchiveReader`.
