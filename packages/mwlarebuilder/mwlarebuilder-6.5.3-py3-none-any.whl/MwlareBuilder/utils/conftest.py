...
# Set a handler for the root-logger to inhibit 'basicConfig()' (called in MwlareBuilder.log) is setting up a stream
...
from MwlareBuilder import __main__ as pyi_main  # noqa: E402
from MwlareBuilder import configure  # noqa: E402
from MwlareBuilder.compat import architecture, is_darwin, is_win  # noqa: E402
from MwlareBuilder.depend.analysis import initialize_modgraph  # noqa: E402
from MwlareBuilder.archive.readers import pkg_archive_contents  # noqa: E402
from MwlareBuilder.utils.tests import gen_sourcefile  # noqa: E402
from MwlareBuilder.utils.win32 import winutils  # noqa: E402
...
class AppBuilder:
    ...
    def test_script(
        ...
        :param pyi_args: Additional arguments to pass to MwlareBuilder when creating executable.
    ...
    def _test_executables(self, name, args, runtime, run_from_path, **kwargs):
        ...
        # Empty list means that MwlareBuilder probably failed to create any executable.
    ...
    def _run_executable(self, prog, args, run_from_path, runtime):
        """
        Run executable created by MwlareBuilder.
    ...
    def _test_building(self, args):
        ...
        :param args: additional CLI options for MwlareBuilder.
        ...
        # TODO: fix return code in running MwlareBuilder programmatically.
        PYI_CONFIG = configure.get_config()
        # Override CACHEDIR for MwlareBuilder and put it into self.tmpdir
...
def pyi_builder(tmpdir, monkeypatch, request, pyi_modgraph):
    ...
    # MwlareBuilder or a test case might manipulate 'sys.path'. Reset it for every test.
    monkeypatch.syspath_prepend(None)
    # Set current working directory to
    monkeypatch.chdir(tmpdir)
    # Clean up configuration and force MwlareBuilder to do a clean configuration for another app/test. The value is same
    # as the original value.
    monkeypatch.setattr('MwlareBuilder.config.CONF', {'pathex': []})
