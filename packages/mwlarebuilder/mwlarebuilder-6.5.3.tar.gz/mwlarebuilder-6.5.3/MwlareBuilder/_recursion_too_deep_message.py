...
2. Build your program by running MwlareBuilder with the .spec file as
   argument::

     mwlarebuilder myprog.spec

3. If this fails, you most probably hit an endless recursion in
   MwlareBuilder. Please try to track this down has far as possible,
   create a minimal example so we can reproduce and open an issue at
   https://github.com/mwlarebuilder/mwlarebuilder/issues following the
...
eating up memory. MwlareBuilder imports modules recursively. If the structure
