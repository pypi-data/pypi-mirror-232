...
from MwlareBuilder.building.datastruct import Tree
...
def _warn_if_activetcl_or_teapot_installed(tcl_root, tcltree):
    ...
    MwlareBuilder does *not* freeze all ActiveTcl dependencies -- including Teapot, which is typically ignorable. Since
    Teapot is *not* ignorable in this case, this function warns of impending failure.

    See Also
    -------
    https://github.com/mwlarebuilder/mwlarebuilder/issues/621
    """
    ...
    if mentions_activetcl and mentions_teapot:
        logger.warning(
            """
You appear to be using an ActiveTcl build of Tcl/Tk, which MwlareBuilder has
difficulty freezing. To fix this, comment out all references to "teapot" in:

     %s

See https://github.com/mwlarebuilder/mwlarebuilder/issues/621 for more information.
            """ % init_resource
        )
...
def _find_tcl_tk(tkinter_ext_file):
    ...
    if compat.is_darwin:
        ...
        # python with python-tk). MwlareBuilder does not bundle data from system frameworks (as it does not not collect
        ...
        # Starting with macOS 11, system libraries are hidden (unless both Python and MwlareBuilder's bootloader are built
