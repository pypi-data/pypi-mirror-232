...
#-----------------------------------------------------------------------------
"""
Main command-line interface to MwlareBuilder.
"""
...
def run_makespec(filenames, **opts):
    ...
    spec_file = MwlareBuilder.building.makespec.main(filenames, **opts)
...
def run_build(pyi_config, spec_file, **kwargs):
    ...
    MwlareBuilder.building.build_main.main(pyi_config, spec_file, **kwargs)
...
def generate_parser() -> _PyiArgumentParser:
    """
    Build an argparse parser for MwlareBuilder's main CLI.
    """
    ...
    parser.prog = "mwlarebuilder"

    parser._add_options(__add_options)
    parser._add_options(MwlareBuilder.building.makespec.__add_options, name="makespec")
    parser._add_options(MwlareBuilder.building.build_main.__add_options, name="build_main")
    parser._add_options(MwlareBuilder.log.__add_options, name="log")
...
def run(pyi_args: list | None = None, pyi_config: dict | None = None):
    """
    pyi_args     allows running MwlareBuilder programmatically without a subprocess
    ...
    try:
        ...
        # Print MwlareBuilder version, Python version, and platform as the first line to stdout. This helps us identify
        # MwlareBuilder, Python, and platform version when users report issues.
        logger.info('MwlareBuilder: %s' % __version__)
        ...
        from MwlareBuilder import _recursion_too_deep_message
...
def _console_script_run():
    # Python prepends the main script's parent directory to sys.path. When MwlareBuilder is ran via the usual
    # `mwlarebuilder` CLI entry point, this directory is $pythonprefix/bin which should not be in sys.path.
