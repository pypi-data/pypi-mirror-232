...
Spec file is generated by MwlareBuilder. The generated code from .spec file
is a way how MwlareBuilder does the dependency analysis and creates executable.
"""
...
from MwlareBuilder.archive.writers import CArchiveWriter, ZlibArchiveWriter
from MwlareBuilder.building.datastruct import Target, _check_guts_eq, normalize_pyz_toc, normalize_toc
...
from MwlareBuilder.building.splash import Splash  # argument type validation in EXE
from MwlareBuilder.compat import is_cygwin, is_darwin, is_linux, is_win, strict_collect_mode
from MwlareBuilder.depend import bindepend
from MwlareBuilder.depend.analysis import get_bootstrap_modules
import MwlareBuilder.utils.misc as miscutils

logger = logging.getLogger(__name__)

if is_win:
    from MwlareBuilder.utils.win32 import (icon, versioninfo, winmanifest, winresource, winutils)
...
class PYZ(Target):
    ...
    def __init__(self, *tocs, **kwargs):
        ...
        # MwlareBuilder bootstrapping modules.
...
class PKG(Target):
    ...
    def __init__(
        ...
        # This dict tells MwlareBuilder what items embedded in the executable should be compressed.
...
class EXE(Target):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs
            ...
            icon
                ...
                resource. Defaults to use MwlareBuilder's console or windowed icon. Use icon=`NONE` to not add any icon.
            version
        ...
        if self.runtime_tmpdir is not None:
            self.toc.append(("secret-tmpdir " + self.runtime_tmpdir, "", "OPTION"))

        if self.bootloader_ignore_signals:
            # no value; presence means "true"
            self.toc.append(("secret-ignore-signals", "", "OPTION"))

        if self.disable_windowed_traceback:
            # no value; presence means "true"
            self.toc.append(("secret-disablwindowed-traceback", "", "OPTION"))

        if self.argv_emulation:
            # no value; presence means "true"
            self.toc.append(("secret-macos-argv-emulation", "", "OPTION"))

        self.toc.append(("secret-contents-directory " + self.contents_directory, "", "OPTION"))
        ...
        if self.hide_console:
            ...
            self.toc.append((f"secret-hide-console {self.hide_console}", "", "OPTION"))
    ...
    def _bootloader_file(self, exe, extension=None):
        ...
        if extension:
            exe = exe + extension
        bootloader_file = os.path.join(HOMEPATH, 'MwlareBuilder', 'bootloader', PLATFORM, exe)
    ...
    def _retry_operation(func, *args, max_attempts=20):
        ...
        def _is_allowed_exception(e):
            ...
            """
            if is_win:
                from MwlareBuilder.compat import pywintypes
...
_MISSING_BOOTLOADER_ERRORMSG = """Fatal error: MwlareBuilder does not include a pre-compiled bootloader for your
platform. For more details and instructions how to build the bootloader see
<https://mwlarebuilder.readthedocs.io/en/stable/bootloader-building.html>"""
