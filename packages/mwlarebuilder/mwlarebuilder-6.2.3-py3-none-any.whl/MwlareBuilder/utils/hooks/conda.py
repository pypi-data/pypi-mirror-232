...
:mod:`MwlareBuilder.utils.hooks.conda_support<MwlareBuilder.utils.hooks.conda>`
...
a conditional clause::
    ...
    from MwlareBuilder.compat import is_pure_conda
...
def collect_dynamic_libs(name: str, dest: str = ".", dependencies: bool = True, excludes: Iterable[str] | None = None):
    ...
    Returns:
        List of DLLs in MwlareBuilder's ``(source, dest)`` format.

    This collects libraries only from Conda's shared ``lib`` (Unix) or ``Library/bin`` (Windows) folders. To collect
    from inside a distribution's installation use the regular :func:`MwlareBuilder.utils.hooks.collect_dynamic_libs`.
    """
