...
class ImportErrorWhenRunningHook(HookError):
    def __str__(self):
        return (
            ...
            "hooks in the manual and provide a pull-request to improve MwlareBuilder.".format(self.args[0], self.args[1])
        )
...
class RemovedCipherFeatureError(SystemExit):
    def __init__(self, message):
        super().__init__(
            f"Bytecode encryption was removed in MwlareBuilder v6.0. {message}"
            " For the rationale and alternatives see https://github.com/mwlarebuilder/mwlarebuilder/pull/6999"
        )


class RemovedExternalManifestError(SystemExit):
    def __init__(self, message):
        super().__init__(f"Support for external executable manifest was removed in MwlareBuilder v6.0. {message}")


class RemovedWinSideBySideSupportError(SystemExit):
    def __init__(self, message):
        super().__init__(
            f"Support for collecting and processing WinSxS assemblies was removed in MwlareBuilder v6.0. {message}"
        )
...
class ImportlibMetadataError(SystemExit):
    def __init__(self):
        super().__init__(
            "MwlareBuilder requires importlib.metadata from python >= 3.10 stdlib or importlib_metadata from "
