...
from MwlareBuilder import DEFAULT_DISTPATH, DEFAULT_WORKPATH, HOMEPATH, compat
from MwlareBuilder import log as logging
from MwlareBuilder.building.api import COLLECT, EXE, MERGE, PYZ
from MwlareBuilder.building.datastruct import (
    TOC, Target, Tree, _check_guts_eq, normalize_toc, normalize_pyz_toc, toc_process_symbolic_links
)
from MwlareBuilder.building.osx import BUNDLE
from MwlareBuilder.building.splash import Splash
from MwlareBuilder.building.utils import (
...
from MwlareBuilder.compat import is_win, is_conda, is_darwin
from MwlareBuilder.depend import bindepend
from MwlareBuilder.depend.analysis import initialize_modgraph
from MwlareBuilder.depend.utils import create_py3_base_library, scan_code_for_ctypes
from MwlareBuilder import isolated
from MwlareBuilder.utils.misc import absnormpath, get_path_to_toplevel_modules, mtime
from MwlareBuilder.utils.hooks import get_package_paths
from MwlareBuilder.utils.hooks.gi import compile_glib_schema_files

if is_darwin:
    from MwlareBuilder.utils import osx as osxutils
...
_init_code_path = os.path.join(HOMEPATH, 'MwlareBuilder', 'loader')
...
This file lists modules MwlareBuilder was not able to find. This does not
...
def discover_hook_directories():
    """
    Discover hook directories via mwlarebuilder40 entry points. Perform the discovery in an isolated subprocess
    ...
    from MwlareBuilder.log import logger
    ...
    entry_points = importlib_metadata.entry_points(group='mwlarebuilder40', name='hook-dirs')

    # Ensure that mwlarebuilder_hooks_contrib comes last so that hooks from packages providing their own take priority.
    entry_points = sorted(entry_points, key=lambda x: x.module == "_mwlarebuilder_hooks_contrib.hooks")
...
class Analysis(Target):
    ...
    def __init__(
        ...
        **_kwargs,
        ...
        if cipher is not None:
            from MwlareBuilder.exceptions import RemovedCipherFeatureError
        ...
        if win_no_prefer_redirects:
            from MwlareBuilder.exceptions import RemovedWinSideBySideSupportError
        ...
        # Set global config variable 'pathex' to make it available for MwlareBuilder.utils.hooks and import hooks. Path
        # extensions for module search.
        CONF['pathex'] = self.pathex
        # Extend sys.path so MwlareBuilder could find all necessary modules.
        ...
        # Add hook directories from MwlareBuilder entry points.
        ...
        # Custom runtime hook files that should be included and started before any existing MwlareBuilder runtime hooks.
    ...
    def _extend_pathex(self, spec_pathex, scripts):
        """
        Normalize additional paths where MwlareBuilder will look for modules and add paths with scripts to the list of
        paths.
        ...
        # In that case MwlareBuilder will not be able find modules in the directory containing 'mymodule'.
        # Add this directory to PYTHONPATH so MwlareBuilder could find it.
        pathex = []
    ...
    def assemble(self):
        ...
        if python_lib is None:
            from MwlareBuilder.exceptions import PythonLibraryNotFoundError
        ...
        # We do not support zipped eggs anymore (MwlareBuilder v6.0), so `zipped_data` and `zipfiles` are always empty.
        self.zipped_data = []
        self.zipfiles = []
...
def build(spec, distpath, workpath, clean_build):
    ...
    # Add 'specname' to workpath and distpath if they point to MwlareBuilder homepath.
    ...
    # Clean MwlareBuilder cache (CONF['cachedir']) and temporary files (workpath) to be able start a clean build.
    if clean_build:
    ...
    spec_namespace = {
        ...
        # MwlareBuilder classes for .spec.
...
def __add_options(parser):
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help="Clean MwlareBuilder cache and remove temporary files before building.",
    )
...
def main(
    ...
    **kw
    ...
    if pyi_config is None:
        import MwlareBuilder.configure as configure
