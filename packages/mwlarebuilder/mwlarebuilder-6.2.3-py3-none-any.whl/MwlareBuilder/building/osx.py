...
from MwlareBuilder.building.api import COLLECT, EXE
from MwlareBuilder.building.datastruct import Target, logger, normalize_toc
from MwlareBuilder.building.utils import _check_path_overlap, _rmtree, process_collected_binary
from MwlareBuilder.compat import is_darwin, strict_collect_mode
from MwlareBuilder.building.icon import normalize_icon_type
import MwlareBuilder.utils.misc as miscutils

if is_darwin:
    import MwlareBuilder.utils.osx as osxutils
...
class BUNDLE(Target):
    def __init__(self, *args, **kwargs):
        ...
        if not self.icon:
            # --icon not specified; use the default in the mwlarebuilder folder
    ...
    #
    # The requirements, framed from MwlareBuilder's perspective, can be summarized as follows:
    #
    ...
    #   source-only form by MwlareBuilder (i.e., the python modules and scripts collected as source .py files). At the
    ...
    #
    # In the light of all above, MwlareBuilder attempts to strictly place all files to their mandated location
    ...
    # the program executable) into `Contents/Frameworks`. This requires the MwlareBuilder's bootloader to detect that it
    ...
    # `MwlareBuilder.utils.osx.collect_files_from_framework_bundles` helper function. So the input TOC that BUNDLE
    ...
    def assemble(self):
        ...
        for dest_name, src_name, typecode in bundle_toc:
            ...
            try:
                ...
                raise SystemExit(
                    f"Mwlarebuilder needs to create a directory at {dest_dir!r}, "
        ...
        try:
            ...
            if os.environ.get("MWLAREBUILDER_STRICT_BUNDLE_CODESIGN_ERROR", "0") == "0":
        ...
        if os.environ.get("MWLAREBUILDER_VERIFY_BUNDLE_SIGNATURE", "0") != "0":
