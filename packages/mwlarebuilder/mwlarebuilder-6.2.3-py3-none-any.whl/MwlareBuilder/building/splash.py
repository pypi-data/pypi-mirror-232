...
from MwlareBuilder.archive.writers import SplashWriter
from MwlareBuilder.building import splash_templates
from MwlareBuilder.building.datastruct import Target
from MwlareBuilder.building.utils import _check_guts_eq, _check_guts_toc, misc
from MwlareBuilder.compat import is_darwin
from MwlareBuilder.depend import bindepend
from MwlareBuilder.utils.hooks import tcl_tk as tcltk_utils
...
# (see `MwlareBuilder.utils.hooks.tcl_tk.collect_tcl_tk_files`).
splash_requirements = [
...
class Splash(Target):
    ...
    def __init__(self, image_file, binaries, datas, **kwargs):
        ...
        try:
            ...
            # not installed. This would cause a runtime error in MwlareBuilder, since this module is imported from
    ...
    def test_tk_version(self):
        ...
        if tcl_version != tk_version:
            logger.warning(
                "The installed version of Tcl (%s) and Tk (%s) do not match. MwlareBuilder is tested against matching "
        ...
        if not tcltk_utils.tcl_threaded:
            ...
            raise SystemExit(
                "The installed tcl version is not threaded. MwlareBuilder only supports the splash screen "
                "using threaded tcl."
            )
