...
#limit at MwlareBuilder startup (i.e., in the
#MwlareBuilder.building.build_main.build() function). For details, see:
#    https://github.com/mwlarebuilder/mwlarebuilder/issues/1919#issuecomment-216016176
...
# The logic in MwlareBuilder.compat ensures that these are available and
# of correct version.
...
#significantly more fine-grained metadata than MwlareBuilder will ever require.
...
#"MwlareBuilder.lib.modulegraph.node" module. This module is much too long.
...
class Node:
    ...
    _deferred_imports : list
        ...
        * `_safe_import_hook_kwargs` is a (typically empty) dictionary of all
        ...
        See mwlarebuilder/mwlarebuilder/#1289 for further details.
...
class ModuleGraph(ObjectGraph):
    ...
    def import_hook(
        ...
        source_module : Node
            ...
            subtle issues in external callers (namely MwlareBuilder, which
        ...
        for attr_name in attr_names:
            ...
            if submodule is None:
                ...
                if submodule is None:
                    ...
                    # submodule! Why?
                    #
                    # Edge cases. MwlareBuilder (and by extension ModuleGraph)
    ...
    def _safe_import_hook(
        ...
        source_module : Node
            ...
            created for the module to be imported will be disconnected and
            hence unreachable from all other nodes -- which frequently causes
            subtle issues in external callers (e.g., MwlareBuilder, which
            silently ignores unreachable nodes).
        target_attr_names : list
            List of the unqualified names of all submodules and attributes to
            ...
            * Efficiency. Unlike packages, modules cannot physically contain
              ...
              case that target module will already be frozen by MwlareBuilder.
            ...
            for target_submodule_partname in target_attr_names:
                ...
                if target_submodule is None:
                    ...
                    try:
                        ...
                        # If this is a SWIG C extension, instruct MwlareBuilder
