...
from MwlareBuilder import HOMEPATH, PACKAGEPATH
from MwlareBuilder import log as logging
from MwlareBuilder.building.utils import add_suffix_to_extension
from MwlareBuilder.compat import (
...
from MwlareBuilder.depend import bytecode
from MwlareBuilder.depend.imphook import AdditionalFilesCache, ModuleHookCache
from MwlareBuilder.depend.imphookapi import (PreFindModulePathAPI, PreSafeImportModuleAPI)
from MwlareBuilder.lib.modulegraph.find_modules import get_implies
from MwlareBuilder.lib.modulegraph.modulegraph import ModuleGraph, DEFAULT_IMPORT_LEVEL, ABSOLUTE_IMPORT_LEVEL
from MwlareBuilder.log import DEBUG, INFO, TRACE
from MwlareBuilder.utils.hooks import collect_submodules, is_package
...
class PyiModuleGraph(ModuleGraph):
    ...
    def __init__(self, pyi_homepath, user_hook_dirs=(), excludes=(), **kwargs):
        ...
        # Homepath to the place where is MwlareBuilder located.
        self._homepath = pyi_homepath
        # modulegraph Node for the main python script that is analyzed by MwlareBuilder.
    ...
    def _cache_hooks(self, hook_type):
        ...
        The cache will include all official hooks defined by the MwlareBuilder codebase _and_ all unofficial hooks
        defined for the current application.

        Parameters
        ----------
        hook_type : str
            Type of hooks to be cached, equivalent to the basename of the subpackage of the `MwlareBuilder.hooks`
    ...
    def _safe_import_module(self, module_basename, module_name, parent_package):
        ...
        this module (e.g., a script `MwlareBuilder.hooks.hook-{module_name}` containing a function
        ...
        if module_name in self._hooks_pre_safe_import_module:
            ...
            for hook in self._hooks_pre_safe_import_module[module_name]:
                ...
                hook_module_name = 'MwlareBuilder_hooks_pre_safe_import_module_' + module_name.replace('.', '_')
    ...
    def _find_module_path(self, fullname, module_name, search_dirs):
        ...
        for this module (e.g., a script `MwlareBuilder.hooks.hook-{module_name}` containing a function
        ...
        if fullname in self._hooks_pre_find_module_path:
            ...
            for hook in self._hooks_pre_find_module_path[fullname]:
                ...
                hook_fullname = 'MwlareBuilder_hooks_pre_find_module_path_' + fullname.replace('.', '_')
    ...
    def _make_toc(self, typecode=None):
        ...
        of MwlareBuilder TOC typecodes. If that list is empty we return the complete flattened graph as a TOC with the
        ModuleGraph note types in place of typecodes -- meant for debugging only. Normally we return ModuleGraph
        nodes whose types map to the requested MwlareBuilder typecode(s) as indicated in the MODULE_TYPES_TO_TOC_DICT.
    ...
    def make_pure_toc(self):
        ...
        """
        # MwlareBuilder should handle special module types without code object.
    ...
    def _node_to_toc(node, typecode=None):
        ...
        # 'str'. But 'marshal.dumps()' function is able to marshal only 'str'. Otherwise on Windows MwlareBuilder might
    ...
    def analyze_runtime_hooks(self, custom_runhooks):
        ...
        # in the list are executed first. Put their graph nodes at the head of the priority_scripts list Mwlarebuilder
    ...
    def add_hiddenimports(self, module_list):
        ...
        some MwlareBuilder features when enabled (e.g., crypto feature).
        """
    ...
    def _metadata_from(self, package, methods=(), recursive_methods=()) -> set:
        ...
        Returns:
            ...
            :func:`MwlareBuilder.utils.hooks.copy_metadata()`.
...
def get_bootstrap_modules():
    ...
    loader_mods = list()
    loaderpath = os.path.join(HOMEPATH, 'MwlareBuilder', 'loader')
