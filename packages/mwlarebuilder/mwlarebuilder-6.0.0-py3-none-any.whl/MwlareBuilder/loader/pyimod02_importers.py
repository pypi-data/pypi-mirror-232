...
# List of modules collected into base_library.zip: MwlareBuilder.compat.PY3_BASE_MODULES
...
class PyiFrozenImporter:
    """
    Load bytecode of Python modules from the executable created by MwlareBuilder.
    ...
    def __init__(self):
        ...
        for pyz_filepath in sys.path:
            try:
                ...
                trace("# MwlareBuilder: PyiFrozenImporter(%s)", pyz_filepath)
                return
            except IOError:
    ...
    def get_data(self, path):
        ...
        on the filesystem. Older versions of MwlareBuilder also supported data embedded in the PYZ archive, but that has
    ...
    def get_filename(self, fullname):
        ...
        # directory where files are unpacked by MwlareBuilder. Then, append the appropriate suffix (__init__.pyc for a
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        if path is not None:
            ...
            for p in path:
                ...
                if entry_name in self.toc:
                    trace("import %s as %s # MwlareBuilder PYZ (__path__ override: %s)", entry_name, fullname, p)
                    break
            else:
                entry_name = None
        ...
        if entry_name is None:
            ...
            if fullname in self.toc:
                entry_name = fullname
                trace("import %s # MwlareBuilder PYZ", fullname)
    ...
    def exec_module(self, module):
        ...
        if spec.submodule_search_locations is not None:
            # Since PYTHONHOME is set in bootloader, 'sys.prefix' points to the correct path where MwlareBuilder should
...
class PyiFrozenResourceReader:
    ...
    However, this should cover the typical use cases (access to data files), as MwlareBuilder collects data files onto
...
def install():
    ...
    for item in sys.meta_path:
        if hasattr(item, '__name__') and item.__name__ == 'WindowsRegistryFinder':
            ...
            break
    # _frozen_importlib.PathFinder is also able to handle Python C extensions. However, MwlareBuilder needs its own
