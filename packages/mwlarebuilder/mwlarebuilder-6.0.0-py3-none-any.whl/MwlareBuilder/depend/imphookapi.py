...
#-----------------------------------------------------------------------------
"""
Classes facilitating communication between MwlareBuilder and import hooks.

MwlareBuilder passes instances of classes defined by this module to corresponding functions defined by external import
hooks, which commonly modify the contents of these instances before returning. MwlareBuilder then detects and converts
...
from MwlareBuilder.building.utils import format_binaries_and_datas
from MwlareBuilder.lib.modulegraph.modulegraph import (RuntimeModule, RuntimePackage)
...
class PreSafeImportModuleAPI:
    ...
    or C extension and all transitive imports thereof to the module graph) back to MwlareBuilder.
...
class PreFindModulePathAPI:
    ...
    _before_ a call to `ModuleGraph._find_module_path()` finding the hooked module's absolute path) back to MwlareBuilder.
...
class PostGraphAPI:
    ...
    this application has been constructed) back to MwlareBuilder.
