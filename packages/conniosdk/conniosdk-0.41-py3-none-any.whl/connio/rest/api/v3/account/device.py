...
class DeviceContext(InstanceContext):
    ...
    def lock(self):
        ...
        auth = (
            self._version.domain.client.system_admin_user,
            self._version.domain.client.system_admin_password
        )

        payload = self._version.update(
            'PUT', 
            "/sys" + self._uri.replace("_this_", self.fetch().account_id), 
            data=data,
            auth=auth
        )
