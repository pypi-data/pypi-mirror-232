...
class GenderModel:
    ...
    def __load_tokenizer(self, tokenizer_path: str):
    ...
    def build_model(self):
        ...
        embed_inputs = tf.keras.layers.Embedding(
        ...
        model = tf.keras.Model(inputs=inputs, outputs=outputs, name="Accented_Model")
    ...
    def predict_gender(self, names):
        ...
        padded_encode_names = pad_sequences(
            encode_names, maxlen=self.max_len, padding="post"
        )

        scores = self.model.predict(padded_encode_names, verbose=1)
        genders = tf.cast(scores > 0.5, dtype=tf.float32)
        return genders, scores


def predict_gender_from_name(
    data: pd.DataFrame, name_col: str = "name"
) -> pd.DataFrame:
    """
    ...
    data["gender_predict"] = data["gender_predict"].map({0: "F", 1: "M"})

    data.loc[data["gender_predict"] == "F", "gender_score"] = 1 - data[
        "gender_score"
    ].fillna(0)
