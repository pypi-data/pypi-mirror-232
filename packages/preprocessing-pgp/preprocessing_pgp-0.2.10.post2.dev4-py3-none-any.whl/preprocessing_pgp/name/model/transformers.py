...
class TransformerModel:
    ...
    def build_model(self, *arg, **kwargs):
        ...
        x = PositionalEmbedding(self.sequence_length, self.vocab_size, self.embed_dim)(
            encoder_inputs
        )
        encoder_outputs = TransformerEncoder(
            self.embed_dim, self.dense_dim, self.num_heads, self.drop_out_enc
        )(x)
        ...
        x = TransformerDecoder(
            self.embed_dim, self.dense_dim, self.num_heads, self.drop_out_dec
        )(x, encoder_outputs)
    ...
    def predict(self, input_sentence):
        ...
        for i in range(min(self.sequence_length, len(input_word))):
            ...
            sampled_token = (
                self.original_index_lookup[sampled_token_index]
                if sampled_token_index != 1
                else input_word[i]
            )
    ...
    def predict_multi(
        ...
        if multiprocessing:
            with Pool(processes=n_cpu) as pool:
                pred_sentences = list(
                    tqdm(
                        pool.imap(self.predict, sentences),
