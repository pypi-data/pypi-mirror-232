...
def clean_email_name(data: pd.DataFrame, name_col: str = "email_name") -> pd.DataFrame:
    """
    ...
    data.loc[data[name_col].apply(is_name_accented), f"cleaned_{name_col}"] = None

    # * Removing names with spaces
    data.loc[
        data[name_col].str.contains(r"\s+", regex=True), f"cleaned_{name_col}"
    ] = None

    # * Remove digit, '.' from name & lower name
    clean_data_mask = data[f"cleaned_{name_col}"].notna()
    data.loc[clean_data_mask, f"cleaned_{name_col}"] = (
        data.loc[clean_data_mask, name_col]
    ...
    data.loc[data[f"cleaned_{name_col}"] == "", f"cleaned_{name_col}"] = None

    return data


def sort_series_by_appearance(series: pd.Series) -> pd.Series:
    """
...
def extract_sub_string(
    sub_regex: str, string: str, take_first: bool = True
) -> Tuple[str, str]:
    """
