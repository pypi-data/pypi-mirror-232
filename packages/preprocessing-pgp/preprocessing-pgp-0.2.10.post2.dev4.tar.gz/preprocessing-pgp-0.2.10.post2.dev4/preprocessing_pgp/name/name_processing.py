...
import pandas as pd
from preprocessing_pgp.name.const import ADHOC_NAME_ELEMENTS, WITH_ACCENT_ELEMENTS
...
from preprocessing_pgp.name.utils import choose_best_name, remove_nicknames
...
class NameProcessor:
    ...
    def __is_adhoc_middle_name(self, name: str) -> bool:
        if len(name.split(" ")) == 1:
            return False
        return name[1] in ADHOC_NAME_ELEMENTS
    ...
    def fill_accent(
        ...
        adhoc_middle_name_mask = name_df[name_col].apply(self.__is_adhoc_middle_name)
        good_accent_name_df = name_df[good_name_mask & ~adhoc_middle_name_mask]
        bad_accent_name_df = name_df[~good_name_mask | adhoc_middle_name_mask]
        ...
        if not bad_accent_name_df.empty:
            ...
            # start_time = time()
            predicted_name["predict"] = predicted_name[
                f"clean_{name_col}"
            ].progress_apply(self.predict_non_accent)
            ...
            predicted_name["final"] = predicted_name.apply(
                lambda row: choose_best_name(row[f"clean_{name_col}"], row["final"]),
                axis=1,
            )
