...
def parse_dataset(dataset: DatasetType) -> Tuple[Path, DatasetFormat]:
    ...
    if isinstance(dataset, (dict, list)):
        dataset_rows = (
            transpose_dict_to_rows(dataset) if isinstance(dataset, dict) else dataset
        )
        if is_pyarrow_available:
            with NamedTemporaryFile(
                mode="wb", suffix=f".{DatasetFormat.feather}", delete=False
            ) as dataset_file:
            ...
            with NamedTemporaryFile(
                mode="wt", suffix=f".{DatasetFormat.csv}", delete=False
            ) as dataset_file:
    ...
    if dataset_path.suffix.endswith(DatasetFormat.csv):
        ...
        reader = DictReader(dataset_path.open("rt"))
