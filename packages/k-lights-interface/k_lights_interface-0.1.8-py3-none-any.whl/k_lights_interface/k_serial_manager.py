...
class KSerialManager(metaclass=KSingletonMeta):
    ...
    def connect_to_all(self, remove_disconnected : bool = False) -> OrderedSet[KDevice]:
        ...
        for port in possible_ports:
            device = self.connect_to_device(port)
            if not device:
                continue
            mcumgr_conn_string = self.__get_mcumgr_conn_string(port, device.serial_number)
            if not mcumgr_conn_string:
                logger.warning(f"Could not find mcumgr port for device {device.name} with serial number {device.serial_number}")
                continue
            device.mcumgr_conn_string = mcumgr_conn_string
        return self.devices


    def connect_to_device(self, port: ListPortInfo, remove_disconnected : bool = False) -> KDevice | None:
