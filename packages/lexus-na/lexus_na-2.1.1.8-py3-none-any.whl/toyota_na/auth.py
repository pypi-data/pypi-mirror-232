...
from .exceptions import LoginError, NotLoggedIn, TokenExpired, OtpRequired
...
class ToyotaOneAuth:
    ACCESS_TOKEN_URL = "https://login.lexusdriverslogin.com/oauth2/realms/root/realms/tmna-native/access_token"
    AUTHORIZE_URL = "https://login.lexusdriverslogin.com/oauth2/realms/root/realms/tmna-native/authorize"
    AUTHENTICATE_URL = "https://login.lexusdriverslogin.com/json/realms/root/realms/tmna-native/authenticate"
    ...
    async def authorize(self, username, password, otp=None, data={}):
    ...
    async def login(self, username, password, otp=None, data={}):
        auth_result = await self.authorize(username, password, otp, data)
        if self.is_otp_required:
            return auth_result
        else:
            await self.request_tokens(auth_result)
