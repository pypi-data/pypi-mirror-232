...
def start_report(
    ...
    """
    Globals.current_report = report_class(name, report_filename)
    return Globals.current_report
...
def stop_report():
    ...
    """
    if Globals.current_report is not None:
        Globals.current_report.end_report()
...
def log(text: str, log_type="LOG"):
    ...
    """
    Globals.current_report.log(text, log_type)
...
def passed(description, details=""):
    ...
    """
    Globals.current_report.passed(description, details)
...
def attach_image(image, name, description) -> Path:
    ...
    file_name = Globals.current_report.get_folder() / 'screenshots' / 'attached' / file_name
    image.save(file_name)
    Globals.current_report.log('Attached image "{name}" has been saved to "{file_name}"')
    Globals.current_report.log(description)
    return file_name
