...
from ..lib import check_types, multiprocess_map, export_eopatch_to_tiff, df_to_csv_manual, load_bbox
...
def lai_to_csv_px(eop_paths:list, patchlet_dir:str, outdir:str, n_jobs:int=16, delete_patchlets:bool=True):
    ...
    """
    print("1. Splitting tiles into patchlets")
    patchlet_size = (1128,1128)
    buffer = 0
    for eop_path in tqdm.tqdm(eop_paths, total=len(eop_paths)):
        success = split_patch_into_patchlets(eop_path=eop_path, patchlet_size=patchlet_size, 
        buffer=buffer, output_dir=patchlet_dir)
        if success is not None:
            print(f"Error with {eop_path}")
    
    # 2. Combine the data for each patchlet
    print("2. Combining dates per patchlet")
    ...
    for pname in tqdm.tqdm(pnames, total=len(pnames)):
    ...
    patchlet_paths.sort()
    for ppath in tqdm.tqdm(patchlet_paths, total=len(patchlet_paths), desc="Extracting timeseries per patchlet"):
...
def load_fields(field_path:str, nrows:int = None, min_area:int = 0, max_area:int = 500000):
    if nrows is None:
        df = gpd.read_file(field_path)
    else:
        df = gpd.read_file(field_path, rows=slice(0,nrows)) 

    # Filter out fields that are too small or too large
...
def field_to_csv(fields:gpd.GeoDataFrame, tiff_path:str,
    ...
    df.to_csv(outpath, index=True, mode=wmode, header=(wmode=="w"))

@check_types
def lai_to_csv_field(eop_paths:list, fields_path:str, outdir:str, nfields:int = None, n_jobs:int = 8, delete_tmp:bool=True):
        # Make temporary tiff dir
        tif_dir = os.path.join(outdir, "tmp_tiffs")
