...
use crate::translations::rust::{Zion, CustomNodeCreateOpts, DenoCodeNodeCreateOpts, GraphBuilder, Handler, NodeHandle, PromptNodeCreateOpts, VectorMemoryNodeCreateOpts};
...
impl Finalize for NodeZion {}

impl NodeZion {

    fn js_new(mut cx: FunctionContext) -> JsResult<JsBox<NodeZion>> {
        ...
        debug!("Creating new Zion instance with file_id={}, url={}, api_token={:?}", file_id, url, "".to_string());
        Ok(cx.boxed(NodeZion {
            c: Arc::new(Mutex::new(Zion::new(file_id, url))),
        }))
    }
    ...
    fn start_server(mut cx: FunctionContext) -> JsResult<JsPromise> {
        let channel = cx.channel();
        let self_ = cx.this()
            .downcast_or_throw::<JsBox<NodeZion>, _>(&mut cx)?;
        let (deferred, promise) = cx.promise();
...
//     // TODO: adding a node should also display any errors
    ...
    fn commit(mut cx: FunctionContext) -> JsResult<JsPromise> {
        ...
        let self_ = cx.this()
            .downcast_or_throw::<JsBox<NodeGraphBuilder>, _>(&mut cx)?;
        let node_zion = cx.argument::<JsBox<NodeZion>>(0)?;
        let branch = cx.argument::<JsNumber>(1).unwrap_or(JsNumber::new(&mut cx, 0.0)).value(&mut cx) as u64;

        let c = Arc::clone(&node_zion.c);
        let g = Arc::clone(&self_.g);

        let rt = runtime(&mut cx)?;
        rt.spawn(async move {
            let mut graph_builder = g.lock().await;
            let mut zion = c.lock().await;
            let m = graph_builder.commit(&mut zion, branch).await;
            deferred.settle_with((&channel), move |mut cx| {
                if let Ok(result) = m {
                    neon_serde3::to_value(&mut cx, &result)
                        .or_else(|e| cx.throw_error(e.to_string()))
...
fn main(mut cx: ModuleContext) -> NeonResult<()> {
    ...
    cx.export_function("zionNew", NodeZion::js_new)?;
    cx.export_function("zionStartServer", NodeZion::start_server)?;
    cx.export_function("zionPlay", NodeZion::play)?;
    cx.export_function("zionPause", NodeZion::pause)?;
    cx.export_function("zionBranch", NodeZion::branch)?;
    cx.export_function("zionQuery", NodeZion::query)?;
    cx.export_function("zionGraphStructure", NodeZion::display_graph_structure)?;
    cx.export_function("zionRegisterCustomNodeHandle", NodeZion::register_custom_node_handle)?;
    cx.export_function("zionRunCustomNodeLoop", NodeZion::run_custom_node_loop)?;
