...
impl PyZion {
    ...
    fn new(file_id: String, url: String, api_token: Option<String>) -> Self {
        debug!("Creating new Zion instance with file_id={}, url={}, api_token={:?}", file_id, url, api_token);
        let c = Zion::new(file_id.clone(), url.clone());
        PyZion {
...
impl PyGraphBuilder {
    ...
    fn commit<'a>(
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let exec_status = graph_builder.commit(&zion, branch).await
...
#[pymodule]
#[pyo3(name = "_zion")]
fn zion(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    // pyo3_log::init();
    m.add_class::<PyZion>()?;
