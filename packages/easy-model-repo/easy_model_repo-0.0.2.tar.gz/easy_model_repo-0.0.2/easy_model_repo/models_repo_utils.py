...
ADMIN_HOST = "dev06.ucd.qzm.stonewise.cn:30020"
...
class EasyModelRepo:
    ...
    def get_model(self, model_name, version, destination_path):
        if version == "":
        ...
        res = self._get_storage_path_req(model_name, version)
    ...
    def _delete_model_version_req(self, model_name):
        base_url = "http://{host}/api/v1/model/{model_name}/version" \
    ...
    def _get_storage_path_req(self, model_name, version):
        base_url = "http://{host}/api/v1/model/{model_name}/{version}/storage" \
            .format(host=ADMIN_HOST, model_name=model_name, version=version)
        res = requests.get(url=base_url)
        print(res.text)
        return res
