# Tea, a version control
# Copyright (C) 2023 Lizardwine
...
def main() -> int:
    ...
    if args.init:
        ...
        if not author and not author_email:
            global_author_info = tea_config.get_author_info()
        ...
        try:
            tea.create_repository(author, author_email)
        ...
        if local_flag:
            if not tea.check_repo_files():
                print('\033[31merror: repository not found\033[m')
                print('(use "tea init" to create a new repository)')
                return 1

            tea.write_author_info(author, author_email)
        else:
            tea_config.write_author_info(author, author_email)
    else:
        # catch exception for all arguments below
        try:
            if args.status:  # print files status
                status = tea.get_files_status()
                ...
                if new_files:
                    ...
                    print('  (use "tea commit" argument to commit files):\n')
                ...
                if untracked:
                    ...
                    print('  (use "tea add" argument to add files):\n')
                ...
                tracked_info = tea.get_tracked_files()
                ...
                if not files_to_add:
                    print('\033[31merror: no file specified to track (use "tea add" argument and '
                ...
                if all_tracked_files_flag:
                    ...
                    for filepath in tracked_info.keys():
                        if tea.file_has_changed(filepath):
                ...
                try:
                    commit, commit_hash = tea.commit(files, message)
                ...
                tracked_files = tea.get_tracked_files()

                for filepath in tracked_files.keys():
                    current_lines = tea.index_file_lines(filepath)
                    previous_lines = tea.join_file_changes(filepath)
                    difference = tea.get_lines_difference(previous_lines, current_lines)
            ...
            print('\033[31merror: no ".tea" repository found in this directory\033[m')
            return 1
