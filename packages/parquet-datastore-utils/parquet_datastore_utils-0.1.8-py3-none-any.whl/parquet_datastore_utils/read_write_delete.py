...
def read(uri: str,
    ...
    Returns
    -------
    pandas.Dataframe : stored data, with timezone-aware DatetimeIndex
    """
    ...
    if start_end is not None:
        index_name = dataset.schema.pandas_metadata['index_columns'][0]
        start, end = start_end
        if start > end:
            raise ValueError(f'Timestamp "start" must be equal or less than "end". You provided '
                             f'start={start.isoformat()}, end = {start.isoformat()}.')

        filters = list(kwargs['filters']) if 'filters' in kwargs else []
        if filters:
            if any([f[0] for f in kwargs['filters']]):
        ...
        def to_utc(d):
            # Return timezone-naive timestamp in UTC, as dataset was stored like that.
            if d.tzinfo is None:
                return d  # Timezone-naive: interpret as UTC
            return d.astimezone(pytz.utc).replace(tzinfo=None)

        filters.append((index_name, '>=', to_utc(start_end[0])))
        filters.append((index_name, '<=', to_utc(start_end[1])))
        kwargs['filters'] = filters

    def read_df() -> pd.DataFrame:
        df = pd.read_parquet(uri, columns=columns, **kwargs)
        # Data was stored in timezone-naive UTC, return timezone-aware
        if len(df):
...
def write(data: pd.DataFrame,
    ...
    if not isinstance(data.index, pd.DatetimeIndex):
        raise ValueError(f'Can only write DataFrame with a DatetimeIndex. You passed a {type(data.index)}.')
    # If DataFrame is timezone-naive
    if data.index.tzinfo is None:
        pass  # Nothing to do, assume it's UTC
    else:
        # Convert to and store data as timezone-naive UTC timestamps
        data.index = data.index.tz_convert(pytz.utc).tz_localize(None)
    ...
    try:
        ...
        if len(update_tables) > 0:
            ...
            data_after = existing_table.filter((pc.field(index_name) > data.index.max()))
...
def delete_between(uri: str,
    ...
    start, end : dict
        ...
        Dict keys are "timestamp" (start / end timestamps, timezone-aware) and a value for each of the partition_cols.
    ...
    Raises
    ------
    ValueError : If start and/or end are not timezone-aware or start > end.

    Notes
    -----
    - This assumes that data have been stored in UTC, as timezone-naive timestamps.
    """
    ...
    if not start['timestamp'].tzinfo or not end['timestamp'].tzinfo:
        raise ValueError('Both "start" and "end" timestamps must be timezone-aware.')

    to_utc = lambda d: d['timestamp'].astimezone(pytz.UTC).replace(tzinfo=None)
    start_utc, end_utc = (to_utc(start), to_utc(end))
    if start_utc > end_utc:
        raise ValueError(f'Timestamp "start" must be equal or less than "end". You provided '
                         f'start={start_utc.isoformat()}, end = {start_utc.isoformat()}.')
