...
class AudioWave:
    ...
    def __eq__(self, __o: object) -> bool:
        if isinstance(__o, AudioWave):
            return (np.array_equal(self.array, __o.array) and self.bit == __o.bit
    ...
    def from_file(
        ...
        """A static method to generate an AudioWave object from a file.
    ...
    def read_file_metadata(cls, filepath: str) -> Tuple[int, int, int]:
        """A static method to read the metadata of an audio file.
    ...
    @classmethod
    def buffer_generator_from_file(cls,
                                   filepath: str,
                                   buffer_size: int = 1024 * 1024 * 8):
        """Return a generator that yields AudioWave objects from a file. The
        generator will read the file in chunks of `buffer_size` bytes.
    ...
    @classmethod
    def from_bytes(cls, raw_data: bytes, bit: int, channels: int,
                   samplerate: int) -> Self:
        """A static method to generate an AudioWave object from a stream of bytes.
        ...
        data = np.array(
            [
                int.from_bytes(
                    raw_data[i:i + bit // 8], byteorder="little", signed=True)
    ...
    def set_sample_rate(self, samplerate: int) -> None:
        ...
        """
        self.array = librosa.resample(self.array,
    ...
    def number_of_frames(self) -> int:
        """Get the number of frames in the audio. A frame is a sample for each
        channel. In a stereo audio, a frame is composed by two samples, one for
        each channel, in the form [left, right], where left and right are the
        samples for the left and right channels respectively.
    ...
    def duration(self) -> float:
        r"""Get the duration of the audio in seconds. The duration is calculated by
        dividing the number of frames by the sample rate.
    ...
    def rms(self,
            mode: Criterion = Criterion.MAX,
            scale: Literal["db", "normal"] = "normal") -> float:
        r"""The root-mean-square of the audio. It measures the power level of the
    ...
    def db_rms(self, mode: Criterion = Criterion.MAX) -> float:
        """A wrapper method for ``self.rms()`` that converts the returned value to
        decibels.
    ...
    def get_silence_slices(self, noise_list: Iterable[Noise],
