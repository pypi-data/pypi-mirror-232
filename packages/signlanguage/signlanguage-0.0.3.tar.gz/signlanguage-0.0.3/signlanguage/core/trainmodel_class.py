            ...
            for i, dm in enumerate(data_model):
                if len(dm) > 0:
                    ...
                    for i, dm_pos in enumerate(dm):
                        ...
                        #init core pool
                        KmeanModel = mk.CorePool(n_clusters=n_clusters, max_iter=len(data)*len(data_index))#len(dm)*len(data)*len(data_index))
                        #train core pool
            ...
            for i, dm in enumerate(data_model):
                if len(dm) > 0:
                    ...
                    for i, dm_pos in enumerate(dm):
                        ...
                        data_Train = np.array(df_real.values.tolist())
                        #init core pool
                        KmeanModel = mk.CorePool(n_clusters=n_clusters, max_iter=len(dm)*len(data)*len(data_index))
                        #train core pool
                        KmeanModel.fit(data_Train)
                        #add core pool, matrix values
                        KmeansModel.append(KmeanModel)
...
class TrainModelStates(Itrain):
    ...
    def Train_models(self, data=None, lenght_valuation=None):
        try:
            ...
            num_datos = data_operations.get(True, 1500)
