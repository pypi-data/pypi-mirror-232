...
# region sudo_owner_filter
async def sudo_owner_filter(_, __: "fipper.Client", m: Message):
    return bool(m.from_user and m.from_user.id in await __.get_sudo() and m.from_user.id == int(__.get_owner()))


sudo_owner = create(sudo_owner_filter)
...
# region sudo_filter
async def sudo_filter(_, __: "fipper.Client", m: Message):
    return bool(m.from_user and m.from_user.id in await __.get_sudo())
...
# region owner_filter
async def owner_filter(_, c: "fipper.Client", m: Message):
    owner = c.get_owner()
    return bool(m.from_user and m.from_user.id == int(owner))


owner = create(owner_filter)
"""Filter messages generated by Owner Bot."""
...
# region ayiin_filter
def ayiin(commands: Union[str, List[str]], case_sensitive: bool = False):
