...
from chainmaker.keys import ArchiveConfig, SystemContractNames
...
from chainmaker.utils.gm.sm3 import sm3_hash
...
def get_hmac(chain_id: str, block_height: int, full_block_data: bytes, secret_key: str) -> str:
    ...
    data += uint64_to_bytes(block_height)
...
class ArchiveUtils:
    ...
    def insert_db_archive_block_data(self, block_height: int):
        create_time = modify_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def get_db_archived_block_height(self):
        sql = 'SELECT v FROM sysinfos WHERE k="archived_block_height";'
        return self.query(sql)
    
    def update_db_archived_block_height(self, block_height: int):
        sql = f'UPDATE sysinfos SET v = {block_height} WHERE k="archived_block_height";'
        self.execute(sql)
    ...
    def is_config_tx(self, tx: Transaction):
        if tx is None:
            return False
        return  SystemContractNames.CHAIN_CONFIG == tx.payload.contract_name
