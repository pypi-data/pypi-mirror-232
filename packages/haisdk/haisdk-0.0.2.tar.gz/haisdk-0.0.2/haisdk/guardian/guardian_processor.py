...
class GuardianProcessor:
    ...
    endpoint = "/unauthenticated/messages/analyse"
    ...
    def validate_request(json_data):
        ...
        if len(json_data["data"]["message_groups"]) % 2 != 0:
            raise Exception(
                "Expected field message_groups should be grouped in batches of size 2"
            )

    def prepare_request(self, json_data):
        url = self.guardian_base_url + self.endpoint
        headers = {
    ...
    def analyse(self, json_data):
        ...
        url, headers, json_data = self.prepare_request(json_data)
