...
def ignored_url(request: Union['HttpRequest', 'HttpResponse']) -> bool:
    ...
    :return: Boolean
    """
    static_media_regex = re.compile(r"{}".format(settings.ignore_urls_regex))
    request_path = request.get_full_path()
    return request_path.strip('/') in settings.ignore_urls or static_media_regex.match(request_path)
