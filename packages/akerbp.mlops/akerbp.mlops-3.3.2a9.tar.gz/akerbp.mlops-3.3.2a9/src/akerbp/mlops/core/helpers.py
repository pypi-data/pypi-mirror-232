...
def input_data_validation(
    required_input: List[str], input: Dict, client: CogniteClient
) -> bool:
    ...
    for i, well_data in enumerate(input_wells):
        try:
            ...
            if len(curves_in_well_data) == 0:
                try:
                    bytes_data = client.files.download_bytes(
                        external_id=well_data["input_file_reference"]
                    )
                    curves_in_well_data = list(
                        json.loads(bytes_data)["input"][i]["input_logs"].keys()
                    )
                except Exception as e:
                    if well_data["input_file_reference"] == "":
                        return True
                    else:
                        raise ValueError(
                            f"Failed to load input file from CDF for input validation ({well_data['input_file_reference']})"
                        ) from e
