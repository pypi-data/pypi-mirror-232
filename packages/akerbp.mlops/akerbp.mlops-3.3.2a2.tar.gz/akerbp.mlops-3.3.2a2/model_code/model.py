...
def predict(data_dict, init_object, secrets) -> Any:
    ...
    for well_data in data_dict["input"]:
        ...
        # TODO: modify to use input file reference
        if len(well_data["input_logs"]) != 0:
            logs = well_data["input_logs"]
        else:
            # TODO: download from CDF, refer to input_file_reference
            bytes_data = client.files.download_bytes(
                external_id=well_data["input_file_reference"]
            )
            logs = pd.DataFrame.from_dict(json.loads(bytes_data))
