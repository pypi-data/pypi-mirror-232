...
def get_model_test_data(test_import_path: str) -> Tuple[Dict, Dict, bool]:
    """
    ...
    input_file_reference = getattr(
        service_test, f"{service_name}_input_file_reference", {}
    )
    check = getattr(service_test, f"{service_name}_check")
    return input, input_file_reference, check
