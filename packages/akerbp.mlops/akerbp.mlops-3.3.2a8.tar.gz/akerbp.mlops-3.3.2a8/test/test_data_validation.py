...
def test_input_data_validation_no_required_input_read_from_settings_file_only_well_in_payload(
    ...
    for setting in project_settings:
        ...
        validated = input_data_validation(
            required_input=required_input, input=input_with_well_only, client=client
        )
        if validated:
...
def test_input_data_validation_with_input_file_reference():
    input_with_logs = {
        "input": [
            {
                "well": "well-123",
    ...
    input_to_upload = input_with_logs["input"][0]["input_logs"]
    content = str(json.dumps(input_to_upload))
    id = uuid.uuid4().hex
    external_file_id = f"test_data_validation_input_{id}.binary"
    client_write.files.upload_bytes(
        content,
        name=external_file_id.split(".")[0],
        external_id=external_file_id,
    )

    input_with_logs["input"][0]["input_logs"] = {}
    input_with_logs["input"][0]["input_file_reference"] = external_file_id
    validated = input_data_validation(
        required_input=required_input,
        input=input_with_logs,
        client=client,
    )
    client_write.files.delete(external_id=external_file_id)
    assert validated
