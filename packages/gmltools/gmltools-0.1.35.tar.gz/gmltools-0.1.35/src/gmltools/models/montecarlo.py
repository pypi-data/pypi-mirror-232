...
class MontecarloInterval:
    ...
    def plot_montecarlo_hourly(self,n_iterations:int=1000,percentiles:list=[10,90],figsize=(2200,600)):
        ...
        # Pronóstico promedio
        mean_forecast = np.mean(self.simulated_paths, axis=0)
        ...
        #if tomorrow in sppot index
        if (datetime.datetime.now() + datetime.timedelta(days=1)).strftime("%Y-%m-%d") in self.spot.index:
