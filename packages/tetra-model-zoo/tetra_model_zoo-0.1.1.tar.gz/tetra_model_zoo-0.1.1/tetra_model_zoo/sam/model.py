...
from tetra_model_zoo.utils.asset_loaders import maybe_clone_git_repo
from tetra_model_zoo.utils.input_spec import InputSpec

SAM_SOURCE_REPO = "https://github.com/tetraai/segment-anything"
SAM_SOURCE_REPO_COMMIT = "db44f30e01c020b2522c6889186198e7c49a51a4"
...
class SAMTetraWrapper:
    """
    Tetra version of segment-anything (https://github.com/tetraai/segment-anything)

    Tetra fork modifies following from parent segment-anything repo:
    ...
    def __init__(self, model_type: str = DEFAULT_MODEL_TYPE):
        (
            self.sam_model_registry,
            self.SamOnnxModel,
            self.ResizeLongestSide,
            self.SamPredictor,
        ) = _patch_sam_with_tetra_modules()
        self.sam = load_sam_model(self.sam_model_registry, model_type)

    def get_sam(self) -> torch.nn.Module:
        return self.sam


class SegmentAnythingEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.sam = sam_tetra_wrapper.get_sam()
        self.transforms = sam_tetra_wrapper.ResizeLongestSide(
    ...
    def get_input_spec(
        self,
        image_size: tuple[int, int] | int = (720, 1280),
    ) -> InputSpec:
        ...
        preprocessed_image = self.preprocess_input_image(
            np.ones((*image_size, 3), dtype=np.uint8)
        )
...
class SegmentAnythingONNXDecoder(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.sam = sam_tetra_wrapper.get_sam()
        self.sam_decoder = sam_tetra_wrapper.SamOnnxModel(
...
def load_sam_model(
    ...
    weights_path = os.path.join(cwd, weights_url.split("/")[-1])
...
def _patch_sam_with_tetra_modules():
    """
    Patches segment-anything with tetra modifications
    from https://github.com/tetraai/segment-anything
    ...
    sam_repo_path = maybe_clone_git_repo(
        SAM_SOURCE_REPO, SAM_SOURCE_REPO_COMMIT, MODEL_NAME
    )
    cwd = os.getcwd()
    try:
