...
def _notify(message, subject):
    ...
    if SECTION_MAIL not in cfg:
        LOGGER.debug(
            f"missing section [{SECTION_MAIL}]: not sending success "
            "message: {message}"
        )
        return
...
def write_slurm_job_file(dryrun, force, verbose, restore, mail, plan):
    ...
    if mail is not None:
        ...
        if email:
            ...
            warnings.warn(
                "No email address found in configuration, slurm will"
                "not send any emails"
            )
    ...
    venv_manager = config.get(
        "slurm", "virtual_env_manager", fallback="pipenv"
    )
    ...
    try:
        ...
        if type(content) == list or type(content) == tuple:
            ...
            try:
                ...
                LOGGER.warning(
                    "Could not find representation for item %s", str(content)
                )
                cleaned = content.__class__.__name__
