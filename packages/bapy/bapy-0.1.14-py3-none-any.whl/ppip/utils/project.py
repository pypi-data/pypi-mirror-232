...
class Project:
    ...
    # TODO: ver que hacer con los requirements de pyproject.toml
    ...
    def latest(self) -> str:
        ...
        latest = stdout(f"{self.git} describe --abbrev=0 --tags @^") or ""
        if not latest:
            latest = "0.0.0"
            self.commit()
            self.tag(latest)
    ...
    def publish(self, part: Bump = Bump.PATCH, force: bool = False, tox: bool = True):
        ...
        if tox:
            if rc := self.tox() != 0:
                sys.exit(rc)
        self.commit()
        if (n := self.next(part=part, force=force)) != self.latest():
            self.tag(n)
            self.push()
            if rc := self.twine() != 0:
                sys.exit(rc)
            self.log.info(f"{self.publish.__name__}: {n}")
        self.clean()
    ...
    def twine(self, part: Bump = Bump.PATCH, force: bool = False, ) -> int:
        ...
        Args:
            ...
            force: force bump
        """
        if (self.pyproject_toml.file and (self.version() != self.next(part=part, force=force))
