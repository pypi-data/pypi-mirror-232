...
class BookSystemSDK:
    ...
    def refresh(self, obj: TypeModel, by: TypeModel | None = None, by_id: int | None = None,  **kwargs) -> TypeModel:
        if by and by_id:
            url = f"{self.api_url}{by.base_path}{by_id}{obj.base_path}"
            return obj.from_json(self._make_request(url=url, method="PATCH", body=obj.body, params=kwargs))
        else:
    ...
    def get(self, model: TypeModel, by: TypeModel | None = None, by_id: int | None = None,  **kwargs) -> TypeModel:
        ...
        if isinstance(json, list):
            return [model.from_json(obj) for obj in json]
