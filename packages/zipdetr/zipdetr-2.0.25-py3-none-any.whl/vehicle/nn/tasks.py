...
class Head(torch.nn.Module):
    ...
    def forward(self, inputs):
        ...
        group_indices = max_indices.clone()
        group_indices[(group_indices >= 0) & (group_indices <= 3)] = 0
        group_indices[(group_indices >= 4) & (group_indices <= 6)] = 1
        group_indices[group_indices == 7] = 2
        group_indices[(group_indices >= 8) & (group_indices <= 11)] = 3
        max_values = torch.unsqueeze(max_values, -1).float()
        max_indices = torch.unsqueeze(max_indices, -1).float()
        group_indices = torch.unsqueeze(group_indices, -1).float()
        output = torch.cat([boxes, max_values, max_indices, group_indices], dim=-1)
        print(output.shape)
