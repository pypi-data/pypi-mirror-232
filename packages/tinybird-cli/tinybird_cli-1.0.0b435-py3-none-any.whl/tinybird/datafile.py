...
class CLIGitRelease:
    ...
    def diff_datafiles(self, current_ws_commit: str) -> List[Diff]:
        ...
        return list(filter(lambda d: d.change_type in [c.value for c in self.ChangeType] and d.a_path and d.a_path not in self.NO_DATAFILES_PATHS and Path(d.a_path).resolve().suffix in self.DATAFILES_SUFFIXES,
...
async def folder_push(
    ...
    if git_release:
        ...
        click.echo(FeedbackManager.info_git_release_diffs(workspace=current_ws['name'], current_release_commit=current_release['commit'],
