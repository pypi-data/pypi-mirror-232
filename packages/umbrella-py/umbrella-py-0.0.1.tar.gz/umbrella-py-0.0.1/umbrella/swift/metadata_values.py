...
class ProtocolContextDescriptorFlags(FlagSet):
    ...
    def get_sepcial_protocol_kind(self) -> SpecialProtocol:
...
class TypeContextDescriptorFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        ...
        self.has_vtable = self.get_flag(15)

        # Set if the context descriptor includes metadata for dynamically
        # installing method overrides at metadata instantiation time.
        self.has_override_table = self.get_flag(14)

        # Set if the context descriptor is for a class with resilient ancestry.
        #
        # Only meaningful for class descriptors.
        self.has_resilient_super_class = self.get_flag(13)

        # Whether the immediate class members in this metadata are allocated
        # at negative offsets.  For now, we don't use this.
        self.immediate_members_negative = self.get_flag(12)

        # The kind of reference that this class makes to its resilient superclass
        # descriptor.  A TypeReferenceKind.
        #
        # Only meaningful for class descriptors.
        self.resilient_superclass_reference_kind = from_flags(
        ...
        self.has_import_info = self.get_flag(2)

        # Set if the type descriptor has a pointer to a list of canonical
        # prespecializations.
        self.has_canonical_metadata_prespecializations = self.get_flag(3)

        # Set if the metadata contains a pointer to a layout string
        self.has_layout_string = self.get_flag(4)

        # Set if the class is an actor.
        #
        # Only meaningful for class descriptors.
        self.is_actor = self.get_flag(7)
        ...
        #
        # Only meaningful for class descriptors.
        self.is_default_actor = self.get_flag(8)

        # Whether there's something unusual about how the metadata is
        # initialized.
        #
        # Meaningful for all type-descriptor kinds.
        self.meta_initialization = from_flags(
...
class ProcolRequirementFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        ...
        self.kind = from_flags(ProtocolRequirementKind, __flags, mask=0x0F)
        self.instance = self.get_flag(4)
...
class FieldRecordFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        ...
        self.is_var = self.get_flag(1)
        # Is this an indirect enum case?
        self.is_indirect_case = self.get_flag(0)
        # Is this an artificial field?
        self.is_artificial = self.get_flag(2)
...
class AnonymousContextDescriptorFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        ...
        self.has_mangled_name = self.get_flag(0)
...
class MethodDescriptorFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        self.flags = __flags
        self.kind = from_flags(MethodDescriptorKind, __flags, mask=0x0F)

        # Is the method marked 'dynamic'?
        self.dynamic = self.get_flag(5)
...
class ContextDescriptorFlags(FlagSet):
    def __init__(self, __flags: int) -> None:
        ...
        self.version = self.get_field(8, (1*8))
        # The most significant two bytes of the flags word, which can have
        # kind-specific meaning.
        self.kind_flags = self.get_field(16, (2*8))
