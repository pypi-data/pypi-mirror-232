...
class TargetContextDescriptor(Virtual):
    ...
    # IMPORTANT: This method is used to determine the parent
    ...
    def get_type(kind: ContextDescriptorKind) -> t.Type[TargetContextDescriptor]:
        ...
        if kind == ContextDescriptorKind.Module:
            #: Basic module descriptor
            cls = TargetModuleContextDescriptor
        elif kind == ContextDescriptorKind.Extension:
            cls = TargetExtensionContextDescriptor
        elif kind == ContextDescriptorKind.Anonymous:
            cls = TargetAnonymousContextDescriptor
        elif kind == ContextDescriptorKind.Class:
            cls = TargetClassDescriptor
        elif kind == ContextDescriptorKind.Protocol:
            cls = TargetProtocolDescriptor
        elif kind == ContextDescriptorKind.Enum:
            cls = TargetEnumDescriptor
        elif kind == ContextDescriptorKind.Struct:
            cls = TargetStructDescriptor
        elif kind == ContextDescriptorKind.Opaque:
            cls = TargetOpaqueTypeDescriptor
        return cls
...
class TargetModuleContextDescriptor(TargetContextDescriptor):
    ...
    name: RelativeDirectPointer[str, NotNull] = csd.csfield(
        #: Note the use of 'String' here. The default 'Runtime' class implements
...
@dc.dataclass
class TargetOpaqueTypeDescriptor(
    TargetContextDescriptor,
    TrailingGenericContextObjects[
        TargetGenericContextDescriptorHeader,
        *TargetGenericSignature,
...
class TargetVTableDescriptorHeader(Virtual):
    ...
    def get_vtable_offset(self, description: TargetClassDescriptor) -> int:
...
class TargetClassMetadata(TargetAnyClassMetadata):
    ...
    #: dynamically.
    description: DirectPointer[TargetClassDescriptor, Nullable] = csd.csfield(
        DirectPointer(TargetClassDescriptor, True)
    )
...
class TargetProtocolConformanceDescriptor(
    ...
    protocol: RelativeIndirectablePointer[
        TargetProtocolDescriptor, Nullable
    ] = csd.csfield(RelativeIndirectablePointer(TargetProtocolDescriptor, True))

    def get_protocol(self) -> t.Optional[TargetProtocolDescriptor]:
