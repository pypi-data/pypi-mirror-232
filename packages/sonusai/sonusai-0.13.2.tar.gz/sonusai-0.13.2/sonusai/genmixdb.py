...
def genmixdb(location: Location,
    ...
    mixdb.mixtures = pp_tqdm_map(_process_mixture, range(total_mixtures), progress=progress, no_par=False)
    progress.close()
    ...
    if logging:
        ...
        logger.info('')
        logger.debug(f'read_audio cache info: {read_audio.cache_info()}')
        logger.debug('')
