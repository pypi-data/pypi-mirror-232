...
def load_checkpoint(filename,
    ...
    if optimizer is not None:
        if 'optimizer' in checkpoint:
            ...
            logger.warning(f'The state dict of optimizer cannot be found in checkpoint file: {filename}')

    if lr_scheduler is not None:
        if 'lr_scheduler' in checkpoint:
            lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
        else:
            logger.warning(f'The state dict of lr_scheduler cannot be found in checkpoint file: {filename}')
...
def save_configuration(target_folder, config: Dict):
    ...
    config_file = os.path.join(target_folder, ModelFile.CONFIGURATION_YAML)
