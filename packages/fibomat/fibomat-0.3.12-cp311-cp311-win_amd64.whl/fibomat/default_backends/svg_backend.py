from fibomat import mill, raster_styles
...
class SVGBackend(BackendBase):
    def __init__(self, pixel_scale: Q_ = Q_('0.01 µm'), stroke_width: float = 0.1, spot_radius: Q_ = Q_('500 nm'), description: Optional[str] = None):
        super().__init__()
    ...
    def line(self, ptn: Pattern[Line]):
        polyline_pattern = Pattern(
            dim_shape=Polyline([ptn.dim_shape.shape.start, ptn.dim_shape.shape.end]) * ptn.dim_shape.unit,
    ...
    def spot(self, ptn: Pattern[Spot]):
        circle_pattern = Pattern(
            dim_shape=Circle(
                self._spot_radius.m_as(ptn.dim_shape.unit),
                ptn.dim_shape.shape.center
