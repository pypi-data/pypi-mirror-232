...
class User:
    ...
    def list(self, team: int) -> dict:
        if not team.isnumeric():
            team = self.parent.team.find(team)['id']

        if users := self.parent.rest.get("/v3.0/users",
                                         headers={"teamid": str(team)}
                                         ):
            return users['users']
        print(f"Error listing Users: team - {team}", file=sys.stderr)
        return {}

    def find(self, team, email: str = None) -> dict:
        if email is None:
            return self.list(team)

        for user in self.list(team):
            if email.lower() == user['email_id'].lower():
