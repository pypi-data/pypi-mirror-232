...
#    current_directory = os.chdir('/opt/airflow/git/jaffle_shop.git/dags/dbt/jaffle_shop/')
...
# Print the list of files and directories
   ...
   with open('/tmp/credentials.json', 'r') as file:

    credentials = json.load(file)

    credential1 = credentials.get('credential1')
    credential2 = credentials.get('credential2')
    credential3 = credentials.get('credential3')

    if credential1 is None or credential2 is None or credential3 is None:
        print("One or more credentials are missing in the configuration file.")
    else:
        print(f'Credential 1: {credential1}')
        print(f'Credential 2: {credential2}')
        print(f'Credential 3: {credential3}')
   ...
   results = []
   cursor.execute("select distinct command_invocation_id, run_started_at from models order by run_started_at desc limit 1")
