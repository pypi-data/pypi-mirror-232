...
class ConfigWS(object):
    ...
    def __new__(cls):
        if cls.config is None:
            ...
            # cls.config['loc'] = config.get('NSLC', 'location', fallback='--')
            # if cls.config['loc'] == '':
            #     cls.config['loc'] = '--'
            ...
            cls.config['data'] = dascls(cls.config['experiment'], cls.config['directory'], iterate='D',
                                        networkcode=cls.config['net'], channelcode=cls.config['cha'],
                                        loglevel=cls.config['loglevel'])
            md = dascls(cls.config['experiment'], cls.config['directory'], iterate='M',
                        networkcode=cls.config['net'], channelcode=cls.config['cha'],
                        loglevel=cls.config['loglevel'])
            # Create list with the line for the level=network
            cls.config['network'] = list()
            cls.config['network'].append((cls.config['net'], cls.config['sitename'], md.starttime.isoformat(),
                                          md.endexperiment.isoformat(), md.numchannels))

            # Create list with the lines for the level=station
            cls.config['station'] = list()
            for c in md.channels:
                cls.config['station'].append((cls.config['net'], '%05d' % c, 0.0, 0.0, 0.0, cls.config['sitename'],
                                              md.starttime.isoformat(), md.endexperiment.isoformat()))

            # Create list with the lines for the level=channel
            cls.config['channel'] = list()
            for c in md.channels:
                cls.config['channel'].append((cls.config['net'], '%05d' % c, '', cls.config['cha'], '', '', '', '', '',
                                              '', '', '', '', '', '', md.starttime.isoformat(), md.endexperiment.isoformat()))
...
@dataselect.get("/application.wadl")
async def dsapplicationwadl():
    ...
    return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content=messdict)


@stationws.get("/application.wadl")
...
async def stquery(network: str = '*', net: str = '*', station: str = '*', sta: str = '*', location: str = '*',
    ...
    # Network
    if network not in ('*', cfgparams['net']) or net not in ('*', cfgparams['net']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")

    # Location
    if location not in ('*', cfgparams['loc']) or loc not in ('*', cfgparams['loc']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")

    # Channel
    if channel not in ('*', cfgparams['cha']) or cha not in ('*', cfgparams['cha']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")
    ...
    # Network and net: Discard the most comprehensive case of '*' and keep the most restricted one
    auxnet = network if net == '*' else net
    # Station and sta: Discard the most comprehensive case of '*' and keep the most restricted one
    auxsta = station if sta == '*' else sta
    # Location and loc: Discard the most comprehensive case of '*' and keep the most restricted one
    auxloc = location if loc == '*' else loc
    # Channel and cha: Discard the most comprehensive case of '*' and keep the most restricted one
    ...
    for s in cfgparams[level]:
        result += '|'.join(['%s' for i in range(len(s))]) % s
        result += '\n'
...
@dataselect.get("/query")
def dsquery(net: str = '*', sta: str = '*', loc: str = '*', cha: str = '*', start: str = None, starttime: str = None,
    ...
    # Network
    # if network not in ('*', cfgparams['net']) or net not in ('*', cfgparams['net']):
    if net not in ('*', cfgparams['net']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")

    # Location
    # if location not in ('*', cfgparams['loc']) or loc not in ('*', cfgparams['loc']):
    if loc not in ('*', cfgparams['loc']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")

    # Channel
    # if channel not in ('*', cfgparams['cha']) or cha not in ('*', cfgparams['cha']):
    if cha not in ('*', cfgparams['cha']):
        return PlainTextResponse(status_code=status.HTTP_204_NO_CONTENT, content="")
    ...
    # Reading experiment
    if cfgparams['format'] == 'TDMS':
        dascls = TDMS
    elif cfgparams['format'] == 'OptoDAS':
        dascls = OptoDAS
    else:
        raise Exception('Format %s not recognized!' % cfgparams['format'])
