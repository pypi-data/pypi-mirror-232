...
from dashvector.core.models.collection_meta_status import (CollectionMeta,
                                                           CollectionStats)
from dashvector.core.models.create_collection_request import \
    CreateCollectionRequest
from dashvector.core.models.create_partition_request import \
    CreatePartitionRequest
from dashvector.core.models.delete_collection_request import \
...
from dashvector.core.models.describe_collection_request import \
...
from dashvector.core.models.list_collections_request import \
    ...
    def __init__(self, *,
        ...
        if not isinstance(endpoint, str):
            raise DashVectorException(
                ...
                reason=f"DashVectorSDK GRPCHandler endpoint({endpoint}) is Invalid")
        ...
        if not isinstance(api_key, str):
            raise DashVectorException(
                code=DashVectorCode.InvalidArgumentType,
                reason=f"DashVectorSDK GRPCHandler api_key({api_key}) is Invalid")
        ...
        if isinstance(timeout, float):
            ...
            raise DashVectorException(
                code=DashVectorCode.InvalidArgumentType,
                reason=f"DashVectorSDK GRPCHandler timeout({timeout}) is Invalid")
        if timeout <= 0.000001:
            timeout = 365.5 * 86400
        ...
        self._headers["x-user-agent"] = self._headers["x-user-agent"] + \
