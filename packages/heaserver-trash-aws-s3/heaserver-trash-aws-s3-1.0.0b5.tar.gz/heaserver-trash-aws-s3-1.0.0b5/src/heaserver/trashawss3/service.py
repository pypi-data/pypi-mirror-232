...
async def get_all_deleted_items_all_buckets(request: web.Request) -> web.Response:
    ...
    for bucket in resp_.get('Buckets', []):
        async def coro(volume_id: str, bucket_id: str, sub: str | None):
            resp_list = []
            async for item in _get_deleted_items_private(s3, volume_id, bucket_id, None, sub):
                resp_list.append(item)
            return resp_list
        coros.append(coro(request.match_info['volume_id'], bucket['Name'], request.headers.get(SUB)))
    return await response.get_all(request, list(itertools.chain(*(await asyncio.gather(*coros)))))
...
async def _get_deleted_items_private(s3: S3Client, volume_id: str, bucket_id: str, prefix: str | None = None,
    ...
    async for response_ in _get_version_objects(s3, bucket_id, prefix, loop_):
        ...
        non_null_versions = list(vers for vers in response_.get('Versions', []) if vers['VersionId'] != 'null' and vers['VersionId'] is not None and (version is None or version == vers['VersionId']))
        for key, versions in itertools.groupby(non_null_versions, itemgetter('Key')):
            if key in delete_markers:
                sorted_versions = sorted(versions, key=itemgetter('LastModified'), reverse=True)
                version = next(version for version in sorted_versions if version['LastModified'] < delete_markers[key])
                logger.debug('Version response for key %s and version %s', key, version)
                if version:
                    ...
                    item.key = key
                    item.version = version['VersionId']
