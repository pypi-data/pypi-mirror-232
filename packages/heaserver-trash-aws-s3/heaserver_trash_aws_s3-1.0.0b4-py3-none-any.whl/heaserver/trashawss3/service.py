...
async def _get_deleted_items_private(s3, volume_id, bucket_id, prefix=None, sub_user=None) -> AsyncIterator[DesktopObjectDict]:
    ...
    async for response_ in _get_version_objects(s3, bucket_id, prefix, loop_):
        ...
        for key, versions in itertools.groupby(sorted(itertools.chain((vers for vers in response_.get('DeleteMarkers', []) if vers['VersionId'] != 'null'), (vers for vers in response_.get('Versions', []) if vers['VersionId'] != 'null')), key=keyfunc), key=keyfunc):
            if prefix is not None and not key.startswith(prefix):
                continue
            delete_markers_to_versions = {}
            def delete_marker_iterator():
                ...
                for is_delete_marker, versions_ in itertools.groupby(versions, key=lambda x: 'Size' not in x):
                    if is_delete_marker:
                        ...
                        yield prev_delete_marker
                    elif prev_delete_marker and (prev_delete_marker['VersionId'] in delete_markers_to_versions):
                        delete_markers_to_versions[prev_delete_marker['VersionId']] = versions_
            for delete_marker in delete_marker_iterator():
