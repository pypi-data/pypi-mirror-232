...
def main():
    ...
    # 遍历bam文件的每一个read
    # 打开BAM文件
    # krak_bamfile = pysam.AlignmentFile(bam_file, "rb")
    with pysam.AlignmentFile(args.bam_file, "rb") as krak_bamfile:
        # 遍历BAM文件和krak2_output数据
        for sread,kread in zip(krak_bamfile, krak2_output.itertuples()):
            read_count += 1
            ...
            try:
                ...
                skipped += 1
                continue
            if len(sread.seq) < int(kread.len)-1:
                continue
            ...
            if kread.kmer_position.strip() == "":
                pass
            else:
                use_count += 1
                kmer_positions_tuple = np.array([list(map(str, info.split(":"))) for info in kread.kmer_position.strip().split()])
