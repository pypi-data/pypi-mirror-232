...
class CommitReporter(Commit):
    ...
    def display_body_report(self, body_error: BodyError) -> None:
        ...
        if body_error.mixed_tag_found:
            for tag in body_error.mixed_tag_found:
                print((f"    Invalid tag found: '{tag}'. Should be '{tag.upper()}'"))

        if body_error.duplicate_jama_line:
            print("    Found duplicate lines starting with 'JAMA:'")
        ...
        if body_error.impact_tag_line_is_short:
            print("    The details provided in the 'IMPACT:' section is too short.")

        if body_error.duplicate_impact_line:
            print("    Found duplicate lines starting with 'IMPACT:'")
