...
class CommitValidator(Commit):
    ...
    def _validate_body(self) -> bool:
        self.body_error = BodyError(
            ...
            impact_tag_line_is_short=False,
            duplicate_impact_line=False,
        )
        ...
        if any(
            [
                ...
                self.body_error.mixed_tag_found,
                self.body_error.duplicate_jama_line,
                self.body_error.duplicate_jama_refs,
                self.body_error.invalid_jama_refs,
                self.body_error.impact_tag_is_missing,
                self.body_error.impact_tag_line_is_short,
                self.body_error.duplicate_impact_line,
            ]
        ):
    ...
    def _check_if_tag_exists(self, tag: str) -> bool:
        if self.body:
            return any([line.lower().startswith(tag.lower()) for line in self.body])
    ...
    def _validate_jama_referencing(self) -> None:
        ...
        """
        exact_jama_tag, jama_tag_mixed_case = self._get_lines_starting_with_tag(
            JAMA_TAG
        )
        if jama_tag_mixed_case:
            self.body_error.mixed_tag_found.extend(jama_tag_mixed_case)
        if len(exact_jama_tag) > 1:
            self.body_error.duplicate_jama_line = True
        elif len(exact_jama_tag) == 1:
            (
                duplicate_jama_refs,
                invalid_jama_refs,
    ...
    def _validate_impact_referencing_for_fixbug(self) -> None:
        ...
        """
        exact_impact_tag, impact_tag_mixed_case = self._get_lines_starting_with_tag(
            IMPACT_TAG
        )
        if impact_tag_mixed_case:
            self.body_error.mixed_tag_found.extend(impact_tag_mixed_case)
        elif len(exact_impact_tag) == 0:
            self.body_error.impact_tag_is_missing = True
        elif len(exact_impact_tag) > 1:
            self.body_error.duplicate_impact_line = True
        else:
            impact_line = exact_impact_tag[0].replace(IMPACT_TAG, "").strip()
            if len(impact_line) <= 10:
                self.body_error.impact_tag_line_is_short = True

    def _get_lines_starting_with_tag(self, tag: str) -> Tuple[List[str], List[str]]:
        """
        Get the lines starting with a given tag (case sensitive and case insensitive)
        ...
        Returns:
            ...
            - exact_tag_matches (List[str]): List of lines starting with exact tag (case sensitive)
            - tag_mixed_case_matches (List[str]): List tag with different case
        """
        exact_tag_matches = []
        tag_mixed_case_matches = []
        if self.body:
            for line in self.body:
                if line.startswith(tag):
                    exact_tag_matches.append(line)
                elif line.lower().startswith(tag.lower()):
                    tag_mixed_case_matches.append(line[: len(tag)])
        return exact_tag_matches, tag_mixed_case_matches

    def _get_jama_ref_line(self) -> str:
        """
        Get the line starting with JAMA_TAG

        Returns:
            str: The line starting with JAMA_TAG
        """
        exact_jama_tag, _ = self._get_lines_starting_with_tag(JAMA_TAG)
        if exact_jama_tag:
            return exact_jama_tag[0].replace(JAMA_TAG, "").strip()
        else:
            return ""
    ...
    def _validate_jama_items(self) -> Tuple[List[str], List[str]]:
        ...
        """
        jama_ref_line = self._get_jama_ref_line()
