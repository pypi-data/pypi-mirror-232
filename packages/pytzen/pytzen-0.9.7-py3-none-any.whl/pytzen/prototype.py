...
class Prototype(ABC):
    ...
    def __init__(self, log_level:str='INFO', **kwargs):
        ...
        self._path_config:str = os.environ.get('PATH_CONFIG', '.')
        self._get_config()
    ...
    def _get_config(self):
        ...
        if not Prototype.config:
            vars = parser.VariablesParser(json_path=self._path_config, 
                                          logs=self.logs)
    ...
    def _get_class_pattern(self):
        ...
        class_path = Prototype.config['path_classes'][class_name]
        with open(class_path) as class_file:
            class_pattern:dict[str,str|dict[str,str]] = json.load(class_file)
