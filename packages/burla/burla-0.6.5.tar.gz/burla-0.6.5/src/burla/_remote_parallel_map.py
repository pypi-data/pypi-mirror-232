...
MAX_CPUS = 4000  # please ask before you increase this <3
MAX_GPUS = 500  # please ask before you increase this <3
...
def _start_job(
    ...
    for pkg in installed_packages:
        if pkg["name"] in imported_modules:
            required_packages.append(pkg)
        elif pkg["name"] == "beautifulsoup4" and "bs4" in imported_modules:
            required_packages.append({"name": "beautifulsoup4", "version": pkg["version"]})
    ...
    function_pkl = dill.dumps(function_, recurse=True)
    ...
    for input_batch in input_batches:
        try:
            ...
            for output_pkl in download_outputs(output_urls):
                output = dill.loads(output_pkl)
                if output["pickled_with_dill"]:
                    return_values.append(dill.loads(output["return_value_pkl"]))
