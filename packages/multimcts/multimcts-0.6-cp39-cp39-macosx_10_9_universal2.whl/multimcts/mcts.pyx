...
cdef class Node:
    ...
    cdef Node best_child(self, double exploration_bias, double rave_bias, double pruning_bias):
        ...
        # Prune any children that are much worse than the best child.
        ...
        if pruning_bias > 0:
            ...
            for (child, move, score) in lessers:
                # If this child's optimistic estimate (UCB) is much worse ("much" defined by pruning_bias) than the best child's pessimistic estimate (LCB), prune it.
