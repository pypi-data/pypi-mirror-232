...
__author__ = "Ricardo Mugica <rmugicag@gmail.com>"
...
def get_chrome_version():
    ...
    if platform == "linux":
        ...
        chrome_path_32 = "C:\\Program Files\\Google\\Chrome\\Application"
        chrome_path_64 = "C:\\Program Files (x86)\\Google\\Chrome\\Application"
        if os.path.isdir(chrome_path_32):
            dirs = [f.name for f in os.scandir(chrome_path_32) if f.is_dir() and re.match("^[0-9.]+$", f.name)]
            if dirs:
                return max(dirs)
        elif os.path.isdir(chrome_path_64):
