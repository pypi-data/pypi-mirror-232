...
class Telescope():
    ...
    def update(self, stepsize=None):
        ...
        with self.dataLock:
            ...
            self.data["acu.azimuth.p_shape"] = self.data["acu.azimuth.p_act"]
            self.data["acu.elevation.p_shape"] = self.data["acu.elevation.p_act"]
            self.data["acu.feed_indexer.p_shape"] = self.data["acu.feed_indexer.p_act"]

            if self.plant_model:
                az, el = self.azel
                noise = np.random.randn() * 0.005 # -> results in ~5" on az and 10" on el
                az_is, el_is = self.plant_model(np.array([az, el, noise]))
            else:
                az_is, el_is = self.azel
            
            self.data["acu.azimuth.p_act"] = az_is
            self.data["acu.elevation.p_act"] = el_is
            self.data["acu.azimuth.v_act"] = (az_is - az_last) / dt_tick
            self.data["acu.elevation.v_act"] = (el_is - el_last) / dt_tick
            ...
            self.additional_update_fun(stepsize=stepsize, dt_tick=dt_tick, t_last=t_last, t_now=t_now,az_last=az_last, el_last=el_last)
