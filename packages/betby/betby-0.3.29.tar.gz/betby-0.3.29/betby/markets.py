...
class Get_markets:
    ...
    def two_way(self, mar=0.05, min_prob=0.02, max_odds=25, prob_tw=0):
        ...
        if self.prob_tw < 0.999 and self.prob_tw > 0.001:
            self.Odd_1 = margin(1 / self.prob_tw, self.mar, self.max_odds)
            self.Odd_2 = margin(
        ...
        new_data = {
            markets.BOTH_TO_SCORE: {
                format_specifiers(markets.BOTH_TO_SCORE, {}): {
                    markets.BOTH_TO_SCORE_YES: get_outcome(
                        self.Odd_1, self.bs_tw),
                    markets.BOTH_TO_SCORE_NO: get_outcome(
