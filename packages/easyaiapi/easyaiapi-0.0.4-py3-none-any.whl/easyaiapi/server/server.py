...
class EasyAIServer(FastAPI):
    ...
    def videostream(self, endpoint: str, resize_shape:Optional[tuple[int,int]]=None, on_connect:Callable[[str], None]=None, on_disconnect:Callable[[str], None]=None):
        ...
        on_received = lambda data: read_base64_image(data, resize_shape)
        return self.stream(endpoint, on_received, on_connect, on_disconnect) 


    def create_model_enpoint(self, endpoint: str, model: Model, connection_type: ConnectionType, resize_shape:Optional[tuple[int, int]]=None):
        
        def index(image: np.ndarray, client_address:str):
            return handle_result(model(image))

        if connection_type == ConnectionType.WEBSOCKET:
            self.videostream(endpoint, resize_shape)(index)
