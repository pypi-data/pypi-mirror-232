...
class Session(object):
    ...
    def __init__(self, address, timeout, loop=None, builder=tcp, reconnect_limit=5):
        ...
        self._request_table = OrderedDict()
    ...
    def cancel_last_request(self):
        """
        Cancel the last request, set error to the future, and stop the running ioloop.

        # todo: make this threadsafe, race could happen when this method is called, the response from remote server also arrived.
        neither _request_table, future or _loop is thread-safe.

        The only use case for now is to cancel the last move_async request that caused collision, in which case race condition is not likely to happen
        """
        if len(self._request_table) == 0:
            return

        msgid, future = self._request_table.popitem(last=True)
        future.set_error("Request canceled")
        self._loop.stop()
