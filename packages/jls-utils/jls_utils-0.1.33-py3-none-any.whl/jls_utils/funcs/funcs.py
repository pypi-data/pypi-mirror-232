...
from jls_utils.abstract.abstract import *
from jls_utils.secondary.const import *
from jls_utils.secondary import model_from_json, Config, show_logs_func
...
def post_register(auth: Optional[AUTH]=None) -> Alias.Info:
    if auth is None:
        auth = (Config.CORE_USERNAME, Config.CORE_PASSWORD)
    send_to_core_modify(REGISTER, User(login=auth[0], password=auth[1]), with_auth=False, with_show=False)
...
def delete_register(auth: Optional[AUTH]=None, *args, **kwargs) -> Alias.Info:
    send_to_core_modify(REGISTER, *args, **kwargs, auth=auth, with_show=False, is_post=False)
...
def get_app_info(id: str, parse: bool, *args, **kwargs) -> Union[Alias.Json, AppFunctionsInfo]:
    res = send_to_core_get(MANAGER_APP_INFO(id), *args, **kwargs)
    if parse:
        return model_from_json(res, AppFunctionsInfo)
    return res


def get_app_info_by_real_id(id: str, *args, **kwargs) -> Alias.Json:
    return send_to_core_get(MANAGER_APP_INFO_REAL_ID(id), *args, **kwargs)
...
def post_manager_task(data: MainTask, with_show: bool, long: bool, long_timeout: int, wait: bool, auth: Optional[AUTH], *args, **kwargs) -> AppLogs:
    check_profile_mode(data.profileMode)
    res = send_to_core_modify(MANAGER_TASK(wait and not long), data, with_show=with_show, show_func=show_logs_func, auth=auth, *args, **kwargs)
    if wait and long:
        res = asyncio.run(__get_result(res, timeout=long_timeout, auth=auth))
    return AppLogs.parse_raw(res)


def post_manager_task_run(data: RunTask, with_show: bool, long: bool, long_timeout: int, wait: bool, auth: Optional[AUTH], *args, **kwargs) -> Optional[AppLogs]:
    check_profile_mode(data.profileMode)
    res = send_to_core_modify(MANAGER_TASK_RUN(wait and not long), data, with_show=with_show, show_func=show_logs_func, auth=auth, *args, **kwargs)
    if wait and long:
        res = asyncio.run(__get_result(res, timeout=long_timeout, auth=auth))
...
async def __get_result(id: str, is_text: bool = True, check_time: float = LONG_SLEEP_TIME, timeout: int = WAIT_RESULT_TIMEOUT, auth: Optional[AUTH]=None) -> str:      # FIXME
    assert Config.HOST_PORT is not None, "host port not set"
    if auth is None:
    ...
    while True:
        try:
            async with aiohttp.ClientSession(auth=auth, connector=aiohttp.TCPConnector(verify_ssl=False), timeout=AIOHTTP_TIMEOUT_MINI) as session:
                while True:
                    async with session.get(f"{Config.HOST_PORT}{OPERATION_RESULTS_ID(id, None)}", headers=HEADERS) as response:
                        if response.ok:
                            if is_text:
...
def send_to_core_get(path: str, with_auth=True, is_text=False, auth: Optional[AUTH]=None) -> Optional[str]:
    assert Config.HOST_PORT is not None, "host port not set"
    if auth is None or not with_auth:
        auth = (Config.CORE_USERNAME, Config.CORE_PASSWORD) if with_auth else None
    response = requests.get(f"{Config.HOST_PORT}{path}", headers=HEADERS, auth=auth)
    check_response(f"{Config.HOST_PORT}{path}", response)
...
def send_to_core_modify(path: str, operation: Optional[Any] = None, with_auth: bool=True, with_show: Optional[bool]=None, show_func: Optional[Callable]=None, is_post: bool=True, auth: Optional[AUTH]=None) -> str:
    ...
    if is_post:
        response = requests.post(f"{Config.HOST_PORT}{path}", data=operation, headers=HEADERS, auth=auth if with_auth else None)
    else:   # delete
        response = requests.delete(f"{Config.HOST_PORT}{path}", data=operation, headers=HEADERS, auth=auth if with_auth else None)
    check_response(f"{Config.HOST_PORT}{path}", response, show_func=show_func)
    if response.status_code == HTTPStatus.NO_CONTENT:
        return ""
...
async def send_to_core_post_async(path: str, operation: Optional[str] = None, with_auth=True, with_show=True, show_func: Optional[Callable]=None, auth: Optional[AUTH]=None) -> str:
    assert Config.HOST_PORT is not None, "host port not set"
    if auth is None or not with_auth:
    ...
    async with aiohttp.ClientSession(auth=auth, connector=aiohttp.TCPConnector(verify_ssl=False), timeout=AIOHTTP_TIMEOUT) as session:
        async with session.post(f"{Config.HOST_PORT}{path}", data=operation, headers=HEADERS) as response:
