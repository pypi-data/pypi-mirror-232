...
def dm_generated_table_refused(path_bui=None,
    ...
    data_fuentes = pd.read_excel(path_bui, sheet_name='ID Fuentes (Source)', skiprows=[0], dtype=str)
    data_columns = [f"f_{remove_accents(col)}" for col in list(data_fuentes.columns)]
    data_fuentes.columns = data_columns
    data_fuentes['f_id2'] = data_fuentes['f_id'].astype(float)
    data_fuentes['f_tabla_master'] = data_fuentes['f_tabla_master'].str.lower()
    data_fuentes['f_tabla_master'] = data_fuentes['f_tabla_master'].str.strip()
    ...
    data_fuentes2 = data_fuentes[['f_id', 'f_id2', 'f_descripcion_de_la_tds', 'f_fuente_tds',
                                  ...
                                  'f_deuda_de_dictamen_o_sustento_tds', 'f_tabla_master']]
    data_fuentes2 = data_fuentes2.sort_values(by="f_id2", ascending=False)
    with open(path_tables) as f:
        tables_lines = [line.rstrip('\n') for line in f]
    data_fuentes3 = data_fuentes2[data_fuentes2["f_tabla_master"].isin(tables_lines)]
    data_fuentes4 = data_fuentes3.drop_duplicates(["f_tabla_master"])
    ...
    frame["nombre_tabla_o_descarga_(si_existiera)"] = data_join["f_tabla_master"]
