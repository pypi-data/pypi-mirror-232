...
class dl_trainer:
    ...
    scheduler: Optional[Callable[[int], float]], default `None`
    ...
    *args, **kwargs:
    ...
    def __init__(self,
        ...
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=scheduler) if scheduler is not None else None
    ...
    def train_epoch(self,
        ...
        for x, y in loader:
            x, y = x.to(self.device), y.to(self.device)
            yhat = self.model(x)
            loss = self.loss(yhat, y) / self.mini_batch
            if mini_batch_countr == 0:
                self.optimizer.zero_grad()
            loss.backward()
            mini_batch_countr = mini_batch_countr + 1
            if mini_batch_countr == self.mini_batch:
                self.optimizer.step()
                mini_batch_countr = 0
        if mini_batch_countr != 0:
            self.optimizer.step()
