...
class ZstdPickleSerializer(PickleSerializer):
    ...
    _cache.level: int = None
    _cache.threads: int = None
    _cache.compressor: zstandard.ZstdCompressor = None
    _cache.decompressor: zstandard.ZstdDecompressor = None
    ...
    def serialize(cls, x, *, level=ZSTD_LEVEL, protocol=None, threads=0):
        ...
        '''
        y = super().serialize(x, protocol=protocol)
        if cls._cache.compressor is None or level != cls._cache.level or threads != cls._cache.threads:
            cls._cache.compressor = zstandard.ZstdCompressor(level=level, threads=threads)
        return cls._cache.compressor.compress(y)
