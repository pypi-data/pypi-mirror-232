...
def initialize(bbox,df,x_mul,y_mul,bbox_cor):
    ...
    return(domain1,df,np.min(xxx),np.min(yyy)) # return the 



def process_building(lineO,lineL,domain1,idxP,gdf,bHeightL,bHeightH,x_mul,y_mul,x_start,y_start,bbox_cor,bldH,area,areaH,res,angleRotate):
    # poly.bounds
    nx = []
    ny = []
    xy = []
    ...
    for i in range(np.shape(gdf['geometry'][idxP])[1]):
        xb = int(np.round((gdf['geometry'][idxP][0][i] - bbox_cor[1])*x_mul))
        yb = int(np.round((gdf['geometry'][idxP][1][i] - bbox_cor[2])*y_mul))
