...
from finlab.utils import check_version, requests, get_tmp_dir
...
class FileStorage():
    def __init__(self, path=get_tmp_dir(), use_cache=True):
        """將歷史資料儲存於檔案中
          Examples:
        ...
        if self._expiry:
            res = requests.get('https://asia-east1-fdata-299302.cloudfunctions.net/data_reset_time')
    ...
    if 'url' in ret and len(ret['url']) > 0:
        ...
        if 'pyodide' in sys.modules:
            if hasattr(requests, 'getBytes'):
                res = requests.getBytes(ret['url'])
                df = pd.read_pickle(BytesIO(res), compression='gzip')
            else:
                res = requests.get(ret['url'])
