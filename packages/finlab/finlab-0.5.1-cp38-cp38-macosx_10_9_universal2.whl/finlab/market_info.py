...
class MarketInfo():
    ...
    def get_trading_price(self, name: str) -> pd.DataFrame:

        """取得回測用價格數據
        
        Args:
            name (str): 選擇回測之還原股價以收盤價或開盤價計算，預設為'close'。可選'close'或'open'。
        Returns:
            (pd.DataFrame): 價格數據
        ...
        if name in ['open', 'close', 'high', 'low']:
            ...
            return (self.get_price('open', adj=True) + self.get_price('close', adj=True)) / 2
        elif name == 'high_low_avg' or 'low_high_avg':
            return (self.get_price('high', adj=True) + self.get_price('low', adj=True)) / 2
        elif name == 'price_avg':
            return (self.get_price('open', adj=True) + self.get_price('close', adj=True)\
                     + self.get_price('high', adj=True) + self.get_price('low', adj=True)) / 4
