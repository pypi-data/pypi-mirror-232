...
async def create_replymarkup5(ent_id, post_id, POST_BUTTON, reply_markup, counters=dict()):
    result = reply_markup
    try:
        if POST_BUTTON is None or POST_BUTTON == '': return
        tmp = []
                ...
                if v[0]:
                    if len(tmp) > 0 and tmp[-1] is None:
                        ...
                        if 'ᴵ' in v[0]:
                            ...
                            buttons = [types.InlineKeyboardButton(text=f"{counter} {str(v[0])}",
                                                                  callback_data=f"pst_{ent_id}_{post_id}_{btn_ix}")]
                            btn_ix += 1
                            pritn(f"up pst_{ent_id}_{post_id}_{btn_ix}")
                        else:
                            buttons = [types.InlineKeyboardButton(text=str(v[0]), url=v[1])]
                            btn_ix += 1
                    else:
                        if 'ᴵ' in v[0]:
                            ...
                            buttons.append(types.InlineKeyboardButton(text=f"{counter} {str(v[0])}",
                                                                      callback_data=f"pst_{ent_id}_{post_id}_{btn_ix}"))
                            btn_ix += 1
                            pritn(f"dn pst_{ent_id}_{post_id}_{btn_ix}")
                        else:
                            buttons.append(types.InlineKeyboardButton(text=str(v[0]), url=v[1]))
                            btn_ix += 1
                tmp.append(v[0])
