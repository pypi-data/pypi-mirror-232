    ...
    def __init__(self, path: str, class_map: Optional[Dict] = None):
        ...
        if os.name == "nt":
            if not os.path.exists(os.path.join(lib_path, "libsensiml.dll")):
                print("ERROR: libsensiml.dll as not found in {}.".format(lib_path))
                raise ImportPathException(
                    "ERROR: libsensiml.dll as not found in {}".format(lib_path)
                )

            clf_lib = CDLL(os.path.join(lib_path, "libsensiml.dll"))

        if os.name == "posix":
            if not os.path.exists(os.path.join(lib_path, "libsensiml.so")):
                print(
                    "Warning: libsensiml.so as not found in {}. Attempting".format(
                        lib_path
                    )
                )
                if not os.path.exists(os.path.join(path, "libsensiml.a")):
                    print("ERROR: libsensiml.a as not found in {}.".format(lib_path))
                ...
                if not os.path.exists(os.path.join(lib_path, "libsensiml.so")):
                    raise ImportPathException(
                        "ERROR: libsensiml.so as not found in {}".format(lib_path)
                    )

            clf_lib = CDLL(os.path.join(lib_path, "libsensiml.so"))
