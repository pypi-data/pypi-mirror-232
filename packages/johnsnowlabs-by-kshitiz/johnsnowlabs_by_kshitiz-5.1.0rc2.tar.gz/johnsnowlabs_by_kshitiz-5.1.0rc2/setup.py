...
class InstallCommand(install):
    def should_install_pyspark(self):
        if environ.get("NO_PYSPARK", "0") == "1":
            return False
        return True

    def install_pyspark(self):
        pyspark_to_install = environ.get("PYSPARK_VERSION", "3.1.2")
        install_standard_pypi_lib(pypi_name="pyspark", version=pyspark_to_install)

    def run(self):
        install.run(self)

        if self.should_install_pyspark():
            self.install_pyspark()
        else:
            print("Not installing pyspark. Please make sure pyspark is available.")
