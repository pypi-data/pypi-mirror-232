...
class ConnectionOptimization():
    ...
    def get_added_section(self) -> Polygon:
        ...
        for n, connection in enumerate(self.connected_fibers):
            ...
            added_section.append(new_added_section)
    ...
    def optimize_virtual_shift(self, bounds: tuple) -> float:
        ...
        if len(self.connected_fibers) == 0:
            raise ValueError('No connected fibers are found.')
    ...
    def add_added_section_to_ax(self, ax: Axis) -> None:
        for connection in self.connected_fibers:
            connection.added_section.facecolor = 'green'
            connection.added_section._render_(ax)

    def add_removed_section_to_ax(self, ax: Axis) -> None:
        for connection in self.connected_fibers:
            connection.removed_section.facecolor = 'red'
            connection.removed_section._render_(ax)
