...
class PD:
    ...
    def recv_nlos_led_signal(self, led:LED, room:Room):
        ...
        for i, (w_pos, angle) in enumerate(room.get_reflect_wall_args()):
            ...
            cos_wall_t_angle_rad = np.where(cos_wall_t_angle_rad>0, cos_wall_t_angle_rad, 0) # 满足墙壁反射入射条件 （入射角小于90）
            ...
            cos_pd_r_angle_rad = np.where(cos_pd_r_angle_rad>0, cos_pd_r_angle_rad, 0) # 满足墙壁反射发射条件（发射角小于90）
