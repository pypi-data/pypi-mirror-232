...
class Reporter(object):
    ...
    def report_records(self, max_records=MAX_MESSAGES_PER_CHECK, status_types_to_return=STATUS_TYPES):
        ...
        """
        all_records = sorted(self._report_records, key=lambda x: STATUS_PRIORITIES[x.result])
        filtered_records = [
            report_record for report_record in all_records if report_record.result in status_types_to_return
        ]
    ...
    def __save_result_message(
        ...
        message_stripped_of_unprintables = "".join(s for s in message if s in string.printable)
    ...
    def report(self, message):
        ...
        reporter_output = self.__format_message(
            message.message, message_file_name=message.file_name, message_line_number=message.line_number
        )
    ...
    def ast_warn(self, message, files_with_ast_results):
        self._ast_report(message, files_with_ast_results, self.warn)
    ...
    def exception(self, exception, category="error"):
        ...
        report_record = ReportRecord(category, message, filename, line_number, code_section, None, None)
        self._report_records.append(report_record)
