        ...
        try:
            ...
            if resource_contents:
                ...
                for resource in resource_contents:
                    ...
                    generated_app_package = AppPackage.generate_app_package_from_file_or_folder(resource_path)
        ...
        while app_package_queue:
            ...
            if package.dependencies_folder is not None:
                ...
                for dependency_path in depdendency_paths:
                    dependency_full_path = os.path.join(package.dependencies_folder, dependency_path)
                    dependency_app_package = AppPackage.generate_app_package_from_file_or_folder(dependency_full_path)
...
class AppPackage(object):
    ...
    def does_dir_contain_default_app_conf(directory):
        ...
        Returns:
            Boolean: True if `default/app.conf` exists
        """
        dir_exists = directory is not None and directory != "" and os.path.isdir(directory)
    ...
    @staticmethod
    def find_prohibited_files(directory_to_search, directory_allow_list=None):
        """Function to locate prohibited directories and files

        Args:
            directory_to_search (String): the absolute path of the directories
                to search
            directory_allow_list (List of Strings): paths of files to ignore
        ...
        """
        if directory_allow_list is None:
            directory_allow_list = []
        file_paths_to_return = []

        directory_name = os.path.basename(directory_to_search)
        directory_path_match = re.findall(AppPackage.NOT_ALLOWED_PATTERN, directory_name)
        ...
        if os.path.isdir(directory_to_search):
            for directory, sub_directories, files in os.walk(directory_to_search):
                ...
                sub_directories[:] = [
                    sub_directory for sub_directory in sub_directories if sub_directory not in directory_allow_list
                ]

                for sub_directory in sub_directories:
                    directory_name_matches = re.findall(AppPackage.NOT_ALLOWED_PATTERN, sub_directory)
    ...
    def find_files_not_part_of_valid_apps(self):
        ...
        if working_app_path_in_contents and working_app_path_is_app:
            ...
            dependencies_is_folder = os.path.isdir(os.path.join(self.working_artifact, self.DEPENDENCIES_LOCATION))
            working_app_path_has_manifest = self.does_working_artifact_contain_app_manifest()
            if dependencies_folder_in_contents and dependencies_is_folder and working_app_path_has_manifest:
...
class CompressedAppPackage(AppPackage):
    ...
    def clean_up(self):
        ...
        Returns:
            None
        """
        if self.extracted_path is not None and self.extracted_path != "" and os.path.isdir(self.extracted_path):
    ...
    def get_content_path_names(self):
