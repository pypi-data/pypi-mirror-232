...
from splunk_appinspect.check_messages import FailMessage, NotApplicableMessage, SkipMessage, WarningMessage
...
class CheckReloadTriggerForAllCustomConfs(Check):
    ...
    def check(self, app):
        ...
        self._config.depends_on_config = {"app"} | {
            os.path.splitext(os.path.basename(conf_file))[0] for conf_file in app.custom_conf_files
        }
    ...
    def check_config(self, app, config):
        ...
        for path_in_app in app.custom_conf_files:
            file_name = os.path.splitext(os.path.basename(path_in_app))[0]
...
class CheckCustomConfReplication(Check):
    ...
    def check(self, app):
        ...
        self._config.depends_on_config = {"server"} | {
            os.path.splitext(os.path.basename(conf_file))[0] for conf_file in app.custom_conf_files
        }
    ...
    def check_config(self, app, config):
        ...
        for custom_conf in app.custom_conf_files:
            file_name = os.path.splitext(os.path.basename(custom_conf))[0]
...
def _is_splunk_default_app_id(app_id):
    splunk_default_apps = {
        "075-cloudworks",
        "100-s2-config",
        "100-whisper",
