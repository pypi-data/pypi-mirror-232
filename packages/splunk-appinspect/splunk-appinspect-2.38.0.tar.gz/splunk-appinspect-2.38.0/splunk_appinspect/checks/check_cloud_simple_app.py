...
from splunk_appinspect.check_messages import FailMessage, ManualCheckMessage, WarningMessage
    ...
    def check_metadata_file(self, app, path_in_app):
        ...
        if os.path.dirname(path_in_app) != "metadata":
            yield WarningMessage(
                ...
                file_name=os.path.dirname(path_in_app),
                remediation="Please remove this directory from the metadata directory.",
            )
        elif not path_in_app.endswith(".meta"):
            yield FailMessage(
    ...
    def check_static_file(self, app, path_in_app):
        ...
        if os.path.dirname(path_in_app) != "static":
            ...
            if filename.lower() == "thumbs.db":
                ...
                yield ManualCheckMessage("Please investigate this file manually.", file_name=path_in_app)
            else:
                yield WarningMessage(
...
class CheckSetupXml(Check):
    ...
    def check_setup_xml(self, app, path_in_app):
        yield FailMessage(
...
def check_transforms_conf_for_external_cmd_cloud(app, reporter):
