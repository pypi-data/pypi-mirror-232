...
def check_for_secret_disclosure(app, reporter):
    ...
    for result, file_path, lineno in matcher.match_results_iterator(
        ...
        if _secret_disclosure_values_allow_list(result):
            continue
        if dashboard_token_keyword in result and file_path.startswith(conditional_excluded_paths):
            continue
        reporter_output = (
...
def _secret_disclosure_values_allow_list(line_message):
    """
...
def _conf_options_secret_disclosure_allow_list(file_path, line_message):
    """Checks if the line message matched by the secret disclosure regex
    is an allow listed configuration option.
    ...
    conf_options_allow_list = {os.path.join("default", "savedsearches.conf"): ["action.email.show_password"]}
...
def _secret_disclosure_commands_allow_list(file_path, messages):
    """
    :param file_path: the file path of this matched line
...
def check_for_insecure_http_calls_in_python_private(app, reporter):
    ...
    def is_keyword_not_secure(call_node, ast_info):
        is_not_secure = False
        possible_argument_keys = {"url", "fullurl", "host"}
        for keyword in call_node.keywords:
            if keyword.arg in possible_argument_keys:
...
def check_for_supported_tls_private(app, reporter):
    ...
    def _report_if_all_kwrgs_found(
        ast_info, file_path, reporter, lib_name, url_param_index, url_param_key, check_kwrgs
    ):
