...
from splunk_appinspect.check_messages import FailMessage, SkipMessage, WarningMessage
...
def check_that_splunk_app_package_has_valid_static_dependencies(app, reporter):
    ...
    for dependency_resource in dependencies_contents:
        resource_path = os.path.join(app.package.dependencies_folder, dependency_resource)
...
def check_that_extracted_splunk_app_does_not_contains_only_app_conf_file(app, reporter):
    """Check that the extracted Splunk App does not contains only  `app.conf`"""
    for directory, filename, _ in app.iterate_files():
        if directory.strip(os.sep) in ("local", "default") and filename != "app.conf":
            return
        elif directory.strip(os.sep) not in ("local", "default"):
            return
...
class CheckValidVersionNumber(Check):
    ...
    @staticmethod
    def _is_valid_version_number(version_number):
    ...
    def check_app_manifest(self, app, path_in_app):
        try:
...
def check_requires_adobe_flash(app, reporter):
    ...
    flash_files = [os.path.join(f[0], f[1]) for f in app.iterate_files(types=flash_file_types)]
