...
class AlertActions(object):
    def __init__(self, app):
        ...
        self.CROSS_PLAT_EXES = splunk_appinspect.iter_ext.intersect(self.WINDOWS_EXES, self.NIX_EXES)
        ...
        for section in configuration_file.sections():
            ...
            for key, value, lineno in self.get_configuration_file().items(section.name):
                if key.lower() == "alert.execute.cmd":
                    alert_action.alert_execute_cmd = os.path.join(self.app.app_dir, "bin/", value)
                if key.lower() == "icon_path":
                    alert_action.relative_icon_path = os.path.join("appserver/static/", value)
                    alert_action.icon_path = os.path.join(self.app.app_dir, alert_action.relative_icon_path)
