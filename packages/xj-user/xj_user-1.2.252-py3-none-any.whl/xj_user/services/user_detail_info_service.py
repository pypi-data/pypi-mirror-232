...
class DetailInfoService:
    ...
    def get_detail(user_id: int = None, search_params: dict = None, filter_fields: "str|list" = None, **kwargs):
        ...
        result = result_dict[int(filter_dict["real_name_is_pass"])]
        if result == "VERIFY_REJECT":  # 审核不通过才需要原因
            reply_record = UserApplyRecord.objects.filter(user_id=user_id, apply_type_id=1, result=result).order_by("-id").values("reject_reason").first()
            if reply_record:
