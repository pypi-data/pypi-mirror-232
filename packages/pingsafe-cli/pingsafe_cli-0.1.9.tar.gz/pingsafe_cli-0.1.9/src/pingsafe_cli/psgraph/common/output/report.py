...
from pingsafe_cli.psgraph.common.pingsafe.code_categories import CodeCategoryType
from pingsafe_cli.psgraph.common.pingsafe.severities import BcSeverities, Severity
from pingsafe_cli.psgraph.common.pingsafe.check_type import CheckType
from pingsafe_cli.psgraph.common.models.enums import CheckResult, ErrorStatus
from pingsafe_cli.psgraph.common.output.ai import OpenAi
from pingsafe_cli.psgraph.common.typing import _ExitCodeThresholds, _ScaExitCodeThresholds
from pingsafe_cli.psgraph.common.output.record import Record, SCA_PACKAGE_SCAN_CHECK_NAME
from pingsafe_cli.psgraph.common.util.consts import PARSE_ERROR_FAIL_FLAG, PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2
from pingsafe_cli.psgraph.common.util.json_utils import CustomJSONEncoder
from pingsafe_cli.psgraph.runner_filter import RunnerFilter

from pingsafe_cli.psgraph.sca_package_2.output import create_cli_output as create_sca_package_cli_output_v2

from pingsafe_cli.psgraph.sca_package.output import create_cli_output as create_sca_package_cli_output_v1

from pingsafe_cli.psgraph.policies_3d.output import create_cli_output as create_3d_policy_cli_output

from pingsafe_cli.psgraph.version import version

if TYPE_CHECKING:
    from pingsafe_cli.psgraph.common.output.baseline import Baseline
...
class Report:
    ...
    def get_dict(self, is_quiet: bool = False, url: str | None = None, full_report: bool = False) -> dict[str, Any]:
        if not url:
            url = "Add an api key '--bc-api-key <api-key>' to see more detailed insights via https://pingsafe.cloud"
        if is_quiet:
            return {
    ...
    def get_exit_code(self, exit_code_thresholds: Union[_ExitCodeThresholds, _ScaExitCodeThresholds]) -> int:
        """
        Returns the appropriate exit codescanner depending on the flags that are passed in.

        :return: Exit codescanner 0 or 1.
        """

        hard_fail_on_parsing_errors = os.getenv(PARSE_ERROR_FAIL_FLAG, "false").lower() == 'true'
        logging.debug(f'In get_exit_code; exit codescanner thresholds: {exit_code_thresholds}, hard_fail_on_parsing_errors: {hard_fail_on_parsing_errors}')
    ...
    def print_console(
        ...
        if self.check_type in (CheckType.SCA_PACKAGE, CheckType.SCA_IMAGE):
            if self.failed_checks or self.skipped_checks:
                create_cli_output = create_sca_package_cli_output_v2 if PINGSAFE_RUN_SCA_PACKAGE_SCAN_V2 else create_sca_package_cli_output_v1
        ...
        if created_baseline_path:
            output_data += colored(
                f"Created a pingsafe baseline file at {created_baseline_path}",
                "blue",
            )
        if baseline:
            output_data += colored(
    ...
    def _create_test_case_failure_output(self, record: Record) -> str:
        ...
        IaC example:
            ...
            File: /main.tf: 71-83
            Guideline: https://docs.pingsafe.io/docs/bc_azr_networking_2
