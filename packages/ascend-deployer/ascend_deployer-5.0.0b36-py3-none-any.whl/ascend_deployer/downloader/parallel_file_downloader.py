...
class DownloadFileInfo:
    ...
    def __init__(self, filename="", url="", sha256="", dst_file_path=""):
...
class CalcSha256Result:

    def __init__(self, file_info: DownloadFileInfo, file_sha256="", is_sha256_equals=False):
        self.file_info = file_info
        self.file_sha256 = file_sha256
        self.is_sha256_equals = is_sha256_equals


def get_calc_sha256_result(file_info: DownloadFileInfo) -> CalcSha256Result:
    file_sha256 = calc_sha256(file_info.dst_file_path)
    return CalcSha256Result(file_info, file_sha256 or "", file_sha256 == file_info.sha256)


class ParallelDownloader:
    _MAX_DOWNLOAD_THREAD_NUM = int(os.environ.get("ASCEND_DEPLOYER_DOWNLOAD_MAX_SIZE", 16))
    _MAX_CALC_SHA256_NUM = multiprocessing.cpu_count()
    ...
    def _parallel_calc_all_file_sha256(self, file_info_list: List[DownloadFileInfo]) -> List[CalcSha256Result]:
        with multiprocessing.Pool(processes=self._MAX_CALC_SHA256_NUM) as process_pool:
            process_results = []
    ...
    def start_download(self):
        calc_all_file_sha256_results = self._parallel_calc_all_file_sha256(self._file_info_list)
        self._need_download_files = [calc_result.file_info for calc_result in calc_all_file_sha256_results
                                     if not calc_result.file_sha256 or not calc_result.is_sha256_equals]
        ...
        if not all(results):
            return results
        calc_download_file_sha256_res = self._parallel_calc_all_file_sha256(self._need_download_files)
        calc_failed_res = [result for result in calc_download_file_sha256_res if
                           result.file_info.sha256 and not result.is_sha256_equals]
        if calc_failed_res:
            for calc_result in calc_failed_res:
                LOG.info('the downloaded file：{} hash is not equal to the hash in file'.format(
                    calc_result.file_info.dst_file_path))
