...
class GeoserverOperatoer:
    ...
    #     self.wms_server = wms_server


    #   # 清除服务缓存
    #     os.system(
    #         'curl -v -u admin:geoserver -X POST http://192.168.3.203:8020/geoserver/rest/reset -H  "accept: application/json" -H  "content-type: application/json"'
    #     )
    #     # 发布服务
    #     os.system(
    #         'curl -v -u admin:geoserver -XPOST -H "Content-type: text/xml" -d '
    #         '"<featureType><name>' + table + '</name></featureType>" '
    #         'http://192.168.3.203:8020/geoserver/rest/workspaces/fujianai/datastores/postgis/featuretypes '
    #     )
    #     # 修改服务样式
    #     os.system(
    #         'curl -v -u admin:geoserver -X PUT http://192.168.3.203:8020/geoserver/rest/workspaces/fujianai/layers/' + table + ' '
    #         '-H "accept: application/json" '
    #         '-H "content-type: application/xml" '
    #         '-d "<layer><defaultStyle><name>fujianai:yellowbhq</name></defaultStyle></layer>"'
    #     )
    #         # 删除GeoServer图层
    #         os.system(
    #             "curl -v -u admin:geoserver -XDELETE vgis_http://" + GEOSERVER_HOST + ":" + GEOSERVER_PORT + "/geoserver/rest/layers/" +
    #             request.query_params['name'] + '?recurse=true')
    #         return Response({'success': True})

    # 初始化
    def __init__(self, wms_server, username, password):
        # "http://192.168.3.77:8085/geoserver"
        self.wms_server = wms_server
        serect = username + ":" + password
        self.basic_auth = "Basic " + str(base64.b64encode(serect.encode("utf-8")), "utf-8")
    ...
    def publish_raster_layer_service(self, tif_path,workspacename, layer_name):
        ...
        url = self.wms_server + "/rest/workspaces/" + workspacename + "/coveragestores/" + layer_name + "/vgis_file.geotiff"
    ...
    def clear_black_raster_layer_service(self, workspacename, datastore_name, layer_name):
        ...
        print("去除黑边")
        url = "{}/rest/workspaces/{}/coveragestores/{}/coverages/{}".format(self.wms_server, workspacename,
    ...
    def is_has_layer(self, layer_name):
        ishas = False
        url = "{}/rest/layers/{}".format(self.wms_server, layer_name)
        payload = ""
        headers = {
            'Authorization': self.basic_auth
        }
    ...
    def publish_shp_layer_service(self, shp_path, layer_name,workspacename, layer_style_color):
        ...
        url = self.wms_server + "/rest/workspaces/" + workspacename + "/datastores/" + layer_name + "/vgis_file.shp"
    ...
    def publish_layer_group_service(self, layer_group_name,workspacename, layer_name_list):
        ...
        layergroup_url = self.wms_server + "/rest/layergroups"
    ...
    def get_layer_info(self, layer_name):
        url = self.wms_server + "/rest/layers/" + layer_name
        payload = {}
        headers = {
            'Authorization': self.basic_auth
        }
    ...
    def delete_layer_service(self, layer_name):
        ...
        curl_delete_layer_url = self.wms_server + "/rest/layers/python:" + layer_name
    ...
    def delete_layer_group_service(self, layer_group_name,workspacename):
        ...
        if layer_group_name != None and layer_group_name.strip() != "":
            curl_delete_layergroup_url = self.wms_server + "/rest/layergroups/" + workspacename + ":" + layer_group_name
    ...
    def make_layer_style(self, style_color,workspacename):
        ...
        url = self.wms_server + "/rest/workspaces/" + workspacename + "/styles?name=" + style_name
    ...
    def delete_layer_style(self, style_name,workspacename):
        ...
        if style_name != None and style_name.strip() != "":
            url = self.wms_server + "/rest/styles/" + workspacename + ":" + style_name
            payload = {}
            headers = {
                'Authorization': self.basic_auth
            }
