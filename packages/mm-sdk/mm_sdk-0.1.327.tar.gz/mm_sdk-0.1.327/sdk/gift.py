...
class UseCode(BaseModel):
    code: str
    service: ServiceCode
    phone: str = Field(description="Телефон клиента, применяющего промокод")
    object_id: str = Field(
    ...
    cost: int = Field(description="Цена без применения промокодов")
    visit_at: datetime.date = Field(
