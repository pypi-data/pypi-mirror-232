...
def _crop_product_value(crop: dict):
    ...
    yield_dm = safe_parse_float(get_crop_lookup_value(MODEL, TERM_ID, term_id, LOOKUPS['crop']), None) or 0
...
def _should_run_product(product: dict):
    ...
    yield_dm = get_crop_lookup_value(MODEL, TERM_ID, term_id, LOOKUPS['crop'])
