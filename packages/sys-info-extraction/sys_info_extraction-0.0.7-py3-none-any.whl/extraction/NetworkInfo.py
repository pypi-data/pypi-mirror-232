...
class network():
    ...
    def interfaces(self):
        ...
        ip_int_name \
            = os.popen("%sip addr | awk '{print $2}' | grep ':$' || %sip addr | awk '{print $2}' | grep ':$'"% (self.sbin_path, self._sbin_path))\
            .read()\
            .split("\n")
        ...
        for ip_length \
            ...
            try:
                dict_ipinfo[
                    ...
                    os.popen("%sip addr | grep '%s' | grep inet' ' | awk '{print $2}' || %sip addr | grep '%s' | grep inet' ' | awk '{print $2}'"% (self.sbin_path,
                                ip_int_name[ip_length]
                                .replace(
                                    ":",
                                    ""
                                ), self._sbin_path, ip_int_name[ip_length].replace(":", "")
                                )
                    ).read()\
                        .replace(
                        "\n",
