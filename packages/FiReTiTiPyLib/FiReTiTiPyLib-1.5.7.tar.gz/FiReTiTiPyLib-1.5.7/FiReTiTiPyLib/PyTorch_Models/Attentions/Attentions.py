...
class NLblock(Module):
	def __init__(self, in_channels: int, parameters: str):
		...
		if params[0] == "NLC":
			...
			self.NL = NonLocalBlock.EmbeddedGaussian(in_channels, param1, int(params[2]), bool(params[3]), bool(params[4]))
		elif params[0] == "NLG":
			self.NL = NonLocalBlock.Gaussian(in_channels, param1, int(params[2]), bool(params[3]), bool(params[4]))
		else:
			raise Exception("Unknown non local block attention type: '" + str(params[0]) + "'. Expected: NLC (concatenation), NLP (product), " +
