...
from FiReTiTiPyLib.CyclicIF import CyclicIF_JavaInterfacer
...
class MaskRCNN(object):
	...
	def Directory(self, PredictionsDirPath, GroundTruthsDirPath, Threshold: int=128, Overlap: float=0.51,
		...
		for i in range(nbImages):
			...
			if not SegmentationOnly: det = imIO.Read(DetectionsPaths[i], True, False)
			...
			if not SegmentationOnly:
				SegAcc, SegSens, SegSpec, SegPrec, SegDice, DetAcc, DetSens, DetPrec = self.EvaluateImage(seg, segGT,
						DetectionsPaths[i], GT[i], Threshold=Threshold, Overlap=Overlap,
						SaveImagesAs=path, SegmentationOnly=SegmentationOnly,
						JavaJarsPath=JavaJarsPath, JavaXms=JavaXms, JavaXmx=JavaXmx)
			else:
				SegAcc, SegSens, SegSpec, SegPrec, SegDice = self.EvaluateImage(seg, segGT, None, None,
	...
	def EvaluateImage(self, SegPred, SegGT, DetPredPath, DetGTPath, Threshold: int=128,
							Overlap: float=0.51, SaveImagesAs: str=None, SegmentationOnly: bool=False,
							JavaJarsPath: str=None, JavaXms: str="4G", JavaXmx: str="8G"):
		...
		Acc, Sensitivity, Specificity, Precision, Dice = self.Segmentation(SegPred, SegGT, Threshold=Threshold,
																			SaveImageAs=SaveSegmentationImageAs)
		
		if not SegmentationOnly:
			Acc2, Sensitivity2, Precision2 = self.Detection2(DetPredPath, DetGTPath, Overlap=Overlap,
															SaveImageAs=SaveDetectionImageAs,
															JavaJarsPath=JavaJarsPath, JavaXms=JavaXms, JavaXmx=JavaXmx)
	...
	def Segmentation(self, Prediction, GroundTruth, Threshold: int=128, SaveImageAs: str=None):
		""" This function evaluates the segmentation result for a single given image.
			Args:
				...
				SaveImageAs (str): The entire path and name to save the result image.
									The result image is generated only if this path is not None.
			Returns:
				Accuracy
				Sensitivity/Recall
				Specificity
	...
	def Detection2(self, Prediction, GroundTruth, Overlap: float=0.51, SaveImageAs: str=None,
		...
		""" This function evaluates the detection results for a single given image.
			...
			Returns:
				...
				Precision
		"""
		return CyclicIF_JavaInterfacer.EvaluateDetection(Prediction, GroundTruth, Overlap, SaveImageAs,
														JavaJarsPath=JavaJarsPath, JavaXms=JavaXms, JavaXmx=JavaXmx)
