...
from FiReTiTiPyLib.PyTorch_Models.Attentions import Attentions
...
class ResUnetA(nn.Module):
	...
	def __init__(self, Inputs: int=1, Depth: int=6, FeatureMaps: int=32, Activations: str="relu",
				...
				PSPpooling: list=None, BatchNormEncoder: list=None, BatchNormDecoder: list=None, InstNormEncoder: list=None,
				InstNormDecoder: list=None,  Attention: list=None,
				DownSampling: str="stride_conv", UpSampling: str="nearest", DropOutPosition: str="Block", DropOut: list=None,
		...
		if Dilations is None:
			...
			raise Exception("The Dilations must be a tuple of three elements: (From, To, Step). Note that To is included.")
		...
		if FirstLastBlock == "ConvBlock":
			...
			self.convleft.append(ResUNetA_Block(Inputs, FeatureMaps,
												...
												BatchNormalization=BatchNormEncoder[0], InstanceNormalization=InstNormEncoder[0],
												DropOutPosition=DropOutPosition, DropOut=DropOut[0]))
		else:
			raise Exception("No last block defined.") # Cannot happen.

		self.down.append(DownSampleBlock(mode=DownSampling, in_fmaps=FeatureMaps, out_fmaps=FeatureMaps, activation=Activations))
		...
		for d in range(1, Depth-1):
			self.convleft.append(ResUNetA_Block(FeatureMaps*coef, FeatureMaps*coef*2,
												...
												AttentionMecanism=Attention[d], DropOutPosition=DropOutPosition, DropOut=DropOut[d]))
			self.down.append(DownSampleBlock(mode=DownSampling, in_fmaps=FeatureMaps*coef*2, out_fmaps=FeatureMaps*coef*2, activation=Activations))

			self.up.append(UpSampleBlock(FeatureMaps*coef*2, FeatureMaps*coef, mode=UpSampling, activation=Activations))
		...
		if PSPpooling is not None:
			...
			self.convright.insert(0, ResUNetA_Block(FeatureMaps*2, FeatureMaps,
												...
												AttentionMecanism=Attention[0], DropOutPosition=DropOutPosition, DropOut=DropOut[0]))
		else:
			raise Exception("No last block defined.") # Cannot happen.
...
class ResUNetA_Block(nn.Module):
	...
	def __init__(self, in_fmaps, out_fmaps, activation: str="relu", dilations: list=[1], ResidualConnection: str="conv",
		...
		for d in dilations:
			self.branches.append(ResUnetA_Branch(in_fmaps, out_fmaps, dilation=d, activation=activation, BatchNormalization=BatchNormalization,
	...
	def forward(self, x):
		...
		#	results.append(torch.cat((x,x)))
			...
			raise Exception("Unknown residual connection '" + self.ResidualConnection + "', must be among {None, conv}.")
...
class ResUnetA_Branch(nn.Module):
	...
	def __init__(self, in_fmaps, out_fmaps, dilation: int=1, activation: str="relu", BatchNormalization: bool=None, InstanceNormalization: bool=None,
				 DropOut: float=0.0):
		super().__init__()
...
class ConvBlock(nn.Module):
	...
	def __init__(self, in_fmaps, out_fmaps, activation: str=None, BatchNormalization: bool=None, InstanceNormalization: bool=None, DropOut: float=0.0):
		super().__init__()
...
class UpSampleBlock(nn.Module):
	...
	def __init__(self, in_fmaps, out_fmaps, mode="nearest", activation: str=None, BatchNormalization: bool=None, InstanceNormalization: bool=None):
		super().__init__()
