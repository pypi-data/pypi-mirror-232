...
-- Maintained & updated by Simon Perneel
...
def ids_from_server(index="appevents",
    ...
    # Log
    log(f"Getting IDs that have logged {index} between {time_range[0]} and {time_range[1]} for {studyKey if studyKey else 'all studies'}.")
        ...
        body['query'] = {
            'bool': {
                'filter':
                    range_restriction,
                'must':
                    study_restriction if studyKey else {}
            }
        }

    except:
        raise Warning("WARNING: Failed to restrict range or study. Getting all data.")
    ...
    # Log
    log(f"Found {len(ids)} active IDs in {studyKey if studyKey else ''} {index}")
...
def fetch(index: str, ids: list, time_range=('2017-01-01T00:00:00.000', '2024-01-01T00:00:00.000'), app=None) -> dict:
    """
    Fetch data from server, for given ids, within certain timeframe, for given app.
    ...
    :param app: only look at data from this app
    ...
    if len(ids) > 1:
        ...
        for idx, id in enumerate(ids):
            ...
            log(f"Getting data: ID {idx + 1}/{len(ids)}: \t{id}")

            try:
                dump_dict[id] = fetch(index=index, ids=[id], time_range=time_range, app=app)[id]
    ...
    # If there's one ID, fetch data
        ...
        try:
            ...
            body['query']['bool']['filter'].append(range_restriction)

        except:
            log("⚠️ WARNING: Failed to restrict range. Getting all data.", lvl=1)

        # Add extra application restriction if asked
        if app and (index in {"appevents", "notifications"}):
            app_restriction = {
                'term': {'application.keyword': app}
            }
            try:
                body['query']['bool']['filter'].append(app_restriction)

            except:
                log("⚠️ WARNING: Failed to restrict to application. Getting all data.", lvl=1)
        ...
        res = es.search(index="mobiledna",
                        ...
                        scroll='10s',  # Get scroll id to get next results
                        doc_type=index)
        ...
        while 0 < temp_size:
            ...
            sys.stdout.write(f"Entries remaining: {remaining} \r")
            sys.stdout.flush()
...
def pipeline(name: str, ids: list, dir: str,
    ...
    :param time_range: only look in this time range
    :param app: only fetch data from this application
    :param pickle: (bool) export as pickle (default = False)
    :param csv_file: (bool) export as CSV file (default = True)
    :return:
    """

    log(f"Begin pipeline for {len(ids)} IDs, in time range {time_range}.")
    ...
    for index in indices:
        ...
        data = fetch(index=index, ids=ids, time_range=time_range, app=app)
...
if __name__ in ['__main__', 'builtins']:
    ...
    time_range = ('2023-06-22T00:00:00.000', '2023-09-22T00:00:00.000')

    ids = ids_from_server(index='appevents', time_range=time_range, studyKey='pilotUWC')
    ids = list(ids.keys())

    # Split up per id
    """data = split_pipeline(ids=ids,
                          dir=os.path.join(hlp.DATA_DIR, 'total_sample_jan_2023'),
                          subfolder=False,
                          indices=(['appevents']),
                          time_range=time_range,
                          parquet=True,
    ...
    # Fetch all data and save to one file
    data = pipeline(ids=ids, subfolder=False,
                    name="test",
                          dir=os.path.join(hlp.DATA_DIR, 'test'),
                          time_range=time_range,
                          indices=(['appevents']),
                          app=None,   # specify if you only want to fetch data from 1 app instead of appevents from all apps
                          parquet=True,
                          csv_file=False)
