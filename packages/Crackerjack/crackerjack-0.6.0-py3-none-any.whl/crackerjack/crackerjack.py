...
class Crakerjack(BaseModel, arbitrary_types_allowed=True):
    ...
    def run_interactive(hook: str) -> None:
        success: bool = False
        while not success:
            fail = call([Commands.pre_commit, "run", hook.lower(), "--all-files"])
            if fail > 0:
    ...
    async def update_pkg_configs(self) -> None:
        ...
        if not len([pkg for pkg in installed_pkgs if "pre-commit" in pkg]):
            ...
            run([commands.pdm, "add", "-d", "pyright"])
    ...
    async def process(self, options: t.Any) -> None:
        ...
        if options.publish:
            check_output([commands.pdm, "bump", options.publish])
        if not options.do_not_update_configs:
            await self.update_pkg_configs()
        if options.interactive:
            for hook in ("refurb", "pyright"):
                self.run_interactive(hook)
        check_all = call([commands.pre_commit, "run", "--all-files"])
        if check_all > 0:
            call([commands.pre_commit, "run", "--all-files"])
        if options.publish:
        ...
        if options.commit:
            ...
            call([commands.git, "push", "origin", "main"])
