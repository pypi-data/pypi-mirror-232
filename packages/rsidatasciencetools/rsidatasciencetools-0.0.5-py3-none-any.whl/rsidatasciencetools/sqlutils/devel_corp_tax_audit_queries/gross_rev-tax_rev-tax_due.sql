/* grossRev

Gross Revenue, Taxable Revenue, Tax Due
For Criteria of “Significant Decrease in Gross Revenue” */

USE MODRPE

SELECT B.business_id, 
    BL.bus_loc_id,
    B.business_legal_name,
    BL.dlis_branch_number,
    Bl.business_status_id,
    BL.naics_code,
    O.obligation_id,
    O.obl_year,
    O.obl_period,
    O.obl_type_id,
    -- OT.obl_type_code,
    Sum(D.gross_amount) AS sum_of_gross_amount,
    Sum(D.taxable_amount) As sum_of_taxable_amount,
    Sum(D.tax_due_amount) AS sum_of_tax_due,
    -- NAICS.Description
    O.paid_amt


FROM stg.obligation AS O
    INNER JOIN stg.tax_detail_charge AS D
                  ON O.obligation_id = D.obligation_id
    INNER JOIN stg.BUSINESS_LOCATION AS BL 
                  ON O.bus_loc_id = BL.bus_loc_id
    INNER JOIN stg.business AS B
                  ON B.business_id = BL.business_id
    -- INNER JOIN stg.OBL_TYPE_CODE as OT ON O.obl_type_id=OT.obl_type_id
    -- INNER JOIN stg.NAICS ON BL.naics_code=stg.NAICS.Code
  
WHERE -- O.obl_year='2019' AND O.obl_period='03' AND
        O.obl_type_id = 10
        AND O.obl_source_code_id In (5,9,15)
        AND (O.delq_excptn_cd_id Is Null Or O.delq_excptn_cd_id=10)
        AND D.detail_date = (SELECT MAX(Di.detail_date)  
                            FROM stg.tax_detail_charge AS Di  
                            WHERE Di.obligation_id = D.obligation_id)

GROUP BY B.business_id, 
    BL.bus_loc_id,
    B.business_legal_name,
    BL.dlis_branch_number,
    BL.business_status_id,
    BL.naics_code,
    O.obligation_id,
    O.obl_year,
    O.obl_period,
    -- OT.obl_type_code,
    -- stg.NAICS.Description,
    O.obl_type_id,
    O.paid_amt

ORDER BY B.business_id ASC
;
