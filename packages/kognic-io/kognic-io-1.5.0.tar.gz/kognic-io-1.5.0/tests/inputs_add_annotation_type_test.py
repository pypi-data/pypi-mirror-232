...
class TestCameras:
    ...
    def test_invalidate_inputs(self, client: IOC.KognicIOClient):
        ...
        add_annotation_type.run(client=client, input_uuid=scene_uuid, annotation_type="second")

        updated_input = get_inputs_example.run(client=client, input_uuids=[scene_uuid])[0]
