...
def put_context_metrics(
    ...
    if context.instance.organization_name != "test":
        context.log.info(f"{context.instance.dagit_url}graphql")
        transport = RequestsHTTPTransport(
    ...
    for metric_graphql_inputs in chunks(metric_graphql_inputs, 5):
        context.log.info(PUT_CLOUD_METRICS_MUTATION)
        context.log.info({"metrics": metric_graphql_inputs})
        try:
    ...
    for result in run_results["results"]:
        ...
        for adapter_response_key in result["adapter_response"]:
            ...
            if adapter_response_key == "query_id":
                ...
                continue
            if (
                isinstance(result["adapter_response"][adapter_response_key], str)
                and result["adapter_response"][adapter_response_key].isnumeric()
            ):
                metric_values.append(
                    {
                        "metricValue": float(result["adapter_response"][adapter_response_key]),
                        "metricName": adapter_response_key,
                    }
                )
            elif isinstance(result["adapter_response"][adapter_response_key], int) or isinstance(
                result["adapter_response"][adapter_response_key], float
            ):
                metric_values.append(
                    {
