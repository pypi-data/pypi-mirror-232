    ...
    try:
        with self.tracer.start_as_current_span(
            ...
            if should_forward_to_handler:
                ...
                raise TykeException(status_code=403)
    finally:
...
class TykeException(HTTPException):
    """An exception our middleware will raise if a filter determines to block the request"""

class FastAPIInstrumentorWrapper(FastAPIInstrumentor, BaseInstrumentorWrapper):
        ...
        try:
            if self.app:
                async def catch_exceptions_middleware(request: Request, call_next):
                    ...
                    except TykeException:
                        return PlainTextResponse(status_code=403)

                # configure fast api instrumentor w hooks
                FastAPIInstrumentorWrapper.instrument_app(app=self.app,
            ...
            logger.debug('FastAPI instrumentation failed: %s', ex.__str__() )
    ...
    def server_request_hook(self, span, req_data, body):
        ...
        try:
            ...
            logger.debug('server_request_hook: %s', ex.__str__() )
    ...
    def client_response_hook(self, span, resp_data):
        ...
        try:
            ...
            if resp_phase == "http.response.start":
                ...
                span.set_attribute('tyke.capture', should_capture_body)

            elif resp_phase == 'http.response.body':
                should_capture = span.attributes.get('tyke.capture')
                if should_capture:
            ...
            logger.debug('client_response_hook: %s', ex.__str__() )
