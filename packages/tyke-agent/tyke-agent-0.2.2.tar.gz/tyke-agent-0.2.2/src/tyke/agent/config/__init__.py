...
from tyke.agent.config import config_pb2
from tyke.agent.config.default import *
...
class AgentConfig:  # pylint: disable=R0902,R0903
    ...
    def __init__(self):  # pylint: disable=R0912,R0915
        ...
        If 'TYKE_CONFIG_FILE' is specified in the environment data would be loaded from that file.
        ...
        logger.info("Tyke Config : %s", config_dict)
...
def _read_from_file():
    ...
    if len(config_file) == 0:  # pylint:disable=R1705:
        # TYKE_CONFIG_FILE can be passed as empty string which is invalid
