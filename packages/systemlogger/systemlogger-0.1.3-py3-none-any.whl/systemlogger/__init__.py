...
class LoggerConfigurator:
    ...
    def get_logger(self, name: str = "default", application: str = "python") -> logging.Logger:
    ...
    def configure_syslog(self, logger: logging.Logger) -> bool:
        ...
        syslog_url = self.config_parser.get(self.config_section, "syslog_url", fallback=None)
        if not syslog_url:
            return False
    ...
    def configure_loki(self, logger: logging.Logger, application: str = "python") -> bool:
        ...
        loki_url = self.config_parser.get(self.config_section, "loki_url", fallback=None)
        log_source = self.config_parser.get(self.config_section, "log_source", fallback="python")
        if not loki_url:
            return False
    ...
    @staticmethod
    def get_config_parser(filename: Union[str, configparser.RawConfigParser]) -> configparser.RawConfigParser:
    ...
    def configure_console(self, logger: logging.Logger) -> bool:
        ...
        use_console = self.config_parser.getboolean(self.config_section, "console", fallback=False)
        if not use_console:
            return False
    ...
    def configure_file(self, logger: logging.Logger, application: str = "python") -> bool:
        """Write a log file."""
        log_directory = self.config_parser.get(self.config_section, "logfile_directory", fallback=None)
        max_size = self.config_parser.getint(self.config_section, "logfile_max_size", fallback=0)
        backup_count = self.config_parser.getint(self.config_section, "logfile_backup_count", fallback=0)
        ...
        try:
            if max_size > 0:
                handler = RotatingFileHandler(filename, maxBytes=max_size, backupCount=backup_count)
            else:
    ...
    def configure_sentry(self, logger: logging.Logger, application: str = "python") -> bool:
        """Configure Sentry, if available and if required."""
        if sentry_sdk is None:
            return False
        log_source = self.config_parser.get(self.config_section, "log_source", fallback="python")
        sentry_dsn = self.config_parser.get(self.config_section, "sentry_dsn", fallback=None)
        if not sentry_dsn:
            return False
...
def getLogger(
    ...
    """
    configurator = LoggerConfigurator(config=config_filename, config_section=config_section)
