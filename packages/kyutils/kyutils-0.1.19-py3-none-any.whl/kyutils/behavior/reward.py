...
def get_licks_rewards(time, Din_well, Dout_pump):
    ...
    first_lick_times = lick_times[np.nonzero(np.diff(lick_times) > lick_train_interval_s)[0]+1]
    ...
    pump_on_times = [pump_times[i] for i in range(len(pump_times)) if Dout_pump['data']['state'][i]==1]
    
    # delay between detection of lick and delivery of reward (unit: seconds)
    reward_delay_s = 0.2
    rewarded_lick_times = [i for i in first_lick_times if np.min(np.abs(pump_on_times - i))<rewarded_lick_times]

    return lick_times, first_lick_times, pump_on_times, rewarded_lick_times

def plot_performance(ax, first_lick_times_left, first_lick_times_center, first_lick_times_right,
                     rewarded_lick_times_left, rewarded_lick_times_center, rewarded_lick_times_right):

    ax.plot(first_lick_times_left, np.ones(len(first_lick_times_left)), 'bo', markersize=10, markerfacecolor='none')
    ax.plot(rewarded_lick_times_left, np.ones(len(rewarded_lick_times_left)), 'bo', markersize=10)

    ax.plot(first_lick_times_center, 2*np.ones(len(first_lick_times_center)), 'ro', markersize=10, markerfacecolor='none')
    ax.plot(rewarded_lick_times_center, 2*np.ones(len(rewarded_lick_times_center)), 'ro', markersize=10)

    ax.plot(first_lick_times_right, 3*np.ones(len(first_lick_times_right)), 'go', markersize=10, markerfacecolor='none')
    ax.plot(rewarded_lick_times_right, 3*np.ones(len(rewarded_lick_times_right)), 'go', markersize=10)
