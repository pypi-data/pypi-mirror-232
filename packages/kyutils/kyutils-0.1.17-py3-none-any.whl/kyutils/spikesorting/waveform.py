...
def plot_waveforms(waveform_extractor, unit_id, scale_x=150, scale_y=0.5, ax=None):
    ...
    for i in channels_to_plot:
        x, y = waveform_extractor.get_channel_locations()[np.nonzero(waveform_extractor.channel_ids==i)[0][0]]
        ax.plot(t+x/scale_x, waveform_extractor.get_template(unit_id=unit_id)[:,i]+y/scale_y, 'k')
    
    ref_loc = waveform_extractor.get_channel_locations()[np.nonzero(waveform_extractor.channel_ids==channels_to_plot[0])[0][0]]
    ax.plot([-1+ref_loc[0]/scale_x, -1+ref_loc[0]/scale_x], [ref_loc[1]/scale_y, 100+ref_loc[1]/scale_y], 'k-')
