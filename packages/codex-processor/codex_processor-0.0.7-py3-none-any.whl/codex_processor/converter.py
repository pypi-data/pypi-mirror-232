...
class Converter:
    ...
    def convert_file(self, *pypandoc_args, **pypandoc_kwargs):
        args = [
            self.pandoc_path,
            pypandoc_args[0],
            "-t",
            pypandoc_args[1],
            "-o",
            pypandoc_kwargs["outputfile"]
        ]
        if "sandbox" in pypandoc_kwargs:
            args.extend(["--sandbox=false"])
        if "extra_args" in pypandoc_kwargs:
            args.extend(pypandoc_kwargs["extra_args"])
    ...
    def latex_lfix_process(
        ...
        self.convert_file(*pypandoc_args, **pypandoc_kwargs)
    ...
    def process_file(
        ...
        if self.output_format in ("md", "markdown"):
            extension = ".md"
            extra_args.extend(["--wrap=none", "-t", "markdown-smart"])
            if bn == orig_bn:
                bn += ".out"
