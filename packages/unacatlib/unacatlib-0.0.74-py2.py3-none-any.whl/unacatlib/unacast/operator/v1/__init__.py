...
@dataclass(eq=False, repr=False)
class PublishLayerSeriesRevisionRequest(betterproto.Message):
    layer_series_id: str = betterproto.string_field(1)
    layer_series_revision: str = betterproto.string_field(2)


@dataclass(eq=False, repr=False)
class PublishLayerSeriesRevisionResponse(betterproto.Message):
    pass
...
class MapOperatorServiceStub(betterproto.ServiceStub):
    ...
    async def publish_layer_series_revision(
        self,
        publish_layer_series_revision_request: "PublishLayerSeriesRevisionRequest",
        *,
        timeout: Optional[float] = None,
        deadline: Optional["Deadline"] = None,
        metadata: Optional["MetadataLike"] = None
    ) -> "PublishLayerSeriesRevisionResponse":
        return await self._unary_unary(
            "/unacast.operator.v1.MapOperatorService/PublishLayerSeriesRevision",
            publish_layer_series_revision_request,
            PublishLayerSeriesRevisionResponse,
            timeout=timeout,
            deadline=deadline,
            metadata=metadata,
...
class MapOperatorServiceBase(ServiceBase):
    ...
    async def publish_layer_series_revision(
        self, publish_layer_series_revision_request: "PublishLayerSeriesRevisionRequest"
    ) -> "PublishLayerSeriesRevisionResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
    ...
    async def __rpc_publish_layer_series_revision(
        self,
        stream: "grpclib.server.Stream[PublishLayerSeriesRevisionRequest, PublishLayerSeriesRevisionResponse]",
    ) -> None:
        request = await stream.recv_message()
        response = await self.publish_layer_series_revision(request)
        await stream.send_message(response)
    ...
    def __mapping__(self) -> Dict[str, grpclib.const.Handler]:
        return {
            ...
            "/unacast.operator.v1.MapOperatorService/PublishLayerSeriesRevision": grpclib.const.Handler(
                self.__rpc_publish_layer_series_revision,
                grpclib.const.Cardinality.UNARY_UNARY,
                PublishLayerSeriesRevisionRequest,
                PublishLayerSeriesRevisionResponse,
            ),
        }
