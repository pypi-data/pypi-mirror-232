...
def wait_for_thumbs(
    tator_api: TatorApi, project_id: int, wait_list: List[Tuple[datetime, int]]
) -> List[Tuple[datetime, int]]:
    # Wait for imports to generate thumbnails
    logger.info("Waiting for transcodes to generate thumbnails for all uploads...")
    pbar = tqdm(total=len(wait_list), dynamic_ncols=True, position=0, ascii=False)
    wait_list_lookup = {item[1]: item for item in wait_list}
    created_ids = list(wait_list_lookup.keys())
    wait_time = 15  # seconds
    iteration = 0
    iterations_per_minute = ceil(60 / wait_time)
    successes = []
    while created_ids:
        n_waiting_on = len(created_ids)
        logger.info("Media gif thumbnail creation not complete for %d media", n_waiting_on)
        completed_transcodes = []
        iteration += 1
        if iteration % iterations_per_minute == 0:
            # Check transcode status
            logger.info("Checking transcode status for %d media", n_waiting_on)
            transcode_list = tator_api.get_transcode_list_by_id(
                project=project_id, media_id_query={"ids": created_ids}
            )
            if isinstance(transcode_list, list):
                for transcode in transcode_list:
                    if (
                        not transcode
                        or not hasattr(transcode, "job")
                        or not hasattr(transcode, "spec")
                    ):
                        continue
                    status = transcode.job.status
        ...
        for media_id in completed_transcodes:
            if media_id in created_ids:
                logger.debug("Transcode unsuccessful for media '%s'", media_id)
                created_ids.remove(media_id)

        iter_finished = n_waiting_on - len(created_ids)
        if iter_finished:
            pbar.update(iter_finished)
        if created_ids:
            sleep(wait_time)
...
def wait_for_imports(tator_api, project_id, responses):
    """
    Waits for all transcodes started by `import_uploads` to complete and logs their status
    """

    def is_complete(job_dict, job_id):
        job = job_dict.get(job_id, None)

        if job:
            if job.status == "Succeeded":
                logger.info(f"Media import succeeded for job '{job_id}'")
                return True

            if job.status in ["Failed", "Error"]:
                logger.info(f"Media import unsuccessful ({job.status}) for job '{job_id}'")
                return True
        else:
            logger.info(f"Job id '{job_id}' not found, removing from list.")
            return True
    ...
    while responses:
        ...
        logger.info(f"Waiting on {n_responses} import{'s' if n_responses != 1 else ''} to finish")
        time.sleep(15)
        job_dict = {job.uid: job for job in tator_api.get_job_list(project_id)}
        responses = [response for response in responses if not is_complete(job_dict, response.uid)]
