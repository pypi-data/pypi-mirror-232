...
def attributes_from_toc(filename: str, toc_folder: str) -> Dict[str, Any]:
    ...
    logger.warning("Could not find file %s in %d toc file(s)", filename, len(toc_files))
    return {}
            ...
            with tempfile.TemporaryDirectory(dir=work_dir) as data_dir, tempfile.TemporaryDirectory(
                ...
                if filename and os.path.isfile(path):
                    if os.stat(path).st_size:
                        ...
                        for p, job_response in tator.util.upload_media(
                            api=tator_api,
                            type_id=media.type,
                            path=path,
                            ...
                            last_progress = p
                        wait_list.append((media.id, response))
                        updated_media = media
                    else:
                        logger.warning(
