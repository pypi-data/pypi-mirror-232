    ...
    if args.prompt and args.config_file:
        ...
        if f'{key_word}_API_KEY' in os.environ:
            ...
            questions = [inquirer.Text("api_key", message=f"{key_word.title()} API key"),
                inquirer.Text("api_base", message=f"{key_word.title()} API base URL." + (f" Enter to use the default {key_word.title()} URL." if api_type != 'azure' else ""), validate=lambda _ , c: True if api_type != 'azure' else len(c.strip()) > 0),
