...
from mkdocs_revealjs.ci import raise_ci_warnings
    ...
    def get_git_commit_timestamp(
        ...
        try:
            ...
            if self.config.get('fallback_to_build_date'):
                logger.warning(
                    "[mkdocs-revealjs-plugin] Unable to find a git directory and/or git is not installed."
            ...
            if self.config.get('fallback_to_build_date'):
                logger.warning(
                    "[mkdocs-revealjs-plugin] Unable to read git logs of '%s'. Is git log readable?"
                    " Option 'fallback_to_build_date' set to 'true': Falling back to build date"
                    % path
                )
                commit_timestamp = time.time()
                ...
                logger.error(
                    "[mkdocs-revealjs-plugin] Unable to read git logs of '%s'. "
                    " To ignore this error, set option 'fallback_to_build_date: true'"
                    % path
                )
                raise err
            ...
            if self.config.get('fallback_to_build_date'):
                logger.warning(
                    "[mkdocs-revealjs-plugin] Unable to perform command: 'git log'. Is git installed?"
                    " Option 'fallback_to_build_date' set to 'true': Falling back to build date"
                )
                commit_timestamp = time.time()
            else:
        ...
        if commit_timestamp == "":
            commit_timestamp = time.time()
            logger.warning(
                "[mkdocs-revealjs-plugin] '%s' has no git logs, using current timestamp"
                % path
            )
    ...
    def add_spans(date_formats: Dict[str, str]) -> Dict[str, str]:
        ...
        for date_type, date_string in date_formats.items():
            date_formats[date_type] = (
                '<span class="mkdocs-revealjs-plugin mkdocs-revealjs-plugin-%s">%s</span>'
