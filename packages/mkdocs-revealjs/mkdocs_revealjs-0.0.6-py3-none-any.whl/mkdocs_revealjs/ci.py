...
def raise_ci_warnings(repo) -> None:
    ...
    if os.getenv("GITLAB_CI") is not None and n_commits < 50:
        ...
        logging.warning(
            """
                [mkdocs-revealjs-plugin] Running on a GitLab runner might lead to wrong
    ...
    # Github Actions
        ...
        logging.warning(
            """
                [mkdocs-revealjs-plugin] Running on GitHub Actions might lead to wrong
                Git revision dates due to a shallow git fetch depth.
    ...
    # Does not limit fetch-depth by default
        ...
        logging.warning(
            """
                [mkdocs-revealjs-plugin] Running on Azure pipelines with limited
    ...
    # Bitbucket pipelines
        ...
        logging.warning(
            """
                [mkdocs-revealjs-plugin] Running on bitbucket pipelines might lead to wrong
                Git revision dates due to a shallow git fetch depth.
...
def is_shallow_clone(repo) -> bool:
    ...
    References & Context:
    https://github.com/timvink/mkdocs-mkdocs-revealjs-plugin/issues/10
