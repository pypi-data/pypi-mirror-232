...
class BaseBrowser(CommonBase):
    ...
    def get_element_by_xpath_wait(self, xpath, attempts=3) :
        ...
        try:
            ...
            self.logger.warn(f"xpath: {xpath} , Element not found, retrying...")
            attempts -= 1
            if attempts <= 0:  # stop after 3 attempts
                self.logger.warn("Failed to find the element after 3 attempts.")
                return None
            self.browser.refresh()
