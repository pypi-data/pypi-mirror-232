...
class GenericCRUD:
    ...
    def select_one_by_id(self, table_name: str, select_clause_value: str = "*", id_column_name: str = None, id_column_value: Any = None) -> tuple:
        ...
        where = f"{id_column_name}={id_column_value}"
        try:
            result = self._select_by_where(table_name, select_clause_value, where, limit=1)[0]
        except Exception as e:
            logger.exception("Error selecting json_data", object=e)
            logger.end()
            raise
    ...
    def select_one_by_where(self, table_name: str, select_clause_value: str = "*", where: str = None) -> list:
        ...
        logger.start(object={"table_name": table_name, "select_clause_value": select_clause_value, "where": where})
        try:
            result =  self._select_by_where(table_name, select_clause_value, where=where, limit=1)[0]
        except Exception as e:
            logger.exception("Error selecting json_data", object=e)
            logger.end()
            raise
