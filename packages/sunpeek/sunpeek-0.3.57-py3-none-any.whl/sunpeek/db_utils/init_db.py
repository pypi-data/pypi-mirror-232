...
from sunpeek.definitions import collector_types, fluid_definitions
...
def init_db():
    ...
    if not database_exists(db_url):
        ...
        with utils.S.begin() as session:
            # Add collector types
            for item in collector_types.all_definitions:
                session.add(item)
...
def apply_db_migrations():
    ...
    col_types = [(item, item.name) for item in collector_types.all_definitions]
    ...
    with utils.S.begin() as session:
        with session.no_autoflush:
            ...
            for col_type, name in col_types:
                try:
                    existing_col = session.query(cmp.CollectorType).filter(cmp.CollectorType.name == name).one()
