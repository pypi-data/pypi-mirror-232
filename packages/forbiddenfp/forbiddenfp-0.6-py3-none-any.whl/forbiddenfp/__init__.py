...
falseful = negate_func(bool)
chain_as("falseful")(falseful)
...
@chainable
def map_pred(self: _P, *pred_and_val: Tuple[_Pred, _R], default: Optional[_R] = None) -> Optional[_R]:
    for pred, val in pred_and_val:
        if pred(self):
            return val
    return default
...
@chainable
def match_pred(self: _P, *pred_and_action: Tuple[_Pred, _R], default: _F = always(None)) -> Optional[_R]:
    for pred, action in pred_and_action:
        if pred(self):
            return action(self)
    return default(self)
...
@chainable
def filter_key(self: Dict[_P, _P2], predicate: Callable[[_P], bool]) -> Dict[_P, _P2]:
