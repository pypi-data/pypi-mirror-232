...
from typing import Callable, List, NoReturn, Optional, Dict, TypeVar, Iterable, Sequence, Tuple, Union, Set
...
@chain_only
def map(self: Iterable[_P], func: _F, /) -> Iterable[_R]:
...
@chain_only
def filter(self: Iterable[_P], predicate: _Pred, /) -> Iterable[_P]:
    return (i for i in self if predicate(i))
...
@chain_only
def next(self: Iterable[_P], predicate: _Pred = always(True), /) -> _P:
...
@chain_only
def partition(self: Iterable[_P], predicate: _Pred, /) -> Tuple[List[_P], List[_P]]:
    true = []
    false = []
    for i in self:
        true.append(i) if predicate(i) else false.append(i)
    return true, false
...
@chain_only
def join(self: Iterable[_P], sep: builtins.str = "", /) -> builtins.str:
    return sep.join(builtins.map(builtins.str, self))
