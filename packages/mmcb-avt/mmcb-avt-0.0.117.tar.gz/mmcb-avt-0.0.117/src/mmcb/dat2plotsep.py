...
def plot_raw(data, units, infile, colour_map, marks):
    """
    ...
    for index, column in enumerate(x for x in data.columns if x != 'timestamp'):
        unit = column.split(' ')[-1]
        myax = data.plot(
            kind='line', x='timestamp', y=column, ax=axes[index],
            alpha=lal, linewidth=lwi, color=colour_map
        )
...
def main():
    ...
    colour_map = dict(zip(unique_sensor_name, itertools.cycle(colours)))
    ...
    for column in columns:
        ...
        cmap[column] = colour_map[matching_sensor]
    ...
    plot_raw(data, units, infile, cmap, marks)
