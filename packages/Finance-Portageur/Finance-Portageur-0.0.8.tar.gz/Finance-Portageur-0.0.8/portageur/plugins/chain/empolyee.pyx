...
class Empolyee(BaseModel):
    ...
    def run(self, code, document_len_limit=2000, review_num=1500, topic=None):
        ...
        review_df = loader_emplyee.fetch_es_reviews(code=code,
                                                    review_num=review_num, topic=topic)
        ...
        reviews = self.summarize_single_docs(
            docs=review_docs, llm_advanced=llm_advanced.instance(), topic=topic) if len(
                review_docs) == 1 else self.summarize_multi_docs(
                    docs=review_docs, llm=llm.instance(), llm_advanced=llm_advanced.instance())
        return reviews
