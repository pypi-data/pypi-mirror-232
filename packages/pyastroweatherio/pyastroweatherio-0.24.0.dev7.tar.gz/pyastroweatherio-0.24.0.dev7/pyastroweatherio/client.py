...
class AstroWeather:
    ...
    async def _get_location_data(self) -> None:
        ...
        for row in self._weather_data_seventimer:
            ...
            item = {
                ...
                "forecast_length": (
                    len(self._weather_data_seventimer) - forecast_skipped
                )
                * 3,
            ...
            if (
                ...
                item["cloud_area_fraction_high"] = details.get(
                    "cloud_area_fraction_high", -1
                )
                item["cloud_area_fraction_low"] = details.get(
                    "cloud_area_fraction_low", -1
                )
                item["cloud_area_fraction_medium"] = details.get(
    ...
    async def _get_deepsky_forecast(self):
        ...
        for row in self._forecast_data:
            ...
            # darkness.
            if (
                hour_of_day < sun_next_setting_astro.hour
                and hour_of_day > sun_next_rising_astro.hour
            ):
                start_forecast_hour = 0
                start_weather = ""
                interval_points = []
            ...
            if self._metno_enabled and cloud_area_fraction > 0:
                interval_points.append(
                    await self.calc_condition_percentage(
                        cloud_area_fraction, seeing, transparency
                    )
                )
            else:
                interval_points.append(
                    await self.calc_condition_percentage(
                        cloudcover, seeing, transparency
                    )
                )
