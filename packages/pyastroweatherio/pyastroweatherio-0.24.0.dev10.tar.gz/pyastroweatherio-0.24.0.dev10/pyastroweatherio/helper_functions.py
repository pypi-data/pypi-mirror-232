...
class AstronomicalRoutines:
    ...
    def calc(
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if len(w):
                if issubclass(w[-1].category, TargetAlwaysUpWarning):
                    ...
                    self._sun_next_setting_civil = self.utc_to_local((time + 1 * u.day).to_datetime())
                    self._sun_next_rising_civil = self.utc_to_local(time.to_datetime())
                w.clear()
            else:
                darkness = "civil"
                ...
                # Compatibility
                self._sun_next_setting_civil = self.utc_to_local(sun_next_setting_civil.to_datetime())
                self._sun_next_rising_civil = self.utc_to_local(sun_next_rising_civil.to_datetime())
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if len(w):
                ...
                # Compatibility
                self._sun_next_setting_nautical = self.utc_to_local(sun_next_setting_nautical.to_datetime())
                self._sun_next_rising_nautical = self.utc_to_local(sun_next_rising_nautical.to_datetime())
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if len(w):
                ...
                # Compatibility
                self._sun_next_setting_astro = self.utc_to_local(sun_next_setting_astronomical.to_datetime())
                self._sun_next_rising_astro = self.utc_to_local(sun_next_rising_astronomical.to_datetime())
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if len(w):
                ...
                # Compatibility
                self._moon_next_setting = self.utc_to_local(moon_next_setting_civil.to_datetime())
                self._moon_next_rising = self.utc_to_local(moon_next_rising_civil.to_datetime())
    ...
    def need_update(self):
        ...
        if (
            ...
            self.calc(self._longitude, self._latitude, self._elevation, self._timezone_info)
        else:
