...
def LoadFont( name='monaco', emWidthInPixels=None, lineHeightInPixels=None ):
	...
	font_size = 1
	def GetFontSize( font, txt ):
		try: font.getbbox
		except: return font.getsize( txt ) # getsize was removed around pillow version 9.5.0
		else: return font.getbbox( txt )[ 2: ]
	while True:
		...
		if emWidthInPixels is not None and GetFontSize( font, 'm' )[ 0 ] >= emWidthInPixels: break
		if lineHeightInPixels is not None and GetFontSize( font, PROBE_NOMINAL_LINE_HEIGHT )[ 1 ] >= lineHeightInPixels: break
		font_size += 1
	return font

def GetTextSizeSafely( draw, string, **kwargs ):
	if string: string = string.strip( '\n' )
	safeString = string if string else ' '
	try: func = draw.textbbox; pargs = [ [ 0, 0 ], safeString ]
	except: func = draw.textsize; pargs = [ safeString ] # textsize was removed around pillow version 9.5.0
	x, y = func( *pargs, **kwargs )[ -2: ]
	if not string: x = 0
	return x, y
...
def RenderTextOntoImage( imageNumpy, position, string, border=( 0, 0 ), linespacing=1.1, align='left', anchor='lower left', bg=None, blockbg=None, defer=False, wrapping=None, **kwargs ):
	...
	lineHeight = GetTextSizeSafely( draw, PROBE_NOMINAL_LINE_HEIGHT, font=font )[ 1 ]
	fullLineHeight = GetTextSizeSafely( draw, PROBE_FULL_LINE_HEIGHT, font=font )[ 1 ]
	...
	string = WrapString( string,
		...
		spacesPerTab = 4,
		widthFunc=lambda s: GetTextSizeSafely( draw, s, font=font )[ 0 ],
		justify = wrapping < 0,
	...
	widths, heights = zip( *[ GetTextSizeSafely( draw, line, font=font ) for line in lines ] )
