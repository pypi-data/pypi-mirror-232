...
class AttenuationCls:
	...
	def set(self, attenuation: float) -> None:
		...
		Snippet: driver.configure.system.z320.attenuation.set(attenuation = 1.0) \n
		No command help available \n
			:param attenuation: No help available
		"""
		param = Conversions.decimal_value_to_str(attenuation)
		self._core.io.write(f'CONFigure:SYSTem:Z320:ATTenuation {param}')

	def get(self, connector_name: str) -> float:
		"""SCPI: [CONFigure]:SYSTem:Z320:ATTenuation \n
		Snippet: value: float = driver.configure.system.z320.attenuation.get(connector_name = '1') \n
		No command help available \n
		...
		response = self._core.io.query_str(f'CONFigure:SYSTem:Z320:ATTenuation? {param}')
