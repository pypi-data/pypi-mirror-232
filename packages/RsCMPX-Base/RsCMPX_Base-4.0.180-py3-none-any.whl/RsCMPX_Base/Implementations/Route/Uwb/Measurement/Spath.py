...
class SpathCls:
	...
	def get_count(self) -> int:
		...
		Snippet: value: int = driver.route.uwb.measurement.spath.get_count() \n
		No command help available \n
			:return: signal_path_count: No help available
		"""
		response = self._core.io.query_str('ROUTe:UWB:MEASurement<Instance>:SPATh:COUNt?')
		return Conversions.str_to_int(response)

	def get_value(self) -> List[str]:
		"""SCPI: ROUTe:UWB:MEASurement<Instance>:SPATh \n
		Snippet: value: List[str] = driver.route.uwb.measurement.spath.get_value() \n
		No command help available \n
	...
	def set_value(self, signal_path: List[str]) -> None:
		"""SCPI: ROUTe:UWB:MEASurement<Instance>:SPATh \n
		Snippet: driver.route.uwb.measurement.spath.set_value(signal_path = ['1', '2', '3']) \n
		No command help available \n
