...
class MultiEvalCls:
	...
	def get_source(self) -> str:
		...
		Snippet: value: str = driver.trigger.lteDl.measurement.multiEval.get_source() \n
		No command help available \n
			:return: trigger: No help available
		"""
		response = self._core.io.query_str('TRIGger:LTEDl:MEASurement<Instance>:MEValuation:SOURce?')
		return trim_str_response(response)

	def set_source(self, trigger: str) -> None:
		"""SCPI: TRIGger:LTEDl:MEASurement<Instance>:MEValuation:SOURce \n
		Snippet: driver.trigger.lteDl.measurement.multiEval.set_source(trigger = '1') \n
		No command help available \n
			:param trigger: No help available
		"""
		param = Conversions.value_to_quoted_str(trigger)
		self._core.io.write(f'TRIGger:LTEDl:MEASurement<Instance>:MEValuation:SOURce {param}')
