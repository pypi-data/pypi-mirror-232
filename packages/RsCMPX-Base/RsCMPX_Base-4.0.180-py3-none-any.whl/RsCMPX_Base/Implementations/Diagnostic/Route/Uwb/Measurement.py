...
class MeasurementCls:
	...
	def get_spath(self) -> List[str]:
		...
		Snippet: value: List[str] = driver.diagnostic.route.uwb.measurement.get_spath() \n
		No command help available \n
			:return: signal_path: No help available
		"""
		response = self._core.io.query_str('DIAGnostic:ROUTe:UWB:MEASurement<Instance>:SPATh?')
		return Conversions.str_to_str_list(response)

	def set_spath(self, signal_path: List[str]) -> None:
		"""SCPI: DIAGnostic:ROUTe:UWB:MEASurement<Instance>:SPATh \n
		Snippet: driver.diagnostic.route.uwb.measurement.set_spath(signal_path = ['1', '2', '3']) \n
		No command help available \n
			:param signal_path: No help available
		"""
		param = Conversions.list_to_csv_quoted_str(signal_path)
		self._core.io.write(f'DIAGnostic:ROUTe:UWB:MEASurement<Instance>:SPATh {param}')
