...
from ...plugins.sqlalchemy.queries import SqlAlchemySyncQuery

from ...integration.database_errors import TableAlreadyExistsError, NonExistingTableError
...
class DatabaseValidator(AbstractDatabaseValidator):
    ...
    def __init__(self, db_connection: AbstractQueryExecutor[AsyncConnection], schema: str, table_name: str) -> None:
    ...
    async def validate_init(self) -> None:
        await self._db_connection.execute(
            SqlAlchemySyncQuery[None](
                lambda connection: self._check_table(connection, self._table_name, self._schema, False)
            )
        )
    ...
    async def validate_sync(self) -> None:
        await self._db_connection.execute(
            SqlAlchemySyncQuery[None](
                lambda connection: self._check_table(connection, self._table_name, self._schema, True)
            )
        )

    @staticmethod
    def _check_table(connection: Connection, table_name: str, table_schema: str, should_exist: bool) -> None:
