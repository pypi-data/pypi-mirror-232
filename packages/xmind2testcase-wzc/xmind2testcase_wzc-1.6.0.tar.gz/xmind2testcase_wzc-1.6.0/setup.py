...
with io.open(os.path.join(here, 'xmind2testcase_wzc', '__about__.py'), encoding='utf-8') as f:  # custom
    exec(f.read(), about)
...
class PyPiCommand(Command):
    ...
    def run(self):
        ...
        try:
            ...
            rmtree(os.path.join(here, 'xmind2testcase_wzc.egg-info'))  # custom
        except OSError:
            pass
...
setup(
    ...
    entry_points={  # custom
        'console_scripts': [
            'xmind2testcase_wzc=xmind2testcase_wzc.cli:cli_main',
        ]
    },
    cmdclass={
