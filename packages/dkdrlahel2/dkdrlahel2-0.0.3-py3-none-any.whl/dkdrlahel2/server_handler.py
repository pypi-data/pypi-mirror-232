...
async def get_password(inquiry_code: str) -> Union[dict[str, Any], None]:
    ...
    return await handle_request(url, data_s, headers)


async def get_password_refresh(
...
async def get_token(
    ...
    payload = await handle_request(url, data_s, headers)
...
async def download_save(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url, data=data_s, headers=headers) as response:
...
async def upload_save_data(
    ...
    if not config_manager.get_config_value_category("SERVER", "UPLOAD_METADATA"):
        items = []
    save_key = await get_save_key(token)
    ...
    return await handle_request(url, body, headers)
...
async def upload_save_data_v2(
    ...
    if not config_manager.get_config_value_category("SERVER", "UPLOAD_METADATA"):
        items = []
    save_key_data = await get_save_key_data(token)
    ...
    return await handle_request(url_2, meta_data, headers_2)


async def upload_metadata_v2(
    token: str,
    inquiry_code: str,
    save_data: bytes,
    play_time: int,
...
async def get_save_key_data(token: str) -> Optional[dict[str, Any]]:
    ...
    payload = await handle_request(url, None, headers, True)
    if payload is not None:
        return payload
...
async def get_save_key(token: str) -> Optional[str]:
    """Gets the save key for the given token
    ...
    """
    data = await get_save_key_data(token)
    if data is not None:
        return data["key"]
    return None
...
async def upload_metadata(
    ...
    return await handle_request(url, managed_item_details_str, headers)
...
async def update_managed_items(
...
async def check_gen_token(
    save_stats: dict[str, Any]
) -> dict[str, Any]:
    """Gets the account auth token"""

    save_stats, password_refresh_data = await check_gen_password(save_stats)
...
async def check_gen_password(
    ...
    password_refresh_data = await get_password_refresh(inquiry_code, password_refresh_token)
    if password_refresh_data is not None:
        return save_stats, password_refresh_data

    password_refresh_data = await get_password(inquiry_code)
    if password_refresh_data is None:
        inquiry_code = get_inquiry_code()
        save_stats["inquiry_code"] = inquiry_code

        return await check_gen_password(save_stats)
...
async def prepare_upload(
    ...
    data = await check_gen_token(save_stats)
    token = data["token"]
...
async def upload_handler(
    ...
    data = await prepare_upload(save_stats)
    if data is None:
        return None
...
async def meta_data_upload_handler(
    save_stats: dict[str, Any]
