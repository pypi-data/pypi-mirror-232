    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if latest == version:
                ...
                print(f"[{blue}Version{reset}]: Subdominator current version {version} is({red}outdated{reset})")
                
                print(f"[{red}ALERT{reset}]: Subdominator new version detected update to latest version check the github page: https://github.com/sanjai-AK47/Subdominator")
...
def api(domain,  filepath):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=200000) as executor:
        ...
        futures = [executor.submit(api_virustotal, domain, filepath),
                  ...
                  executor.submit(api_binaryegde, domain, filepath),
