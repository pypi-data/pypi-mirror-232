...
def get_next_cluster_device_order(db_controller):
    ...
    for node in db_controller.get_storage_nodes():
        for dev in node.cluster_devices:
    ...
    for index, nvme in enumerate(nvme_devs):
        ...
        for nvm_part in ret:
            ...
            cluster_devices.append({
                ...
                "port": "4420",
                "status": "online",
                "cluster_device_order": dev_order
            })
            dev_order += 1
    ...
    for snode in snodes:
        ...
        for i, dev in enumerate(snode.cluster_devices):
            ...
            if snode.get_id() == target_node.get_id():
                ...
                for remote_dev in target_node.remote_devices:
                    if remote_dev['id'] == dev['id']:
                        name = remote_dev['remote_name']
                        break
            dev_map[f"{index}"] = {  # could be dev[order]
                "UUID": dev['id'],
                "bdev_name": name,
                "status": "online"}
            dev_w_map.append({
