...
with description('CndApp') as self:
    with before.each:
        ...
        self.instance = self.instance.host

    with context("__init__"):
        with it("shoud get an instance"):
            expect(isinstance(self.instance, aap._host.Host)).to(equal(True))

    with context("data"):
        with it("shoud support the right field"):
            result = self.instance.data("name", "inventory_id", description="description", variables={'c': 'd'})
            expected_result = {
                'name': 'name',
