"""
   ...
   NTRIP module of pygnss. Based on NtripClient_.
   .. _NtripClient: https://github.com/jcmb/NTRIP
"""
...
class NtripClient(object):
    def __init__(self,
        ...
        self.buffer = buffer
        self.user = user   # base64.b64encode(user)
        self.out = out
        self.port = port
        self.caster = caster
    ...
    def getGGAString(self, now=None):
        if not now:
            now = datetime.datetime.utcnow()
        ggaString = ("GPGGA,%02d%02d%04.2f,%02d%011.8f,%1s,%03d%011.8f,%1s"
                     ",1,05,0.19,+00400,M,%5.3f,M,,") % \
            (now.hour, now.minute, now.second, self.latDeg, self.latMin,
             self.flagN, self.lonDeg, self.lonMin, self.flagE, self.height)
        ...
        try:
            while reconnectTry <= maxReconnect:
                ...
                if error_indicator == 0:
                    ...
                    print(self.getMountPointString())
                    ...
                    if reconnectTry < maxReconnect:
                        sys.stderr.write(("%s No Connection to NtripCaster."
                                          "  Trying again in %i seconds\n") % (
                    ...
                    self.socket = None
                    if self.verbose:
                        print("Error indicator: " + str(error_indicator))
...
if __name__ == '__main__':
    ...
    parser.add_option("-o", "--org", type="string", dest="org",
                      help=("Use IBSS and the provided organization for"
                            " the user. Caster and Port are not needed"
                            " in this case Default: %default"))
    parser.add_option("-b", "--baseorg", type="string", dest="baseorg",
                      help=("The org that the base is in. IBSS Only,"
                            " assumed to be the user org"))
    parser.add_option("-t", "--latitude", type="float", dest="lat",
                      default=50.09, help="Your latitude.  Default: %default")
    parser.add_option("-g", "--longitude", type="float", dest="lon",
                      default=8.66, help="Your longitude.  Default: %default")
    parser.add_option("-e", "--height", type="float", dest="height",
                      default=1200,
    ...
    parser.add_option("-s", "--ssl", action="store_true", dest="ssl",
                      default=False, help="Use SSL for the connection")
    parser.add_option("-H", "--host", action="store_true", dest="host",
                      default=False,
                      help="Include host header, should be on for IBSS")
    parser.add_option("-r", "--Reconnect", type="int",
                      dest="maxReconnect", default=1,
    ...
    parser.add_option("-2", "--V2", action="store_true", dest="V2",
                      default=False, help="Make a NTRIP V2 Connection")
    parser.add_option("-f", "--outputFile", type="string", dest="outputFile",
                      default=None,
                      help="Write to this file, instead of stdout")
    parser.add_option("-m", "--maxtime", type="int", dest="maxConnectTime",
                      default=None,
                      help="Maximum length of the connection, in seconds")

    parser.add_option("--Header", action="store_true", dest="headerOutput",
                      default=False, help="Write headers to stderr")
    parser.add_option("--HeaderFile", type="string", dest="headerFile",
                      default=None,
