...
"""
   ...
   I/O module of pygnss. Including reading and writting rinex file,
...
def parse_brdc_block(fstream):
    ...
    ndata = np.zeros((len(pd_time_set), len(prn_set),
                      n_valid_items), dtype=np.float) + np.nan
...
def _parse_ssr_block(fread):
    ...
    clk_np = np.array(clock_srr, dtype=np.float)
    ...
    orb_np = np.array(orbit_srr, dtype=np.float)
...
def _parse_sp3_blocks(fstream, header):
    ...
    ndata = np.array(epochs_data, dtype=np.float)
...
def read_south_csv(filename):
    ...
    with open(filename, "rb") as fr:
        ...
        coords = {'time': obj_time_list,
                  'data': ['x', 'y', 'z']}
        dims = ['time', 'data']
        ndata = np.array(xyz_list, dtype=np.float)
...
def _parse_map_block(map_lines, header):
    ...
    data = np.zeros((nrow, ncol), dtype=np.float) + np.nan
...
def read_ascii_raster(filename):
    ...
    for i, line in enumerate( lines[6:]):
        ...
        raster[i] = np.asarray(data, dtype=np.float)
...
def read_GGOS_VMFG(fn,lowB,upperB,leftL,rightL):
    ...
    with open(fn, "r") as fs:
        ...
        matZTD = np.zeros((rows,cols),dtype=np.float)
...
def read_ztd_file(filename):
    ...
    ndata = np.zeros((len(obj_time_list), 3), dtype=np.float)
    ndata[:, 0] = content["zhd"]
    ndata[:, 1] = content["zwd"]
    ndata[:, 2] = content["ztd"]
    coords = {'time': obj_time_list,
