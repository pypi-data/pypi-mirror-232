...
// OWA Addition ###############################################################
// Inflow validation function for OWA toolkit API use
...
// Return: returns Error Code
{
    ...
    {
        errcode = ERR_TKAPI_POLLUT_INDEX;
    }
    // Validate Type
    ...
    {
        errcode = ERR_TKAPI_TSERIES_INDEX;
    }
    // Validate Timepattern Index
    else if (basePat >= Nobjects[TIMEPATTERN])
    {
        errcode = ERR_TKAPI_PATTERN_INDEX;
    }
    else
    {
        // --- assign type & cf values for a FLOW inflow
...
{
    // OWA EDIT ################################################################
    // OWA SWMM addition for inflow_setExtInflow to return error if 
    // inflow does not validate. EPA SWMM always returns error code 0
    int errcode = 0;
    TExtInflow* inflow;            // external inflow object

    // Validate Inflow for OWA toolkit API use
    ...
    // ##########################################################################
        ...
        inflow->extIfaceInflow = 0.0;   // OWA addition - inflow struct prop for holding external inflow set by OWA toolkit API
    }
    return(errcode);
}
...
//
{
    ...
    double extIfaceInflow = inflow->extIfaceInflow;  // OWA Addition - for toolkit API external interfacing inflow
    ...
    // OWA Edit #############################################################
    // EPA removed extIfaceInflow usage in SWMM 5.2.0.     
    // OWA keeps it to use with toolkit API in addition to new apiExtInflow
    // used by EPA API
