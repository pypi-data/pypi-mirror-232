...
text = ["the keys to the cabinet are", "the keys to the cabinet is"]

# we want to test if p(are | prefix) > p(is | prefix)
model = "gpt-3.5-turbo-instruct"
query = mo.OpenAIQuery(model, text)
...
# NOTE: this will not be a 100% match since it seems OpenAI adds a little noise to its outputs.
