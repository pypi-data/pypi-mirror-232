...
class ImageCrop:
    ...
    def self_adaption_width_crop(self, top, bottom, crop_point_result_dict, origin_slot, img=None, img_width=None,
                                  img_height=None):
        # img_self-adaption: true,false     图片自适应
        # layout_self-adaption: true,false  画布自适应
        ...
        # 加上内边界后 裁剪点 x1,x2
        height_scale = origin_slot_height / img_height
        transform_img_width = height_scale * img_width
        scale_x = scale_y = height_scale
        x = 0  # slot左边 距离 原点的位置
        y = 0
        ...
        amend_layout_width = (origin_slot_width - transform_img_width) / 2

        self.si.img_scale_ratio(scale_x, scale_y)
        # print(f'自适应裁剪: slot高度{origin_slot_height} -> {amend_layout_height}')
        return x * img_width, y * img_height, crop_width, crop_height, scale_x, scale_y, amend_layout_width
    ...
    def _need_center_crop(self, crop_method, slot_width, slot_height, img):
        ...
        if crop_method == 'w':  # 宽固定，高自适应
            ...
            get_crop_point_result_dict = {'x1': 0, 'x2': 1}
            crop_x, crop_y, crop_width, crop_height, scale_x, scale_y, amend_layout_left = self.self_adaption_width_crop(
                0, 1, get_crop_point_result_dict, [slot_width, slot_height], img
            )
            return crop_x, crop_y, crop_width, crop_height, scale_x, scale_y, 0, img.width, img.height, amend_layout_left
        else:
