...
def get_KEGG_annot_df(annotation_dir, stats_level, annotation_df_absolute_num, annotation_df_pct, annotation_df_pct_by_all, D2ABCD_dict, with_depth, pct_by_all):
    ...
    for each_ko in all_identified_ko_list:
        ko_with_desc = '%s_%s' % (each_ko, D2ABCD_dict[each_ko])
...
def Annotation_KEGG(args):
    ...
    if os.path.isdir(input_file_folder) is True:
        ...
        for ko_level in ['A', 'B', 'C', 'D']:
            ...
            get_KEGG_annot_df(output_folder, ko_level, annotation_df_GeneNumber, annotation_df_GeneNumber_pct, annotation_df_GeneNumber_pct_by_all, D2ABCD_dict, with_depth=False, pct_by_all=pct_by_all)
            ...
            if depth_file is not None:
                get_KEGG_annot_df(output_folder, ko_level, annotation_df_TotalDepth, annotation_df_TotalDepth_pct, annotation_df_TotalDepth_pct_by_all, D2ABCD_dict, with_depth=True, pct_by_all=pct_by_all)
