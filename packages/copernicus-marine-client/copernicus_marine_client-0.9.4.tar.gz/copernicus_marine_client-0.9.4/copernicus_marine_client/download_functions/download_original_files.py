...
def download_original_files(
    ...
    return download_files(endpoint, bucket, filenames_in, filenames_out)
...
def _download_header(
    ...
    raw_filenames = list_files_on_marine_data_lake_s3(
        endpoint_url, bucket, path
    )
    filename_filtered = []
...
def _download_files(
    tuple_original_files_filename: Tuple[
        str, str, list[str], list[pathlib.Path]
    ],
) -> list[pathlib.Path]:
