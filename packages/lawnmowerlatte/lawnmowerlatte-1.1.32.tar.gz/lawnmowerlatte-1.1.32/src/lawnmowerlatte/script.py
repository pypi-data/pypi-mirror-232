...
from lawnmowerlatte.imports import mock, doctest
...
"""
    ...
    def test_globs(self):
        ...
        ['mock', 'script']
        """
        from lawnmowerlatte.imports import mock

        globs = {"mock": mock, "script": type(self)("")}
        return globs

    def test(self, mock_test=False):
        module = sys.modules[self.__class__.__module__]

        result = test_module(module, self.test_globs())

        if result is not None:
            self.error(result)


def test_module(module, test_globs=None):
