...
for count_file in os.listdir(sys.argv[1]):
	...
	# Filter files
	if count_file.endswith('.summary'): continue
	...
	if merged_dataframe.empty: merged_dataframe = pd.DataFrame(count_series, columns = [sample])
	else: merged_dataframe[sample] = count_series
