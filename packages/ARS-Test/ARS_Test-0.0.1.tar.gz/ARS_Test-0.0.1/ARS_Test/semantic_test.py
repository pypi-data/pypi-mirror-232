...
def test_must_have_curie(query_type, input_curie, output_curie, expected_output, ARS_URL):
...
def run_semantic_test(env, query_type, input_curie, output_curie, expected_output):
    
    ars_env = env_spec[env]
    ARS_URL = f'https://{ars_env}.transltr.io/ars/api/'
    #get the report cards
    report_cards = test_must_have_curie(query_type, input_curie, output_curie, expected_output, ARS_URL)
