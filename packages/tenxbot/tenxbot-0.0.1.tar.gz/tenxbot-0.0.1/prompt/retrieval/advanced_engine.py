...
class AdvancedWeaviateQueryEngine(SimpleWeaviateQueryEngine):
    def query(self, query_string: str, model: str) -> tuple:
        ...
        query_results = (
            SimpleWeaviateQueryEngine.client.query.get(
                class_name="Chunk",
        ...
        try:
            ...
            completion = openai.ChatCompletion.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are a Retrieval Augmented Generation chatbot. "
                        "Try to answer this user query {query_string} with only the provided "
                        "context. If the provided documentation does not provide enough information, "
                        "say so. If the answer requires code examples encapsulate them "
                        "with ```programming-language-name ```. Don't do pseudo-code.",
                    },
