...
def save_auth(env_name: str, endpoint: str, access_key: str, secret_key: str):
    env_cfg = {'use': '', 'envs': {}}
    if ENV_PATH.exists():
        try:
            env_cfg = get_env_cfg()
...
def make_request(access_key, secret_key, method, url, params=None, json=None, headers=None,
...
def get_user_agent(user_agent: Union[dict, str, None] = None, ) -> str:
        ...
        ua = 'scoamp/%s; python/%s; platform/%s; processor/%s' % (
            __version__,
