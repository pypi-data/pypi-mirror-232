...
def trade_func(
    ...
    def UNIVERSE(GUBUN):
        ...
        : 한국거래소(유가증권시장 및 코스닥시장) 종목 중 다음 조건을 만족하는 종목을 스크리닝 한다.
         1) <성장/가치주 > 시가총액이 2000억원 이상 포함, <혼합주> 시가총액이 1350억원 이상, 10조 이하 포함 & 금융/유틸리티 제외
         2) 주식시장 통합시장조치가 투자주의/투자경고/투자위험/불성실공시/투자유의/관리지정/거래정지/거래중단/정리매매 종목 제외
         3) 우선주 제외
        ...
        if GUBUN == 'mix' :
            universe = code_tgt[code_tgt.MARKETCAP >= 135000000000].reset_index(drop=True)
    ...
    def GET_DUR(RISK_FREE, MARKET_RATE, TICKERS, PERIOD, STDDATE):
        '''
        주식 듀레이션 산출하는 함수
        : 1) ROE와 자본증가율이 수렴하는 기간을 10년으로 가정, 이후 성장률은 고정된 것으로 가정
          2) 무위험수익률 4%, 시장수익률 8% 가정하여 Discount Cash Flow Valuation 기반으로 듀레이션 산출
    ...
    def GET_FEATURED_SCORE(RISK_FREE, MARKET_RATE, TICKERS, PERIOD, STDDATE) :
        ...
        for i in range(0,len(grp)) :
            ...
            ls_srp = pd.DataFrame({'NI' : df.sort_values('NI', ascending = False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('NI', ascending = True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'OI' : df.sort_values('OI', ascending = False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('OI', ascending = True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'REV' : df.sort_values('REV', ascending = False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('REV', ascending = True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'FCF' : df.sort_values('FCF', ascending = False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('FCF', ascending = True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'BP' : df.sort_values('BP', ascending = False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('BP', ascending = True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'EP': df.sort_values('EP', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('EP', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'CP': df.sort_values('CP', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('CP', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'GPA': df.sort_values('GPA', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('GPA', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'NCV': df.sort_values('NCV', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('NCV', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'EPS': df.sort_values('EPS', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('EPS', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               ,'DIV': df.sort_values('DIV', ascending=False).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean() - df.sort_values('DIV', ascending=True).reset_index(drop=True)[0:int(len(df) / 5)].RET.mean()
               }, index = [0]).T
    ...
    def GET_FINAL_TICKERS(DF) :
        ...
        :return: 최종 투자대상종목 관련 데이터
        '''
        df_spec = DF[['STDDATE', 'SYMBOL', 'NAME', 'MARKET', 'CLOSE', 'MARKETCAP', 'GUBUN', 'FEAT1', 'FEAT2', 'FEAT3', 'FTSCORE', \
        ...
        for i in range(0, len(grp)):
            if grp[i] == 'mix':
                temp = df_spec[df_spec.GUBUN == grp[i]][0:15]
            else:
                temp = df_spec[df_spec.GUBUN == grp[i]][0:10]
            df_all = pd.concat([df_all, temp])
        df_all['WGT'] = 1 / len(df_all)
        df_all['AMT'] = 995000000 * df_all['WGT']  # 0.5% 현금버퍼
    ...
    if date == dt.date(2023,10,4):  # 투자시작일
        ...
        final = GET_FINAL_TICKERS(feat_wgt) # 최종 투자대상종목
