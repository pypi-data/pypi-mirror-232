...
@pass_config
def job_run(config, source_id, source_version_id, pack_id, pack_version_id, job={}):
    logger.info("------------- Job Run -------------")
...
def post_run(
    ...
    #########################################################
    ## Schemas
    if os.path.exists(run_path + "/schema.json"):
                ...
                if routine["source"]["id"] in source_ids:
                    ...
                    if isinstance(jobs, list):
                        ...
                        if last_job != None:
                            ...
                            if is_time_for_job(
                                last_job_end_date_str=agent_start_datetime.strftime("%Y-%m-%d %H:%M:%S"),
                                cron_expression=routine["schedule"],
                                start_date_str=routine["start_date"],
                            ):
                                create_scheduled_job(routine, agent_conf)
