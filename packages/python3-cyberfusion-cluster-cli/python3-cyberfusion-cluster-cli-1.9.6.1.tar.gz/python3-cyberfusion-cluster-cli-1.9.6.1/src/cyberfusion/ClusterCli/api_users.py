...
def create(
    ...
    is_provisioning_user: bool = typer.Option(False, "--provisioning-user"),
    is_superuser: bool = typer.Option(False, "--superuser"),
    trusted_ip_networks: List[str] = typer.Option(default=...),
) -> None:
    """Create API user."""
...
@app.command()
@catch_api_exception
def update_provisioning_user(
    username: str,
    state: bool = typer.Argument(default=..., help=BOOL_MESSAGE),
) -> None:
    """Update is provisioning user."""
    api_user = get_object(get_support().api_users, username=username)

    api_user.is_provisioning_user = state
    api_user.update()
