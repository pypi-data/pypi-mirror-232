...
def create(
    ...
    primary_node_hostname: Optional[str] = typer.Option(
        default=None,
        help="Default is first found node with Redis group",
    ),
) -> None:
    ...
    if primary_node_hostname:
        primary_node_id = get_object(
            get_support().nodes, hostname=primary_node_hostname
        ).id
    else:
        primary_node_id = get_object(
            get_support().nodes, cluster_id=cluster.id, groups=NodeGroup.REDIS
        ).id
