...
def get_certificate_(
    ...
    if output_file:
        output_file.write_text(certificate.certificate)

        return

    console.print(certificate.certificate)
...
def get_ca_chain(
    ...
    if output_file:
        output_file.write_text(certificate.ca_chain)

        return

    console.print(certificate.ca_chain)
...
def get_private_key(
    ...
    private_key = get_object(get_support().certificates, id_=id_).private_key

    if not private_key:
        handle_manual_error("Private key is empty")

    output_file.write_text(private_key)
