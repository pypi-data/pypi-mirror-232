...
def cpi_reindexed(cpi, ts, gap = None):
    ...
    if is_ts(cpi):
        ...
        if gap == 0 or gap is None:
            gap = int(12 / observations_per_year(cpi))
...
def ilb_total_return(price, coupon, funding, cpi, base_cpi = None, floor = 1, rate_fmt = 100, 
    ...
    if gap is None or gap == 0: 
        ...
        if n <= 12: ## this is the raw timeseries
            notional = cpi_reindexed(cpi, ts = price, gap = int(12/n))
        else: ## this is a daily timeseries already
            notional = df_reindex(cpi, price, method = 'ffill')
    else: ## user asserted this needs cpi-reindexing
        notional = cpi_reindexed(cpi, ts = price, gap = int(gap))        
