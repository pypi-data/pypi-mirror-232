...
@op3
def rotate_stretch(
    sdf, a, b=None, axis=None, e=ease.linear, symmetric=False, verbose=False
):
    """
    ...
    Args:
        a, b (point vectors): the control points, must not be collinear.
        axis (vector): the axis. auto-determined from a and b if not given.
            ...
            don't like the direction of rotation of the default.
    ...
    bn = _normalize(b)
    if axis is None:
        rotax = np.cross(an, bn)  # rotation axis
        if np.allclose(L := np.linalg.norm(rotax), 0):
            raise errors.SDFCADError(
                f"rotate_stretch(): "
                f"Your stretch points {a = } and {b = } are collinear "
    ...
    basemat = np.array([(an_ := an), (bn_ := _normalize(np.cross(rotax, an)))])

    def angle_on_plane(p):
        p_on_plane = basemat @ p.T
        if verbose:
            rich.print(f"angle_on_plane(): {p = }, {p_on_plane = }")
    ...
    a_b_angle = angle_on_plane(np.array([b]))[0]

    if verbose:
        rich.print(f"{a_b_angle = }, {np.rad2deg(a_b_angle) = }")
        rich.print(f"{a,b = }")
        rich.print(f"{an, bn = }")
        rich.print(f"{rotax = }")
        rich.print(f"{an_, bn_ = }")
    ...
    def f(p):
        ...
        p_angle_fraction = np.clip(
            p_angle_on_plane / a_b_angle, -1 if symmetric else 0, 1
        )
        if verbose:
            rich.print(f"{p_angle_fraction = }")
        back_angle = e(p_angle_fraction) * a_b_angle
        if verbose:
            rich.print(f"{back_angle = }")
        # create many rotation matrices (along 3rd dim) for all angles
        rotmatrix = rotation_matrix(axis=rotax, angle=-back_angle)
        # (this is a slow Python loop, I wonder how to optmize this 🤔)
        rotated = np.array([m @ p for p, m in zip(p, rotmatrix)])
        if verbose:
            rich.print(f"{rotated = }")
        # look up sdf at rotated location
        return sdf(rotated)
