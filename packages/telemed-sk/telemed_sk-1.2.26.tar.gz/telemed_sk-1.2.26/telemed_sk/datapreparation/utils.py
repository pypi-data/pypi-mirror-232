...
def map_columns(hierarchy: Dict[str, str], df: pd.DataFrame, 
    ...
    """
    func_str = 'telemedpredictive.datapreparation.utils.map_columns'
    # Verify that hierarchy is of type dict
    check_not_isinstance(obj = hierarchy, data_type = dict, func = func_str)
    # Verify that conversion is of type dict
    check_not_isinstance(obj = conversion, data_type = dict, func = func_str)
    # Verify that df is of type pd.DataFrame
    check_not_isinstance(obj = df, data_type = pd.DataFrame, func = func_str)
    ...
    for conversion_level in conversion.keys():
        if conversion[conversion_level]:
            ...
            check_not_in_iterable(obj = name_column, iterable = df.columns, func = func_str)
...
def column_to_int(hierarchy: Dict[str, str], df: pd.DataFrame, 
    ...
    pd.DataFrame
        Containing the modified dataframe if there is a conversion to apply
    """
    func_str = 'telemedpredictive.datapreparation.utils.column_to_int'
    # Verify that hierarchy is of type dict
    check_not_isinstance(obj = hierarchy, data_type = dict, func = func_str)
    # Verify that conversion is of type dict
    check_not_isinstance(obj = conversion, data_type = dict, func = func_str)
    ...
    for conversion_level in conversion.keys():
        if conversion[conversion_level]:
            ...
            for column_val in df[name_column].unique():
                check_datatype_convertible(obj = column_val, data_type_conversion = int, func = func_str)
...
def column_to_date(df: pd.DataFrame, 
    ...
    pd.DataFrame
        Containing the modified dataframe if there is a conversion to apply
    """
    func_str = 'telemedpredictive.datapreparation.utils.column_to_date'
    # Verify that df is of type pd.DataFrame
    check_not_isinstance(obj = df, data_type = pd.DataFrame, func = func_str)
...
def generate_time_series(df: pd.DataFrame, date_col: str,
    ...
    """
    func_str = 'telemedpredictive.datapreparation.utils.generate_time_series'
    # Verify that df is of type pd.DataFrame
    check_not_isinstance(obj = df, data_type = pd.DataFrame, func = func_str)
    # Verify that date_col is a column in df
    check_not_in_iterable(obj = date_col, iterable = df.columns, func = func_str)
...
def clean_df_columns(df: pd.DataFrame, hierarchy: Dict[str, str]) -> pd.DataFrame:
    ...
    pd.DataFrame
        Cleaned dataframe
    """
    func_str = 'telemedpredictive.datapreparation.utils.clean_df_columns'
    # Verify that hierarchy is of type dict
    check_not_isinstance(obj = hierarchy, data_type = dict, func = func_str)
