...
from ..utility.resources import get_configuration, logger
...
class HierarchicalReconcileModel():
    ...
    def prepare_dataframe_to_hf(self, X_pred: pd.DataFrame, model_name: str, X_true: pd.DataFrame =None) -> pd.DataFrame:
        ...
        """
        func_str = 'telemedpredictive.algorithms.models.prepare_dataframe_to_hf'
        check_not_isinstance(obj=X_pred, data_type=pd.DataFrame, func=func_str)
        check_not_in_iterable(obj='timestamp', iterable=X_pred.columns, func=func_str)
        ...
        if type(X_true) != type(None):
            check_not_isinstance(obj=X_true, data_type=pd.DataFrame, func=func_str)
