...
def generate_values(dictionary: Dict[str, Dict[str, Union[int, float]]], default_step: Union[int, float]) -> List[Union[int, float]]:
    ...
    """
    func_str = 'telemedpredictive.algorithms.utils.generate_values'
    if len(list(dictionary.keys())) == 3:
        for key in dictionary.keys():
            check_not_in_iterable(obj=key, iterable=['min', 'max', 'step'], func=func_str,
...
def grid_values_hyperparameters(config_hyperparameters: Dict[str, Union[List[str], Dict[str, Union[int, float]]]],
    ...
    """
    func_str = 'telemedpredictive.algorithms.utils.grid_values_hyperparameters'
    grid_parameters = {}
    for param in config_hyperparameters.keys():
        if type(config_hyperparameters[param]) == list:
            ...
            check_not_in_iterable(obj=param, iterable=list(default_steps.keys()), func=func_str,
...
def prepare_dataframe_to_hf(X_true: pd.DataFrame, X_pred: pd.DataFrame, model_name: str) -> pd.DataFrame:
    ...
    """
    func_str = 'telemedpredictive.algorithms.utils.prepare_dataframe_to_hf'
    check_not_in_iterable(
        obj='timestamp', iterable=X_pred.columns, func=func_str)
    check_not_in_iterable(
        obj='pred_mean', iterable=X_pred.columns, func=func_str)
    check_not_in_iterable(
        obj='id_pred', iterable=X_pred.columns, func=func_str)
