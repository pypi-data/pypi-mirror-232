...
from .resources import log_exception, logger
...
def config_key_parsing(config_file: Dict[str, Any], config_file_parsing: Dict[str, Any]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.config_key_parsing'
    for key in config_file.keys():
        check_not_in_iterable(obj = key, iterable = config_file_parsing.keys(), func = func_str)
...
def check_usecases(usecase: str, possible_usecases: List[str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_usecases'
    check_not_isinstance(obj =  usecase, data_type = str, func = func_str)
    check_not_isinstance(obj =  possible_usecases, data_type = list, func = func_str)
    check_not_in_iterable(obj = usecase, iterable = possible_usecases, func = func_str)
...
def check_model(model_name: str, reference_model_name: str, apply_reconciler: str, reference_apply_reconciler: List[str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_model'
    check_not_isinstance(obj =  model_name, data_type = str, func = func_str)
    check_not_isinstance(obj =  reference_model_name, data_type = str, func = func_str)
    check_not_isinstance(obj =  apply_reconciler, data_type = str, func = func_str)
    check_not_in_iterable(obj = apply_reconciler, iterable = reference_apply_reconciler, func = func_str)
    if model_name != reference_model_name:
        raise InputTypeError(func = func_str, 
...
def check_hyperparameters(hyperparameters: List[str], possible_hyperparameters: List[str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_hyperparameters'	
    check_not_isinstance(obj = hyperparameters, data_type = list, func = func_str)
    check_not_isinstance(obj = possible_hyperparameters, data_type = list, func = func_str) 
    for hyperparam in hyperparameters:
        check_not_isinstance(obj = hyperparam, data_type = str, func = func_str)
        check_not_in_iterable(obj = hyperparam, iterable = possible_hyperparameters, func = func_str)
...
def check_hyperparameters_value(hyperparameters: Dict[str, Union[List[str], Dict[str, Union[int, float]]]]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_hyperparameters_value'
...
def check_missing_data_strategy(missing_data_strategy:  Union[str,int,dict], possible_strategies: List[str], 
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_missing_data_strategy'	
    if isinstance(missing_data_strategy, str):    
        check_not_in_iterable(obj = missing_data_strategy, iterable = possible_strategies, func = func_str)
        ...
        for key, _ in missing_data_strategy.items():
            check_not_in_iterable(obj = key, iterable = possible_dictionary_keys, func = func_str)
        check_not_in_iterable(obj = missing_data_strategy['interpolation'], iterable = possible_interpolation_methods, func = func_str)
        check_not_isinstance(obj = missing_data_strategy['order'], data_type= int, func = func_str)
    else:
...
def check_time_granularity(time_granularity: str, possible_time_granularities: List[str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_time_granularity'	
    check_not_isinstance(obj = time_granularity, data_type = str, func = func_str)
    check_not_isinstance(obj = possible_time_granularities, data_type = list, func = func_str) 
    check_not_in_iterable(obj = time_granularity, iterable = possible_time_granularities, func = func_str)
...
def check_preprocessing(target_col: str, date_col: str):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_preprocessing'	
    check_not_isinstance(obj = target_col, data_type = str, func= func_str)
    check_not_isinstance(obj = date_col, data_type = str, func= func_str)
...
def check_split_forecasting(time_granularity: str, validation_obs: int, test_obs: int,
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_split_forecasting'	
    check_not_isinstance(obj = time_granularity, data_type = str, func = func_str)
    check_not_isinstance(obj = validation_obs, data_type = int, func = func_str)
    check_not_isinstance(obj = test_obs, data_type = int, func = func_str)
    check_not_isinstance(obj = num_forecast_periods, data_type = int, func = func_str)
    check_not_isinstance(obj = start_date, data_type = str, func = func_str)
    check_datatype_convertible(obj = start_date, data_type_conversion = pd.to_datetime, func = func_str)
...
def check_hierarchy(hierarchy: Dict[str, str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_hierarchy'	
    for hier_val in hierarchy.values():
        check_not_isinstance(obj = hier_val, data_type = str, func= func_str)
...
def check_conversion(conversion: Dict[str, str]):
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.check_conversion'	
    for conversion_val in conversion.values():
        check_not_isinstance(obj = conversion_val, data_type = str, func= func_str)
...
def config_parsing(config_path: str, 
    ...
    """
    func_str = 'telemedpredictive.utility.configuration_utils.config_parsing'	
    check_not_isinstance(obj = config_path, data_type = str, func = func_str)
    check_not_isinstance(obj = config_filename, data_type = str, func = func_str)
    check_not_isinstance(obj = config_filename_parsing, data_type = str, func = func_str)
