...
class ExperimentClass():
    ...
    def check_consistency(self, X: pd.DataFrame):
        ...
        X : pd.DataFrame
            Dataframe to check
        """
        func_str = 'telemedpredictive.algorithms.experiment.check_consistency'
        check_not_isinstance(obj=X, data_type=pd.DataFrame, func=func_str)
        check_not_in_iterable(obj='unique_id', iterable=X.reset_index().columns, func=func_str)
        check_not_in_iterable(obj='ds', iterable=X.columns, func=func_str)
        check_not_in_iterable(obj='y', iterable=X.columns, func=func_str)
    ...
    def score(self, X_true: pd.DataFrame, X_pred: pd.DataFrame, y=None, full_scoring=True) -> Tuple[pd.Series, Union[pd.DataFrame, None]]:
        ...
        """
        func_str = 'telemedpredictive.algorithms.experiment.score'
        self.check_consistency(X=X_true)
        check_not_in_iterable(obj='id_pred', iterable=X_pred.columns, func=func_str)
        check_not_in_iterable(obj='pred_mean', iterable=X_pred.columns, func=func_str)
        check_not_in_iterable(obj='pi_upper_95', iterable=X_pred.columns, func=func_str)
        check_not_in_iterable(obj='pi_lower_95', iterable=X_pred.columns, func=func_str)
