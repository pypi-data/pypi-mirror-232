...
def radon_cut(I0,I1,I2,supergrid0,supergrid1,supergrid2, spin_case, i_case, mask, **params):
    ...
    for i in range(len(radonangles)):
        ...
        filename=path+"Visamp_%s_a_%s_i_%s_dx0_%s_dx1_%s_dx2_%s_sk_%s_bphi_%s_br_%s_bvapp_%s_gfact_%s_gp_%s_sp_%s_fudge_%s_Ncut_%s.h5"%(radonangle,spin_case,i_case, dx0, dx1, dx2, sub_kep, betaphi, betar,  bvapp, gfactor, gammap, sigmap, fudge, int(Ncut))
