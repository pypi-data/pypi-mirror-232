...
class DecoderParamsBuilder():
    ...
    def request_individual_decoder_knowledge(self, input_json, decoder_id):
        ...
        try:
            if self._input_proto_type == RemoteDecoderInput:
                raise ValueError("Can not support get hyp knowledge from individual decoder with RemoteDecoderInput proto")
            else:
                knowledge = ModelContextParser.get_individual_decoder_knowledge(self._input_proto, decoder_id)
        except Exception as e:
            print(e)
            self.logger.info(f"fail to get hyp knowledge from individual decoder with decoder key as {decoder_id}")
        return knowledge 
