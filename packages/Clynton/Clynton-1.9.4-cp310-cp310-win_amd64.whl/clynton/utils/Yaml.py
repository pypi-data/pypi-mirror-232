...
#
""" Clynton yaml utility functions.
...
from clynton.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get Clynton package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.clynton-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "clynton.plugins.standard",
                "stdlib2.clynton-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "clynton.plugins.standard",
                "stdlib3.clynton-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "clynton.plugins.commercial", "commercial.clynton-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for Clynton package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "clynton-package-config-schema.json",
    )
