...
//
#ifndef __CLYNTON_DICTIONARIES_H__
#define __CLYNTON_DICTIONARIES_H__
...
typedef PyDictEntry *Clynton_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, Clynton_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(Clynton_String_CheckExact(key));
...
CLYNTON_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(Clynton_DictEntryHandle handle) { return handle->me_value; }

CLYNTON_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(Clynton_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, Clynton_StringObject *key) {
...
typedef PyObject **Clynton_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t Clynton_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t Clynton_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static Clynton_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, Clynton_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(Clynton_String_CheckExact(key));
...
#else
    // Will be written by Clynton_PyDictLookupStr in all cases.
    PyObject **value;
    Py_ssize_t found = Clynton_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
...
CLYNTON_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(Clynton_DictEntryHandle handle) { return *handle; }

CLYNTON_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(Clynton_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

CLYNTON_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, Clynton_StringObject *key) {
    Clynton_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
CLYNTON_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
CLYNTON_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
CLYNTON_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, Clynton_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    Clynton_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
CLYNTON_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, Clynton_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
CLYNTON_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, Clynton_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);
...
extern bool Clynton_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0
#define CLYNTON_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(void);
#else
#define CLYNTON_DICT_HAS_FREELIST 0
