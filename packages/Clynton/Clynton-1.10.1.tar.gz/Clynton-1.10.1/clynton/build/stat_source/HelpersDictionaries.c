...
#else
    PyObject **value_addr;
    Py_ssize_t ix = Clynton_PyDictLookup(dict_object, key, hash, &value_addr);
#endif
...
static inline PyObject *_MAKE_DICT_ITERATOR(PyDictObject *dict, PyTypeObject *type, bool is_iteritems) {
...
#if PYTHON_VERSION < 0x300
    dictiterobject *di = (dictiterobject *)Clynton_GC_New(type);
    CHECK_OBJECT(di);
    Py_INCREF(dict);
    di->di_dict = dict;
    ...
    return (PyObject *)di;
#else
    _PyDictViewObject *dv = (_PyDictViewObject *)Clynton_GC_New(type);
    CHECK_OBJECT(dv);
...
static PyObject *_MAKE_DICT_VIEW(PyDictObject *dict, PyTypeObject *type) {
    ...
    dictviewobject *dv = (dictviewobject *)Clynton_GC_New(type);
...
static inline int Clynton_py_get_index_from_order(PyDictObject *mp, Py_ssize_t i) {
...
static inline Py_ssize_t Clynton_Py_dictkeys_get_index(const PyDictKeysObject *keys, Py_ssize_t i) {
...
static inline Py_hash_t Clynton_Py_unicode_get_hash(PyObject *o) { return _PyASCIIObject_CAST(o)->hash; }

// From CPython
#define PERTURB_SHIFT 5

static Py_ssize_t Clynton_Py_unicodekeys_lookup_generic(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                       Py_hash_t hash) {
    ...
    while (1) {
        Py_ssize_t ix = Clynton_Py_dictkeys_get_index(dk, i);
        ...
        if (ix >= 0) {
            ...
            if (Clynton_Py_unicode_get_hash(ep->me_key) == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                clynton_bool cmp = RICH_COMPARE_EQ_NBOOL_UNICODE_OBJECT(startkey, key);
                Py_DECREF(startkey);

                if (unlikely(cmp == CLYNTON_BOOL_EXCEPTION)) {
                    return DKIX_ERROR;
                }

                if (dk == mp->ma_keys && ep->me_key == startkey) {
                    if (cmp == CLYNTON_BOOL_TRUE) {
                        return ix;
                    }
                } else {
    ...
    CLYNTON_CANNOT_GET_HERE("Clynton_Py_unicodekeys_lookup_generic failed");
}

// TODO: Make use of this one in Clynton_PyDictLookupStr
static Py_ssize_t Clynton_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash) {
    ...
    while (true) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_key == key || (Clynton_Py_unicode_get_hash(ep->me_key) == hash &&
        ...
        i = mask & (i * 5 + perturb + 1);
        ix = Clynton_Py_dictkeys_get_index(dk, i);
    ...
    CLYNTON_CANNOT_GET_HERE("Clynton_Py_unicodekeys_lookup_unicode failed");
}

// Search key from Generic table.
static Py_ssize_t Clynton_Py_dictkeys_generic_lookup(PyDictObject *mp, PyDictKeysObject *dk, PyObject *key,
                                                    Py_hash_t hash) {
    ...
    while (1) {
        ...
        if (ix >= 0) {
            ...
            if (ep->me_hash == hash) {
                PyObject *startkey = ep->me_key;
                Py_INCREF(startkey);
                clynton_bool cmp = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(startkey, key);
                Py_DECREF(startkey);
                if (cmp == CLYNTON_BOOL_EXCEPTION) {
                    return DKIX_ERROR;
                }
                if (dk == mp->ma_keys && ep->me_key == startkey) {
                    if (cmp == CLYNTON_BOOL_TRUE) {
...
Py_ssize_t Clynton_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    PyDictKeysObject *dk;
    DictKeysKind kind;
    Py_ssize_t ix;
...
restart:
    ...
    if (kind != DICT_KEYS_GENERAL) {
        if (PyUnicode_CheckExact(key)) {
            ix = Clynton_Py_unicodekeys_lookup_unicode(dk, key, hash);
        } else {
            ix = Clynton_Py_unicodekeys_lookup_generic(mp, dk, key, hash);
        ...
        ix = Clynton_Py_dictkeys_generic_lookup(mp, dk, key, hash);
...
Py_ssize_t Clynton_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr) {
    assert(PyUnicode_CheckExact(key));

    return Clynton_PyDictLookup(mp, key, hash, value_addr);
}

#endif

bool Clynton_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr) {
    CHECK_OBJECT(dict);
    assert(PyDict_CheckExact(dict));
    assert(key_ptr);
    assert(value_ptr);
    ...
    *value_ptr = value;
    ...
    if (mp->ma_values) {
        ...
        int index = Clynton_py_get_index_from_order(mp, i);
...
PyObject *MAKE_DICT_EMPTY(void) {
    ...
    // No Clynton_GC_Track for the empty dictionary.
