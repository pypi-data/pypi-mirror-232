...
from clynton.nodes.shapes.BuiltinTypeShapes import tshape_bool
from clynton.nodes.shapes.StandardShapes import tshape_unknown
from clynton.PythonOperators import (
...
def getRichComparisonCode(
    ...
    if needs_check:
        ...
        # instead can use the CTypeClyntonBoolEnum that will easily convert to
        # it.
        if helper_type is CTypeClyntonVoidEnum:
            helper_type = CTypeClyntonBoolEnum
        ...
        # avoid the CTypeClyntonBoolEnum (NBOOL) helpers except for things that
    ...
    if helper_function is None and not report_missing:
        helper_type, helper_function = selectCodeHelper(
            ...
            specialized_helpers_set=specialized_helpers_set,
            non_specialized_helpers_set=non_specialized_helpers_set,
            result_type=CTypeClyntonBoolEnum,
            left_shape=left_shape,
            right_shape=right_shape,
            left_c_type=left_c_type,
            right_c_type=right_c_type,
...
def generateSubtypeCheckCode(to_name, expression, emit, context):
    ...
    emit(
        "%s = Clynton_Type_IsSubtype((PyTypeObject *)%s, (PyTypeObject *)%s);"
        % (res_name, left_name, right_name)
    )
