...
#if defined(_CLYNTON_ONEFILE_MODE) && defined(_WIN32)
...
DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    ...
    Sleep(_CLYNTON_ONEFILE_CHILD_GRACE_TIME_INT);
...
// Empty "sys.path" again, the above adds program directory to it.
    ...
    if (unlikely(is_multiprocessing_fork)) {
        ...
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("CLYNTON_ONEFILE_PARENT", buffer, sizeof(buffer));
