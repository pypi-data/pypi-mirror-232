...
from clynton.Errors import ClyntonAssumptionError, ClyntonNodeDesignError
...
def _checkBases(name, bases):
    ...
    for base in bases:
        ...
        if base is not object and "__slots__" not in base.__dict__:
            raise ClyntonNodeDesignError(name, "All bases must set __slots__.", base)
        ...
        if "python_version_spec" in dictionary:
            ...
            if not eval(condition):
                ...
                def __init__(self, *args, **kwargs):
                    raise ClyntonAssumptionError(name, "assumption violated", condition)
    ...
    def __init__(cls, name, bases, dictionary):
        if not name.endswith(("Base", "Mixin")):
            if "kind" not in dictionary:
                raise ClyntonNodeDesignError(name, "Must provide class variable 'kind'")
