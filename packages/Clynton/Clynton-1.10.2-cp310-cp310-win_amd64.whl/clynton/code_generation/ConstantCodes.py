...
from clynton.utils.Distributions import getDistributionTopLevelPackageNames
from clynton.Version import getClyntonVersionTuple
...
def getConstantsDefinitionCode():
    """Create the code code "__constclyn.c" and "__constclyn.h" files.
    ...
    constant_accessor = ConstantAccessor(
        data_filename="__constclyn.const", top_level_name="global_constants"
    )
    ...
    header = template_header_guard % {
        "header_guard_name": "__CLYNTON_GLOBAL_CONSTANTS_H__",
        "header_body": "\n".join(lines),
    }

    major, minor, micro, is_final, _rc_number = getClyntonVersionTuple()
    ...
    body = template_constants_reading % {
        ...
        "clynton_version_major": major,
        "clynton_version_minor": minor,
        "clynton_version_micro": micro,
        "clynton_version_level": "release" if is_final else "candidate",
