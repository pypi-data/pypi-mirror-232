...
from clynton import Options, OutputDirectories
from clynton.build.SconsInterface import (
    asBoolStr,
    cleanSconsDirectory,
    getSconsDataPath,
    runScons,
...
from clynton.Options import getOnefileTempDirSpec, isOnefileTempDirMode
from clynton.OutputDirectories import getResultFullpath
from clynton.plugins.Plugins import Plugins
from clynton.PostProcessing import executePostProcessingResources
from clynton.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from clynton.Tracing import simplfy_logger, postprocessing_logger
...
from clynton.utils.InstalledPythons import findInstalledPython
from clynton.utils.SharedLibraries import cleanupHeaderForAndroid
from clynton.utils.Signing import addMacOSCodeSignature
...
from clynton.utils.WindowsResources import RT_RCDATA, addResourceToFile


def packDistFolderToOnefile(dist_dir):
    """Pack distribution to onefile, i.e. a single file that is directly executable."""

    simplfy_output_filename = getResultFullpath(onefile=True)

    packDistFolderToOnefileBootstrap(simplfy_output_filename, dist_dir)

    Plugins.onOnefileFinished(simplfy_output_filename)


def _runOnefileScons(simplfy_compression):
    ...
    env_values["_CLYNTON_SIMPLFY_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_CLYNTON_SIMPLFY_TEMP_BOOL"] = "1" if isOnefileTempDirMode() else "0"
    env_values["_CLYNTON_SIMPLFY_COMPRESSION_BOOL"] = "1" if simplfy_compression else "0"
    env_values["_CLYNTON_SIMPLFY_BUILD_BOOL"] = "1" if simplfy_compression else "0"
    ...
    result = runScons(
        ...
        scons_filename="Simplfy.scons",
    )

    # Exit if compilation failed.
    if not result:
        simplfy_logger.sysexit("Error, onefile bootstrap binary build failed.")
...
def runOnefileCompressor(
    compressor_python, dist_dir, simplfy_output_filename, start_binary
):
    ...
    if compressor_python is None or areSamePaths(
        ...
        from clynton.tools.simplfy_compressor.OnefileCompressor import (
            attachOnefilePayload,
        )

        attachOnefilePayload(
            dist_dir=dist_dir,
            simplfy_output_filename=simplfy_output_filename,
        ...
        simplfy_compressor_path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "tools", "simplfy_compressor")
        )
        ...
        mapping["CLYNTON_PROGRESS_BAR"] = "1" if Options.shallUseProgressBar() else "0"
        ...
        with withEnvironmentVarsOverridden(mapping):
            subprocess.check_call(
                [
                    compressor_python.getPythonExe(),
                    simplfy_compressor_path,
                    dist_dir,
                    getExternalUsePath(simplfy_output_filename, only_dirname=True),
                    start_binary,
                    str(file_checksums),
                    str(win_path_sep),
...
def packDistFolderToOnefileBootstrap(simplfy_output_filename, dist_dir):
    postprocessing_logger.info(
        "Creating single file from dist folder, this may take a while."
    )

    simplfy_logger.info("Running bootstrap binary compilation via Scons.")
    ...
    simplfy_payload_filename = os.path.join(source_dir, "__simplloada.bin")
    ...
    if payload_used_in_build:
        runOnefileCompressor(
            ...
            dist_dir=dist_dir,
            simplfy_output_filename=simplfy_payload_filename,
            start_binary=getResultFullpath(onefile=False),
        )

    # Create the bootstrap binary for unpacking.
    _runOnefileScons(
        simplfy_compression=compressor_python is not None,
    )
    ...
    if isAndroidBasedLinux():
        cleanupHeaderForAndroid(simplfy_output_filename)

    Plugins.onBootstrapBinary(simplfy_output_filename)

    if isMacOS():
        addMacOSCodeSignature(filenames=[simplfy_output_filename])

    if not payload_used_in_build:
        runOnefileCompressor(
            compressor_python=compressor_python,
            dist_dir=dist_dir,
            simplfy_output_filename=(
                simplfy_payload_filename
                if isWin32Windows()
                else simplfy_output_filename
            ),
            start_binary=getResultFullpath(onefile=False),
        )
        ...
        if isWin32Windows():
            addResourceToFile(
                target_filename=simplfy_output_filename,
                data=getFileContents(simplfy_payload_filename, mode="rb"),
    ...
    if Options.isRemoveBuildDir():
        simplfy_logger.info("Removing onefile build directory '%s'." % source_dir)

        removeDirectory(path=source_dir, ignore_errors=False)
        assert not os.path.exists(source_dir)
    else:
        simplfy_logger.info("Keeping onefile build directory '%s'." % source_dir)
