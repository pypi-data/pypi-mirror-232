...
#include "simplfy_definitions.h"
#else
#define _CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_CACHING
#define _CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_HANDLING
#define _CLYNTON_SIMPLFY_TEMP_BOOL 0
#define _CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT 5000
#define _CLYNTON_SIMPLFY_TEMP_SPEC "%TEMP%/siplfyunique_%PID%_%TIME%"

#define _CLYNTON_EXPERIMENTAL_DEBUG_AUTO_UPDATE
#define _CLYNTON_AUTO_UPDATE_BOOL 1
#define _CLYNTON_AUTO_UPDATE_URL_SPEC "https://..."

#if __APPLE__
#define _CLYNTON_PAYLOAD_FROM_MACOS_SECTION
#endif

#endif

#if _CLYNTON_SIMPLFY_COMPRESSION_BOOL == 1
...
#if defined(_WIN32)
    SetEnvironmentVariable("CLYNTON_JUST_ANPARENT", value);
#else
...
#ifdef _CLYNTON_PAYLOAD_FROM_MACOS_SECTION
...
#if _CLYNTON_SIMPLFY_TEMP_BOOL == 0
...
#if _CLYNTON_SIMPLFY_TEMP_BOOL == 1
...
static int waitpid_timeout(pid_t pid) {
    ...
    struct timespec timeout = {
        _CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT / 1000,
        (_CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT % 1000) * 1000,
    };
...
static void cleanupChildProcess(bool send_sigint) {
    ...
    if (handle_process != 0) {
        ...
        if (send_sigint) {
#if defined(_CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_HANDLING)
...
#if _CLYNTON_SIMPLFY_TEMP_BOOL == 1 || 1
        CLYNTON_PRINT_TRACE("Waiting for child to exit.\n");
#if defined(_WIN32)
        if (WaitForSingleObject(handle_process, _CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT) != 0) {
...
#endif
        CLYNTON_PRINT_TRACE("Child is exited.\n");
#endif
    }

#if _CLYNTON_SIMPLFY_TEMP_BOOL == 1
    if (payload_created) {
#if _CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_HANDLING
...
BOOL WINAPI ourConsoleCtrlHandler(DWORD fdwCtrlType) {
    ...
    case CTRL_C_EVENT:
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_HANDLING
        puts("Ctrl-C event");
#endif
...
#if _CLYNTON_AUTO_UPDATE_BOOL && !defined(__IDE_ONLY__)
#include "clynton_simplfy_auto_updater.h"
#endif

#if _CLYNTON_AUTO_UPDATE_BOOL
extern bool exe_file_updatable;
#endif

#ifdef _CLYNTON_clynton_splshscrn
#ifdef __cplusplus
extern "C" {
#endif
...
int main(int argc, char **argv) {
#endif
#endif
    CLYNTON_PRINT_TIMING("ONEFILE: Entered main().");

    filename_char_t const *pattern = FILENAME_EMPTY_STR _CLYNTON_SIMPLFY_TEMP_SPEC;
...
#ifdef _CLYNTON_AUTO_UPDATE_BOOL
    checkAutoUpdates();
#endif

    CLYNTON_PRINT_TIMING("ONEFILE: Unpacking payload.");

    initPayloadData();

#if !defined(_CLYNTON_PAYLOAD_FROM_MACOS_SECTION) && !defined(_WIN32)
    const off_t size_end_offset = exe_file_mapped.file_size;

    CLYNTON_PRINT_TIMING("ONEFILE: Determining payload start position.");
    ...
    CLYNTON_PRINT_TIMING("ONEFILE: Checking header for compression.");
...
// The 'X' stands for no compression, 'Y' is compressed, handle that.
...
#if _CLYNTON_clynton_splshscrn
    CLYNTON_PRINT_TIMING("ONEFILE: Splash screen.");

    initSplashScreen();
#endif

    CLYNTON_PRINT_TIMING("ONEFILE: Entering decompression.");
...
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_SIMPLFY_CACHING
    ...
    CLYNTON_PRINT_TIMING("ONEFILE: Finishing decompression, cleanup payload.");
    ...
    // might change it by the time the child looks at its parent.
    {
        ...
        setEnvironVar("CLYNTON_JUST_ANPARENT", buffer);
    }

    CLYNTON_PRINT_TIMING("ONEFILE: Preparing forking of slave process.");
    ...
    CLYNTON_PRINT_TIMING("ONEFILE: Started slave process.");
    ...
    CLYNTON_PRINT_TIMING("ONEFILE: Exiting.");
