...
#if CLYNTON_TUPLE_HAS_FREELIST
static struct _Py_tuple_state *_Clynton_Py_get_tuple_state(void) {
...
PyObject *MAKE_TUPLE_EMPTY(Py_ssize_t size) {
    ...
    struct _Py_tuple_state *state = _Clynton_Py_get_tuple_state();
    ...
    if ((size < PyTuple_MAXSAVESIZE) && (result_tuple = state->free_list[size]) != NULL) {
        ...
        Clynton_Py_NewReference((PyObject *)result_tuple);
    } else {
        // Check for overflow
        if ((size_t)size >
        ...
        result_tuple = (PyTupleObject *)Clynton_GC_NewVar(&PyTuple_Type, size);
    }
