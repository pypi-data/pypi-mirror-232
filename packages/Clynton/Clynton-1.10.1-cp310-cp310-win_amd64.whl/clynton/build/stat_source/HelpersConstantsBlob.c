...
#if defined(_CLYNTON_CONSTANTS_FROM_LINKER)
...
#elif defined(_CLYNTON_CONSTANTS_FROM_CODE)
...
#if defined(_CLYNTON_CONSTANTS_FROM_INCBIN)
...
static Py_hash_t Clynton_FastHashBytes(const void *value, Py_ssize_t size) {
    if (unlikely(size == 0)) {
        return 0;
    }
...
static Py_hash_t our_list_hash(PyListObject *list) {
    return Clynton_FastHashBytes(&list->ob_item[0], Py_SIZE(list) * sizeof(PyObject *));
}
...
static Py_hash_t our_tuple_hash(PyTupleObject *tuple) {
    return Clynton_FastHashBytes(&tuple->ob_item[0], Py_SIZE(tuple) * sizeof(PyObject *));
}
...
static Py_hash_t our_set_hash(PyObject *set) {
    ...
    while (_PySet_Next(set, &pos, &key)) {
        result *= 1000003;
        result ^= Clynton_FastHashBytes(&key, sizeof(PyObject *));
    }
#else
    Py_hash_t unused;
...
static Py_hash_t our_dict_hash(PyObject *dict) {
    ...
    while (Clynton_DictNext(dict, &pos, &key, &value)) {
        result *= 1000003;
        result ^= Clynton_FastHashBytes(&key, sizeof(PyObject *));
        result *= 1000003;
        result ^= Clynton_FastHashBytes(&value, sizeof(PyObject *));
    }
...
static PyObject *our_dict_richcompare(PyObject *a, PyObject *b, int op) {
    ...
    if (Py_SIZE(a) != Py_SIZE(b)) {
        ...
        // effect intended.
        while (Clynton_DictNext(a, &pos1, &key1, &value1)) {
            int res = Clynton_DictNext(b, &pos2, &key2, &value2);
            assert(res != 0);
...
PyObject **Clynton_Long_SmallValues;
#elif PYTHON_VERSION >= 0x300
PyObject *Clynton_Long_SmallValues[CLYNTON_STATIC_SMALLINT_VALUE_MAX - CLYNTON_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
static void initCaches(void) {
    ...
    Clynton_Long_SmallValues = (PyObject **)_PyInterpreterState_GET()->small_ints;
#elif PYTHON_VERSION >= 0x300
    for (long i = CLYNTON_STATIC_SMALLINT_VALUE_MIN; i < CLYNTON_STATIC_SMALLINT_VALUE_MAX; i++) {
        PyObject *value = PyLong_FromLong(i);
        Clynton_Long_SmallValues[CLYNTON_TO_SMALL_VALUE_OFFSET(i)] = value;
    }
#endif
#endif
...
static PyObject *_unpackAnonValue(unsigned char anon_index) {
    ...
    default:
        ...
        CLYNTON_CANNOT_GET_HERE("Corrupt constants blob");
    }
}
    ...
    for (int _i = 0; _i < count; _i++) {
        ...
        *output = NULL;
...
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_CONSTANTS
        ...
        case 'D': {
            ...
            if (size > 0) {
                CLYNTON_DYNAMIC_ARRAY_DECL(keys, PyObject *, size);
                CLYNTON_DYNAMIC_ARRAY_DECL(values, PyObject *, size);
                ...
                if (size == 0) {
                    ...
                    if (empty_frozenset == NULL) {
                        empty_frozenset = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, (PyObject *)&PyFrozenSet_Type,
                                                                        Clynton_Bytes_FromStringAndSize("", 0));
                    }
        ...
        case 'l': {
            ...
            PyObject *l = Clynton_LongFromCLong(value);

            // Avoid the long cache, won't do anything useful for small ints
#if PYTHON_VERSION >= 0x300
            if (value < CLYNTON_STATIC_SMALLINT_VALUE_MIN || value >= CLYNTON_STATIC_SMALLINT_VALUE_MAX)
#endif
            {
        ...
        case 'c': {
            ...
            PyObject *b = Clynton_Bytes_FromStringAndSize((const char *)data, size);
            CHECK_OBJECT(b);
        ...
        case 'd': {
            ...
            *output = s;
#else
            PyObject *b = Clynton_Bytes_FromStringAndSize((const char *)data, 1);
            data += 1;
            *output = b;
#endif
...
#if _CLYNTON_CONSTANTS_FROM_MACOS_SECTION
...
void loadConstantsBlob(PyThreadState *tstate, PyObject **output, char const *name) {
    ...
    if (init_done == false) {
        CLYNTON_PRINT_TIMING("loadConstantsBlob(): One time init.");
...
#elif defined(_CLYNTON_CONSTANTS_FROM_RESOURCE)
#ifdef _CLYNTON_EXE
        ...
        assert(constant_bin);
#elif _CLYNTON_CONSTANTS_FROM_MACOS_SECTION
        constant_bin = findMacOSBinarySection();

        assert(constant_bin);
#endif
        CLYNTON_PRINT_TIMING("loadConstantsBlob(): Found blob, decoding now.");
        DECODE(constant_bin);

        CLYNTON_PRINT_TIMING("loadConstantsBlob(): CRC32 that blob for correctness.");
        ...
        CLYNTON_PRINT_TIMING("loadConstantsBlob(): One time init complete.");
