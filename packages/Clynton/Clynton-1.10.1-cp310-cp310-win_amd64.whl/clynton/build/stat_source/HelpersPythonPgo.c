...
//
/**
 * This is responsible for collection of Clynton Python PGO information. It writes
...
void PGO_Initialize(void) {
    ...
    // shall be written to.
    char const *output_filename = getenv("CLYNTON_PGO_OUTPUT");

    if (unlikely(output_filename == NULL)) {
        CLYNTON_CANNOT_GET_HERE("CLYNTON_PGO_OUTPUT needs to be set");
    }
