...
from clynton.nodes.CallNodes import ExpressionCallNoKeywords
...
from clynton.nodes.ModuleAttributeNodes import ExpressionModuleAttributeFileRef
from clynton.nodes.ModuleNodes import CompiledPythonPackage
        ...
        if package.canHaveExternalImports():
            ...
            for count in range(len(parts)):
                path_part = _makeCall(
                    ...
                    source_ref,
                    makeConstantRefNode(
                        constant="CLYNTON_PACKAGE_%s" % "_".join(parts[: count + 1]),
                        source_ref=source_ref,
                    ),
                    makeConstantRefNode(
