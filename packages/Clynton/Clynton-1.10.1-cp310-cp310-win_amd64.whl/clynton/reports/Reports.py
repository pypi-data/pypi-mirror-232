...
from clynton import TreeXML
from clynton.__past__ import unicode
from clynton.build.DataComposerInterface import getDataComposerReportValues
from clynton.containers.OrderedSets import OrderedSet
from clynton.freezer.IncludedDataFiles import getIncludedDataFiles
from clynton.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from clynton.importing.Importing import getPackageSearchPath
from clynton.ModuleRegistry import (
    getDoneModules,
...
from clynton.plugins.Plugins import getActivePlugins
from clynton.PythonFlavors import getPythonFlavorName
from clynton.PythonVersions import getSystemPrefixPath, python_version_full_str
from clynton.Tracing import ReportingSystemExit, reports_logger
from clynton.utils.Distributions import getDistributionsFromModuleName
from clynton.utils.FileOperations import getReportPath, putTextFileContents
from clynton.utils.Jinja2 import getTemplate
from clynton.utils.MemoryUsage import getMemoryInfos
from clynton.utils.Utils import getArchitecture, getOS
...
def _getReportInputData(aborted):
    ...
    clynton_version = getClyntonVersion()
    clynton_commercial_version = getCommercialVersion() or "not installed"

    clynton_aborted = aborted

    clynton_exception = sys.exc_info()
...
def _getReportPathPrefixes():
    ...
    if _report_prefixes is None:
        ...
        sys_prefix = os.environ.get("CLYNTON_SYS_PREFIX", sys.prefix)
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["clynton_aborted"]:
        completion = "yes"
    elif report_input_data["clynton_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["clynton_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["clynton_exception"][1].code
    elif report_input_data["clynton_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["clynton_exception"][1].code
        )
        exit_message = report_input_data["clynton_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "clynton-compilation-report",
        clynton_version=report_input_data["clynton_version"],
        clynton_commercial_version=report_input_data["clynton_commercial_version"],
...
_crash_report_filename = "clynton-crash-report.xml"
