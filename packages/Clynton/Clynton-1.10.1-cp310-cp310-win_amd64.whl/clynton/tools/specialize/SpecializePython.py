...
import sys
import clynton.Options

clynton.Options.is_full_compat = False

# isort:start

from collections import namedtuple

import clynton.code_generation.BinaryOperationHelperDefinitions
import clynton.code_generation.CodeGeneration
import clynton.code_generation.ComparisonCodes
import clynton.code_generation.Namify
import clynton.nodes.PackageMetadataNodes
import clynton.nodes.PackageResourceNodes
import clynton.nodes.SideEffectNodes
import clynton.specs.BuiltinBytesOperationSpecs
import clynton.specs.BuiltinDictOperationSpecs
import clynton.specs.BuiltinListOperationSpecs
import clynton.specs.BuiltinStrOperationSpecs
import clynton.specs.BuiltinTypeOperationSpecs
import clynton.specs.HardImportSpecs
import clynton.tree.Building
from clynton.containers.OrderedSets import OrderedSet
from clynton.nodes.ImportNodes import hard_modules_non_stdlib
from clynton.nodes.NodeMetaClasses import NodeCheckMetaClass
from clynton.nodes.shapes.BuiltinTypeShapes import (
    tshape_bool,
    tshape_bytes,
    tshape_dict,
    tshape_int,
...
from clynton.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "clynton.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_dict",
    clynton.specs.BuiltinDictOperationSpecs,
    python2_dict_methods,
    python3_dict_methods,
    ("fromkeys",),
)
...
processTypeShapeAttribute(
    "tshape_str",
    clynton.specs.BuiltinStrOperationSpecs,
    python2_str_methods,
    python3_str_methods,
)
...
processTypeShapeAttribute(
    "tshape_bytes",
    clynton.specs.BuiltinBytesOperationSpecs,
    (),
    python3_bytes_methods,
)
...
processTypeShapeAttribute(
    "tshape_list",
    clynton.specs.BuiltinListOperationSpecs,
    python2_list_methods,
    python3_list_methods,
)
...
processTypeShapeAttribute(
    "tshape_type",
    clynton.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def makeAttributeNodes():
    filename_python = "clynton/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from clynton.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from clynton.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            "from clynton.nodes.NodeMakingHelpers import wrapExpressionWithNodeSideEffects"
        )

        emit(
            "from clynton.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from clynton.nodes.AttributeNodes import makeExpressionAttributeLookup")
...
def makeBuiltinOperationNodes():
    filename_python = "clynton/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "clynton/nodes/ChildrenHavingMixins.py"
    filename_python2 = "clynton/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "clynton/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from clynton.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "clynton/nodes/HardImportNodesGenerated.py"
...
"""
        ...
        for spec_descriptions in getSpecVersions(clynton.specs.HardImportSpecs):
