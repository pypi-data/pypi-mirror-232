...
from clynton import Tracing
from clynton.__past__ import urlretrieve
...
def getCachedDownload(
    ...
    clynton_download_dir = getDownloadCacheDir()

    clynton_download_dir = os.path.join(
        clynton_download_dir, os.path.basename(binary).replace(".exe", "")
    )

    if is_arch_specific:
        clynton_download_dir = os.path.join(clynton_download_dir, is_arch_specific)

    if specificity:
        clynton_download_dir = os.path.join(clynton_download_dir, specificity)

    download_path = os.path.join(clynton_download_dir, os.path.basename(url))
    exe_path = os.path.join(clynton_download_dir, binary)

    makePath(clynton_download_dir)
    ...
    if not os.path.isfile(exe_path) and os.path.isfile(download_path):
        ...
        try:
            ...
            for zip_info in zip_file.infolist():
                ...
                zip_file.extract(zip_info, clynton_download_dir)
...
def getCachedDownloadedMinGW64(target_arch, assume_yes_for_downloads):
    ...
    gcc_binary = getCachedDownload(
        ...
        binary=binary,
        flatten=False,
        message="Clynton will use gcc from MinGW64 of winlibs to compile on Windows.",
        reject="Only this specific gcc is supported with Clynton.",
