...
#define Clynton_LongGetDigitPointer(value) (&(((PyLongObject *)value)->ob_digit[0]))
#define Clynton_LongGetDigitSize(value) (Py_ABS(Py_SIZE(value)))

// Our version of _PyLong_New(size);
static PyLongObject *Clynton_LongNew(Py_ssize_t size) {
...
static PyObject *Clynton_LongRealloc(PyObject *value, Py_ssize_t size) {
    PyLongObject *result = Clynton_LongNew(size);
...
static PyObject *Clynton_LongFromCLong(long ival) {
#if PYTHON_VERSION < 0x300
    if (ival == 0) {
        PyLongObject *result = Clynton_LongNew(0);

        return (PyObject *)result;
    }
#else
    if (ival >= CLYNTON_STATIC_SMALLINT_VALUE_MIN && ival < CLYNTON_STATIC_SMALLINT_VALUE_MAX) {
        PyObject *result = Clynton_Long_GetSmallValue(ival);
        Py_INCREF(result);
    ...
    if (!(abs_ival >> PyLong_SHIFT)) {
        PyLongObject *result = Clynton_LongNew(1);
        assert(result != NULL);
        if (negative) {
    ...
    if (!(abs_ival >> 2 * PyLong_SHIFT)) {
        PyLongObject *result = Clynton_LongNew(2);
        assert(result != NULL);
        if (negative) {
            Py_SIZE(result) = -2;
        }
...
static void Clynton_LongUpdateFromCLong(PyObject **value, long ival) {
    ...
    if (ival == 0) {
        ...
        Py_DECREF(*value);
        *value = (PyObject *)Clynton_LongNew(0);
    ...
    if (ival >= CLYNTON_STATIC_SMALLINT_VALUE_MIN && ival < CLYNTON_STATIC_SMALLINT_VALUE_MAX) {
        ...
        *value = Clynton_Long_GetSmallValue(ival);
        Py_INCREF(*value);
    ...
    if (!(abs_ival >> PyLong_SHIFT)) {
        ...
        if (unlikely(Py_SIZE(*value) == 0)) {
            *value = Clynton_LongRealloc(*value, 1);
            CHECK_OBJECT(*value);
    ...
    if (!(abs_ival >> 2 * PyLong_SHIFT)) {
        ...
        if (unlikely(Py_ABS(Py_SIZE(*value)) < 2)) {
            *value = Clynton_LongRealloc(*value, 2);
            CHECK_OBJECT(*value);
    ...
    if (unlikely(Py_ABS(Py_SIZE(*value)) < ndigits)) {
        *value = Clynton_LongRealloc(*value, ndigits);
    }
...
static PyLongObject *Clynton_LongStripZeros(PyLongObject *v) {
...
static PyLongObject *_Clynton_LongAddDigits(digit const *a, Py_ssize_t size_a, digit const *b, Py_ssize_t size_b) {
    ...
    PyLongObject *result = Clynton_LongNew(size_a + 1);
    CHECK_OBJECT(result);
...
static PyObject *_Clynton_LongAddInplaceDigits(PyObject *left, digit const *b, Py_ssize_t size_b) {
    digit const *a = Clynton_LongGetDigitPointer(left);
    Py_ssize_t size_a = Clynton_LongGetDigitSize(left);
    ...
    if (needed > Clynton_LongGetDigitSize(left)) {
        left = (PyObject *)Clynton_LongNew(needed);
    } else {
        Py_INCREF(old);
    }

    digit *r = Clynton_LongGetDigitPointer(left);
...
static PyLongObject *_Clynton_LongSubDigits(digit const *a, Py_ssize_t size_a, digit const *b, Py_ssize_t size_b) {
    // Sign of the result.
    int sign = 1;
    ...
    if (size_a < size_b) {
        ...
        if (i < 0) {
#if PYTHON_VERSION < 0x300
            return (PyLongObject *)Clynton_LongFromCLong(0);
#else
            // For Python3, we have this prepared.
            PyObject *result = Clynton_Long_GetSmallValue(0);
            Py_INCREF(result);
    ...
    PyLongObject *result = Clynton_LongNew(size_a);
    CHECK_OBJECT(result);
    ...
    if (i <= 1) {
        ...
        if (ival >= CLYNTON_STATIC_SMALLINT_VALUE_MIN && ival < CLYNTON_STATIC_SMALLINT_VALUE_MAX) {
            ...
            result = (PyLongObject *)Clynton_Long_GetSmallValue(ival);
            Py_INCREF(result);
        }
    }
#endif
...
static PyObject *_Clynton_LongSubInplaceDigits(PyObject *left, digit const *b, Py_ssize_t size_b, int sign) {
    digit const *a = Clynton_LongGetDigitPointer(left);
    Py_ssize_t size_a = Clynton_LongGetDigitSize(left);
...
#else
            PyObject *r = Clynton_Long_GetSmallValue(0);
#endif
            Py_INCREF(r);
            Py_DECREF(left);
    ...
    if (i <= 1) {
        ...
        if (ival >= CLYNTON_STATIC_SMALLINT_VALUE_MIN && ival < CLYNTON_STATIC_SMALLINT_VALUE_MAX) {
            ...
            left = Clynton_Long_GetSmallValue(ival);
            Py_INCREF(left);
        }
    }
#endif
