...
#
""" CType classes for PyObject *, PyObject **, and struct Clynton_CellObject *

"""

from clynton.__past__ import iterItems, xrange
from clynton.code_generation.ErrorCodes import (
...
from clynton.Constants import getConstantValueGuide, isMutable
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToClyntonBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? CLYNTON_BOOL_FALSE : CLYNTON_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = Clynton_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct Clynton_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "Clynton_Cell_New1(%s)" % init_from
        else:
            return "Clynton_Cell_Empty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "Clynton_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct Clynton_CellObject *%s" % variable_code_name
