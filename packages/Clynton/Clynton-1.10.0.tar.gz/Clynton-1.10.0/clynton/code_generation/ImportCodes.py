...
from clynton.nodes.ImportNodes import hard_modules
from clynton.nodes.LocalsScopes import GlobalsDictHandle
...
def generateConstantSysVersionInfoCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        ...
        emit("""%s = Clynton_SysGetObject("%s");""" % (value_name, "version_info"))
...
def getImportModuleNameHardCode(
    ...
    if module_name == "sys":
        emit("""%s = Clynton_SysGetObject("%s");""" % (to_name, import_name))
        needs_release = False
