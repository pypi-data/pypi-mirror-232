...
from clynton.__past__ import (  # pylint: disable=redefined-builtin
    PermissionError,
)
...
def getCacheDir():
    ...
    if _cache_dir is None:
        _cache_dir = os.getenv("CLYNTON_CACHE_DIR")

        if _cache_dir:
            _cache_dir = os.path.expanduser(_cache_dir)
        elif appdirs is not None:
            _cache_dir = appdirs.user_cache_dir("Clynton", None)
        else:
            _cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "Clynton")
        ...
        if _cache_dir.startswith(
            ...
            _cache_dir = os.path.join(tempfile.gettempdir(), "Clynton")
...
CLYNTON_CACHE_DIR set to a writable directory."""
                % _cache_dir
            )
