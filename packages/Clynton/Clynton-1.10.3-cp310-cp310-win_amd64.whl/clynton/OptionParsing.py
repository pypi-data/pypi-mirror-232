...
#
""" Command line options of Clynton.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "clynton.Options" values is not in
...
from clynton.PythonFlavors import getPythonFlavorName
from clynton.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from clynton.utils.FileOperations import getFileContentByLine
from clynton.utils.Utils import (
    getArchitecture,
...
from clynton.Version import getCommercialVersion, getClyntonVersion

# Indicator if we were called as "clynton-run" in which case we assume some
# other defaults and work a bit different with parameters.
is_clynton_run = os.path.basename(sys.argv[0]).lower().endswith("-run")

if not is_clynton_run:
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run Clynton, most
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run Clynton, this
...
for Scons. Otherwise Clynton can use what you run Clynton with or a Python
...
and Clynton will then find it and include it and all the modules found below that
...
and Clynton will then find it and include it in the binary or extension module
...
simplfy_group = parser.add_option_group("Onefile options")

simplfy_group.add_option(
    "--onefile-tempdir-spec",
    action="store",
    dest="justrandom_tmpdir",
    metavar="JUSTRANDOM_TMPDIR",
    default=None,
    help="""\
Use this as a folder to unpack to in onefile mode. Defaults to
'%TEMP%/siplfyunique_%PID%_%TIME%', i.e. user temporary directory
...
warnings_group = parser.add_option_group("Control the warnings to be given by Clynton")
...
warnings_group.add_option(
    ...
    default=False,
    help="""\
Allow Clynton to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the Clynton website. The mnemonic is the part
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a ".pyi" file for extension modules created by Clynton. This is
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in Clynton, do not use for
...
debug_group.add_option(
    ...
    default=False,
    help="""\
This is not incremental compilation, but for Clynton development only. Takes
...
Note: This is experimental and not working with standalone modes of Clynton yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of Clynton commercial.
...
def _considerPluginOptions(logger):
    ...
    from clynton.plugins.Plugins import (
...
def getClyntonProjectOptions(logger, filename_arg, module_mode):
    """Extract the Clynton project options.

    Note: This is used by Clynton project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of Clynton handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)clynton-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'clynton-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'clynton-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'clynton-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'clynton-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'clynton-project-else' not currently allowed after nested clynton-project-if.",
                    )
...
def _considerGithubWorkflowOptions(phase):
    ...
    for key, value in json.loads(os.environ["CLYNTON_WORKFLOW_INPUTS"]).items():
        if not value:
            continue
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getClyntonProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
Error, need filename argument with python module or main program."""
    ...
    if options.plugin_list:
        from clynton.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from clynton.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_data:
        from clynton.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.edit_module_code:
        from clynton.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from clynton.tools.environments.CreateEnvironment import (
...
pass them to the compiled program execution."""
    ...
    return is_clynton_run, options, positional_args, extra_args
