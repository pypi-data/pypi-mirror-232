...
""" Main front-end to the tests of Clynton.
...
from clynton.tools.Basics import goHome
...
from clynton.utils.Execution import check_call, check_output, getNullOutput
...
from clynton.utils.Timing import TimerReport
from clynton.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS
...
def parseOptions():
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if Clynton is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if Clynton handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if Clynton handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if Clynton handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if Clynton handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if Clynton does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if Clynton standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if Clynton works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles Clynton with Clynton, and then Clynton with the
compile Clynton and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow Clynton to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            clynton_id = check_output("git rev-parse HEAD".split())
        clynton_id = clynton_id.strip()

        if sys.version_info > (3,):
            clynton_id = clynton_id.decode()

        data_file.write("CLYNTON_SOURCE_DIR='%s'\n" % source_dir)
        data_file.write("CLYNTON_COMMIT='%s'\n" % clynton_id)
...
def main():
    ...
    def execute_tests(where, use_python, flags):
        ...
        if options.simplfy_tests and not options.coverage:
