...
Clynton can detect some things that PyLint and PyDev will complain about too,
and sometimes it's a false alarm, so people add disable markers into their
source code. Clynton does it itself.

This tries to parse the code for these markers and uses hooks to prevent Clynton
from warning about things, disabled to PyLint or Eclipse. The idea is that we
won't have another mechanism for Clynton, but use existing ones instead.

The code for this is very incomplete, barely good enough to cover Clynton's own
...
class ClyntonPluginPylintEclipseAnnotations(ClyntonPluginBase):
...
if False:
    ...
    class ClyntonPluginDetectorPylintEclipseAnnotations(ClyntonPluginBase):
        detector_for = ClyntonPluginPylintEclipseAnnotations
