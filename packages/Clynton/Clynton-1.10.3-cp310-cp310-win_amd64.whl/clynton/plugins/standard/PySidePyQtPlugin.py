...
from clynton.plugins.PluginBase import ClyntonPluginBase
...
from clynton.PythonFlavors import isAnacondaPython
from clynton.PythonVersions import python_version
from clynton.utils.FileOperations import getFileList, listDir
...
class ClyntonPluginQtBindingsPluginBase(ClyntonPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "clynton_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_clynton_patch_level', 0)"
                    ),
                ),
    ...
    def _getClyntonPatchLevel(self):
        """Does it include the Clynton patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().clynton_patch_level
...
if not path.startswith(__clynton_binary_dir):
    os.environ["PATH"] = __clynton_binary_dir + ";" + path
"""
            yield (
                code,
...
class ClyntonPluginPyQt5QtPluginsPlugin(ClyntonPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        ClyntonPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the Clynton support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = ClyntonPluginQtBindingsPluginBase._getQtInformation(self)
...
class ClyntonPluginDetectorPyQt5QtPluginsPlugin(ClyntonPluginBase):
    detector_for = ClyntonPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == ClyntonPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact Clynton commercial if you need it."
            )


class ClyntonPluginPySide2Plugins(ClyntonPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getClyntonPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://clynton.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getClyntonPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return ClyntonPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in ClyntonPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getClyntonPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: Clynton does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class ClyntonPluginDetectorPySide2Plugins(ClyntonPluginBase):
    detector_for = ClyntonPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == ClyntonPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class ClyntonPluginPySide6Plugins(ClyntonPluginQtBindingsPluginBase):
...
class ClyntonPluginDetectorPySide6Plugins(ClyntonPluginBase):
    detector_for = ClyntonPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == ClyntonPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class ClyntonPluginPyQt6Plugins(ClyntonPluginQtBindingsPluginBase):
...
class ClyntonPluginNoQt(ClyntonPluginBase):
