...
#ifndef CLYNTON_MAIN_MODULE_NAME
#define CLYNTON_MAIN_MODULE_NAME "__main__"
#endif

// It doesn't work for MinGW64 to update the standard output handles early on,
// so make a difference here.
#if defined(CLYNTON_FORCED_STDOUT_PATH) || defined(CLYNTON_FORCED_STDERR_PATH)
#if defined(__MINGW64__) || defined(__MINGW32__)
#define CLYNTON_STANDARD_HANDLES_EARLY 0
#else
#define CLYNTON_STANDARD_HANDLES_EARLY 1
#endif
#else
#define CLYNTON_STANDARD_HANDLES_EARLY 0
#endif
...
static void prepareFrozenModules(void) {
    ...
    struct _frozen *merged =
        (struct _frozen *)malloc(sizeof(struct _frozen) * (_CLYNTON_FROZEN + pre_existing_count + 1));
...
#if SYSFLAG_NO_RANDOMIZATION == 1 || SYSFLAG_UNBUFFERED == 1 || defined(_CLYNTON_STANDALONE)
...
static void undoEnvironmentVariable(PyThreadState *tstate, char const *variable_name,
    ...
    PyObject *variable_name_str = Clynton_String_FromString(variable_name);
...
#ifdef _WIN32
        PyObject *env_value = ClyntonUnicode_FromWideChar(old_value, -1);
#else
        PyObject *env_value = Clynton_String_FromString(old_value);
#endif
...
static environment_char_t const *old_env_anrandomhome;
...
static void prepareStandaloneEnvironment(void) {
    ...
    old_env_anrandomhome = getEnvironmentVariable("ANRANDOMHOME");
#if defined(_WIN32)
    setEnvironmentVariable("ANRANDOMHOME", getBinaryDirectoryWideChars(true));
#else
    setEnvironmentVariable("ANRANDOMHOME", getBinaryDirectoryHostEncoded(true));
#endif
    ...
    CLYNTON_PRINTF_TRACE("main(): Binary dir is %s\n", binary_directory);

    Py_Setanrandomhome(binary_directory);
...
static void restoreStandaloneEnvironment(void) {
    ...
    // CLYNTON_PRINTF_TRACE("Final PySys_GetPath is 's'.\n", PySys_GetPath());
#elif PYTHON_VERSION < 0x370
    PySys_SetPath(getBinaryDirectoryWideChars(true));
    Py_SetPath(getBinaryDirectoryWideChars(true));
    // CLYNTON_PRINTF_TRACE("Final Py_GetPath is '%ls'.\n", Py_GetPath());
#endif

#ifdef _CLYNTON_EXPERIMENTAL_DUMP_PY_PATH_CONFIG
...
#ifdef _WIN32
#define _CLYNTON_NATIVE_WCHAR_ARGV 1
#else
#define _CLYNTON_NATIVE_WCHAR_ARGV 0
#endif

// Types of command line arguments are different between Python2/3.
#if PYTHON_VERSION >= 0x300 && _CLYNTON_NATIVE_WCHAR_ARGV == 0
...
#if PYTHON_VERSION >= 0x350
    PRINT_FORMAT("Compiled Coroutines: %d | %d | %d\n", count_active_Clynton_Coroutine_Type,
                 count_allocated_Clynton_Coroutine_Type, count_released_Clynton_Coroutine_Type);
    PRINT_FORMAT("Compiled Coroutines Wrappers: %d | %d | %d\n", count_active_Clynton_CoroutineWrapper_Type,
                 count_allocated_Clynton_CoroutineWrapper_Type, count_released_Clynton_CoroutineWrapper_Type);

    PRINT_FORMAT("Compiled Coroutines AIter Wrappers: %d | %d | %d\n", count_active_Clynton_AIterWrapper_Type,
                 count_allocated_Clynton_AIterWrapper_Type, count_released_Clynton_AIterWrapper_Type);
#endif
#if PYTHON_VERSION >= 0x360
    PRINT_FORMAT("Compiled Asyncgen: %d | %d | %d\n", count_active_Clynton_Asyncgen_Type,
                 count_allocated_Clynton_Asyncgen_Type, count_released_Clynton_Asyncgen_Type);
    PRINT_FORMAT("Compiled Asyncgen Wrappers: %d | %d | %d\n", count_active_Clynton_AsyncgenValueWrapper_Type,
                 count_allocated_Clynton_AsyncgenValueWrapper_Type, count_released_Clynton_AsyncgenValueWrapper_Type);
    PRINT_FORMAT("Compiled Asyncgen Asend: %d | %d | %d\n", count_active_Clynton_AsyncgenAsend_Type,
                 count_allocated_Clynton_AsyncgenAsend_Type, count_released_Clynton_AsyncgenAsend_Type);
    PRINT_FORMAT("Compiled Asyncgen Athrow: %d | %d | %d\n", count_active_Clynton_AsyncgenAthrow_Type,
                 count_allocated_Clynton_AsyncgenAthrow_Type, count_released_Clynton_AsyncgenAthrow_Type);
#endif

    PRINT_FORMAT("Compiled Frames: %d | %d | %d (cache usage may occur)\n", count_active_Clynton_Frame_Type,
                 count_allocated_Clynton_Frame_Type, count_released_Clynton_Frame_Type);
...
static int HANDLE_PROGRAM_EXIT(void) {
    ...
    if (HAS_ERROR_OCCURRED(tstate)) {
        ...
        while (tstate->curexc_traceback) {
            ...
            if (0 == strcmp(PyUnicode_AsUTF8(Clynton_Frame_GetCodeObject(frame)->co_filename),
...
static PyObject *EXECUTE_MAIN_MODULE(char const *module_name) {
    CLYNTON_INIT_PROGRAM_LATE(module_name);

    PyThreadState *tstate = PyThreadState_GET();

#if CLYNTON_MAIN_PACKAGE_MODE
    {
...
#ifdef _CLYNTON_PLUGIN_WINDOWS_SERVICE_ENABLED
#include "clynton_windows_service.h"

// Callback from Windows Service logic.
bool SvcStartPython(void) {
    EXECUTE_MAIN_MODULE(CLYNTON_MAIN_MODULE_NAME);
...
#if _CLYNTON_NATIVE_WCHAR_ARGV == 0
...
#endif
    if (initial) {
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_SELF_FORKING
#if _CLYNTON_NATIVE_WCHAR_ARGV == 0
        printf("Commandline: ");
        for (int i = 0; i < argc; i++) {
            if (i != 0) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (i == 1) {
                ...
                if (strcmpFilename(argv[i], FILENAME_EMPTY_STR "install") == 0) {
                    CLYNTON_PRINT_TRACE("main(): Calling plugin SvcInstall().");

                    SvcInstall();
                    CLYNTON_CANNOT_GET_HERE("SvcInstall must not return");
                }
#endif
            }
...
#if !defined(_CLYNTON_DEPLOYMENT_MODE) && !defined(_CLYNTON_NO_DEPLOYMENT_SELF_EXECUTION)
...
#if defined(_CLYNTON_SIMPLFY_MODE) && defined(_WIN32)

static long simplfy_ppid;

DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    CLYNTON_PRINT_TRACE("Onefile parent monitoring starts.");

    for (;;) {
        Sleep(1000);

        HANDLE handle = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, simplfy_ppid);
    ...
    CLYNTON_PRINT_TRACE("Onefile parent monitoring causes KeyboardInterrupt.");
    PyErr_SetInterrupt();

    Sleep(_CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT);

    CLYNTON_PRINT_TRACE("Onefile parent monitoring hard kills after ignored KeyboardInterrupt.");
    ExitProcess(1);
...
#if CLYNTON_FORCED_STDOUT_NULL_BOOL
    if (strcmp(name, "stdout") == 0) {
        return true;
    }
#endif

#if CLYNTON_FORCED_STDERR_NULL_BOOL
    if (strcmp(name, "stderr") == 0) {
        return true;
    }
#endif

    PyObject *sys_std_handle = Clynton_SysGetObject(name);
...
static void setStdinHandle(PyThreadState *tstate, PyObject *stdin_file) {
    ...
    Clynton_SysSetObject("stdin", stdin_file);
...
static void setStdoutHandle(PyThreadState *tstate, PyObject *stdout_file) {
    ...
    Clynton_SysSetObject("stdout", stdout_file);
...
static void setStderrHandle(PyThreadState *tstate, PyObject *stderr_file) {
    ...
    Clynton_SysSetObject("stderr", stderr_file);
...
#if CLYNTON_STANDARD_HANDLES_EARLY == 0
#if defined(CLYNTON_FORCED_STDOUT_PATH) || defined(CLYNTON_FORCED_STDERR_PATH)
#ifdef _WIN32
static PyObject *getExpandedTemplatePath(wchar_t const *template_path) {
    ...
    return ClyntonUnicode_FromWideChar(filename_buffer, -1);
}
#else
static PyObject *getExpandedTemplatePath(char const *template_path) {
    ...
    return Clynton_String_FromString(filename_buffer);
}
#endif
#endif
#endif
...
// reason.
#if CLYNTON_STANDARD_HANDLES_EARLY == 1 && PYTHON_VERSION >= 0x370
#if defined(CLYNTON_FORCED_STDOUT_PATH) || defined(CLYNTON_FORCED_STDERR_PATH)
    PyObject *args = MAKE_DICT_EMPTY();

    DICT_SET_ITEM(args, const_str_plain_encoding, Clynton_String_FromString("utf-8"));
    DICT_SET_ITEM(args, const_str_plain_line_buffering, Py_True);

#if defined(CLYNTON_FORCED_STDOUT_PATH)
    {
        PyObject *sys_stdout = Clynton_SysGetObject("stdout");
...
#if defined(CLYNTON_FORCED_STDERR_PATH)
    {
        PyObject *sys_stderr = Clynton_SysGetObject("stderr");
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" CLYNTON_FORCED_STDOUT_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(CLYNTON_FORCED_STDOUT_PATH);
#endif
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" CLYNTON_FORCED_STDERR_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(CLYNTON_FORCED_STDERR_PATH);
#endif
    ...
    {
#if defined(_WIN32)
        PyObject *devnull_filename = Clynton_String_FromString("NUL:");
#else
        PyObject *devnull_filename = Clynton_String_FromString("/dev/null");
#endif
...
#if CLYNTON_FORCED_STDOUT_NONE_BOOL
    setStdoutHandle(Py_None);
#endif

#if CLYNTON_FORCED_STDERR_NONE_BOOL
...
static void Clynton_Py_Initialize(void) {
#if PYTHON_VERSION < 0x380 || defined(_CLYNTON_EXPERIMENTAL_OLD_PY_INITIALIZE)
    Py_Initialize();
#else
    ...
    CLYNTON_PRINT_TIMING("Clynton_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
// short lived.
#if SYSFLAG_ISOLATED
    Clynton_SysSetObject("path", PyList_New(0));
#endif
#endif
...
#if CLYNTON_STANDARD_HANDLES_EARLY == 1
#if defined(_WIN32)
...
#ifdef _CLYNTON_WINMAIN_ENTRY_POINT
...
int main(int argc, char **argv) {
...
#if defined(CLYNTON_FORCED_STDOUT_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_OUTPUT_HANDLE, stdout, L"" CLYNTON_FORCED_STDOUT_PATH);
#else
    changeStandardHandleTarget(stdout, CLYNTON_FORCED_STDOUT_PATH);
#endif
#endif
#if defined(CLYNTON_FORCED_STDERR_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_ERROR_HANDLE, stderr, L"" CLYNTON_FORCED_STDERR_PATH);
#else
    changeStandardHandleTarget(stderr, CLYNTON_FORCED_STDERR_PATH);
#endif
#endif
#if defined(CLYNTON_FORCED_STDIN_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_INPUT_HANDLE, stdin, L"" CLYNTON_FORCED_STDIN_PATH);
#else
    changeStandardHandleTarget(stdin, CLYNTON_FORCED_STDIN_PATH);
#endif
#endif
#endif
    ...
    CLYNTON_PRINT_TIMING("main(): Entered.");
    CLYNTON_INIT_PROGRAM_EARLY(argc, argv);
...
#ifdef _CLYNTON_STANDALONE
    CLYNTON_PRINT_TIMING("main(): Prepare standalone environment.");
...
#if _CLYNTON_FROZEN > 0
    CLYNTON_PRINT_TIMING("main(): Preparing frozen modules.");
...
#ifdef CLYNTON_PYTHON_STATIC
    CLYNTON_PRINT_TIMING("main(): Preparing static modules.");
...
#if PYTHON_VERSION >= 0x300 && _CLYNTON_NATIVE_WCHAR_ARGV == 0
    CLYNTON_PRINT_TRACE("main(): Calling convertCommandLineParameters.");
    orig_argv = convertCommandLineParameters(argc, argv);
#elif PYTHON_VERSION < 0x300 && _CLYNTON_NATIVE_WCHAR_ARGV == 1
...
orig_argv = argv;
    ...
    CLYNTON_PRINT_TRACE("main(): Calling initial setCommandLineParameters.");
...
#if PYTHON_VERSION < 0x300
    CLYNTON_PRINT_TRACE("main(): Prepare run environment '" PYTHON_HOME_PATH "'.");
    Py_Setanrandomhome(PYTHON_HOME_PATH);
#else
    CLYNTON_PRINTF_TRACE("main(): Prepare run environment '%S'.\n", L"" PYTHON_HOME_PATH);
    Py_Setanrandomhome(L"" PYTHON_HOME_PATH);
    // Make sure the above Py_Setanrandomhome call has effect already.
    Py_GetPath();
#endif
#endif
    ...
    CLYNTON_PRINT_TRACE("main(): Disabling Python warnings.");
    {
#if PYTHON_VERSION >= 0x300
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    CLYNTON_PRINT_TIMING("main(): Calling Clynton_Py_Initialize to initialize interpreter.");
    Clynton_Py_Initialize();

    PyThreadState *tstate = PyThreadState_GET();

#ifdef _CLYNTON_STANDALONE
    CLYNTON_PRINT_TRACE("main(): Restore standalone environment.");
    ...
    CLYNTON_PRINT_TRACE("main(): Calling setCommandLineParameters.");
...
// Empty "sys.path" again, the above adds program directory to it.
    ...
    CLYNTON_PRINT_TRACE("main(): Calling _initBuiltinModule().");
    _initBuiltinModule();

    /* Initialize the Python constant values used. This also sets
     * "sys.executable" while at it.
     */
    CLYNTON_PRINT_TIMING("main(): Calling createGlobalConstants().");
    createGlobalConstants(tstate);
    CLYNTON_PRINT_TIMING("main(): Returned createGlobalConstants().");

    /* Complex call helpers need "__main__" constants, even if we only
     * go into "__parents__main__" module as a start point.
     */
    CLYNTON_PRINT_TIMING("main(): Calling createMainModuleConstants().");
    createMainModuleConstants(tstate);
    CLYNTON_PRINT_TIMING("main(): Returned createMainModuleConstants().");

    CLYNTON_PRINT_TRACE("main(): Calling _initBuiltinOriginalValues().");
...
#if PYTHON_VERSION < 0x300
    PyStructSequence_SET_ITEM(Clynton_SysGetObject("flags"), 9, const_int_0);
#else
    PyStructSequence_SetItem(Clynton_SysGetObject("flags"), 6, const_int_0);
#endif
#endif

    /* Initialize the compiled types of Clynton. */
    ...
    CLYNTON_PRINT_TRACE("main(): Calling enhancePythonTypes().");
    enhancePythonTypes();

    CLYNTON_PRINT_TRACE("main(): Calling patchTypeComparison().");
    patchTypeComparison();

    CLYNTON_PRINT_TRACE("main(): Calling patchTracebackDealloc().");
    patchTracebackDealloc();

#ifndef CLYNTON_USE_PYCORE_THREAD_STATE
    /* Allow to override the ticker value, to remove checks for threads in
     * CPython core from impact on benchmarks. */
    char const *ticker_value = getenv("CLYNTON_TICKER");
    ...
    CLYNTON_PRINT_TRACE("main(): Setting Python input/output handles.");
    ...
    CLYNTON_PRINT_TRACE("main(): Calling setEarlyFrozenModulesFileAttribute().");

    setEarlyFrozenModulesFileAttribute(tstate);
#endif

#if _CLYNTON_FROZEN > 0
    CLYNTON_PRINT_TRACE("main(): Removing early frozen module table again.");
    PyImport_FrozenModules = old_frozen;
#endif

    CLYNTON_PRINT_TRACE("main(): Calling setupMetaPathBasedLoader().");
...
#if NO_PYTHON_WARNINGS && PYTHON_VERSION >= 0x342 && PYTHON_VERSION < 0x3a0 && defined(_CLYNTON_FULL_COMPAT)
...
#if PYTHON_VERSION >= 0x300
    CLYNTON_PRINT_TRACE("main(): Calling patchInspectModule().");
    patchInspectModule(tstate);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_NO_RANDOMIZATION == 1
    CLYNTON_PRINT_TRACE("main(): Reverting to initial 'PYTHONHASHSEED' value.");
    undoEnvironmentVariable(tstate, "PYTHONHASHSEED", old_env_hash_seed);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_UNBUFFERED == 1
    CLYNTON_PRINT_TRACE("main(): Reverting to initial 'PYTHONUNBUFFERED' value.");
    undoEnvironmentVariable(tstate, "PYTHONUNBUFFERED", old_env_unbuffered);
#endif

#ifdef _CLYNTON_STANDALONE
    // Restore the PATH, so the program can use it.
    CLYNTON_PRINT_TRACE("main(): Reverting to initial 'PATH' value.");
    undoEnvironmentVariable(tstate, "PATH", old_env_path);
    undoEnvironmentVariable(tstate, "ANRANDOMHOME", old_env_anrandomhome);
#endif
...
#ifdef _CLYNTON_PLUGIN_MULTIPROCESSING_ENABLED
    if (unlikely(is_multiprocessing_fork)) {
        CLYNTON_PRINT_TRACE("main(): Calling __parents_main__.");
        EXECUTE_MAIN_MODULE("__parents_main__");

        int exit_code = HANDLE_PROGRAM_EXIT();

        CLYNTON_PRINT_TRACE("main(): Calling __parents_main__ Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(is_joblib_popen_loky_posix)) {
        CLYNTON_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix.");
        ...
        int res = PyList_SetSlice(Clynton_SysGetObject("argv"), 0, 2, const_tuple_empty);
        assert(res == 0);
        ...
        CLYNTON_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(multiprocessing_resource_tracker_arg != -1)) {
        CLYNTON_PRINT_TRACE("main(): Launching as 'multiprocessing.resource_tracker'.");
        ...
        CLYNTON_PRINT_TRACE("main(): Calling resource_tracker Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(loky_resource_tracker_arg != -1)) {
        CLYNTON_PRINT_TRACE("main(): Launching as 'joblib.externals.loky.backend.resource_tracker'.");
        ...
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("CLYNTON_JUST_ANPARENT", buffer, sizeof(buffer));

            if (size > 0 && size < 127) {
                simplfy_ppid = atol(buffer);

                CreateThread(NULL, 0, doOnefileParentMonitoring, NULL, 0, NULL);
            }
        }
#endif
        PyDict_DelItemString(Clynton_GetSysModules(), CLYNTON_MAIN_MODULE_NAME);

#if _CLYNTON_PLUGIN_WINDOWS_SERVICE_ENABLED
        CLYNTON_PRINT_TRACE("main(): Calling plugin SvcLaunchService() entry point.");
        SvcLaunchService();
#else
    /* Execute the "__main__" module. */
    CLYNTON_PRINT_TIMING("main(): Calling " CLYNTON_MAIN_MODULE_NAME ".");
    EXECUTE_MAIN_MODULE(CLYNTON_MAIN_MODULE_NAME);
    CLYNTON_PRINT_TIMING("main(): Exited from " CLYNTON_MAIN_MODULE_NAME ".");
    ...
    CLYNTON_PRINT_TIMING("main(): Calling Py_Exit.");
    Py_Exit(exit_code);

    // The "Py_Exit()" calls is not supposed to return.
    CLYNTON_CANNOT_GET_HERE("Py_Exit does not return");
}
