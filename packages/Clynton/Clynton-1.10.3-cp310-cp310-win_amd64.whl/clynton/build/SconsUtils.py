...
from clynton.__past__ import basestring, unicode
...
from clynton.utils.FileOperations import getFileContentByLine, openTextFile
from clynton.utils.Utils import isLinux, isMacOS, isPosixWindows
...
def _enableFlagSettings(env, name, experimental_flags):
    for flag_name in experimental_flags:
        ...
        if value:
            env.Append(CPPDEFINES=[("_CLYNTON_%s" % flag_name, value)])
        else:
            env.Append(CPPDEFINES=["_CLYNTON_%s" % flag_name])
...
def createEnvironment(
    ...
    env.clynton_python = getArgumentBool("clynton_python", False)
...
def provideStaticSourceFile(sub_path, clynton_src, source_dir, c11_mode):
    source_filename = os.path.join(clynton_src, "stat_source", sub_path)
    target_filename = os.path.join(source_dir, "stat_source", os.path.basename(sub_path))
...
def createDefinitionsFile(source_dir, filename, definitions):
    for env_name in os.environ["_CLYNTON_BUILD_DEFINITIONS_CATALOG"].split(","):
    ...
    with openTextFile(build_definitions_filename, "w", encoding="utf8") as f:
        for key, value in sorted(definitions.items()):
            if key == "_CLYNTON_BUILD_DEFINITIONS_CATALOG":
                continue
    ...
    if not _linker_arch_determined:
        if win_target:
            ...
            _linker_arch = _getBinaryArch(
                binary=os.environ["CLYNTON_PYTHON_EXE_PATH"], mingw_mode=mingw_mode
            )
...
b) Using "--mingw64" lets Clynton download MinGW64 for you. Note: MinGW64
