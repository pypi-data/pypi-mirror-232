...
#
""" This is the main actions of Clynton.
...
from clynton.build.DataComposerInterface import runDataComposer
from clynton.build.SconsUtils import getSconsReportValue, readSconsReport
from clynton.code_generation.ConstantCodes import addDistributionMetadataValue
from clynton.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from clynton.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from clynton.importing import Importing, Recursion
from clynton.Options import (
    getPythonPgoInput,
...
from clynton.PostProcessing import executePostProcessing
from clynton.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from clynton.PythonFlavors import (
    isApplePython,
...
from clynton.PythonVersions import (
    getPythonABI,
    getSupportedPythonVersions,
    python_version,
    python_version_str,
)
from clynton.Serialization import ConstantAccessor
from clynton.Tracing import general, inclusion_logger
from clynton.tree import SyntaxErrors
from clynton.tree.ReformulationMultidist import createMultidistMainSourceCode
from clynton.utils import InstanceCounters
from clynton.utils.Distributions import getDistribution
...
from clynton.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from clynton.utils.ModuleNames import ModuleName
from clynton.utils.ReExecute import callExecProcess, reExecuteClynton
from clynton.utils.StaticLibraries import getSystemStaticLibPythonPath
from clynton.utils.Utils import isMacOS, isWin32Windows
...
def pickSourceFilenames(source_dir, modules):
    ...
    def _getModuleFilenames(module):
        base_filename = os.path.join(source_dir, "simplefile." + module.getFullName())
...
def makeSourceDirectory():
    ...
    writeSourceCode(
        filename=os.path.join(source_dir, "__helpfriendly.h"), source_code=helper_decl_code
    )

    writeSourceCode(
        filename=os.path.join(source_dir, "__helpfriendly.c"), source_code=helper_impl_code
    )

    writeSourceCode(
        filename=os.path.join(source_dir, "__constclyn.h"),
        source_code=constants_header_code,
    )

    writeSourceCode(
        filename=os.path.join(source_dir, "__constclyn.c"),
...
def _runCPgoBinary():
    ...
    if _wasMsvcMode():
        ...
        gcc_constants_pgo_filename = os.path.join(
            OutputDirectories.getSourceDirectoryPath(), "__constclyn.gcda"
        )
...
def _runPythonPgoBinary():
    ...
    with withEnvironmentVarOverridden("CLYNTON_PGO_OUTPUT", pgo_filename):
        _exit_code = _runPgoBinary()
...
def runSconsBackend():
    ...
    options = {
        ...
        "source_dir": OutputDirectories.getSourceDirectoryPath(),
        "clynton_python": asBoolStr(isClyntonPython()),
    ...
    if Options.isOnefileMode():
        options["simplfy_mode"] = asBoolStr(True)

        if Options.isOnefileTempDirMode():
            options["simplfy_temp_mode"] = asBoolStr(True)
    ...
    if int(os.environ.get("CLYNTON_NOSITE_FLAG", Options.hasPythonFlagNoSite())):
    ...
    if Options.shallCreatePgoInput():
        ...
        result = runScons(
            ...
            scons_filename="randomBack.scons",
        )
...
def compileTree():
    ...
    if not Options.shallOnlyExecCCompilerCall():
        ...
        bytecode_accessor = ConstantAccessor(
            data_filename="__bt_code.const", top_level_name="bytecode_data"
        )
        ...
        writeSourceCode(
            filename=os.path.join(source_dir, "__simplfyloader.c"), source_code=loader_code
        )

    else:
        source_dir = OutputDirectories.getSourceDirectoryPath()

        if not os.path.isfile(os.path.join(source_dir, "__helpfriendly.h")):
...
Clynton is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m clynton', e.g. use this
'python%s -m clynton' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of Clynton

    At this point, options will be parsed already, Clynton will be executing
    ...
    general.info(
        "Starting Python compilation with Clynton %r on Python %r commercial grade %r."
        % (
            getClyntonVersion(),
            python_version_str,
    ...
    if Options.shallCreatePgoInput():
        ...
        reExecuteClynton(pgo_filename=pgo_filename)
