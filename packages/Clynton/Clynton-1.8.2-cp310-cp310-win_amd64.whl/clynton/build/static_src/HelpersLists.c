...
#if CLYNTON_LIST_HAS_FREELIST
static struct _Py_list_state *_Clynton_Py_get_list_state(void) {
...
PyObject *MAKE_LIST_EMPTY(Py_ssize_t size) {
    ...
    struct _Py_list_state *state = _Clynton_Py_get_list_state();
    ...
    if (state->numfree) {
        ...
        Clynton_Py_NewReference((PyObject *)result_list);
    } else {
        result_list = (PyListObject *)Clynton_GC_New(&PyList_Type);
    }
...
#if _CLYNTON_EXPERIMENTAL_DISABLE_LIST_OPT
...
#else
    // TODO: Actually to be fully correct, we will need a "Clynton_LongFromCSsize_t" which
    // we do not have yet.
    return Clynton_LongFromCLong((long)value);
#endif
}
...
PyObject *LIST_COUNT(PyObject *list, PyObject *item) {
    ...
    for (Py_ssize_t i = 0; i < Py_SIZE(list); i++) {
        ...
        Py_INCREF(element);
        clynton_bool nbool_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(element, item);
        Py_DECREF(element);
...
PyObject *LIST_INDEX3(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start) {
    ...
    PyObject *start_index = Clynton_Number_IndexAsLong(start);
...
PyObject *LIST_INDEX4(PyThreadState *tstate, PyObject *list, PyObject *item, PyObject *start, PyObject *stop) {
    ...
    PyObject *stop_index = Clynton_Number_IndexAsLong(stop);
...
bool LIST_INSERT(PyThreadState *tstate, PyObject *list, PyObject *index, PyObject *item) {
    ...
    PyObject *index_long = Clynton_Number_IndexAsLong(index);
...
static void _Clynton_ReverseObjectsSlice(PyObject **lo, PyObject **hi) {
...
void LIST_REVERSE(PyObject *list) {
    ...
    if (Py_SIZE(list_object) > 1) {
        _Clynton_ReverseObjectsSlice(list_object->ob_item, list_object->ob_item + Py_SIZE(list_object));
    }
}
...
PyObject *MAKE_LIST(PyThreadState *tstate, PyObject *iterable) {
    ...
    if (_PyObject_HasLen(iterable)) {
        Py_ssize_t iter_len = Clynton_PyObject_Size(iterable);
