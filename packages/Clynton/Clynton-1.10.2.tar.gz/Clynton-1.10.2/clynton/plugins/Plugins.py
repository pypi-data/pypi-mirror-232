...
Plugins: Welcome to Clynton! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper Clynton, and some of which are waiting to be created and submitted for
inclusion by you.
...
from clynton.__past__ import basestring, iter_modules
from clynton.build.DataComposerInterface import deriveModuleConstantsBlobName
from clynton.containers.OrderedDicts import OrderedDict
from clynton.containers.OrderedSets import OrderedSet
from clynton.freezer.IncludedDataFiles import IncludedDataFile
from clynton.freezer.IncludedEntryPoints import IncludedEntryPoint
from clynton.ModuleRegistry import addUsedModule
from clynton.PythonVersions import python_version
from clynton.Tracing import plugins_logger, printLine
from clynton.utils.FileOperations import (
    getDllBasename,
    makePath,
    putTextFileContents,
)
from clynton.utils.Importing import importFileAsModule
from clynton.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import ClyntonPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, ClyntonPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "ClyntonPluginDetector", ""
            ) != plugin_class.__name__.replace("ClyntonPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match ClyntonPlugin* and ClyntonPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import clynton.plugins.standard

    _loadPluginClassesFromPackage(clynton.plugins.standard)

    try:
        import clynton.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(clynton.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from clynton.importing import Recursion
        from clynton.importing.Importing import getModuleNameAndKindFromFilename
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from clynton.tree.Building import buildModule
    ...
    def onModuleInitialSet():
        ...
        from clynton.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from clynton.ModuleRegistry import getDoneModules
    ...
    def _getExtraCodeFiles(for_onefile):
        ...
        for plugin in getActivePlugins():
            ...
            if value is not None:
                ...
                for key, value in sorted(value.items()):
                    if (for_onefile and not "simplfy_" in key) or (
                        not for_onefile and "simplfy_" in key
                    ):
                        continue

                    if not key.startswith("clynton_"):
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from clynton.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in Clynton".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not ClyntonPluginBase
            and issubclass(obj, ClyntonPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or Clynton arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of Clynton plugin '%s' not given."
