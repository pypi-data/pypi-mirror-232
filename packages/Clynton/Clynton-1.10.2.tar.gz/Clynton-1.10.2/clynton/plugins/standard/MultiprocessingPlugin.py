...
class ClyntonPluginMultiprocessingWorkarounds(ClyntonPluginBase):
    """This is to make multiprocessing work with Clynton and use compiled code.
...
""" % (
                "__clynton_binary_exe" if Options.isStandaloneMode() else "argv0"
            )
            return (
                code,
                """\
...
Monkey patching "multiprocessing" for compiled methods.""",
    ...
    def createFakeModuleDependency(module):
        ...
        # into the code and let Clynton compile time optimize it, which will also
        # make sure joblib support is there for Python2 as well.
        if python_version >= 0x340:
            source_code += """
def __clynton_freeze_support():
    import sys
...
__import__("multiprocessing.forking").forking.freeze_support()"""
    ...
    def getPreprocessorSymbols():
        if getModuleInclusionInfoByName("__parents_main__"):
            return {"_CLYNTON_PLUGIN_MULTIPROCESSING_ENABLED": "1"}
