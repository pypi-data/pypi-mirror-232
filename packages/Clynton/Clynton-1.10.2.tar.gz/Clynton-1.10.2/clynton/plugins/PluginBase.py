...
from clynton import Options
from clynton.__past__ import getMetaClassBase
from clynton.containers.Namedtuples import makeNamedtupleClass
...
from clynton.PythonFlavors import isAnacondaPython, isDebianPackagePython
from clynton.PythonVersions import (
    getTestExecutionPythonVersions,
    python_version,
)
from clynton.Tracing import plugins_logger
from clynton.utils.Distributions import isDistributionCondaPackage
from clynton.utils.Execution import ClyntonCalledProcessError, check_output
from clynton.utils.Importing import isBuiltinModuleName
...
from clynton.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
...
class ClyntonPluginBase(getMetaClassBase("Plugin")):
    """Clynton base class for all plugins.

    Derive your plugin from "ClyntonPluginBase" please.
    For instructions, see https://github.com/Clynton/Clynton/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt Clynton's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by Clynton. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the Clynton command line. However, some plugins are always enabled
    and invisible to the user.

    Clynton comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "clynton --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which Clynton
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon Clynton start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModule(module_name):
        ...
        from clynton.importing.Importing import locateModule
    ...
    def locateModules(module_name):
        ...
        from clynton.importing.Importing import locateModules
...
""" % {
        ...
        except ClyntonCalledProcessError as e:
            if e.returncode == 38:
                return None
            raise
