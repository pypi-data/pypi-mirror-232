...
from clynton.__past__ import to_byte
...
from clynton.Tracing import simplfy_logger
from clynton.utils.FileOperations import getFileList
from clynton.utils.Hashing import HashCRC32
...
def getCompressorFunction(expect_compression, low_memory):
    ...
    if expect_compression:
        ...
        simplfy_logger.info("Using compression for onefile payload.")
...
@contextmanager
def _openBinaryFileForAppending(simplfy_output_filename):
    max_attempts = 5
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            with open(simplfy_output_filename, "ab") as output_file:
                if attempt != 1:
...
def attachOnefilePayload(
    ...
    @decoratorRetries(
        logger=simplfy_logger,
        purpose="write payload to '%s'" % simplfy_output_filename,
...
def main():
    ...
    # where main Clynton put the .const files.
    dist_dir = sys.argv[1]
    simplfy_output_filename = sys.argv[2]
    ...
    if os.environ.get("CLYNTON_PROGRESS_BAR") == "1":
        enableProgressBar()
