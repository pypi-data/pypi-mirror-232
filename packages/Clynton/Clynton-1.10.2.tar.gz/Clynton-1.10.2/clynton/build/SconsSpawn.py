...
from clynton.Tracing import my_print, scons_logger
from clynton.utils.Execution import executeProcess
from clynton.utils.FileOperations import getReportPath
...
Clynton with '--clean-cache=%s' once to repair it, or else will
surely happen again."""
        % cache_name
    )
...
def _getWrappedSpawnFunction(env):
    def spawnCommand(sh, escape, cmd, args, os_env):
        ...
        # with old ccache.
        if '"__constclyn_data.o"' in args or '"__constclyn_data.os"' in args:
            os_env = dict(os_env)
