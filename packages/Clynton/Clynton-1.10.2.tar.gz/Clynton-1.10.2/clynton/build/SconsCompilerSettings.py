...
from clynton.Tracing import scons_details_logger, scons_logger
from clynton.utils.Download import getCachedDownloadedMinGW64
...
be usable with Clynton. Use the Visual Studio installer for \
adding it."""
        )
...
def _enableC11Settings(env):
    ...
    if not c11_mode:
        env.Append(CPPDEFINES=["_CLYNTON_NON_C11_MODE"])
...
def _enableLtoSettings(
    ...
    elif env.clynton_python:
        lto_mode = True
        reason = "known to be supported (Clynton-Python)"
    elif (
        env.debian_python
        and env.gcc_mode
...
C compiler ('%s' -> '%s') arches, that compiler is ignored!"""
                    % (
                        os.environ["CLYNTON_PYTHON_EXE_PATH"],
                        linker_arch,
                        compiler_path,
                        compiler_arch,
                    )
...
def decideConstantsBlobResourceMode(env, module_mode):
    if "CLYNTON_RESOURCE_MODE" in os.environ:
        resource_mode = os.environ["CLYNTON_RESOURCE_MODE"]
...
def addConstantBlobFile(env, blob_filename, resource_desc, target_arch):
    ...
    if resource_mode == "win_resource":
        # On Windows constants can be accessed as a resource by Clynton at run time afterwards.
        env.Append(CPPDEFINES=["_CLYNTON_CONSTANTS_FROM_RESOURCE"])
    elif resource_mode == "mac_section":
        env.Append(CPPDEFINES=["_CLYNTON_CONSTANTS_FROM_MACOS_SECTION"])
        ...
        env.Append(CPPDEFINES=["_CLYNTON_CONSTANTS_FROM_INCBIN"])

        constants_generated_filename = os.path.join(
            env.source_dir, "__constclyn_data.c"
        )
...
#define INCBIN_LOCAL
#ifdef _CLYNTON_EXPERIMENTAL_WRITEABLE_CONSTANTS
#define INCBIN_OUTPUT_SECTION ".data"
#endif

#include "clynton/incbin.h"
...
"""
        ...
        env.Append(CPPDEFINES=["_CLYNTON_CONSTANTS_FROM_LINKER"])
        ...
        env.Append(
            LINKFLAGS=[
                ...
                "-Wl,-defsym",
                "-Wl,%sconstant_bin_data=_binary_%s___constclyn_bin_start"
                % (
        ...
        env.Append(CPPDEFINES=["_CLYNTON_CONSTANTS_FROM_CODE"])
...
#if !defined(_CLYNTON_EXPERIMENTAL_WRITEABLE_CONSTANTS)
const
#endif
...
def setupCCompiler(env, lto_mode, pgo_mode, job_count):
    ...
    if env.gcc_mode and env.lto_mode:
        if env.debug_mode:
            ...
            # thing, and Clynton-Python is not affected.
            env.Append(
                LINKFLAGS=[
                    "-O3"
                    if env.clynton_python or os.name == "nt" or not env.static_libpython
                    else "-O2"
                ]
            )
    ...
    if env.debug_mode:
        ...
        env.Append(CPPDEFINES=["__CLYNTON_NO_ASSERT__"])
    ...
    if env.mingw_mode or env.msvc_mode:
        if env.disable_console:
            env.Append(CPPDEFINES=["_CLYNTON_WINMAIN_ENTRY_POINT"])
...
def _enablePgoSettings(env, pgo_mode):
    if pgo_mode == "no":
        ...
        env.Append(CPPDEFINES=["_CLYNTON_PGO_PYTHON"])
    elif pgo_mode == "generate":
        env.progressbar_name = "Profile"
        env.Append(CPPDEFINES=["_CLYNTON_PGO_GENERATE"])
        ...
        env.Append(CPPDEFINES=["_CLYNTON_PGO_USE"])
