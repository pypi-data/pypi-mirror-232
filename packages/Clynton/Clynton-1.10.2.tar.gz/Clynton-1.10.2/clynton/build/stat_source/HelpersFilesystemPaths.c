...
#include "clynton/checksum_tools.h"
...
static DWORD Clynton_GetFinalPathNameByHandleW(HANDLE hFile, LPWSTR lpszFilePath, DWORD cchFilePath, DWORD dwFlags) {
...
static void resolveFileSymbolicLink(wchar_t *resolved_filename, wchar_t const *filename, DWORD resolved_filename_size,
    ...
    if (resolve_symlinks) {
        ...
        DWORD len = Clynton_GetFinalPathNameByHandleW(file_handle, resolved_filename, resolved_filename_size,
            ...
            if (w == NULL) {
                ...
                *w = 0;
                ...
                if (wcsicmp(var_name, L"TEMP") == 0) {
                    ...
                    is_path = true;
#ifdef CLYNTON_COMPANY_NAME
                } else if (wcsicmp(var_name, L"COMPANY") == 0) {
                    appendWStringSafeW(target, L"" CLYNTON_COMPANY_NAME, buffer_size);
#endif
#ifdef CLYNTON_PRODUCT_NAME
                } else if (wcsicmp(var_name, L"PRODUCT") == 0) {
                    appendWStringSafeW(target, L"" CLYNTON_PRODUCT_NAME, buffer_size);
#endif
#ifdef CLYNTON_VERSION_COMBINED
                } else if (wcsicmp(var_name, L"VERSION") == 0) {
                    appendWStringSafeW(target, L"" CLYNTON_VERSION_COMBINED, buffer_size);
#endif
            ...
            if (w == NULL) {
                ...
                *w = 0;
                ...
                if (strcasecmp(var_name, "TEMP") == 0) {
                    ...
                    appendStringSafe(target, CLYNTON_COMPANY_NAME, buffer_size);
#endif
#ifdef CLYNTON_PRODUCT_NAME
                } else if (strcasecmp(var_name, "PRODUCT") == 0) {
                    appendStringSafe(target, CLYNTON_PRODUCT_NAME, buffer_size);
#endif
#ifdef CLYNTON_VERSION_COMBINED
                } else if (strcasecmp(var_name, "VERSION") == 0) {
                    appendStringSafe(target, CLYNTON_VERSION_COMBINED, buffer_size);
#endif
