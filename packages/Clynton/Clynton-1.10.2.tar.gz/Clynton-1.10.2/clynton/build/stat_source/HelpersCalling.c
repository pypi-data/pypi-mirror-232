...
PyObject *CALL_METHOD_WITH_POSARGS(PyThreadState *tstate, PyObject *source, PyObject *attr_name,
    ...
    if (PyInstance_Check(source)) {
        ...
        if (called_object != NULL) {
            ...
            if (descr_get == Clynton_Function_Type.tp_descr_get) {
                return Clynton_CallMethodFunctionPosArgs(tstate, (struct Clynton_FunctionObject const *)called_object,
...
#endif
    {
        ...
        if (type->tp_getattro != NULL) {
            ...
            called_object = (*type->tp_getattr)(source, (char *)Clynton_String_AsString_Unchecked(attr_name));
        } else {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT2(PyExc_AttributeError, "'%s' object has no attribute '%s'",
                                                type->tp_name, Clynton_String_AsString_Unchecked(attr_name));
...
char const *GET_CALLABLE_NAME(PyObject *object) {
    if (Clynton_Function_Check(object)) {
        return Clynton_String_AsString(Clynton_Function_GetName(object));
    } else if (Clynton_Generator_Check(object)) {
        return Clynton_String_AsString(Clynton_Generator_GetName(object));
    } else if (PyMethod_Check(object)) {
        return PyEval_GetFuncName(PyMethod_GET_FUNCTION(object));
    } else if (PyFunction_Check(object)) {
        return Clynton_String_AsString(((PyFunctionObject *)object)->func_name);
    }
#if PYTHON_VERSION < 0x300
    else if (PyInstance_Check(object)) {
        return Clynton_String_AsString(((PyInstanceObject *)object)->in_class->cl_name);
    } else if (PyClass_Check(object)) {
        return Clynton_String_AsString(((PyClassObject *)object)->cl_name);
    }
#endif
...
char const *GET_CALLABLE_DESC(PyObject *object) {
    if (Clynton_Function_Check(object) || Clynton_Generator_Check(object) || PyMethod_Check(object) ||
...
char const *GET_CLASS_NAME(PyObject *klass) {
    if (klass == NULL) {
        ...
        if (PyClass_Check(klass)) {
            return Clynton_String_AsString(((PyClassObject *)klass)->cl_name);
        }
#endif
...
void formatCannotInstantiateAbstractClass(PyThreadState *tstate, PyTypeObject *type) {
    ...
    PyObject *comma = Clynton_String_FromString(", ");
    CHECK_OBJECT(comma);
#if PYTHON_VERSION < 0x300
    PyObject *joined = CALL_METHOD_WITH_SINGLE_ARG(tstate, comma, const_str_plain_join, sorted_methods);

    char const *joined_str = Clynton_String_AsString(joined);
    ...
    SET_CURRENT_EXCEPTION_TYPE0_FORMAT3(PyExc_TypeError,
                                        ...
                                        method_count > 1 ? "s" : "", Clynton_String_AsString(joined));
