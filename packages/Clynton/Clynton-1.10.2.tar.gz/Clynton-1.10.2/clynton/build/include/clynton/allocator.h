...
//
#ifndef __CLYNTON_ALLOCATOR_H__
#define __CLYNTON_ALLOCATOR_H__
...
#if PYTHON_VERSION < 0x270 && !defined(__CLYNTON_NO_ASSERT__)
#define _PyObject_GC_IS_TRACKED(obj) (1)
#endif

#if PYTHON_VERSION >= 0x380
// Need to make Py_DECREF a macro again that doesn't call an API
static inline void _Clynton_Py_DECREF(PyObject *ob) {
...
#undef Py_DECREF
#define Py_DECREF(ob) _Clynton_Py_DECREF((PyObject *)(ob))

// Need to make Py_XDECREF a macro again that doesn't call an API
static inline void _Clynton_Py_XDECREF(PyObject *ob) {
    if (ob != NULL) {
...
#undef Py_XDECREF
#define Py_XDECREF(ob) _Clynton_Py_XDECREF((PyObject *)(ob))
...
static void Clynton_Py_NewReference(PyObject *op) {
#ifdef Py_REF_DEBUG
    _Py_RefTotal++;
#endif
    Py_SET_REFCNT(op, 1);
...
#else
#define Clynton_Py_NewReference(op) _Py_NewReference(op)
#endif

static inline int Clynton_PyType_HasFeature(PyTypeObject *type, unsigned long feature) {
    return ((type->tp_flags & feature) != 0);
}

#if PYTHON_VERSION >= 0x3b0

static inline size_t Clynton_PyType_PreHeaderSize(PyTypeObject *tp) {
    return _PyType_IS_GC(tp) * sizeof(PyGC_Head) +
           Clynton_PyType_HasFeature(tp, Py_TPFLAGS_MANAGED_DICT) * 2 * sizeof(PyObject *);
}

extern void Clynton_PyObject_GC_Link(PyObject *op);

static PyObject *Clynton_PyType_AllocNoTrackVar(PyTypeObject *type, Py_ssize_t nitems) {
    ...
    const size_t pre_size = Clynton_PyType_PreHeaderSize(type);
    ...
    if (Clynton_PyType_HasFeature(type, Py_TPFLAGS_HEAPTYPE)) {
        Py_INCREF(type);
    }

    Clynton_Py_NewReference(obj);

    return obj;
}

static PyObject *Clynton_PyType_AllocNoTrack(PyTypeObject *type) {
    // TODO: This ought to be static for all our types, so remove it as a call.
    const size_t pre_size = Clynton_PyType_PreHeaderSize(type);
...
CLYNTON_MAY_BE_UNUSED static void *Clynton_GC_NewVar(PyTypeObject *type, Py_ssize_t nitems) {
    assert(nitems >= 0);
    ...
    Clynton_Py_NewReference((PyObject *)op);

    return op;
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = Clynton_PyType_AllocNoTrackVar(type, nitems);
#endif
    assert(Py_SIZE(op) == nitems);
    return op;
}

CLYNTON_MAY_BE_UNUSED static void *Clynton_GC_New(PyTypeObject *type) {
#if PYTHON_VERSION < 0x3b0
...
#else
    // TODO: We ought to inline this probably too, no point as a separate function.
    PyObject *op = Clynton_PyType_AllocNoTrack(type);
#endif
    return op;
}
