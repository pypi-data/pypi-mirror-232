...
from clynton.__past__ import Iterable, unicode
from clynton.containers.OrderedSets import OrderedSet
from clynton.importing.Importing import (
    addMainScriptDirectory,
    decideModuleSourceRef,
    flushImportCache,
    locateModule,
)
from clynton.Tracing import wheel_logger
from clynton.utils.Execution import check_call
from clynton.utils.FileOperations import deleteFile, getFileList, renameFile
from clynton.utils.ModuleNames import ModuleName


def setupClyntonDistutilsCommands(dist, keyword, value):
    # If the user project setup.py includes the key "build_with_clynton=True" all
    # build operations (build, bdist_wheel, install etc) will run via Clynton.
    ...
    dist.cmdclass["bdist_wheel"] = bdist_clynton
...
class build(distutils.command.build.build):
        ...
        for _is_package, module_name_orig in builds:
            ...
            if os.path.isdir(main_filename):
                if not getFileList(main_filename, only_suffixes=(".py",)):
                    wheel_logger.info(
                        "Skipping '%s' from Clynton compilation due to containing no Python code."
                        % module_name_orig
                    )
                    continue
    ...
    def _build(self, build_lib):
        ...
        for is_package, module_name in self._findBuildTasks():
            # Clynton wants the main package by filename, probably we should stop
            # needing that.
            ...
            toml_filename = os.environ.get("CLYNTON_TOML_FILE")
            if toml_filename:
                ...
                # Cannot use FileOperations.getFileContents() here, because of non-Clynton process
                # pylint: disable=unspecified-encoding

                with open(toml_filename) as toml_file:
                    toml_options = toml_loads(toml_file.read())

                for option, value in toml_options.get("clynton", {}).items():
                    command.extend(self._parseOptionsEntry(option, value))

            # Process any extra options from setuptools
            if "clynton" in self.distribution.command_options:
...
# Required by distutils, used as command name, pylint: disable=invalid-name
class bdist_clynton(wheel.bdist_wheel.bdist_wheel):
    ...
    def write_wheelfile(self, wheelfile_base, generator=None):
        if generator is None:
            ...
            generator = "Clynton (%s)" % getClyntonVersion()
