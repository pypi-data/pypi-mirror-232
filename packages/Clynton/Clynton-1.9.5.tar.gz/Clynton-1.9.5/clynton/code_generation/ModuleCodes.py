...
from clynton.code_generation import Emission
from clynton.Version import getClyntonVersion, getClyntonVersionYear
...
def generateClyntonLoaderCreationCode(to_name, expression, emit, context):
    with withObjectCodeTemporaryAssignment(
        to_name, "clynton_loader_value", expression, emit, context
    ) as result_name:
        emit("%s = Clynton_Loader_New(loader_entry);" % result_name)
