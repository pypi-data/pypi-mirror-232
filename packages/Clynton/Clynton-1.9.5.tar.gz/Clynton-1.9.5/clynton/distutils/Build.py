...
#
""" Clynton python -m build integration """
...
class ClyntonBuildMetaBackend(setuptools.build_meta._BuildMetaBackend):
    def build_wheel(
        self, wheel_directory, config_settings=None, metadata_directory=None
    ):
        os.environ["CLYNTON_TOML_FILE"] = os.path.join(os.getcwd(), "pyproject.toml")

        with suppress_known_deprecation():
            return self._build_with_temp_dir(
                ["bdist_clynton"], ".whl", wheel_directory, config_settings
            )


_BACKEND = ClyntonBuildMetaBackend()
