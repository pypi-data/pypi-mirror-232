...
""" Runner for plugins tests of Clynton.
...
def main():
    ...
    for filename in sorted(os.listdir(".")):
        ...
        if filename == "parameters":
            os.environ["CLYNTON_EXTRA_OPTIONS"] = extra_options + " --trace-my-plugin"
        else:
            os.environ["CLYNTON_EXTRA_OPTIONS"] = extra_options
        ...
        if active:
            ...
            if filename in ("code_signing",):
                if getCommercialVersion() is None:
                    reportSkip(
                        "Plugin only available in Clynton commercial", ".", filename
                    )
                    continue
