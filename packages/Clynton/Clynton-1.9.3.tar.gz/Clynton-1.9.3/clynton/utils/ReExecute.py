...
#
"""Ability to restart Clynton, needed for removing site module effects and using Python PGO after compile.
...
def reExecuteClynton(pgo_filename):
    ...
    if "clynton.__main__" in sys.modules:
        our_filename = sys.modules["clynton.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    os.environ["CLYNTON_BINARY_NAME"] = sys.modules["__main__"].__file__
    os.environ["CLYNTON_PACKAGE_HOME"] = os.path.dirname(
    ...
    if pgo_filename is not None:
        ...
        os.environ["CLYNTON_SYS_PREFIX"] = sys.prefix

    # Same arguments as before.
    args += sys.argv[1:]

    from clynton.importing.PreloadedPackages import (
        detectPreLoadedPackagePaths,
        detectPthImportedPackages,
    )

    os.environ["CLYNTON_NAMESPACES"] = repr(detectPreLoadedPackagePaths())
    ...
    if "site" in sys.modules:
        ...
        os.environ["CLYNTON_SITE_FILENAME"] = site_filename

        # Note: As side effect, this might modify the "sys.path" too.
        os.environ["CLYNTON_PTH_IMPORTED"] = repr(detectPthImportedPackages())

    os.environ["CLYNTON_PYTHONPATH"] = repr(sys.path)

    # In some environments, initial "sys.path" does not contain enough to load
    # "ast" module, which however we use to decode "CLYNTON_PYTHONPATH", this
    # helps solve the chicken and egg problem.
    import ast

    os.environ["CLYNTON_PYTHONPATH_AST"] = os.path.dirname(ast.__file__)

    if sys.flags.no_site:
        os.environ["CLYNTON_NOSITE_FLAG"] = "1"

    os.environ["PYTHONHASHSEED"] = "0"

    os.environ["CLYNTON_REEXECUTION"] = "1"
