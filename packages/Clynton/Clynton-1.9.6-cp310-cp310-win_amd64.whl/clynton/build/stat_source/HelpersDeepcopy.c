...
static PyObject *Clynton_CapsuleNew(void *pointer) {
...
#define Clynton_CapsuleGetPointer(capsule) (((Clynton_PyCapsule *)(capsule))->pointer)

#else
#define Clynton_CapsuleGetPointer(capsule) (PyCObject_AsVoidPtr(capsule))
#endif

#if PYTHON_VERSION >= 0x3a0
PyTypeObject *Clynton_PyUnion_Type;
#endif

static PyObject *_makeDeepCopyFunctionCapsule(copy_func func) { return Clynton_CapsuleNew((void *)func); }
...
static void _initDeepCopy(void) {
...
#if PYTHON_VERSION >= 0x3a0
    {
        ...
        Clynton_PyUnion_Type = Py_TYPE(union_value);

        PyDict_SetItem(_deep_copy_dispatch, (PyObject *)Clynton_PyUnion_Type, _deep_noop);
...
static PyObject *DEEP_COPY_ITEM(PyThreadState *tstate, PyObject *value, PyTypeObject **type, copy_func *copy_function) {
    *type = Py_TYPE(value);
    ...
    if (unlikely(dispatcher == NULL)) {
        CLYNTON_CANNOT_GET_HERE("DEEP_COPY encountered unknown type");
    }
    ...
    if (dispatcher == Py_None) {
        ...
        *copy_function = (copy_func)(Clynton_CapsuleGetPointer(dispatcher));
...
PyObject *DEEP_COPY(PyThreadState *tstate, PyObject *value) {
    ...
    if (dispatcher == Py_None) {
        ...
        copy_func copy_function = (copy_func)(Clynton_CapsuleGetPointer(dispatcher));
        return copy_function(tstate, value);
    }
...
Py_hash_t DEEP_HASH(PyThreadState *tstate, PyObject *value) {
    ...
    if (PyType_Check(value)) {
        ...
        while (Clynton_DictNext(value, &pos, &key, &dict_value)) {
...
#endif
#if PYTHON_VERSION >= 0x3a0
    } else if (Py_TYPE(value) == Clynton_PyUnion_Type) {
        Py_hash_t result = DEEP_HASH_INIT(tstate, value);
        ...
        CLYNTON_CANNOT_GET_HERE("Unknown type hashed");
...
void CHECK_OBJECT_DEEP(PyObject *value) {
    ...
    if (PyTuple_Check(value)) {
        ...
        while (Clynton_DictNext(value, &pos, &dict_key, &dict_value)) {
...
static PyObject *_DEEP_COPY_ELEMENT_GUIDED(PyThreadState *tstate, PyObject *value, char const **guide) {
    ...
    *guide += 1;
    ...
    default:
        CLYNTON_CANNOT_GET_HERE("Illegal type guide");
        abort();
    }
}
