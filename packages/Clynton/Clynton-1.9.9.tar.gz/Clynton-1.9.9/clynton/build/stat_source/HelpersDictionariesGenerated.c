...
static inline PyDictValues *_Clynton_PyDict_new_values(Py_ssize_t size) {
...
static struct _Py_dict_state *_Clynton_Py_get_dict_state(void) {
...
static PyDictObject *_Clynton_AllocatePyDictObject(void) {
    PyDictObject *result_mp;

#if CLYNTON_DICT_HAS_FREELIST
    struct _Py_dict_state *state = _Clynton_Py_get_dict_state();

    if (state->numfree) {
        result_mp = state->free_list[--state->numfree];

        Clynton_Py_NewReference((PyObject *)result_mp);
...
#endif
    {
        result_mp = (PyDictObject *)Clynton_GC_New(&PyDict_Type);
    }
...
#if PYTHON_VERSION >= 0x360 && !defined(_CLYNTON_EXPERIMENTAL_DISABLE_DICT_OPT)
static Py_ssize_t _Clynton_Py_PyDict_KeysSize(PyDictKeysObject *keys) {
#if PYTHON_VERSION < 0x360
...
PyObject *DICT_COPY(PyObject *dict_value) {
#if _CLYNTON_EXPERIMENTAL_DISABLE_DICT_OPT
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            PyDictValues *new_values = _Clynton_PyDict_new_values(size);
            ...
            if (dict_mp->ma_values == NULL && IS_COMPACT(dict_mp)) {
                ...
                Py_ssize_t keys_size = _Clynton_Py_PyDict_KeysSize(dict_mp->ma_keys);
            ...
            while (Clynton_DictNext((PyObject *)dict_mp, &pos, &key, &value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(value);
...
PyObject *DEEP_COPY_DICT(PyThreadState *tstate, PyObject *dict_value) {
    ...
    while (Clynton_DictNext(dict_value, &pos, &key, &value)) {
...
static PyObject *COPY_DICT_KW(PyObject *dict_value) {
    ...
    if (((PyDictObject *)dict_value)->ma_used == 0) {
        ...
        if (_PyDict_HasSplitTable(dict_mp)) {
            ...
            while (Clynton_DictNext((PyObject *)dict_mp, &pos, &key, &_value)) {
                CHECK_OBJECT(key);
                CHECK_OBJECT(_value);
