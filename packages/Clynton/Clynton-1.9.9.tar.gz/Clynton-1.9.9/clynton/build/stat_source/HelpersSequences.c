...
Py_ssize_t Clynton_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
...
PyObject *Clynton_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!Clynton_Index_Check(item))) {
...
PyObject *Clynton_Number_IndexAsLong(PyObject *item) {
    PyObject *result = Clynton_Number_Index(item);
