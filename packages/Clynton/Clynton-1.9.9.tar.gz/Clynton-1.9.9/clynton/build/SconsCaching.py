...
from clynton.utils.Importing import importFromInlineCopy
from clynton.utils.Utils import hasMacOSIntelSupport, isMacOS, isWin32Windows
...
def _injectCcache(env, cc_path, python_prefix, assume_yes_for_downloads):
    ccache_binary = os.environ.get("CLYNTON_CCACHE_BINARY")
    ...
    if ccache_binary is None:
        ...
        if ccache_binary is None:
            if isWin32Windows():
                ...
                ccache_binary = getCachedDownload(
                    ...
                    flatten=True,
                    binary="ccache.exe",
                    message="Clynton will make use of ccache to speed up repeated compilation.",
                    reject=None,
                ...
                if tuple(int(d) for d in platform.release().split(".")) >= (18, 2):
                    url = "https://clynton.net/ccache/v4.2.1/ccache-4.2.1.zip"
        ...
        scons_details_logger.info(
            "Using ccache '%s' from CLYNTON_CCACHE_BINARY environment variable."
            % ccache_binary
        )
