...
Clynton can detect some things that "pkg_resources" may not even be able to during
...
from clynton.utils.Utils import withNoDeprecationWarning


class ClyntonPluginResources(ClyntonPluginBase):
...
class ClyntonProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__clynton_loader_type, ClyntonProvider)
"""

        yield (
            code,
            """Registering Clynton loader with "pkg_resources".""",
        )
