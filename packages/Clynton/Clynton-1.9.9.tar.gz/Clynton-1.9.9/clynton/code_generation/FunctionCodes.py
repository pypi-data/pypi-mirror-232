...
def getFunctionCreationArgs(
    ...
    if closure_variables:
        result.append("struct Clynton_CellObject **closure")
...
def getFunctionMakerCode(
    ...
    if is_constant_returning:
        ...
        if constant_return_value is None:
            ...
            constant_return_code = "Clynton_Function_EnableConstReturnTrue(result);"
        elif constant_return_value is False:
            constant_return_code = "Clynton_Function_EnableConstReturnFalse(result);"
        else:
            constant_return_code = (
                "Clynton_Function_EnableConstReturnGeneric(result, %s);"
...
def getClosureCopyCode(closure_variables, context):
    ...
    if closure_variables:
        closure_name = context.allocateTempName(
            "closure", "struct Clynton_CellObject *[%d]" % len(closure_variables)
        )
    else:
        closure_name = None
...
def _getFunctionCode(
    ...
    if context.isForCreatedFunction():
        parameter_objects_decl = ["struct Clynton_FunctionObject const *self"]
    else:
...
def getExportScopeCode(cross_module):
    if cross_module:
        return "CLYNTON_CROSS_MODULE"
    else:
        return "CLYNTON_LOCAL_MODULE"
    ...
    if expression.isExpressionOutlineFunctionBase():
        ...
        for variable in expression.getUserLocalVariables():
            ...
            if variable_declaration.getCType() is CTypeCellObject:
                emit("%s = Clynton_Cell_Empty();" % variable_declaration)
