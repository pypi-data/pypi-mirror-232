...
from .c_types.CTypeClyntonBooleans import CTypeClyntonBoolEnum
from .c_types.CTypeClyntonInts import CTypeClyntonIntOrLongStruct
from .c_types.CTypeClyntonVoids import CTypeClyntonVoidEnum
...
class VariableDeclaration(object):
    ...
    def __init__(self, c_type, code_name, init_value, heap_name):
        if c_type.startswith("CLYNTON_MAY_BE_UNUSED"):
    ...
    def makeCFunctionLevelDeclaration(self):
        ...
        return "%s%s%s%s%s%s;" % (
            "CLYNTON_MAY_BE_UNUSED " if self.maybe_unused else "",
            lead_c_type,
    ...
    def getCType(self):
        ...
        elif c_type == "struct Clynton_CellObject *":
            return CTypeCellObject
        elif c_type == "PyObject **":
            return CTypePyObjectPtrPtr
        elif c_type == "clynton_bool":
            return CTypeClyntonBoolEnum
        elif c_type == "bool":
            return CTypeBool
        elif c_type == "clynton_ilong":
            return CTypeClyntonIntOrLongStruct
        elif c_type == "module_var":
            return CTypeModuleDictVariable
        elif c_type == "clynton_void":
            return CTypeClyntonVoidEnum
        elif c_type == "long":
            return CTypeCLong
        elif c_type == "clynton_digit":
...
class VariableStorage(object):
    ...
    def addFrameCacheDeclaration(self, frame_identifier):
        return self.addVariableDeclarationFunction(
            "static struct Clynton_FrameObject *", "cache_%s" % frame_identifier, "NULL"
        )
    ...
    def getExceptionVariableDescriptions(self):
        if self.exception_variable_declarations is None:
            self.exception_variable_declarations = (
                ...
                self.addVariableDeclarationTop(
                    "CLYNTON_MAY_BE_UNUSED int", "exception_lineno", "0"
                ),
            )
