...
#
""" CType classes for clynton_ilong, a struct to represent long values.

"""


from clynton.code_generation.templates.CodeTemplatesVariables import (
...
class CTypeClyntonIntOrLongStruct(CTypeBase):
    c_type = "clynton_ilong"
    ...
    def emitVariableAssignCode(
        ...
        if tmp_name.c_type == "clynton_ilong":
            ...
            if tmp_name.c_type == "PyObject *":
                emit("%s.validity = CLYNTON_ILONG_OBJECT_VALID;" % value_name)
    ...
    def emitVariantAssignmentCode(cls, int_name, value_name, int_value, emit, context):
        ...
        if value_name is None:
            ...
            if int_value is None:
                emit("%s.validity = CLYNTON_ILONG_OBJECT_VALID;" % int_name)
                emit("%s.ilong_object = %s;" % (int_name, value_name))
            else:
                emit("%s.validity = CLYNTON_ILONG_BOTH_VALID;" % int_name)
                emit("%s.ilong_object = %s;" % (int_name, value_name))
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s.validity != CLYNTON_ILONG_UNASSIGNED);" % value_name)
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToClyntonIntOrLong(
                to_name=to_name,
    ...
    def getInitValue(cls, init_from):
        if init_from is None:
            ...
            return "{CLYNTON_ILONG_UNASSIGNED, NULL, 0}"
        else:
    ...
    def getInitTestConditionCode(cls, value_name, inverted):
        return "%s.validity %s CLYNTON_ILONG_UNASSIGNED" % (
            value_name,
            "==" if inverted else "!=",
        )
    ...
    def getReleaseCode(cls, value_name, needs_check, emit):
        emit(
            "if ((%s.validity & CLYNTON_ILONG_OBJECT_VALID) == CLYNTON_ILONG_OBJECT_VALID) {"
            % value_name
        )
