...
if (%(orig_name)s != %(tmp_name)s) {
    UPDATE_STRING_DICT_INPLACE(moduledict_%(module_identifier)s, (Clynton_StringObject *)%(variable_name_str)s, %(tmp_name)s);
}"""
                % {
                    "orig_name": orig_name,
                    "tmp_name": tmp_name,
            ...
            emit(
                "UPDATE_STRING_DICT%s(moduledict_%s, (Clynton_StringObject *)%s, %s);"
                % (
                    ref_count,
