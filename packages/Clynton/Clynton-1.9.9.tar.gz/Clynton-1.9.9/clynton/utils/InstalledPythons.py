...
from clynton.__past__ import WindowsError  # pylint: disable=I0021,redefined-builtin
from clynton.containers.OrderedSets import OrderedSet
from clynton.PythonVersions import python_version_str

from .Execution import (
    ClyntonCalledProcessError,
    check_output,
    getExecutablePath,
)
...
class InstalledPython(object):
    ...
    def checkUsability(self, module_name, module_version):
        ...
        if self.python_exe != sys.executable:
            ...
            except ClyntonCalledProcessError:
                return False
            except OSError:
                return False
