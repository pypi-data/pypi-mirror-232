...
static void _initBuiltinTypeMethods(void) {
    ...
    CLYNTON_PRINT_TRACE("main(): Calling _initStrBuiltinMethods().");
    _initStrBuiltinMethods();
#else
    CLYNTON_PRINT_TRACE("main(): Calling _initBytesBuiltinMethods().");
    _initBytesBuiltinMethods();
#endif
    CLYNTON_PRINT_TRACE("main(): Calling _initUnicodeBuiltinMethods().");
    _initUnicodeBuiltinMethods();
    CLYNTON_PRINT_TRACE("main(): Calling _initDictBuiltinMethods().");
    _initDictBuiltinMethods();
    CLYNTON_PRINT_TRACE("main(): Calling _initListBuiltinMethods().");
...
PyObject *BUILTIN_RANGE(PyThreadState *tstate, PyObject *boundary) {
    ...
    if (start == -1 && DROP_ERROR_OCCURRED(tstate)) {
        CLYNTON_ASSIGN_BUILTIN(range);

        PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, CLYNTON_ACCESS_BUILTIN(range), boundary_temp);
...
PyObject *BUILTIN_RANGE2(PyThreadState *tstate, PyObject *low, PyObject *high) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POSARGS2(tstate, CLYNTON_ACCESS_BUILTIN(range), pos_args);
...
PyObject *BUILTIN_RANGE3(PyThreadState *tstate, PyObject *low, PyObject *high, PyObject *step) {
    ...
    if (fallback) {
        ...
        PyObject *result = CALL_FUNCTION_WITH_POSARGS3(tstate, CLYNTON_ACCESS_BUILTIN(range), pos_args);
...
static PyObject *getLengthOfRange(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    clynton_bool nbool_res = RICH_COMPARE_GT_NBOOL_OBJECT_LONG(step, const_int_0);

    if (unlikely(nbool_res == CLYNTON_BOOL_EXCEPTION)) {
        return NULL;
    }
    ...
    if (nbool_res == CLYNTON_BOOL_TRUE) {
        lo = start;
        hi = stop;
        ...
        if (unlikely(nbool_res == CLYNTON_BOOL_TRUE)) {
    ...
    if (nbool_res != CLYNTON_BOOL_FALSE) {
        Py_DECREF(step);
...
static PyObject *MAKE_XRANGE(PyThreadState *tstate, PyObject *start, PyObject *stop, PyObject *step) {
    start = Clynton_Number_IndexAsLong(start);
    if (unlikely(start == NULL)) {
        return NULL;
    }
    stop = Clynton_Number_IndexAsLong(stop);
    if (unlikely(stop == NULL)) {
        return NULL;
    }
    step = Clynton_Number_IndexAsLong(step);
    if (unlikely(step == NULL)) {
        return NULL;
    }
...
#else
    PyObject *stop = Clynton_Number_IndexAsLong(high);
...
PyObject *BUILTIN_LEN(PyThreadState *tstate, PyObject *value) {
    ...
    Py_ssize_t res = Clynton_PyObject_Size(value);
...
bool PRINT_NEW_LINE_TO(PyObject *file) {
    ...
    if (likely(file == NULL)) {
        result = CALL_FUNCTION_NO_ARGS(tstate, CLYNTON_ACCESS_BUILTIN(print));
    } else {
        ...
        // uses a tuple.
        result = CALL_FUNCTION_WITH_KEYARGS(tstate, CLYNTON_ACCESS_BUILTIN(print), kw_args);
...
// print statement, so this code is really necessary.
    ...
    PyObject *result = CALL_FUNCTION(tstate, CLYNTON_ACCESS_BUILTIN(print), print_args, print_kw);
...
PyObject *GET_STDOUT(void) {
    PyObject *result = Clynton_SysGetObject("stdout");
...
PyObject *GET_STDERR(void) {
    PyObject *result = Clynton_SysGetObject("stderr");
...
static int clynton_class_setattr(PyClassObject *klass, PyObject *attr_name, PyObject *value) {
...
static PyObject *clynton_class_getattr(PyClassObject *klass, PyObject *attr_name) {
    char const *sattr_name = PyString_AsString(attr_name);
    ...
    descrgetfunc tp_descr_get = ClyntonType_HasFeatureClass(type) ? type->tp_descr_get : NULL;
...
void enhancePythonTypes(void) {
    ...
    // not doing that.
    PyClass_Type.tp_setattro = (setattrofunc)clynton_class_setattr;
    PyClass_Type.tp_getattro = (getattrofunc)clynton_class_getattr;
#endif
}
...
static bool MAKE_QUICK_ITERATOR(PyThreadState *tstate, PyObject *sequence, struct Clynton_QuickIterator *qiter) {
    if (Clynton_Generator_Check(sequence)) {
        qiter->iterator_mode = ITERATOR_COMPILED_GENERATOR;
        qiter->iterator_data.generator = (struct Clynton_GeneratorObject *)sequence;
...
static PyObject *QUICK_ITERATOR_NEXT(PyThreadState *tstate, struct Clynton_QuickIterator *qiter, bool *finished) {
    PyObject *result;
    ...
    case ITERATOR_COMPILED_GENERATOR:
        result = Clynton_Generator_qiter(tstate, qiter->iterator_data.generator, finished);
...
PyObject *BUILTIN_SUM1(PyThreadState *tstate, PyObject *sequence) {
    struct Clynton_QuickIterator qiter;
...
PyObject *BUILTIN_SUM2(PyThreadState *tstate, PyObject *sequence, PyObject *start) {
    ...
    PyObject *result = CALL_FUNCTION_WITH_POSARGS2(tstate, CLYNTON_ACCESS_BUILTIN(sum), pos_args);
...
static PyTypeObject Clynton_BuiltinModule_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_module", // tp_name
    sizeof(PyModuleObject),                           // tp_size
};

int Clynton_BuiltinModule_SetAttr(PyModuleObject *module, PyObject *name, PyObject *value) {
    CHECK_OBJECT(module);
    CHECK_OBJECT(name);
    ...
    if (res == 1) {
        CLYNTON_UPDATE_BUILTIN(open, value);
        found = true;
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            CLYNTON_UPDATE_BUILTIN(__import__, value);
            found = true;
        }
    }
    ...
    if (found == false) {
        ...
        if (res == 1) {
            CLYNTON_UPDATE_BUILTIN(print, value);
            found = true;
        }
    }
#endif
    ...
    if (found == false) {
        ...
        if (res == 1) {
            CLYNTON_UPDATE_BUILTIN(super, value);
            found = true;
        }
    }
...
static PyObject *getPathSeparatorStringObject(void) {
    ...
    if (sep_str == NULL) {
        sep_str = Clynton_String_FromString(sep);
    }
...
CLYNTON_MAY_BE_UNUSED static void stripFilenameW(wchar_t *path) {
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = ClyntonUnicode_FromWideChar(exe_filename, -1);
#else
...
// there e.g. code objects expect Python2 strings.
    ...
    *binary_object_target = ClyntonUnicode_FromWideChar(bin_directory, -1);
#else
...
void _initBuiltinModule(void) {
    CLYNTON_PRINT_TRACE("main(): Calling _initBuiltinTypeMethods().");
    _initBuiltinTypeMethods();
    CLYNTON_PRINT_TRACE("main(): Calling _initDeepCopy().");
    _initDeepCopy();
...
#ifdef _CLYNTON_STANDALONE
    int res = PyDict_SetItemString((PyObject *)dict_builtin, "__clynton_binary_dir", getBinaryDirectoryObject(true));
    assert(res == 0);
    PyDict_SetItemString((PyObject *)dict_builtin, "__clynton_binary_exe", getBinaryFilenameObject(true));
    assert(res == 0);
#endif

    // init Clynton_BuiltinModule_Type, PyType_Ready won't copy all member from
    ...
    // tp_setattro to Clynton_BuiltinModule_SetAttr and we can detect value
    // change. Set tp_base to PyModule_Type and PyModule_Check will pass.
    Clynton_BuiltinModule_Type.tp_dealloc = PyModule_Type.tp_dealloc;
    Clynton_BuiltinModule_Type.tp_repr = PyModule_Type.tp_repr;
    Clynton_BuiltinModule_Type.tp_setattro = (setattrofunc)Clynton_BuiltinModule_SetAttr;
    Clynton_BuiltinModule_Type.tp_getattro = PyModule_Type.tp_getattro;
    Clynton_BuiltinModule_Type.tp_flags = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE;
    Clynton_BuiltinModule_Type.tp_doc = PyModule_Type.tp_doc;
    Clynton_BuiltinModule_Type.tp_traverse = PyModule_Type.tp_traverse;
    Clynton_BuiltinModule_Type.tp_members = PyModule_Type.tp_members;
    Clynton_BuiltinModule_Type.tp_base = &PyModule_Type;
    Clynton_BuiltinModule_Type.tp_dictoffset = PyModule_Type.tp_dictoffset;
    Clynton_BuiltinModule_Type.tp_init = PyModule_Type.tp_init;
    Clynton_BuiltinModule_Type.tp_alloc = PyModule_Type.tp_alloc;
    Clynton_BuiltinModule_Type.tp_new = PyModule_Type.tp_new;
    Clynton_BuiltinModule_Type.tp_free = PyModule_Type.tp_free;
    int res2 = PyType_Ready(&Clynton_BuiltinModule_Type);
    assert(res2 >= 0);

    // Replace type of builtin module to take over.
    ((PyObject *)builtin_module)->ob_type = &Clynton_BuiltinModule_Type;
...
#else
        our_path_object = Clynton_String_FromString(getDllDirectory());
#endif
    }
...
CLYNTON_DEFINE_BUILTIN(type)
CLYNTON_DEFINE_BUILTIN(len)
CLYNTON_DEFINE_BUILTIN(repr)
CLYNTON_DEFINE_BUILTIN(int)
CLYNTON_DEFINE_BUILTIN(iter)
#if PYTHON_VERSION < 0x300
CLYNTON_DEFINE_BUILTIN(long)
#else
CLYNTON_DEFINE_BUILTIN(range);
#endif

void _initBuiltinOriginalValues(void) {
    CLYNTON_ASSIGN_BUILTIN(type);
    CLYNTON_ASSIGN_BUILTIN(len);
    CLYNTON_ASSIGN_BUILTIN(range);
    CLYNTON_ASSIGN_BUILTIN(repr);
    CLYNTON_ASSIGN_BUILTIN(int);
    CLYNTON_ASSIGN_BUILTIN(iter);
#if PYTHON_VERSION < 0x300
    CLYNTON_ASSIGN_BUILTIN(long);
#endif
...
// Used for threading.
#if PYTHON_VERSION >= 0x300 && !defined(CLYNTON_USE_PYCORE_THREAD_STATE)
