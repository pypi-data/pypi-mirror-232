...
from clynton.Errors import ClyntonForbiddenDLLEncounter
from clynton.plugins.Plugins import Plugins
from clynton.PythonFlavors import isAnacondaPython, isClyntonPython
from clynton.PythonVersions import python_version
from clynton.Tracing import inclusion_logger
from clynton.utils.FileOperations import areSamePaths, isFilenameBelowPath
from clynton.utils.Importing import getSharedLibrarySuffixes
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isClyntonPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although Clynton Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the Clynton Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise ClyntonForbiddenDLLEncounter(binary_filename, plugin_name)
...
def fixupBinaryDLLPathsMacOS(
    ...
    except ClyntonForbiddenDLLEncounter:
