...
#pragma warning(pop)
    ...
    if (Py_ABS(Py_SIZE(operand1_long_object)) <= 1 && Py_ABS(Py_SIZE(operand2_long_object)) <= 1) {
        ...
        if (Py_REFCNT(*operand1) == 1) {
            Clynton_LongUpdateFromCLong(&*operand1, r);
            goto exit_result_ok;
        } else {
            PyObject *obj = Clynton_LongFromCLong(r);
    ...
    if (Py_REFCNT(*operand1) == 1) {
        ...
        if (a_negative) {
            if (b_negative) {
                *operand1 = _Clynton_LongAddInplaceDigits(*operand1, b_digits, b_digit_count);
                Py_SET_SIZE(*operand1, -Py_ABS(Py_SIZE(*operand1)));
            } else {
                *operand1 = _Clynton_LongSubInplaceDigits(*operand1, b_digits, b_digit_count, -1);
            }
        } else {
            if (b_negative) {
                *operand1 = _Clynton_LongSubInplaceDigits(*operand1, b_digits, b_digit_count, 1);
