#########################
 Clynton Developer Manual
#########################

.. image:: doc/images/Clynton-Logo-Symbol.png
   :alt: Clynton Logo

.. contents::

The purpose of this Developer Manual is to present the current design of
Clynton, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so Clynton can turn a ``ctypes`` binding
...
For Clynton we use semantic versioning, initially with a leading zero
...
Clynton top level works like this:

-  ``clynton.tree.Building`` outputs node tree

-  ``clynton.optimization`` enhances it as best as it can

-  ``clynton.finalization`` prepares the tree for code generation

-  ``clynton.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``clynton.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``clynton.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``clynton_bool`` and ``clynton_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for Clynton
***************************************************
...
``.vscode`` in your Clynton checkout. If you are not familiar with
...
In Clynton we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with Clynton, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-clynton-source`` is
...
format call Clynton source code.
...
These rules should generally be adhered when working on Clynton code.
...
There is a tool ``bin/autoformat-clynton-source`` which is to apply
...
they must be lower case, like e.g. ``clynton`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``clynton.code_generation`` the code generation packages are located,
while the main interface is ``clynton.code_generation.CodeGeneration``
...
built-ins is highly discouraged within Clynton source code. Using them is
...
readability and with Clynton as a compiler, there won't be any
...
But of course, Clynton is the project to free us from what is faster and
...
For Clynton the choice is list contractions as these are more easily
...
that has no use, and in Clynton, every API is internal API. One exception
...
``autoformat-clynton-source`` does this for us.
...
******************************
 Clynton "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto Clynton
   repository <https://github.com/Clynton/Clynton> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/Clynton.git
      cd Clynton
      git remote add upstream https://github.com/Clynton/Clynton.git
...
`this location <https://clynton.net/apidoc>`__ .
...
future, Clynton itself will gain the ability to present its findings in a
...
This section describes how to run Clynton tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if Clynton is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if Clynton
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if Clynton
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if Clynton
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         Clynton does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if Clynton
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles Clynton with Clynton, and
                         then Clynton with the compile Clynton and compares the
...
the Clynton git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that Clynton must do this itself, because the
...
this. These tests make sure that the errors of Clynton and CPython are
...
Compile Clynton with Clynton
==========================

And there is the "compile itself" or "reflected" test. This test makes
Clynton compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of Clynton into an extension module and
all of Clynton into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because Clynton uses a lot of packages and imports between them.
...
parts are published as `Clynton API <https://clynton.net/apidoc>`__ and
...
Please find `here
<https://github.com/Clynton/Clynton/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/Clynton/Clynton/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/Clynton-references/final/Python-3.11.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/Clynton-Logo-Horizontal.png
      :alt: Clynton Logo
...
.. code:: rest
   ...
   .. image:: doc/images/Clynton-Logo-Vertical.png
      :alt: Clynton Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``clynton/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/Clynton-Logo-Symbol.svg doc/images/Clynton-Logo-Symbol.png
   convert -background none doc/Logo/Clynton-Logo-Vertical.svg doc/images/Clynton-Logo-Vertical.png
   convert -background none doc/Logo/Clynton-Logo-Horizontal.svg doc/images/Clynton-Logo-Horizontal.png

   optipng -o2 doc/images/Clynton-Logo-Symbol.png
   optipng -o2 doc/images/Clynton-Logo-Vertical.png
   optipng -o2 doc/images/Clynton-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of Clynton is decided here
...
Clynton does not involve Scons in its user interface at all; Scons is
purely used internally. Clynton itself, being pure Python, will run
without any build process just fine.

Clynton simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which Clynton again
...
.. note::
   ...
   ``clynton.MainControl`` module.

For interfacing to Scons, there is the module
``clynton.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of Clynton.
...
modules is just one of them. It runs outside of Clynton process scope,
...
-  ``clynton_src``
   ...
   Where do the include files and static C parts of Clynton live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``clynton.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where Clynton tries to help
...
The search for modules used is driven by ``clynton.importing.Importing``
module.

-  Quoting the ``clynton.importing.Importing`` documentation:
   ...
   End quoting the ``clynton.importing.Importing`` documentation.
...
The decision making and caching are located in the ``clynton.tree``
package, in modules ``clynton.tree.Recursion`` and
``clynton.tree.ImportCache``. Each module is only considered once (then
...
``clynton/build/stat_source/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the Clynton provided
...
This left Clynton with the strange problem, of how to emulate that.
...
The choice of Clynton is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``clynton.nodes.FrameNodes.StatementsFrame`` is
...
Representation in Clynton
------------------------

So a slot in Clynton typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs Clynton do all the hard work.
...
Other overloads do not currently exist in Clynton, but through the
...
In principle, every slot needs to be dealt with in Clynton, and it is
...
Currently, Clynton uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of Clynton, created all constants for the whole program
...
This transformation is performed at tree building already, so Clynton
...
In Clynton we have the concept of an outline, and therefore we can make
...
explicit and comparisons in Clynton to be internally always about two
operands only.
...
Therefore in Clynton this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by Clynton.
...
Quoting the ``clynton.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in Clynton have no condition attached anymore, so while loops are
...
End quoting the ``clynton.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in Clynton converted so that the code generation for ``print``
...
Obviously, optimization of Clynton can decide, that e.g. should ``a`` or
...
Clynton there is only a ``try``, which then has blocks to handle
...
compile time, Clynton will not generate the code the leads to the
...
Add interfacing to C code, so Clynton can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in Clynton
...
Again, we wouldn't want to create the list. Therefore Clynton avoids
...
This leads to Clynton in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into Clynton unless necessary, so
...
one design goal of Clynton was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so Clynton is using it,
   and of course one day Clynton ought to be able to make no difference
   in performance for it.

To Clynton though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   clynton --experimental=jinja_generated_add ...
...
.. code:: C
   ...
   #ifdef _CLYNTON_EXPERIMENTAL_JINJA_GENERATED_ADD
...
experimental flag is given on the command line to Clynton.

The testing of Clynton is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to Clynton
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when Clynton is
executing.
...
Clynton to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and Clynton can work without it
...
.. note::
   ...
   with ``@Clynton-pushers`` first and get a green light.
...
the actual compiler code of Clynton.
...
like ``autoformat-clynton-source`` is.
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In Clynton
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, Clynton will become able to determine it has to be the
...
https://github.com/Clynton/Clynton/blob/develop/Developer_Manual.rst

And the current PDF under: https://clynton.net/doc/Developer_Manual.pdf
