...
from clynton.freezer.DependsExe import getDependsExePath, parseDependsExeOutput
...
from clynton.utils.FileOperations import deleteFile
from clynton.utils.Utils import isFreeBSD, isMacOS, isWin32Windows
...
def _getRuntimeTraceOfLoadedFilesWin32(logger, command, required):
    ...
    # TODO: Move the handling of this into clynton.tools.Execution module methods.
    try:
...
def _takeSystemCallTraceOutput(logger, path, command):
    ...
    with withEnvironmentVarOverridden("LD_PRELOAD", None):
        if os.environ.get("CLYNTON_TRACE_COMMANDS", "0") != "0":
...
def main():
    from clynton.Tracing import tools_logger
