...
""" Tool to compare output of CPython and Clynton.
...
from clynton.OptionParsing import getClyntonProjectOptions
from clynton.PythonVersions import python_version
...
from clynton.tools.testing.OutputComparison import compareOutput
from clynton.Tracing import my_print
from clynton.utils.Execution import (
    callProcess,
    check_output,
    createProcess,
    executeProcess,
...
from clynton.utils.Timing import StopWatch
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.environ.get("CLYNTON_HASH_SALT", "")
...
def main():
    ...
    extra_options = os.environ.get("CLYNTON_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if "CLYNTON" in os.environ:
        # Would need to extract which "python" this is going to use.
        assert not coverage_mode, "Not implemented for binaries."

        clynton_call = [os.environ["PYTHON"], os.environ["CLYNTON"]]
    else:
        if comparison_mode:
            clynton_call = [
                os.environ["PYTHON"],
                "-m",
                "clynton.__main__",  # Note: Needed for Python2.6
            ]
        else:
    ...
    if coverage_mode:
        # Coverage modules hates Clynton to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses CLYNTON_EXTRA_OPTIONS still.
    ...
    # Now build the command to run Clynton.
    if not two_step_execution:
        ...
        clynton_cmd = clynton_call + extra_options + ["--run", filename]

        if no_site:
            clynton_cmd.insert(len(clynton_cmd) - 1, "--python-flag=-S")

    else:
        if module_mode:
            clynton_cmd1 = (
                clynton_call + extra_options + ["--module", os.path.abspath(filename)]
            )
        elif standalone_mode:
            clynton_cmd1 = clynton_call + extra_options + ["--standalone", filename]
        else:
            clynton_cmd1 = clynton_call + extra_options + [filename]

        if no_site:
            clynton_cmd1.insert(len(clynton_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        clynton_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        clynton_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("Clynton command", clynton_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(clynton_package_dir):
                process = createProcess(command=clynton_cmd)

            stdout_clynton, stderr_clynton = process.communicate()
            exit_clynton = process.returncode

            if checkNoPermissionError(stdout_clynton) and checkNoPermissionError(
                stderr_clynton
            ):
                break

            my_print("Retrying clynton exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("Clynton command 1:", clynton_cmd1)

        for _i in range(5):
            with withPythonPathChange(clynton_package_dir):
                stdout_clynton1, stderr_clynton1, exit_clynton1 = executeProcess(
                    clynton_cmd1
                )
...
%s
"""
                        % (clynton_cmd1, python_path_used, stderr_clynton1)
                    )

                exit_clynton = exit_clynton1
                stdout_clynton, stderr_clynton = stdout_clynton1, stderr_clynton1
                stdout_clynton2 = b"not run due to compilation error:\n" + stdout_clynton1
                stderr_clynton2 = stderr_clynton1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(clynton_cmd2[0][:-4] + ".cmd"):
                        clynton_cmd2[0] = clynton_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("Clynton command 2:", clynton_cmd2)

                    # Need full manual control
                    process = createProcess(command=clynton_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "Clynton compiled program", process.pid
                            ),
                        )

                    stdout_clynton2, stderr_clynton2 = process.communicate()
                    stdout_clynton = stdout_clynton1 + stdout_clynton2
                    stderr_clynton = stderr_clynton1 + stderr_clynton2
                    exit_clynton = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_clynton in (-11, -6) and sys.platform != "nt":
                        clynton_cmd2 = wrapCommandForDebuggerForSubprocess(*clynton_cmd2)

                        callProcess(clynton_cmd2, shell=False)
                else:
                    exit_clynton = exit_clynton1
                    stdout_clynton, stderr_clynton = stdout_clynton1, stderr_clynton1
    ...
    stop_watch.stop()
    clynton_time = stop_watch.getDelta()

    if not silent_mode:
        displayOutput(stdout_clynton, stderr_clynton)

        if coverage_mode:
            assert not stdout_clynton
            assert not stderr_clynton
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_clynton2 if two_step_execution else stdout_clynton,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_clynton2 if two_step_execution else stderr_clynton,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_clynton

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_clynton:d} (Clynton)""".format(
                        exit_cpython=exit_cpython, exit_clynton=exit_clynton
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.environ.get("CLYNTON_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_clynton in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            clynton_cmd.insert(len(clynton_cmd) - 1, "--debugger")

            with withPythonPathChange(clynton_package_dir):
                callProcess(clynton_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(clynton_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(clynton_cmd2[0] + ".away"):
                        os.unlink(clynton_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(clynton_cmd2[0], clynton_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs Clynton." % (cpython_time, clynton_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


clynton_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, clynton_package_dir)
