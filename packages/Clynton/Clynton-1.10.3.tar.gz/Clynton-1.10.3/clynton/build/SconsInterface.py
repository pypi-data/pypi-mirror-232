...
from clynton import Options, Tracing
from clynton.__past__ import unicode
from clynton.containers.OrderedDicts import OrderedDict
from clynton.Options import getOnefileChildGraceTime, isOnefileMode
...
from clynton.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while Clynton works with older Python, Scons does not, and therefore
Clynton needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2(scons_filename):
    is_backend = scons_filename == "randomBack.scons"
    ...
    if is_backend and isWin32Windows() and not Options.shallUseStaticLibPython():
        ...
        os.environ["CLYNTON_PYTHON_DLL_PATH"] = getTargetPythonDLLPath()

    os.environ["CLYNTON_PYTHON_EXE_PATH"] = sys.executable
    ...
    if python_version >= 0x300:
        ...
        if "ANRANDOMHOME" in os.environ:
            old_anrandomhome = os.environ["ANRANDOMHOME"]
            del os.environ["ANRANDOMHOME"]

    import clynton

    os.environ["CLYNTON_PACKAGE_DIR"] = os.path.abspath(clynton.__path__[0])
    ...
    if old_anrandomhome is not None:
        os.environ["ANRANDOMHOME"] = old_anrandomhome

    if "CLYNTON_PYTHON_DLL_PATH" in os.environ:
        del os.environ["CLYNTON_PYTHON_DLL_PATH"]

    del os.environ["CLYNTON_PYTHON_EXE_PATH"]

    del os.environ["CLYNTON_PACKAGE_DIR"]
...
def _setupSconsEnvironment(scons_filename):
    ...
    For the target Python we provide "CLYNTON_PYTHON_DLL_PATH" to see where the
    Python DLL lives, in case it needs to be copied, and then also the
    "CLYNTON_PYTHON_EXE_PATH" to find the Python binary itself.

    We also need to preserve "PYTHONPATH" and "ANRANDOMHOME", but remove it
...
def runScons(options, env_values, scons_filename):
    with _setupSconsEnvironment(scons_filename):
        env_values["_CLYNTON_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())
        ...
        if "source_dir" in options and Options.shallCompileWithoutBuildDirectory():
            ...
            options["clynton_src"] = getExternalUsePath(options["clynton_src"])
        ...
        env_values["CLYNTON_QUIET"] = "1" if Tracing.is_quiet else "0"
...
def cleanSconsDirectory(source_dir):
    ...
    if os.path.isdir(source_dir):
        ...
        static_dir = os.path.join(source_dir, "stat_source")
...
def setCommonSconsOptions(options):
    ...
    if "CompanyName" in string_values:
        env_values["CLYNTON_COMPANY_NAME"] = string_values["CompanyName"]
    if "ProductName" in string_values:
        env_values["CLYNTON_PRODUCT_NAME"] = string_values["ProductName"]
    ...
    if effective_version:
        env_values["CLYNTON_VERSION_COMBINED"] = effective_version

    if isClyntonPython() and not isWin32OrPosixWindows():
    ...
    # it.
    if isOnefileMode():
        env_values["_CLYNTON_SIMPLFY_CHILD_GRACE_TIME_INT"] = str(
