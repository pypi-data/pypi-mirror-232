...
class CTypeClyntonVoidEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "clynton_void"
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s == CLYNTON_VOID_OK);" % value_name)

    @classmethod
    def emitReinitCode(cls, value_name, emit):
        emit("%s = CLYNTON_VOID_OK;" % value_name)
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        ...
        if Options.is_debug:
            emit("%s = CLYNTON_VOID_OK;" % to_name)
    ...
    def getExceptionCheckCondition(cls, value_name):
        return "%s == CLYNTON_VOID_EXCEPTION" % value_name
