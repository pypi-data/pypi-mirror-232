...
def generateAssignmentVariableCode(statement, emit, context):
    ...
    if variable.isModuleVariable():
        ...
        if source_shape is tshape_bool and variable_declaration.c_type == "clynton_bool":
            tmp_name = context.allocateTempName("assign_source", "clynton_bool")
        elif (
            source_shape is tshape_int_or_long
            and variable_declaration.c_type == "clynton_ilong"
        ):
            tmp_name = context.allocateTempName("assign_source", "clynton_ilong")
        else:
            tmp_name = context.allocateTempName("assign_source")
...
def getVariableReferenceCode(
    ...
    if variable.isModuleVariable():
        ...
        with withObjectCodeTemporaryAssignment2(
            ...
            emit(
                """\
%(value_name)s = GET_STRING_DICT_VALUE(moduledict_%(module_identifier)s, (Clynton_StringObject *)%(var_name)s);
