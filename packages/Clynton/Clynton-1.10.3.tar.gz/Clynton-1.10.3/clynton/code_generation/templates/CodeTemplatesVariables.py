...
{
    PyObject *old = Clynton_Cell_GET(%(identifier)s);
    PyCell_SET(%(identifier)s, %(tmp_name)s);
    Py_XDECREF(old);
}
"""
...
assert(Clynton_Cell_GET(%(identifier)s) == NULL);
PyCell_SET(%(identifier)s, %(tmp_name)s);
"""
...
%(tmp_name)s = Clynton_Cell_GET(%(identifier)s);
"""
...
%(access_code)s
    ...
    UPDATE_STRING_DICT0((PyDictObject *)%(dict_name)s, (Clynton_StringObject *)%(var_name)s, value);
} else {
