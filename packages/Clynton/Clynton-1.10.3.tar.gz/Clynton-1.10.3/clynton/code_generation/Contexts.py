...
from clynton.__past__ import getMetaClassBase, iterItems
from clynton.Constants import isMutable
from clynton.PythonVersions import python_version
from clynton.Serialization import ConstantAccessor
from clynton.utils.Hashing import getStringHash
from clynton.utils.InstanceCounters import (
    counted_del,
    counted_init,
    isCountingInstances,
)
...
class TempMixin(object):
    ...
    def allocateTempName(self, base_name, type_name="PyObject *", unique=False):
        ...
        if unique:
            ...
            if result is None:
                ...
                if type_name.startswith("CLYNTON_MAY_BE_UNUSED"):
    ...
    def addExceptionPreserverVariables(self, preserver_id):
        ...
        if preserver_id not in self.preserver_variable_declaration:
            ...
            if needs_init:
                preserver_obj_init = "Clynton_ExceptionStackItem_Empty"
            else:
    ...
    def addCleanupTempName(self, tmp_name):
        ...
        assert (
            tmp_name.c_type != "clynton_void" or tmp_name.code_name == "tmp_unused"
        ), tmp_name
    ...
    def pushFrameHandle(self, code_identifier, is_light):
        ...
        if is_light:
            ...
            frame_identifier = self.variable_storage.addVariableDeclarationTop(
                "struct Clynton_FrameObject *", frame_handle, None
            )

        self.variable_storage.addVariableDeclarationTop(
            "CLYNTON_MAY_BE_UNUSED char const *",
