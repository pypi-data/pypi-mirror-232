...
from clynton.utils.Execution import getExecutablePath
from clynton.utils.FileOperations import (
    isPathExecutable,
    openTextFile,
    resolveShellPatternToFilenames,
)
from clynton.utils.Images import checkIconUsage
...
def _checkOnefileTargetSpec():
    checkPathSpec(
        options.justrandom_tmpdir,
        arg_name="--onefile-tempdir-spec",
        allow_disable=False,
    )

    if os.path.normpath(options.justrandom_tmpdir) == ".":
        Tracing.options_logger.sysexit(
            """\
...
as that would overwrite it and cause locking issues as well."""
    ...
    if options.justrandom_tmpdir.count("%") == 0:
        Tracing.options_logger.warning(
    ...
    if os.path.isabs(options.justrandom_tmpdir):
        Tracing.options_logger.warning(
...
very well known environment: anchor with e.g. %%TEMP%%, %%CACHE_DIR%% is recommended: '%s'"""
    ...
    elif not options.justrandom_tmpdir.startswith(
        ("%TEMP%", "%HOME%", "%CACHE_DIR%")
    ):
        Tracing.options_logger.warning(
...
def parseArgs():
    ...
    global is_clynton_run, options, positional_args, extra_args, is_debug, is_non_debug
...
Error, the Python from Windows app store is not supported.""",
    ...
    is_clynton_run, options, positional_args, extra_args = parseOptions(
    ...
    if options.quiet or int(os.environ.get("CLYNTON_QUIET", "0")):
        Tracing.setQuiet()
    ...
    if os.environ.get("CLYNTON_REEXECUTION") and not isAllowedToReexecute():
        Tracing.general.sysexit(
    ...
    if options.clean_caches:
        from clynton.CacheCleanup import cleanCaches
...
when '--onefile' is not specified."""
    ...
    if options.simplfy_child_grace_time is not None:
        if not options.simplfy_child_grace_time.isdigit():
            Tracing.options_logger.sysexit(
                """\
    ...
    if python_version_str not in getSupportedPythonVersions():
        ...
        if "PYTHON" not in os.environ:
            Tracing.general.warning(
                """\
The Python version '%s' is only experimentally supported by Clynton '%s', \
but an upcoming release will change that. In the mean time use Python \
version '%s' instead or newer Clynton."""
                % (
                    python_version_str,
                    getClyntonVersion(),
    ...
    if python_version_str in getNotYetSupportedPythonVersions():
        if not isExperimental("python" + python_version_str):
            Tracing.general.sysexit(
                """\
The Python version '%s' is not supported by Clynton '%s', but an upcoming \
...
def _shallUseStaticLibPython():
    ...
    if options.static_libpython == "auto":
        # Clynton-Python is good to to static linking.
        if isClyntonPython():
            return True, "Clynton-Python is broken."

        # Debian packages with are usable if the OS is new enough
        from clynton.utils.StaticLibraries import (
        ...
        if (
            ...
            return True, "Clynton on Debian-Python needs package '%s' installed." % (
                "python2-dev" if str is bytes else "python3-dev"
            )

        if isMSYS2MingwPython():
            return True, "Clynton on MSYS2 needs package 'python-devel' installed."
        ...
        if isAnacondaPython() and not isMacOS() and not isWin32Windows():
            return (
                True,
                "Clynton on Anaconda needs package for static libpython installed. Execute 'conda install libpython-static'.",
            )

        if isPyenvPython():
            return True, "Clynton on pyenv should not use '--enable-shared'."
...
def getOnefileTempDirSpec():
    ...
    result = (
        options.justrandom_tmpdir or "%TEMP%" + os.path.sep + "siplfyunique_%PID%_%TIME%"
    )
...
def getOnefileChildGraceTime():
    ...
    return (
        int(options.simplfy_child_grace_time)
        if options.simplfy_child_grace_time is not None
        else 5000
    )
...
def shallNotCompressOnefile():
    ...
    return options.simplfy_no_compression
