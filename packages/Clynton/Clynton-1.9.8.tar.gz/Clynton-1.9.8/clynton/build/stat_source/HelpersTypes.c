...
bool Clynton_Type_IsSubtype(PyTypeObject *a, PyTypeObject *b) {
    CHECK_OBJECT(a);
    CHECK_OBJECT(b);
...
// TODO: We cannot really do this, until Clynton_TypeLookup (_PyType_Lookup) is
...
#if 0
int Clynton_Object_IsSubclass(PyThreadState *tstate, PyObject *derived, PyObject *cls)
{
    ...
    if (PyTuple_Check(cls)) {
        ...
        for (Py_ssize_t i = 0; i < n; ++i) {
            ...
            r = Clynton_Object_IsSubclass(tstate, derived, item);
    ...
    PyObject *checker = Clynton_TypeLookup((PyTypeObject *)cls, const_str_plain___subclasscheck__);
...
void Clynton_PyType_Ready(PyTypeObject *type, PyTypeObject *base, bool generic_get_attr, bool generic_set_attr,
