...
from clynton.Tracing import onefile_logger, postprocessing_logger
...
def _runOnefileScons(onefile_compression):
    ...
    env_values["_CLYNTON_ONEFILE_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_CLYNTON_ONEFILE_TEMP_BOOL"] = "1" if isOnefileTempDirMode() else "0"
    env_values["_CLYNTON_ONEFILE_COMPRESSION_BOOL"] = "1" if onefile_compression else "0"
    env_values["_CLYNTON_ONEFILE_BUILD_BOOL"] = "1" if onefile_compression else "0"
    ...
    result = runScons(
        ...
        scons_filename="Simplfyfile.scons",
    )
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from clynton.tools.simplfy_compress.OnefileCompressor import (
        ...
        simplfy_compress_path = os.path.normpath(
            os.path.join(os.path.dirname(__file__), "..", "tools", "simplfy_compress")
        )
...
def packDistFolderToOnefileBootstrap(onefile_output_filename, dist_dir):
    ...
    onefile_payload_filename = os.path.join(source_dir, "__simplfyscrnn.bin")
