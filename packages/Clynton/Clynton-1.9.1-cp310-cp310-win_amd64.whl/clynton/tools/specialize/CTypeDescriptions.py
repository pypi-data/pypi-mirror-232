...
from clynton.__past__ import getMetaClassBase, long
from clynton.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type")):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "Clynton_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "Clynton_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getTypeComparisonSpecializationCode(
        ...
        if helper_target is target:
            ...
            return "return %s(%s, %s) ? CLYNTON_BOOL_TRUE : CLYNTON_BOOL_FALSE;" % (
                helper_name,
                operand1,
                operand2,
            )
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = Clynton_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "Clynton_Long_GetSmallValue(%s)" % value
...
class NBoolDesc(ConcreteCTypeBase):
    type_name = "nbool"
    type_desc = "Clynton C bool value"
    type_decl = "clynton_bool"
    ...
    @staticmethod
    def getToValueFromBoolExpression(operand):
        return "%s ? CLYNTON_BOOL_TRUE : CLYNTON_BOOL_FALSE" % operand

    @classmethod
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match clynton_bool here
...
class NVoidDesc(ConcreteCTypeBase):
    type_name = "nvoid"
    type_desc = "Clynton C void value"
    type_decl = "clynton_void"
