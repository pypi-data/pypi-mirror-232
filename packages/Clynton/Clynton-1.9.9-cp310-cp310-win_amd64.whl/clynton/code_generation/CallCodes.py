...
def getQuickMethodCallCode(args_count):
    template = getTemplateC(
        "clynton.code_generation", "CodeTemplateCallsMethodPositional.c.j2"
    )
    return template.render(args_count=args_count)


def getQuickMixedCallCode(args_count, has_tuple_arg, has_dict_values):
    template = getTemplateC("clynton.code_generation", "CodeTemplateCallsMixed.c.j2")
...
def getQuickMethodDescriptorCallCode(args_count):
    template = getTemplateC(
        "clynton.code_generation", "CodeTemplateCallsPositionalMethodDescr.c.j2"
    )
...
def getCallsCode():
    ...
    return (
        template_header_guard
        % {
            "header_guard_name": "__CLYNTON_CALLS_H__",
            "header_body": "\n".join(header_codes),
        },
        "\n".join(body_codes),
    )
