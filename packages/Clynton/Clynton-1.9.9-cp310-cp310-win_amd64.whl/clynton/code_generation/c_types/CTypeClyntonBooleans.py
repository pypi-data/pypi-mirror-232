...
#
""" CType classes for clynton_bool, an enum to represent True, False, unassigned.

"""

from clynton.code_generation.ErrorCodes import getReleaseCode

from .CTypeBases import CTypeBase, CTypeNotReferenceCountedMixin


class CTypeClyntonBoolEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "clynton_bool"
    ...
    def emitVariableAssignCode(
        ...
        if tmp_name.c_type == "clynton_bool":
    ...
    @classmethod
    def emitAssignmentCodeToClyntonIntOrLong(
    ...
    def getTruthCheckCode(cls, value_name):
        return "%s == CLYNTON_BOOL_TRUE" % value_name
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s != CLYNTON_BOOL_UNASSIGNED);" % value_name)
    ...
    def emitAssignConversionCode(cls, to_name, value_name, needs_check, emit, context):
        if value_name.c_type == cls.c_type:
            ...
            value_name.getCType().emitAssignmentCodeToClyntonBool(
                to_name=to_name,
    ...
    def emitAssignmentCodeFromConstant(
        ...
        emit(
            "%s = %s;"
            % (to_name, "CLYNTON_BOOL_TRUE" if constant else "CLYNTON_BOOL_FALSE")
        )

    @classmethod
    def getInitValue(cls, init_from):
        if init_from is None:
            return "CLYNTON_BOOL_UNASSIGNED"
        else:
    ...
    def getInitTestConditionCode(cls, value_name, inverted):
        return "%s %s CLYNTON_BOOL_UNASSIGNED" % (value_name, "==" if inverted else "!=")

    @classmethod
    def emitReinitCode(cls, value_name, emit):
        emit("%s = CLYNTON_BOOL_UNASSIGNED;" % value_name)
    ...
    def getDeleteObjectCode(
        ...
        if not needs_check:
            ...
            emit("%s = %s != CLYNTON_BOOL_UNASSIGNED;" % (to_name, value_name))
            emit("%s = CLYNTON_BOOL_UNASSIGNED;" % value_name)

    @classmethod
    def emitAssignmentCodeFromBoolCondition(cls, to_name, condition, emit):
        emit(
            "%(to_name)s = (%(condition)s) ? CLYNTON_BOOL_TRUE : CLYNTON_BOOL_FALSE;"
    ...
    def emitAssignInplaceNegatedValueCode(cls, to_name, needs_check, emit, context):
        ...
        emit("assert(%s != CLYNTON_BOOL_EXCEPTION);" % to_name)

        cls.emitAssignmentCodeFromBoolCondition(
            to_name=to_name, condition="%s == CLYNTON_BOOL_FALSE" % to_name, emit=emit
        )

    @classmethod
    def getExceptionCheckCondition(cls, value_name):
        return "%s == CLYNTON_BOOL_EXCEPTION" % value_name
