...
//
/**
 * This is responsible for updating parts of CPython to better work with Clynton
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (Clynton_Generator_Check(object)) {
        struct Clynton_GeneratorObject *generator = (struct Clynton_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (Clynton_Coroutine_Check(object)) {
        struct Clynton_CoroutineObject *coroutine = (struct Clynton_CoroutineObject *)object;
...
static PyObject *_types_coroutine_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (Clynton_Function_Check(func)) {
...
#if PYTHON_VERSION >= 0x300
#if defined(_CLYNTON_EXE) && !defined(_CLYNTON_STANDALONE)
...
";
    ...
    {
        PyObject *module = PyImport_ExecCodeModule("clynton_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        bool bool_res = Clynton_DelModuleString(tstate, "clynton_types_patch");
...
static PyObject *Clynton_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&Clynton_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&Clynton_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&Clynton_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&Clynton_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&Clynton_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&Clynton_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&Clynton_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&Clynton_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&Clynton_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&Clynton_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = Clynton_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct Clynton_FrameObject *frame, int lineno) {
#if 0
...
static void Clynton_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    Clynton_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, Clynton_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)Clynton_tb_dealloc; }
