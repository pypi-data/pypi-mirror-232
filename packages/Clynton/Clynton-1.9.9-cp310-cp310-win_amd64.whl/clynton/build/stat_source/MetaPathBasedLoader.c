...
extern PyTypeObject Clynton_Loader_Type;

struct Clynton_LoaderObject {
...
#elif _CLYNTON_SYSFLAG_VERBOSE
static inline bool isVerbose(void) { return true; }
#else
static inline bool isVerbose(void) { return false; }
#endif

static struct Clynton_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
#if defined(_WIN32) && defined(_CLYNTON_STANDALONE)
...
CLYNTON_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = Clynton_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyObject *module, PyCodeObject *code_object, char const *name,
    ...
    bool b_res = Clynton_SetModuleString(name, module);
    assert(b_res != false);
    ...
    PyObject *module_path_name = Clynton_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, Clynton_String_AsString(module_path));
...
static struct Clynton_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct Clynton_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);

    while (current->name != NULL) {
        if ((current->flags & CLYNTON_TRANSLATED_FLAG) != 0) {
            current->name = UNTRANSLATE(current->name);
            current->flags -= CLYNTON_TRANSLATED_FLAG;
        }
...
#ifndef _CLYNTON_STANDALONE
static struct Clynton_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct Clynton_MetaPathBasedLoaderEntry *current = loader_entries;
    ...
    while (current->name != NULL) {
        ...
        if ((current->flags & CLYNTON_PACKAGE_FLAG) != 0) {
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = Clynton_GetSysModules();
    ...
    char const *module_relname_str = Clynton_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (Clynton_String_CheckExact(filename)) {
                char const *filename_str = Clynton_String_AsString(filename);
    ...
    PRINT_STRING(Clynton_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = Clynton_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = Clynton_String_AsString(candidate);
...
#endif
#else
    char const *extension_module_filename_str = Clynton_String_AsString(extension_module_filename);
#endif

    return callIntoExtensionModule(tstate, Clynton_String_AsString(module_name), extension_module_filename_str);
}

#endif

static char const *getEntryModeString(struct Clynton_MetaPathBasedLoaderEntry const *entry) {
    char const *mode = "compiled";

    if ((entry->flags & CLYNTON_EXTENSION_MODULE_FLAG) != 0) {
        mode = "extension";
    } else if ((entry->flags & CLYNTON_BYTECODE_FLAG) != 0) {
        mode = "bytecode";
    }
...
static PyObject *_clynton_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = Clynton_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_module)\n", name);
    }

    struct Clynton_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&Clynton_Loader_Type;
...
static PyObject *_clynton_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, char const *filename) {
    ...
    if (needs_update) {
#ifdef _WIN32
        int res = SET_ATTRIBUTE(tstate, module, const_str_plain___file__, ClyntonUnicode_FromWideChar(filename, -1));
#else
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const char *filename) {
...
#ifndef _CLYNTON_EXPERIMENTAL_DEBUG_STANDALONE
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, Clynton_SysGetObject("getdlopenflags"));
    }
    ...
    Clynton_DelModuleString(tstate, full_name);
...
#if PYTHON_VERSION < 0x300
    PyObject *module = Clynton_GetModuleString(tstate, full_name);
#endif
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        ...
        PyObject *full_name_obj = Clynton_String_FromString(full_name);
        ...
        Clynton_SetModule(full_name_obj, module);
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = Clynton_String_FromString(package2);
...
#ifdef _WIN32
    PyObject *filename_obj = ClyntonUnicode_FromWideChar(filename, -1);
#else
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct Clynton_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & CLYNTON_ABORT_MODULE_FLAG) != 0) {
                printf("Critical error loading %s.\n", trigger_module_name);
                abort();
            } else {
                PyObject *trigger_module_name_str = Clynton_String_FromString(trigger_module_name);
...
static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct Clynton_MetaPathBasedLoaderEntry const *entry) {
#ifdef _CLYNTON_STANDALONE
    if ((entry->flags & CLYNTON_EXTENSION_MODULE_FLAG) != 0) {
...
#endif
        if ((entry->flags & CLYNTON_BYTECODE_FLAG) != 0) {
        ...
        return loadModuleFromCodeObject(module, code_object, entry->name, (entry->flags & CLYNTON_PACKAGE_FLAG) != 0);
    } else {
        assert((entry->flags & CLYNTON_EXTENSION_MODULE_FLAG) == 0);
        assert(entry->python_initfunc);

        bool res = Clynton_SetModule(module_name, module);
        assert(res != false);
    ...
    return Clynton_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = Clynton_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = Clynton_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = Clynton_GetModule(tstate, module_name);
...
#if PYTHON_VERSION < 0x350
    if (unlikely(result == NULL)) {
        Clynton_DelModule(tstate, module_name);
    }
#endif
...
static PyObject *_clynton_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(Clynton_String_Check(module_name));
...
static PyObject *_clynton_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        result = BOOL_FROM((entry->flags & CLYNTON_PACKAGE_FLAG) != 0);
    } else {
...
static PyObject *_clynton_loader_iter_modules(struct Clynton_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = Clynton_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = Clynton_String_FromString(current->name);
        }
        ...
        PyTuple_SET_ITEM0(r, 1, BOOL_FROM((current->flags & CLYNTON_PACKAGE_FLAG) != 0));
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct Clynton_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *dir_name = ClyntonUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = Clynton_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & CLYNTON_PACKAGE_FLAG) != 0) {
        module_name = Clynton_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = Clynton_String_FromString(buffer);
    }
...
static PyObject *_clynton_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                Clynton_GetFilenameObject(tstate, module));
}
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct Clynton_MetaPathBasedLoaderEntry const *entry) {
...
#endif
    } else if ((entry->flags & CLYNTON_PACKAGE_FLAG) != 0) {
    ...
    PyObject *module_filename = Clynton_String_FromString(filename_buffer);
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(module_name, (PyObject *)&Clynton_Loader_Type);
...
static PyObject *_clynton_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = Clynton_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_spec)\n", full_name);
    }

    struct Clynton_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("import %s # claimed responsibility (%s)\n", Clynton_String_AsString(module_name),
                          getEntryModeString(entry));
    }

    return createModuleSpec(tstate, module_name, getModuleFileValue(tstate, entry),
                            (entry->flags & CLYNTON_PACKAGE_FLAG) != 0);
}

#if PYTHON_VERSION >= 0x350
static char const *_kw_list_create_module[] = {"spec", NULL};

static PyObject *_clynton_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
static PyObject *_clynton_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_clynton_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return Clynton_ResourceReader_New(entry);
    }
...
static PyObject *_clynton_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (Clynton_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            clynton_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);

            if (unlikely(cmp_res == CLYNTON_BOOL_EXCEPTION)) {
                Py_DECREF(name);
                return NULL;
            }

            include = cmp_res == CLYNTON_BOOL_TRUE;
        }

        if (include) {
            // Create a distribution object from our data.
            PyObject *distribution = Clynton_Distribution_New(tstate, distribution_name);
...
static PyObject *_clynton_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct Clynton_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);

    PyThreadState *tstate = PyThreadState_GET();

    while (entry->name != NULL) {
        if ((entry->flags & CLYNTON_TRANSLATED_FLAG) != 0) {
            entry->name = UNTRANSLATE(entry->name);
            entry->flags -= CLYNTON_TRANSLATED_FLAG;
        }
        ...
        if ((entry->flags & CLYNTON_PACKAGE_FLAG) != 0) {
            ...
            clynton_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == CLYNTON_BOOL_TRUE) {
                return Clynton_Loader_New(entry);
            }
        }
...
static PyMethodDef Clynton_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_clynton_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_clynton_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_clynton_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_clynton_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_clynton_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x340
    {"module_repr", (PyCFunction)_clynton_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_clynton_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_clynton_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_clynton_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_clynton_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_clynton_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_clynton_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL}
};

static PyObject *Clynton_Loader_tp_repr(struct Clynton_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return Clynton_String_FromString("<clynton_module_loader>");
    } else {
        return Clynton_String_FromFormat("<clynton_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct Clynton_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void Clynton_Loader_tp_dealloc(struct Clynton_LoaderObject *loader) {
    Clynton_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int Clynton_Loader_tp_traverse(struct Clynton_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *Clynton_Loader_get__module__(struct Clynton_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___clynton__;

    Py_INCREF(result);
    return result;
}

static PyGetSetDef Clynton_Loader_getsets[] = {{(char *)"__module__", (getter)Clynton_Loader_get__module__, NULL, NULL},
                                              {NULL}};

PyTypeObject Clynton_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "clynton_module_loader",
    sizeof(struct Clynton_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)Clynton_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)Clynton_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)Clynton_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    Clynton_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    Clynton_Loader_getsets,                   // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *Clynton_Loader_New(struct Clynton_MetaPathBasedLoaderEntry const *entry) {
    struct Clynton_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct Clynton_LoaderObject, Clynton_Loader_Type);
...
void registerMetaPathBasedUnfreezer(struct Clynton_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    if (isVerbose()) {
        PySys_WriteStderr("Setup clynton compiled module/bytecode/extension importer.\n");
    }

#ifdef _CLYNTON_MODULE
    if (_Py_PackageContext != NULL) {
        char const *last_dot = strrchr(_Py_PackageContext, '.');

        if (last_dot != NULL) {
            struct Clynton_MetaPathBasedLoaderEntry *current = _loader_entries;
            assert(current);
    ...
    Clynton_PyType_Ready(&Clynton_Loader_Type, NULL, true, false, false, false, false);

#ifdef _CLYNTON_EXE
    int res = PyDict_SetItemString((PyObject *)dict_builtin, "__clynton_loader_type", (PyObject *)&Clynton_Loader_Type);
    assert(res == 0);
#endif

#if PYTHON_VERSION >= 0x370
    Clynton_PyType_Ready(&Clynton_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = Clynton_Loader_New(NULL);

    LIST_INSERT_CONST(Clynton_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
    ...
    LIST_APPEND1(Clynton_SysGetObject("path_hooks"), PyObject_GetAttrString(global_loader, "sys_path_hook"));
}

#if defined(_CLYNTON_STANDALONE)
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = Clynton_SysGetObject("builtin_module_names");

    while (Clynton_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(Clynton_String_AsString(key), is_package, false);
