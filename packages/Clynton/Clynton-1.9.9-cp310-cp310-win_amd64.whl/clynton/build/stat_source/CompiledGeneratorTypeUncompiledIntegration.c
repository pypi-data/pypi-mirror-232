...
static inline void Clynton_PyGen_exc_state_clear(_PyErr_StackItem *exc_state) {
...
#if PYTHON_VERSION < 0x3b0
static inline bool Clynton_PyFrameHasCompleted(PyFrameObject *const frame) {
...
static PyObject *Clynton_PyGen_Send(PyThreadState *tstate, PyGenObject *gen, PyObject *arg) {
#if defined(PY_NOGIL)
    PyObject *res;
    ...
    if (likely(gen->return_value == Py_None)) {
        ...
        Clynton_SetStopIterationValue(tstate, gen->return_value);
        return NULL;
    } else {
...
#if PYTHON_VERSION < 0x3b0
    if (f == NULL || Clynton_PyFrameHasCompleted(f)) {
#else
...
#else
        Clynton_PyGen_exc_state_clear(&gen->gi_exc_state);
#endif
...
#define CLYNTON_UNCOMPILED_THROW_INTEGRATION 1
#endif

// TODO: Disabled for Python3.12 until release of it
#if PYTHON_VERSION >= 0x3c0
#undef CLYNTON_UNCOMPILED_THROW_INTEGRATION
#define CLYNTON_UNCOMPILED_THROW_INTEGRATION 0
#endif
...
static PyObject *Clynton_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args);
static int Clynton_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf);
...
const uint8_t Clynton_PyOpcode_Deopt[256] = {
#if PYTHON_VERSION >= 0x3c0
...
PyObject *Clynton_PyGen_yf(PyGenObject *gen) {
    PyObject *yf = NULL;
    ...
    if (gen->gi_frame_state < FRAME_CLEARED) {
        ...
        if (Clynton_PyOpcode_Deopt[_Py_OPCODE(next)] != RESUME || _Py_OPARG(next) < 2) {
            return NULL;
        }
...
static PyFrameObject *_Clynton_PyFrame_New_NoTrack(PyCodeObject *code) {
...
// Also not exported.
static PyFrameObject *_Clynton_PyFrame_MakeAndSetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    PyFrameObject *f = _Clynton_PyFrame_New_NoTrack(frame->f_code);
...
static inline PyFrameObject *_Clynton_PyFrame_GetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    return _Clynton_PyFrame_MakeAndSetFrameObject(tstate, frame);
}

// Also not exported, taking over a frame object.

static void _Clynton_take_ownership(PyThreadState *tstate, PyFrameObject *f, _PyInterpreterFrame *frame) {
    assert(frame->owner != FRAME_OWNED_BY_FRAME_OBJECT);
    assert(frame->owner != FRAME_CLEARED);
    ...
    if (prev) {
        ...
        // TODO: Clynton doesn't do that for compiled frames ever, is that really needed?
        PyFrameObject *back = _Clynton_PyFrame_GetFrameObject(tstate, prev);
    ...
    if (!_PyObject_GC_IS_TRACKED((PyObject *)f)) {
        Clynton_GC_Track((PyObject *)f);
    }
}

// Cleanup up the frame is also not exported.
static void _Clynton_PyFrame_Clear(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    if (frame->frame_obj) {
        ...
        if (Py_REFCNT(f) > 1) {
            _Clynton_take_ownership(tstate, f, frame);
            Py_DECREF(f);
...
static PySendResult Clynton_PyGen_gen_send_ex2(PyThreadState *tstate, PyGenObject *gen, PyObject *arg,
    ...
    *presult = NULL;
    ...
    _Clynton_PyFrame_Clear(tstate, frame);

    *presult = result;
    return result ? PYGEN_RETURN : PYGEN_ERROR;
}

static PyObject *Clynton_PyGen_gen_send_ex(PyThreadState *tstate, PyGenObject *gen, PyObject *arg, int exc,
                                          int closing) {
    PyObject *result;

    if (Clynton_PyGen_gen_send_ex2(tstate, gen, arg, &result, exc, closing) == PYGEN_RETURN) {
...
//   be normalized.
static PyObject *Clynton_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
                                                  PyObject *exception_type, PyObject *exception_value,
                                                  PyTracebackObject *exception_tb) {
#if _DEBUG_GENERATOR
    PRINT_STRING("Clynton_UncompiledGenerator_throw: Enter ");
    PRINT_ITEM((PyObject *)gen);
    PRINT_EXCEPTION(exception_type, exception_value, exception_tb);
    PRINT_NEW_LINE();
#endif

    PyObject *yf = Clynton_PyGen_yf(gen);
    ...
    if (yf != NULL) {
        ...
        if (close_on_genexit && EXCEPTION_MATCH_BOOL_SINGLE(tstate, exception_type, PyExc_GeneratorExit)) {
            ...
            int err = Clynton_PyGen_gen_close_iter(tstate, yf);
            ...
            if (err < 0) {
                ...
                return Clynton_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            // Handing exception ownership to "Clynton_UncompiledGenerator_throw".
            ret = Clynton_UncompiledGenerator_throw(tstate, (PyGenObject *)yf, close_on_genexit, exception_type,
        ...
        if (ret == NULL) {
            ...
            if (_PyGen_FetchStopIterationValue(&val) == 0) {
                ret = Clynton_PyGen_gen_send_ex(tstate, gen, val, 0, 0);
                Py_DECREF(val);
            } else {
                ret = Clynton_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        }
...
throw_here:
    ...
    return Clynton_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
...
static PyObject *Clynton_PyGen_yf(PyGenObject *gen) {
    PyFrameObject *f = gen->gi_frame;
    ...
    if (yf != NULL) {
        ...
        if (ret == NULL) {
            ...
            if (_PyGen_FetchStopIterationValue(&exception_value) == 0) {
                ret = Clynton_PyGen_gen_send_ex(tstate, gen, exception_value, 0, 0);
...
throw_here:
    ...
    RESTORE_ERROR_OCCURRED(tstate, exception_type, exception_value, (PyTracebackObject *)exception_tb);
...
static int Clynton_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf) {
    ...
    if (PyGen_CheckExact(yf)
        ...
        assert(false);
        retval = Clynton_PyGen_gen_close(tstate, (PyGenObject *)yf, NULL);
...
static PyObject *Clynton_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args) {
    ...
    if (yf != NULL) {
        ...
        gen->gi_running = 1;
#endif
        err = Clynton_PyGen_gen_close_iter(tstate, yf);
    ...
    PyObject *retval = Clynton_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
