...
logging_info_re = re.compile(r"^Clynton.*?:INFO")
logging_warning_re = re.compile(r"^Clynton.*?:WARNING")

# Python3.11 style traceback carets are not done by Clynton (yet?)
...
def makeDiffable(output, ignore_warnings, syntax_errors):
    ...
    for line in lines:
        ...
        if line.startswith("Clynton-Inclusion:WARNING: Cannot follow import to module"):
            continue
        if line.startswith("Clynton:WARNING: Cannot detect Linux distribution"):
            continue
        if line.startswith(
            "Clynton-Options:WARNING: You did not specify to follow or include"
        ):
            continue
        if line.startswith("Clynton:WARNING: Using very slow fallback for ordered sets"):
            continue
        if line.startswith("Clynton:WARNING: On Windows, support for input/output"):
            continue
        if line.startswith("Clynton:WARNING:     Complex topic"):
            continue
        ...
        # This URL is updated, and Clynton outputs the new one, but we test
        ...
        # we are going to ignore the issue as Clynton is fine.
        if (
            line
            == """\
...
def compareOutput(
    kind, out_cpython, out_clynton, ignore_warnings, syntax_errors, trace_result=True
):
    from_date = ""
    to_date = ""
    ...
    diff = difflib.unified_diff(
        ...
        makeDiffable(out_clynton, ignore_warnings, syntax_errors),
        "{program} ({detail})".format(program=os.environ["PYTHON"], detail=kind),
        "{program} ({detail})".format(program="clynton", detail=kind),
        from_date,
        to_date,
        n=3,
    )
