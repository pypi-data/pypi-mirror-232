...
import clynton.specs.BuiltinUnicodeOperationSpecs
from clynton.code_generation.BinaryOperationHelperDefinitions import (
    getSpecializedBinaryOperations,
    parseTypesFromHelper,
)
from clynton.code_generation.c_types.CTypePyObjectPointers import (
    make_list_constant_direct_threshold,
    make_list_constant_hinted_threshold,
)
from clynton.code_generation.CallCodes import (
    getQuickCallCode,
...
from clynton.code_generation.ComparisonHelperDefinitions import (
    getSpecializedComparisonOperations,
)
from clynton.code_generation.ImportCodes import getImportModuleHardCodeName
from clynton.nodes.ImportNodes import (
    hard_modules,
...
from clynton.utils.Jinja2 import getTemplateC
...
def getDoExtensionUsingTemplateC(template_name):
    return getTemplateC(
        package_name="clynton.code_generation",
...
def makeHelpersComparisonOperation(operand, op_code):
    ...
    filename_c = "clynton/build/stat_source/HelpersComparison%s.c" % op_code.capitalize()
    filename_h = "clynton/build/include/clynton/helper/comparisons_%s.h" % op_code.lower()
...
def makeHelpersBinaryOperation(operand, op_code):
    ...
    filename_c = (
        "clynton/build/stat_source/HelpersOperationBinary%s.c" % op_code.capitalize()
    )
    filename_h = (
        "clynton/build/include/clynton/helper/operations_binary_%s.h" % op_code.lower()
    )
...
def makeHelpersInplaceOperation(operand, op_code):
    ...
    filename_c = (
        "clynton/build/stat_source/HelpersOperationInplace%s.c" % op_code.capitalize()
    )
    filename_h = (
        "clynton/build/include/clynton/helper/operations_inplace_%s.h" % op_code.lower()
    )
...
def makeHelpersImportHard():
    filename_c = "clynton/build/stat_source/HelpersImportHard.c"
    filename_h = "clynton/build/include/clynton/helper/import_hard.h"
...
def makeHelperCalls():
    filename_c = "clynton/build/stat_source/HelpersCallingGenerated.c"
    filename_h = "clynton/build/include/clynton/helper/calling_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "clynton.code_generation", "CodeTemplateCallsPositional.c.j2"
            )
            ...
            template = getTemplateC(
                "clynton.code_generation", "CodeTemplateCallsMixed.c.j2"
            )
...
def makeHelperLists():
    filename_c = "clynton/build/stat_source/HelpersListsGenerated.c"
    filename_h = "clynton/build/include/clynton/helper/lists_generated.h"
    ...
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
            ...
            template = getTemplateC(
                "clynton.code_generation", "CodeTemplateMakeListSmall.c.j2"
            )
            ...
            template = getTemplateC(
                "clynton.code_generation", "CodeTemplateMakeListHinted.c.j2"
            )
...
generate_builtin_type_operations = [
    ...
    # that we do not have easy access to, but we might one day for Clynton-Python
    ...
    (
        "tshape_unicode",
        unicode_desc,
        clynton.specs.BuiltinUnicodeOperationSpecs,
        (
            "strip",
            "rstrip",
            "lstrip",
...
def makeDictCopyHelperCodes():
    filename_c = "clynton/build/stat_source/HelpersDictionariesGenerated.c"
...
def _getCheckForShape(shape):
    ...
    if shape is tshape_str:
        return "Clynton_String_CheckExact"
...
def makeHelperBuiltinTypeMethods():
    ...
    filename_c = "clynton/build/stat_source/HelpersBuiltinTypeMethods.c"
    filename_h = "clynton/build/include/clynton/helper/operations_builtin_types.h"
    with withFileOpenedAndAutoFormatted(filename_c) as output_c:
        with withFileOpenedAndAutoFormatted(filename_h) as output_h:
