...
from clynton.__past__ import subprocess
from clynton.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from clynton.Tracing import OurLogger, my_print
from clynton.tree.SourceHandling import readSourceCodeFromFilename
from clynton.utils.AppDirs import getCacheDir
...
from clynton.utils.Utils import getOS, isMacOS, isWin32Windows
...
def setup(suite="", needs_io_encoding=False, silent=False, go_main=True):
    ...
    os.environ["CLYNTON_TEST_SUITE"] = suite
...
def compareWithCPython(
    ...
    if os.getenv("CLYNTON_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "clynton.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate Clynton comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in Clynton with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in Clynton, good for test coverage.
Defaults to off.""",
    )
...
Defaults to off.""",
    ...
    if options.show_commands:
        os.environ["CLYNTON_TRACE_COMMANDS"] = "1"
...
def addExtendedExtraOptions(*args):
    old_value = os.environ.get("CLYNTON_EXTRA_OPTIONS")
    ...
    os.environ["CLYNTON_EXTRA_OPTIONS"] = value
...
def withExtendedExtraOptions(*args):
    ...
    if old_value is None:
        del os.environ["CLYNTON_EXTRA_OPTIONS"]
    else:
        os.environ["CLYNTON_EXTRA_OPTIONS"] = old_value
...
def getTestingCPythonOutputsCacheDir():
    ...
    result = os.path.join(
        cache_dir, "cpython_outputs", os.environ.get("CLYNTON_TEST_SUITE", "")
    )
...
def setupCacheHashSalt(test_code_path):
    ...
    os.environ["CLYNTON_HASH_SALT"] = salt_value.hexdigest()
...
#     "# clynton-skip-unless-expression: expression to be evaluated"
#       OR
#     "# clynton-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# clynton-skip-unless-"):
...
def extractClyntonVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("Clynton V")]
