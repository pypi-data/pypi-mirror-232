...
CLYNTON_DEFINE_BUILTIN(compile)
...
#endif
{
    ...
    CLYNTON_ASSIGN_BUILTIN(compile);

    PyObject *result = CALL_FUNCTION(tstate, CLYNTON_ACCESS_BUILTIN(compile), pos_args, kw_args);
...
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, CLYNTON_ACCESS_BUILTIN(open), args, arg_names, 3);
}
#else
PyObject *BUILTIN_OPEN(PyThreadState *tstate, PyObject *file_name, PyObject *mode, PyObject *buffering,
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, CLYNTON_ACCESS_BUILTIN(open), args, arg_names, 8);
}

#endif

CLYNTON_DEFINE_BUILTIN(input);

PyObject *BUILTIN_INPUT(PyThreadState *tstate, PyObject *prompt) {
    CLYNTON_ASSIGN_BUILTIN(input);

#if CLYNTON_STDERR_NOT_VISIBLE && (PYTHON_VERSION >= 0x300 || !defined(_WIN32))
    if (prompt != NULL) {
        PRINT_ITEM(prompt);
        prompt = NULL;
    }
    ...
    if (prompt == NULL) {
        return CALL_FUNCTION_NO_ARGS(tstate, CLYNTON_ACCESS_BUILTIN(input));
    } else {
        return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, CLYNTON_ACCESS_BUILTIN(input), prompt);
    }
}
...
CLYNTON_DEFINE_BUILTIN(staticmethod)

PyObject *BUILTIN_STATICMETHOD(PyThreadState *tstate, PyObject *value) {
    CLYNTON_ASSIGN_BUILTIN(staticmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, CLYNTON_ACCESS_BUILTIN(staticmethod), value);
}
...
CLYNTON_DEFINE_BUILTIN(classmethod)

PyObject *BUILTIN_CLASSMETHOD(PyThreadState *tstate, PyObject *value) {
    CLYNTON_ASSIGN_BUILTIN(classmethod);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, CLYNTON_ACCESS_BUILTIN(classmethod), value);
}
...
CLYNTON_DEFINE_BUILTIN(bytes);

PyObject *BUILTIN_BYTES1(PyThreadState *tstate, PyObject *value) {
    CLYNTON_ASSIGN_BUILTIN(bytes);

    return CALL_FUNCTION_WITH_SINGLE_ARG(tstate, CLYNTON_ACCESS_BUILTIN(bytes), value);
}
...
PyObject *BUILTIN_BYTES3(PyThreadState *tstate, PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    return CALL_BUILTIN_KW_ARGS(tstate, CLYNTON_ACCESS_BUILTIN(bytes), args, arg_names, 3);
}
#endif
...
static long Clynton_HashFromPointer(void *p) {
    size_t y = (size_t)p;
...
PyObject *BUILTIN_HASH(PyThreadState *tstate, PyObject *value) {
    ...
    if (likely(type->tp_compare == NULL && RICHCOMPARE(type) == NULL)) {
        Py_hash_t hash = Clynton_HashFromPointer(value);
        return PyInt_FromLong(hash);
    }
#endif
...
#if PYTHON_VERSION < 0x300
    if (likely(type->tp_compare == NULL && RICHCOMPARE(type) == NULL)) {
        return Clynton_HashFromPointer(value);
    }
#endif
...
CLYNTON_DEFINE_BUILTIN(bytearray)

PyObject *BUILTIN_BYTEARRAY3(PyThreadState *tstate, PyObject *string, PyObject *encoding, PyObject *errors) {
    CHECK_OBJECT(string);
    CHECK_OBJECT(encoding);

    CLYNTON_ASSIGN_BUILTIN(bytearray);

    if (errors == NULL) {
        PyObject *args[] = {string, encoding};

        PyObject *result = CALL_FUNCTION_WITH_ARGS2(tstate, CLYNTON_ACCESS_BUILTIN(bytearray), args);

        return result;
    } else {
        PyObject *args[] = {string, encoding, errors};

        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, CLYNTON_ACCESS_BUILTIN(bytearray), args);
...
PyObject *BUILTIN_ITER2(PyObject *callable, PyObject *sentinel) {
    calliterobject *result = (calliterobject *)Clynton_GC_New(&PyCallIter_Type);
...
PyObject *BUILTIN_TYPE3(PyThreadState *tstate, PyObject *module_name, PyObject *name, PyObject *bases, PyObject *dict) {
    ...
    if (likely(Clynton_Type_IsSubtype(type, &PyType_Type))) {
        if (ClyntonType_HasFeatureClass(type) && type->tp_init != NULL) {
...
CLYNTON_DEFINE_BUILTIN(super);

PyObject *BUILTIN_SUPER2(PyThreadState *tstate, PyDictObject *module_dict, PyObject *type, PyObject *object) {
    CHECK_OBJECT(type);
    CHECK_OBJECT_X(object);

    PyObject *super_value = GET_STRING_DICT_VALUE(module_dict, (Clynton_StringObject *)const_str_plain_super);

    if (super_value == NULL) {
        CLYNTON_ASSIGN_BUILTIN(super);

        super_value = CLYNTON_ACCESS_BUILTIN(super);
    }
...
#if PYTHON_VERSION < 0x300
    if (unlikely(!Clynton_String_Check(value) && !PyUnicode_Check(value))) {
        SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_TypeError, "int() can't convert non-string with explicit base");
        return NULL;
    }

    char *value_str = Clynton_String_AsString(value);
