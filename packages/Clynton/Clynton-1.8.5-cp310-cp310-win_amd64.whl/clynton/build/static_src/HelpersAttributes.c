...
PyObject *LOOKUP_ATTRIBUTE(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
...
#if _CLYNTON_EXPERIMENTAL_DISABLE_ATTR_OPT
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = Clynton_TypeLookup(type, attr_name);
        descrgetfunc func = NULL;

        if (descr != NULL) {
            Py_INCREF(descr);

            if (ClyntonType_HasFeatureClass(Py_TYPE(descr))) {
...
// TODO: This is what Python 3.11 requires us to add.
...
#if PYTHON_VERSION < 0x300
    else if (type->tp_getattro == PyInstance_Type.tp_getattro && Clynton_String_CheckExact(attr_name)) {
        ...
        PyObject *result = (*type->tp_getattr)(source, (char *)Clynton_String_AsString_Unchecked(attr_name));
        return result;
    } else {
        PyErr_Format(PyExc_AttributeError, "'%s' object has no attribute '%s'", type->tp_name,
                     Clynton_String_AsString_Unchecked(attr_name));
...
PyObject *LOOKUP_ATTRIBUTE_DICT_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = Clynton_TypeLookup(type, const_str_plain___dict__);
        descrgetfunc func = NULL;
...
PyObject *LOOKUP_ATTRIBUTE_CLASS_SLOT(PyThreadState *tstate, PyObject *source) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        PyObject *descr = Clynton_TypeLookup(type, const_str_plain___class__);
        descrgetfunc func = NULL;
...
int HAS_ATTR_BOOL2(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    if (hasTypeGenericGetAttr(type)) {
        ...
        if (descr != NULL) {
            // Hold a refcount, Clynton_TypeLookup does not give any.
            Py_INCREF(descr);
...
#if (PYTHON_VERSION < 0x300 || defined(_CLYNTON_USE_UNEXPOSED_API)) && !defined(_CLYNTON_EXPERIMENTAL_DISABLE_ATTR_OPT)
...
bool SET_ATTRIBUTE(PyThreadState *tstate, PyObject *target, PyObject *attr_name, PyObject *value) {
...
#if PYTHON_VERSION < 0x300 || defined(_CLYNTON_USE_UNEXPOSED_API)
    ...
    if (type->tp_setattr != NULL) {
        int status = (*type->tp_setattr)(target, (char *)Clynton_String_AsString_Unchecked(attr_name), value);
...
PyObject *LOOKUP_SPECIAL(PyThreadState *tstate, PyObject *source, PyObject *attr_name) {
    ...
    PyObject *result = Clynton_TypeLookup(Py_TYPE(source), attr_name);
...
PyObject *LOOKUP_MODULE_VALUE(PyDictObject *module_dict, PyObject *var_name) {
    PyObject *result = GET_STRING_DICT_VALUE(module_dict, (Clynton_StringObject *)var_name);

    if (unlikely(result == NULL)) {
        result = GET_STRING_DICT_VALUE(dict_builtin, (Clynton_StringObject *)var_name);
    }
...
PyObject *GET_MODULE_VARIABLE_VALUE_FALLBACK(PyThreadState *tstate, PyObject *variable_name) {
    PyObject *result = GET_STRING_DICT_VALUE(dict_builtin, (Clynton_StringObject *)variable_name);
