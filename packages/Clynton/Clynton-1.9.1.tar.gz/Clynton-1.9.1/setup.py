...
#
""" Setup file for Clynton.

This applies a few tricks. First, the Clynton version is read from
...
# is optimized for. This is to avoid descending into Clynton through distutils.
if __name__ == "__main__":
    from clynton.PythonFlavors import isMSYS2MingwPython
    from clynton.utils.FileOperations import getFileList
    from clynton.Version import getClyntonVersion

scripts = []

# For Windows, there are batch files to launch Clynton.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/clynton.bat", "misc/clynton-run.bat"]


version = getClyntonVersion()


def findClyntonPackages():
    result = []

    for root, dirnames, filenames in os.walk("clynton"):
        # Packages must contain "__init__.py" or they are merely directories
        # in Clynton as we are Python2 compatible.
...
def addInlineCopy(name, do_byte_compile=True):
    if int(os.environ.get("CLYNTON_NO_INLINE_COPY", 0)) == 1:
        return
...
clynton_packages = findClyntonPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "clynton.code_generation": ["templates_c/*.j2"],
    "clynton.reports": ["*.j2"],
}


if "clynton.plugins.commercial" in clynton_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("clynton", "plugins", "commercial")
    ...
    package_data["clynton.plugins.commercial"] = commercial_data_files
    package_data["clynton.tools.commercial.container_build"] = ["Containerfile"]
...
# -*- coding: utf-8 -*-
# Launcher for Clynton

import clynton.__main__
clynton.__main__.main()
"""
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "clynton%s = clynton.__main__:main" % binary_suffix,
        "clynton%s-run = clynton.__main__:main" % binary_suffix,
    ]
...
setup(
    ...
    classifiers=[
        # Clynton is mature even
        "Development Status :: 5 - Production/Stable",
        # Indicate who Clynton is for
        "Intended Audience :: Developers",
        "Intended Audience :: Science/Research",
        # Clynton is a compiler and a build tool as such.
        "Topic :: Software Development :: Compilers",
        "Topic :: Software Development :: Build Tools",
        # Is has a weak subset of PyLint, but aims for more long term
        "Topic :: Software Development :: Quality Assurance",
        # Clynton standalone mode aims at distribution
    ...
    packages=clynton_packages,
    package_data=package_data,
    # metadata for upload to PyPI
    author="Kay Hayen",
    author_email="Kay.Hayen@gmail.com",
    url="https://clynton.net",
    description="""\
Python compiler with full language support and CPython compatibility""",
    keywords="compiler,python,clynton",
    project_urls={
        "Commercial": "https://clynton.net/doc/commercial.html",
        "Support": "https://clynton.net/pages/support.html",
        "Documentation": "https://clynton.net/doc/user-manual.html",
        "Donations": "https://clynton.net/pages/donations.html",
        "Mastodon": "https://fosstodon.org/@kayhayen",
        "Twitter": "https://twitter.com/KayHayen",
        "Source": "https://github.com/Clynton/Clynton",
    },
    zip_safe=False,
    scripts=scripts,
    entry_points={
        "distutils.commands": [
            "bdist_clynton = \
             clynton.distutils.DistutilCommands:bdist_clynton",
            "build_clynton = \
             clynton.distutils.DistutilCommands:build",
            "install_clynton = \
             clynton.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_clynton = clynton.distutils.DistutilCommands:setupClyntonDistutilsCommands"
        ],
