...
#else
#define _CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
#define _CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
#define _CLYNTON_ONEFILE_TEMP_BOOL 0
#define _CLYNTON_ONEFILE_CHILD_GRACE_TIME_INT 5000
#define _CLYNTON_ONEFILE_TEMP_SPEC "%TEMP%/onefile_%PID%_%TIME%"
...
#if _CLYNTON_ONEFILE_COMPRESSION_BOOL == 1
...
#if defined(_WIN32)
    SetEnvironmentVariable("CLYNTON_ONEFILE_PARENT", value);
#else
...
#if _CLYNTON_ONEFILE_TEMP_BOOL == 0
...
#if _CLYNTON_ONEFILE_TEMP_BOOL == 1
...
static int waitpid_timeout(pid_t pid) {
    ...
    struct timespec timeout = {
        _CLYNTON_ONEFILE_CHILD_GRACE_TIME_INT / 1000,
        (_CLYNTON_ONEFILE_CHILD_GRACE_TIME_INT % 1000) * 1000,
    };
...
static void cleanupChildProcess(bool send_sigint) {
    ...
    if (handle_process != 0) {
        ...
        if (send_sigint) {
#if defined(_CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING)
...
#if _CLYNTON_ONEFILE_TEMP_BOOL == 1 || 1
        CLYNTON_PRINT_TRACE("Waiting for child to exit.\n");
#if defined(_WIN32)
        if (WaitForSingleObject(handle_process, _CLYNTON_ONEFILE_CHILD_GRACE_TIME_INT) != 0) {
...
#if _CLYNTON_ONEFILE_TEMP_BOOL == 1
    if (payload_created) {
#if _CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
...
BOOL WINAPI ourConsoleCtrlHandler(DWORD fdwCtrlType) {
    ...
    case CTRL_C_EVENT:
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_HANDLING
        puts("Ctrl-C event");
#endif
...
#include "clynton_onefile_auto_updater.h"
#endif
...
#ifdef _CLYNTON_ONEFILE_SPLASH_SCREEN
#ifdef __cplusplus
extern "C" {
#endif
...
int main(int argc, char **argv) {
    ...
    filename_char_t const *pattern = FILENAME_EMPTY_STR _CLYNTON_ONEFILE_TEMP_SPEC;
...
// The 'X' stands for no compression, 'Y' is compressed, handle that.
...
#if _CLYNTON_ONEFILE_SPLASH_SCREEN
...
#ifdef _CLYNTON_EXPERIMENTAL_DEBUG_ONEFILE_CACHING
    ...
    // might change it by the time the child looks at its parent.
    {
        ...
        setEnvironVar("CLYNTON_ONEFILE_PARENT", buffer);
    }
