...
PyObject *STRING_FROM_CHAR(unsigned char c) {
    ...
    return Clynton_String_FromStringAndSize(s, 1);
}
...
#if PYTHON_VERSION < 0x300 && defined(_CLYNTON_FULL_COMPAT)
...
static void _ClyntonUnicode_FastCopyCharacters(PyObject *to, Py_ssize_t to_start, PyObject *from, Py_ssize_t from_start,
                                              Py_ssize_t how_many) {
...
static int _ClyntonUnicode_modifiable(PyObject *unicode) {
...
static PyObject *_ClyntonUnicode_New(Py_ssize_t length) {
    assert(length != 0);
...
static PyObject *_ClyntonUnicode_resize_copy(PyObject *unicode, Py_ssize_t length) {
    if (_PyUnicode_KIND(unicode) != PyUnicode_WCHAR_KIND) {
        ...
        _ClyntonUnicode_FastCopyCharacters(copy, 0, unicode, 0, copy_length);

        return copy;
    } else {
        PyObject *w = _ClyntonUnicode_New(length);
        if (unlikely(w == NULL)) {
            return NULL;
        }
...
static PyObject *_ClyntonUnicode_resize_compact(PyObject *unicode, Py_ssize_t length) {
    ...
    if (unlikely(new_unicode == NULL)) {
        Clynton_Py_NewReference(unicode);
...
static int _ClyntonUnicode_resize_inplace(PyObject *unicode, Py_ssize_t length) {
    assert(!PyUnicode_IS_COMPACT(unicode));
...
static int _ClyntonUnicode_resize(PyObject **p_unicode, Py_ssize_t length) {
    ...
    if (!_ClyntonUnicode_modifiable(unicode)) {
        PyObject *copy = _ClyntonUnicode_resize_copy(unicode, length);
        if (copy == NULL)
            return -1;
        Py_DECREF(*p_unicode);
        *p_unicode = copy;
    ...
    if (PyUnicode_IS_COMPACT(unicode)) {
        PyObject *new_unicode = _ClyntonUnicode_resize_compact(unicode, length);
        if (new_unicode == NULL)
            return -1;
    ...
    return _ClyntonUnicode_resize_inplace(unicode, length);
}
...
PyObject *UNICODE_CONCAT(PyThreadState *tstate, PyObject *left, PyObject *right) {
    ...
    _ClyntonUnicode_FastCopyCharacters(result, 0, left, 0, left_len);
    _ClyntonUnicode_FastCopyCharacters(result, left_len, right, 0, right_len);
...
bool UNICODE_APPEND(PyThreadState *tstate, PyObject **p_left, PyObject *right) {
    ...
    if (_ClyntonUnicode_modifiable(left) && PyUnicode_KIND(right) <= PyUnicode_KIND(left) &&
        !(PyUnicode_IS_ASCII(left) && !PyUnicode_IS_ASCII(right))) {
        if (unlikely(_ClyntonUnicode_resize(p_left, new_len) != 0)) {
            return false;
        }

        _ClyntonUnicode_FastCopyCharacters(*p_left, left_len, right, 0, right_len);
    } else {
        Py_UCS4 maxchar = PyUnicode_MAX_CHAR_VALUE(left);
        Py_UCS4 maxchar2 = PyUnicode_MAX_CHAR_VALUE(right);
        ...
        _ClyntonUnicode_FastCopyCharacters(res, 0, left, 0, left_len);
        _ClyntonUnicode_FastCopyCharacters(res, left_len, right, 0, right_len);
...
PyObject *ClyntonUnicode_FromWideChar(const wchar_t *str, Py_ssize_t size) {
#if PYTHON_VERSION < 0x300
    if (size == -1) {
        size = wcslen(str);
    }
...
PyObject *BUILTIN_UNICODE3(PyObject *value, PyObject *encoding, PyObject *errors) {
    ...
    if (encoding == NULL) {
        encoding_str = NULL;
    } else if (Clynton_String_Check(encoding)) {
        encoding_str = Clynton_String_AsString_Unchecked(encoding);
    }
#if PYTHON_VERSION < 0x300
        ...
        encoding_str = Clynton_String_AsString_Unchecked(uarg2);
    }
#endif
    else {
    ...
    if (errors == NULL) {
        errors_str = NULL;
    } else if (Clynton_String_Check(errors)) {
        errors_str = Clynton_String_AsString_Unchecked(errors);
    }
#if PYTHON_VERSION < 0x300
        ...
        errors_str = Clynton_String_AsString_Unchecked(uarg3);
    }
#endif
    else {
