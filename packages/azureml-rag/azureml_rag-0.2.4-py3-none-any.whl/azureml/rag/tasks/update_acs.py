...
def create_index_from_raw_embeddings(
    ...
    with track_activity(logger, "update_acs", custom_dimensions={"num_documents": len(emb._document_embeddings)}) as activity_logger:
        ...
        # Upload documents
        include_embeddings = str(acs_config.get("push_embeddings", "true")).lower() == "true" and emb and emb.kind != "none"
...
def main(args, logger, activity_logger):
    try:
        ...
        if args.connection_id is not None:
            ...
            from azureml.rag.utils.connections import get_connection_by_id_v2, get_metadata_from_connection, get_target_from_connection
