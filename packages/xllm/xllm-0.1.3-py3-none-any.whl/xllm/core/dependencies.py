...
def build_model(
    ...
    need_prepare_model_for_kbit_training: bool = True,
) -> Tuple[Union[PeftModel, PreTrainedModel], Optional[LoraConfig]]:
    ...
    if quantization_config is not None or need_prepare_model_for_kbit_training:
    ...
    if config.apply_lora:
        model, lora_config = apply_lora(config=config, model=model)
    else:
        lora_config = None

    # stabilize
    if config.stabilize:
        model = stabilize_training(model=model)
