# Copyright 2023 Komplete AI Team. All rights reserved.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

import json
import os
import random
from abc import ABC, abstractmethod
from typing import List, Optional, Tuple

from loguru import logger
from torch.utils.data import Dataset

from ..core.config import HuggingFaceConfig
from ..core.constants import DATASETS_MUST_HAVE_KEYS
from ..types import RawSample
from ..utils.miscellaneous import have_missing_keys


class BaseDataset(Dataset[RawSample], ABC):
    _DATASET_MUST_HAVE_KEYS = DATASETS_MUST_HAVE_KEYS

    def __init__(self, data: List[RawSample]):
        super().__init__()
        self.data = data

    @classmethod
    def prepare(cls, config: HuggingFaceConfig) -> None:
        train_data, valid_data = cls.download(config=config)

        if config.eval_local_path_to_data is None and valid_data is not None:
            logger.warning("valid_local_path_to_data is None, but valid_data is not None")
            if config.add_valid_to_train_if_no_path:
                train_data += valid_data
                logger.info("Add valid data to train")

        if valid_data is not None and config.eval_local_path_to_data is not None:
            if len(valid_data) > config.max_eval_samples and config.max_eval_samples > 0:
                train_data += valid_data[config.max_eval_samples :]
                valid_data = valid_data[: config.max_eval_samples]
                logger.info(f"Validation data size truncated to {config.max_eval_samples}")
            else:
                logger.info(f"Validation data size: {len(valid_data)}")

            with open(config.eval_local_path_to_data, mode="w") as file_object:
                for raw_sample in valid_data:
                    file_object.write(json.dumps(raw_sample) + "\n")
        else:
            logger.warning("Valid data or valid_local_path_to_data is None")

        if config.shuffle:
            random.shuffle(train_data)
            logger.info("Train data shuffled")

        with open(config.train_local_path_to_data, mode="w") as file_object:
            for raw_sample in train_data:
                file_object.write(json.dumps(raw_sample) + "\n")

        logger.info(f"Train data size: {len(train_data)}")

        return None

    @classmethod
    def load(cls, path_to_data: str) -> "BaseDataset":
        data = list()

        if not os.path.isfile(path_to_data):
            raise FileNotFoundError(
                f"File {path_to_data} not found. Probably you should run download_and_prepare before"
            )

        with open(path_to_data) as file_object:
            for line in file_object:
                sample = json.loads(line)
                data.append(sample)

        dataset = cls(data=data)

        return dataset

    def __len__(self) -> int:
        return len(self.data)

    def __getitem__(self, index: int) -> RawSample:
        sample = self.get_sample(index=index)
        flag, difference = have_missing_keys(data=sample, must_have_keys=self._DATASET_MUST_HAVE_KEYS)
        if flag:
            raise ValueError(f"RawSample from {self.__class__.__name__} must have {difference} keys")
        return sample

    @classmethod
    @abstractmethod
    def download(cls, config: HuggingFaceConfig) -> Tuple[List[RawSample], Optional[List[RawSample]]]:
        raise NotImplementedError

    @abstractmethod
    def get_sample(self, index: int) -> RawSample:
        raise NotImplementedError
