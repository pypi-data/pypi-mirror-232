...
def calc_syntactic_similarity(df_test, df_training):
    ...
    parsed_training = [FastKassim.parse_document(doc) for doc in df_test["training"].values]
    print("parsing test")
    parsed_test = [FastKassim.parse_document(doc) for doc in df_test["test"].values]
    for i, test_text in tqdm.tqdm(enumerate(parsed_test)):
        for j, training_text in parsed_training:
            similarities[i, j] = FastKassim.compute_similarity(test_text, training_text)
