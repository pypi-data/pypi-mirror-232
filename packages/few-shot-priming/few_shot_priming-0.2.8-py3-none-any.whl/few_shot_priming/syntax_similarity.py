...
def calc_syntactic_similarity(df_test, df_training):
    ...
    df_test_dask = ddf.from_pandas(df_test, npartitions=40)
    similarities = np.zeros(((len(df_test),len(df_training))))
    FastKassim = fkassim.FastKassim(fkassim.FastKassim.LTK)
    print(FastKassim.compute_similarity("I love", "I like"))
    parallelized_similarities = df_test_dask["text"].map(lambda x: [(j, FastKassim.compute_similarity(x, train_text)) for j, train_text in enumerate(df_training["text"].values)]).compute()
    print(parallelized_similarities)
    for i, test_text in enumerate(df_test["text"]):
        for j, similarity in parallelized_similarities[i]:
