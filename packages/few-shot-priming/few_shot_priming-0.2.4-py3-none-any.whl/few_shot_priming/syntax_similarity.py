...
def calc_syntactic_similarity(df_test, df_training):
    ...
    parallelized_similarities = df_test_dask["text"].map(lambda x: x ).compute() #[(j, FastKassim.compute_similarity(x,"train_text")) for j, train_text in enumerate(df_training["text"].values)]).compute()
