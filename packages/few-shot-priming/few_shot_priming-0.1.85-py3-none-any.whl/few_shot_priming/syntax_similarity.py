...
def calc_syntactic_similarity(df_test, df_training):
    df_test_dask = ddf.from_pandas(df_test, npartitions=16)
    similarities = np.zeros(((len(df_test),len(df_training))))
    FastKassim = fkassim.FastKassim(fkassim.FastKassim.LTK)
    parallelized_similarities = df_test_dask["text"].map(lambda x: [(j, FastKassim.compute_similarity(x,train_text)) for j, train_text in enumerate(df_training["text"])])
