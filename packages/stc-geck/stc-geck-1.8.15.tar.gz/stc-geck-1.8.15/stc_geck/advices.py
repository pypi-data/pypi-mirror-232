...
default_field_aliases = {
    ...
    'extension': 'cids.extension',
    'format': 'cids.extension',
    'isbn': 'metadata.isbns',
...
class BaseDocumentHolder:
    ...
    def get_links(self):
        if self.has_field('cids') and self.links:
...
class LinksWrapper:
    ...
    def to_list(self):
        links = []
        visited = set()
        for other_cid in self.links:
            if other_cid not in visited:
                links.append(self.links[other_cid])
            visited.add(other_cid)
        return links

    def add(self, link):
        if link['cid'] in self.links:
            old_link = self.links[link['cid']]
