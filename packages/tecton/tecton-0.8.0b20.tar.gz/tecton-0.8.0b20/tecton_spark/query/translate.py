...
def _execute_snowflake_fv(snowflake_fv_node_ref: NodeRef, spark: pyspark.sql.SparkSession) -> DataNode:
    ...
    conf.set("SQL_DIALECT", "snowflake")
    sql_string = fv_node.to_sql(pretty_sql=True)
    # TODO(TEC-16302): Find a better way to build the SQL query rather than setting and unsetting SQL_DIALECT.
    conf.unset("SQL_DIALECT")
    ...
    return DataNode(SparkDataFrame(snowflake_df))
