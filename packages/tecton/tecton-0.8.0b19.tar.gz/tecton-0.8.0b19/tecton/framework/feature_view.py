...
from tecton_core.time_window_wrapper import TimeWindowWrapper
...
class MaterializedFeatureView(FeatureView):
    ...
    def _derive_schemas(self) -> None:
        ...
        if (
            ...
            view_schema, materialization_schema = self._derive_schemas_for_snowflake(
                transformation_specs, data_source_specs
            )
        else:
            view_schema, materialization_schema = self._derive_schemas_for_spark(
                transformation_specs, data_source_specs
            )
    ...
    def _derive_schemas_for_snowflake(
        self, transformation_specs: List[TransformationSpec], data_source_specs: List[DataSourceSpec]
    ) -> Tuple[schema_pb2.Schema, schema_pb2.Schema]:
        view_schema = snowflake_api.derive_view_schema_for_feature_view(
            self._args, transformation_specs, data_source_specs
        )
        materialization_schema = snowflake_api.derive_materialization_schema_for_feature_view(view_schema, self._args)
        return view_schema, materialization_schema

    def _derive_schemas_for_spark(
        self, transformation_specs: List[TransformationSpec], data_source_specs: List[DataSourceSpec]
    ) -> Tuple[schema_pb2.Schema, schema_pb2.Schema]:
        push_sources = [
            ds_spec
        ...
        if is_push_source:
            view_schema = self._derive_push_source_schema(transformation_specs, push_sources)
        else:
            view_schema = self._derive_non_push_source_schema_for_spark(transformation_specs, data_source_specs)
    ...
    def _derive_push_source_schema(
        ...
        if len(transformation_specs) > 0:
            ...
            return self._args.materialized_feature_view_args.schema
        else:
    ...
    def _derive_non_push_source_schema_for_spark(
        self, transformation_specs: List[TransformationSpec], data_source_specs: List[DataSourceSpec]
    ) -> schema_pb2.Schema:
        has_explicit_schema = self._args.materialized_feature_view_args.HasField("schema")
        should_run_query_validation = self._args.materialized_feature_view_args.run_query_validation

        if has_explicit_schema:
            if should_run_query_validation:
                derived_view_schema = spark_api.derive_view_schema_for_feature_view(
                    self._args, transformation_specs, data_source_specs
                )
                if not Schema(self._args.materialized_feature_view_args.schema).is_equivalent(
                    Schema(derived_view_schema)
                ):
                    raise errors.SCHEMAS_DO_NOT_MATCH(
                        self._args.materialized_feature_view_args.schema, derived_view_schema
                    )
            return self._args.materialized_feature_view_args.schema
        else:
            if should_run_query_validation:
                raise errors.SCHEMA_REQUIRED(self.name)
    ...
    def aggregations(self) -> Optional[configs.Aggregation]:
        ...
        return [
            configs.Aggregation(
                ...
                # TODO(sanika): change to user-facing TimeWindow class once added to the SDK
                time_window=TimeWindowWrapper.from_data(agg.time_window).window_duration,
...
def batch_feature_view(
    *,
    ...
    :param run_query_validation: If `True`, Tecton will execute the Feature View transformations during tecton plan/apply
        ...
        with Tecton's validation (e.g. some pip dependencies). Default is True for Spark and Snowflake Feature Views,
...
def _construct_secondary_key_output_list_name(column: str, window: datetime.timedelta) -> str:
    window_name = to_human_readable_str(window)
    return f"{column}_keys_{window_name}".replace(" ", "")
