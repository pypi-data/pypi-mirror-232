    ...
    def _get_aggregations(self, window_order_col: str, partition_cols: List[str]) -> List[Term]:
        ...
        for feature in time_aggregation.features:
            ...
            if feature.HasField("window"):
                ...
                time_window = TimeWindowWrapper.from_data(feature.time_window)
            ...
            if not aggregation_plan or not aggregation_plan.is_supported(dialect()):
                msg = f"Aggregation {feature.function} is not supported by {dialect()}"
                raise NotImplementedError(msg)
        ...
        for feature in time_aggregation.features:
            ...
            if time_aggregation.is_continuous:
                if aggregation_plan.continuous_aggregation_query_terms is None:
                    msg = (
                        f"Continuous mode is not supported for aggregation {feature.function} with dialect {dialect()}"
                    )
                    raise NotImplementedError(msg)
