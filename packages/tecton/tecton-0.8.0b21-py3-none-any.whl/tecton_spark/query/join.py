        ...
        for feature in time_aggregation.features:
            ...
            agg = aggregation_plan.full_aggregation_transform(
                ...
                self._generate_aggregation_window_spec(
                    TimeWindowWrapper.from_data(feature.time_window).window_start,
                    join_spec,
                ),
            )
        ...
        if self.fdw.aggregation_secondary_key:
            for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
                aggregations.append(
                    ...
                    # TODO(sanika): refactor this to use window_start and window_end once generate_aggregation_window is refactored
                    .over(self._generate_aggregation_window_spec(-secondary_key_output.time_window, join_spec)).alias(
                        f"{TECTON_SECONDARY_KEY_AGGREGATION_INDICATOR_COL}_{to_human_readable_str(secondary_key_output.time_window)}"
                    )
                )
        return aggregations

    # TODO(sanika): Refactor this function to take in both window_start and window_end for use in getting the range
    def _generate_aggregation_window_spec(
        self, window_start: datetime.timedelta, join_spec: _AsofJoinerAndWindowAggregator
        ...
        return join_spec.get_range_between_window_spec(
            self.fdw.earliest_anchor_time_from_window_start(window_start) * 2 - 1, spark_window.Window.currentRow
        )
...
class AggregationSecondaryKeyRollupSparkNode(SparkExecNode):
    ...
    def _window_to_features_map(self) -> Dict[str, List[str]]:
        ...
        # TODO(sanika): fix time window for secondary keys
        for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
            ...
            features = [
                ...
                if agg_feature.window.ToTimedelta() == secondary_key_output.time_window
            ]
    ...
    def _temp_struct_column_name(self, window: datetime.timedelta) -> str:
        return f"{self.TECTON_TEMP_STRUCT_PREFIX}_{to_human_readable_str(window)}"

    def _temp_indictor_column_name(self, window: datetime.timedelta) -> str:
        return f"{TECTON_SECONDARY_KEY_AGGREGATION_INDICATOR_COL}_{to_human_readable_str(window)}"

    def _filter_aggregation_secondary_key_by_indicator(
        self, df: pyspark.sql.DataFrame, window: datetime.timedelta, temp_struct_column_name: str
    ...
    def _extract_features_columns_from_struct_columns(
        ...
        for agg_feature in self.fdw.trailing_time_window_aggregation.features:
            if agg_feature.window.ToTimedelta() == window:
                df = df.withColumn(
                    agg_feature.output_feature_name,
