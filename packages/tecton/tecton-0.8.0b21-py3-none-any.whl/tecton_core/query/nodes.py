...
class FeatureViewPipelineNode(QueryNode):
    ...
    def _to_query(self) -> pypika.Query:
        ...
        return pipeline_builder.get_pipeline_query(
            input_name_to_query_map={k: v._to_query() for k, v in self.inputs_map.items()}
        )
...
class StagingNode(QueryNode):
    ...
    def _to_staging_query_sql(self) -> str:
        fields = [Field(col).as_(col) for col in self.columns]
        input_query = self.input_node._to_query()
        # Explicitly selecting fields to respect view schema. DuckDB is case insensitive when querying fields,
        # but exports to respect the original case. When exporting out of DuckDB, this coerces the names to respect
        # the explicit schema naming (e.g. instead of leaking Snowflake auto-uppercasing)
        #
        # TODO(TEC-16563): Fix Snowflake case handling and remove this
        aliased_input = self.input_node.node.__class__.__name__ + "_" + uuid.uuid4().hex[:16]
        sql = Query().with_(input_query, aliased_input).from_(AliasedQuery(aliased_input)).select(*fields).get_sql()
...
class StagedTableScanNode(QueryNode):
    ...
    def from_staging_node(cls, query_node: StagingNode) -> QueryNode:
        return cls(
            ...
            query_tree_step=query_node.query_tree_step,
        )
