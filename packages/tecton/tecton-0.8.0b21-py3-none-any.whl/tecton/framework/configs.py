...
class Aggregation(object):
    ...
    def _to_proto(self, aggregation_interval: datetime.timedelta, is_continuous: bool):
        ...
        if isinstance(self.time_window, datetime.timedelta):
            proto.time_window.CopyFrom(
                feature_view_pb2.TimeWindow(
                    window_duration=time_utils.timedelta_to_proto(self.time_window), offset=Duration(seconds=0)
                )
            )
        ...
        if self.name:
            ...
            proto.name = feature_view_utils.construct_aggregation_output_feature_name(
                ...
                TimeWindowWrapper.from_args(proto.time_window),
