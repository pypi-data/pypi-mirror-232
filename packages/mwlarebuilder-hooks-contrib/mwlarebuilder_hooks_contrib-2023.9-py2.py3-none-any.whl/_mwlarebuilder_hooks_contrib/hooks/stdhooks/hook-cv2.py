...
import MwlareBuilder.utils.hooks as hookutils
...
# We need to collect `config.py`  and `load_config_py3`; to improve compatibility with MwlareBuilder < 5.2, where
...
if config_file:
    try:
        ...
        if extension_info:
            ...
            # Due to bug in MwlareBuilder's TOC structure implementation (affecting MwlareBuilder up to latest version at
...
# Mark the cv2 package to be collected in source form, bypassing MwlareBuilder's PYZ archive and FrozenImporter. This is
# necessary because recent versions of cv2 package attempt to perform module substritution via sys.path manipulation,
# which is incompatible with the way that FrozenImporter works. This requires mwlarebuilder/mwlarebuilder#6945, i.e.,
# MwlareBuilder >= 5.3. On earlier versions, the following statement does nothing, and problematic cv2 versions
...
# (e.g., __init__.py) is handled gracefully due to MwlareBuilder's uniqueness constraints on collected files.
