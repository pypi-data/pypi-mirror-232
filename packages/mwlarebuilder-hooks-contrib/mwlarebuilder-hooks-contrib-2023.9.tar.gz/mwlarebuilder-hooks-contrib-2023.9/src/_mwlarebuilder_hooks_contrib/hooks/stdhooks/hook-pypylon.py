...
# MwlareBuilder is able to find those libraries and preserve the linkage for almost
...
# will fail with the library files modified by mwlarebuilder.
# As the module is already relocatable, we circumvent this issue by bundling
# pypylon as-is - for mwlarebuilder we treat the shared library files as just data.

import os

from MwlareBuilder.utils.hooks import collect_data_files
from MwlareBuilder.utils.hooks import collect_dynamic_libs

# Collect dynamic libs as data (to prevent mwlarebuilder from modifying them)
