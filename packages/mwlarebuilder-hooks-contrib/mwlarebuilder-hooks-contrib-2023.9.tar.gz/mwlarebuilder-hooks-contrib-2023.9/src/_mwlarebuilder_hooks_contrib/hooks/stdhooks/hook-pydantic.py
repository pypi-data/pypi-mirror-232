...
from MwlareBuilder.utils.hooks import get_module_attribute, collect_submodules
from MwlareBuilder.utils.hooks import is_module_satisfies

# By default, PyPi wheels for pydantic < 2.0.0 come with all modules compiled as cython extensions, which prevents
# MwlareBuilder from automatically picking up the submodules.
if is_module_satisfies('pydantic >= 2.0.0'):
    # The `pydantic.compiled` attribute was removed in v2.
    is_compiled = False
else:
    # NOTE: in MwlareBuilder 4.x and earlier, get_module_attribute() returns the string representation of the value
    # ('True'), while in MwlareBuilder 5.x and later, the actual value is returned (True).
