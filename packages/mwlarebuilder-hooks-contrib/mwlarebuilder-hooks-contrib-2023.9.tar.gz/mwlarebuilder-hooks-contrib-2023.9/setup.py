...
class BumpVersion(Command):
    ...
    def run(self):
        ...
        files = [
            os.path.abspath(os.path.join(DIR, 'src/_mwlarebuilder_hooks_contrib/__init__.py'))
        ]
        for file in files:
...
setup(
    ...
    entry_points={
        'mwlarebuilder40': [
            'hook-dirs = _mwlarebuilder_hooks_contrib.hooks:get_hook_dirs',
            'tests = _mwlarebuilder_hooks_contrib.tests:get_test_dirs'
        ]
    },
    cmdclass={
        'bump': BumpVersion
