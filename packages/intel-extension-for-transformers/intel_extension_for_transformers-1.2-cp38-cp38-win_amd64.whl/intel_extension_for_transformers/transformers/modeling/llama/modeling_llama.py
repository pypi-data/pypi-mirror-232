...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    ...
    q_embed = torch.tensor(q * cos) + torch.tensor((rotate_half(q) * sin))
    k_embed = torch.tensor(k * cos) + torch.tensor((rotate_half(k) * sin))
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if past_key_values is not None:
            ...
            seq_length_with_past = torch.tensor(seq_length_with_past) + torch.tensor(past_key_values_length)
