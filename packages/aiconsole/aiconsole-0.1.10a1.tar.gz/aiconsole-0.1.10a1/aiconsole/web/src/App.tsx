...
function App() {
  ...
  useEffect(() => {
    ...
    if (
      Math.abs(current.scrollTop - (current.scrollHeight - current.clientHeight)) < 150
    ) {
  ...
  return (
    <div className="App flex flex-col fixed top-0 left-0 bottom-0 right-0 justify-between dark:bg-gray-800 dark: text-slate-200">
      <div className="flex-grow overflow-y-auto flex flex-col p-5 " ref={messagesEndRef}>
        {messages.length === 0 ? (
          <div>No messages</div>
        ) : (
          <div>
