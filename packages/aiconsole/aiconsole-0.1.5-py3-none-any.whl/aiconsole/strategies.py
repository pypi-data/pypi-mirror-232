...
PRESET_STRATEGIES_DIRECTORY = "./presets/strategies"
...
class Manuals:
    ...
    def __init__(self):
        ...
        if not os.path.exists(STRATEGIES_DIRECTORY):
            print("Creating strategies directory from presets")
            os.mkdir(STRATEGIES_DIRECTORY)
            for filename in os.listdir(PRESET_STRATEGIES_DIRECTORY):
                if filename.endswith('.py'):
                    shutil.copy(os.path.join(PRESET_STRATEGIES_DIRECTORY, filename),
                        os.path.join(STRATEGIES_DIRECTORY, filename))
