...
def get_missing_openai_models(openai_api_key):
    missing_models = ["davinci", "curie", "babbage", "ada"]  # These are OpenAI's GPT-3 models

    try:
        response = requests.get(openai_api_url, headers={
        ...
        if response_data and "data" in response_data:
            ...
            missing_models = list(filter(lambda model: model not in available_models, missing_models))
