...
def init(sweep = False, sweep_name = "", run_name="", save_files = False):
    ...
    if sweep == True:
      ...
      os.mkdir(run_dir_global)
      config_dict.update({'run_name': sweep_name, 'run_path': run_dir_global})
...
def get_run(run_id = None):
    ...
    response = requests.post(protocol+'://'+IP+'/api/get_run', data=data)

    return_dict = {}

    if 'train' in response:
      with open(response['train'], "rb") as file:
          train = dill.load(file)
          return_dict.update({'train': train})
          
    if 'val' in response:
      with open(response['val'], "rb") as file:
          val = dill.load(file)
          return_dict.update({'val': val})
          
    if 'test' in response:
      with open(response['test'], "rb") as file:
          test = dill.load(file)
          return_dict.update({'test': test})
          
    if 'train_labels' in response:
      with open(response['train_labels'], "rb") as file:
          train_labels = dill.load(file)
          return_dict.update({'train_labels': train_labels})
          
    if 'val_labels' in response:
      with open(response['val_labels'], "rb") as file:
          val_labels = dill.load(file)
          return_dict.update({'val_labels': val_labels})
          
    if 'test_labels' in response:
      with open(response['test_labels'], "rb") as file:
          test_labels = dill.load(file)
          return_dict.update({'test_labels': test_labels})
          

    if 'pytorch' in response:
      model_statedict = torch.load(response['pytorch_model_statedict'])
      model_network = torch.load(response['pytorch_model_network'])
      return_dict.update({'best_model_statedict': model_statedict, 'best_model_network': model_network})
      
    elif 'keras' in response:
      model = tf.keras.models.load_model(model_path)
      eturn_dict.update({'best_model': model})
