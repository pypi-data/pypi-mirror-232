...
def preprocess_bbcode(text: str) -> Iterable[str]:
    ...
    for match in BBCODE_TAG_PATTERN.finditer(text):
        ...
        # Closing tag(s)?
        if span.startswith("[/"):
            # If no styles were specified, close the most recently opened
            # style(s)
            styles = span[2:-1].split()

            if not styles:
                styles = most_recent_styles
                most_recent_styles = []

            for style in styles:
                # This style is no longer active
                current_styles.discard(style)
