...
from .bb_parser import escape, unescape
...
def ask(options: Dict[str, T], *, prompt: str = "") -> T:
    ...
    # Display the prompt
    if prompt:
        sys.stdout.write(unescape(prompt))
        sys.stdout.write("\n\n")

    # Display the initial menu
    for index, (name, value) in enumerate(opt_list):
        _display_menu_line(index, name, index == active_index)
        sys.stdout.write("\n")

    sys.stdout.write(unescape("\n[dim]Use arrow keys ⇅ or enter a number[/dim]"))
    sys.stdout.flush()
