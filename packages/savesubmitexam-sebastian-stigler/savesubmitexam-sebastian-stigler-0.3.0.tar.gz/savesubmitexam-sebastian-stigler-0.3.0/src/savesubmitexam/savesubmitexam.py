...
from .notebook_path import get_notebook_path
...
en = Text(
    ...
    button_submit_error="An Error occurred!",
...
class GenericSaveSubmitExam:
    ...
    def run(self, course: str, exam_type: ExamType):
        ...
        def on_submit_exam(instance):
            ...
            current_filename = get_notebook_path()  # os.environ.get('JPY_SESSION_NAME')
            ...
            try:
                requests.get("http://127.0.0.1:8000/api/health", timeout=(60, None))
                res = requests.post(
                ...
                res = ResDummy(
                    status_code=500,
                    text="Backend server is down! Has the submission time expired?",
                )
            except Exception as err:
                res = ResDummy(status_code=500, text=str(err))
            time.sleep(1)
