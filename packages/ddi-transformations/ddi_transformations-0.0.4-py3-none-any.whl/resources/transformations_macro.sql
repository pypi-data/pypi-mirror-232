...
{% for job_data in job_details['mapIngestionJobFieldColumnDTO'] if sys_name == job_data['systemName'] %}
    {% set config = job_data['mapping'][0]['config']['transformations'] %}
    {% if not config == None %}
        {% set function_name = job_data['mapping'][0]['config']['transformations'][0]['functionName'] %}
        {% set function_type = job_data['mapping'][0]['config']['transformations'][0]['type'] %}
        {% set ns.transformed_value = ns1.transformation_types[function_name] %}
        {% set ns.transformed_value = ns.transformed_value.replace('column_name', column_name) %}

        {% if function_type == 'TEXT' %}
            {% set function_input = job_data['mapping'][0]['config']['transformations'][0]['value'] %}
			{% set ns.transformed_value = ns.transformed_value.replace('value', "'" + function_input + "'") %}
        {% elif function_type == 'RANGE' %}
            {% set function_input = job_data['mapping'][0]['config']['transformations'][0]['value'] %}
            {% set value1 = (function_input|string).split(':')[0] %}
            {% set value2 = (function_input|string).split(':')[1] %}
            {% set ns.transformed_value = ns.transformed_value.replace('value1', "'" + value1 + "'") %}
            {% set ns.transformed_value = ns.transformed_value.replace('value2', "'" + value2 + "'") %}
        {% endif %}
    {% endif %}
{% endfor %}
