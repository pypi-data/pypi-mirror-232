...
class Validate:
	...
	def __basic(self):
		...
		print("Chad Extractor v4.4 ( github.com/ivan-sincek/chad )")
		print("")
	...
	def __validate(self, key, value):
		...
		if len(value) > 0:
			...
			# --------------------
				...
				if os.path.isdir(self.__args["excludes"]):
					self.__error("File with regular expressions is a directory")
				elif os.path.isfile(self.__args["excludes"]):
	...
	def run(self):
		...
		# --------------------
			...
			if self.__args["template"] is None or self.__args["results"] is None or self.__args["out"] is None or not self.__check(argc):
