...
class Validate:
	...
	def __basic(self):
		...
		print("Chad v4.4 ( github.com/ivan-sincek/chad )")
		print("")
	...
	def __validate(self, key, value):
		...
		if len(value) > 0:
			...
			if key == "-q" and self.__args["queries"] is None:
				...
				if os.path.isdir(self.__args["queries"]):
					self.__error("File with Google Dorks is a directory")
				elif os.path.isfile(self.__args["queries"]):
			...
			# --------------------
				...
				if os.path.isdir(self.__args["proxies"]):
					self.__error("File with proxies is a directory")
				elif os.path.isfile(self.__args["proxies"]):
	...
	def run(self):
		...
		# --------------------
			...
			if self.__args["queries"] is None or not self.__check(argc):
