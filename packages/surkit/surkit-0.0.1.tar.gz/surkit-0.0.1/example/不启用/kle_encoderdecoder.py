...
from surkit.nn.pytorch.dense_ed import DenseED
...
icbc = ["$p@left$ = 1", "$p@right$ = 0", "$uy@top$ = 0", "$uy@down$ = 0"]
...
train2d(sampler=train_loader, inputs=['k'], constant={}, output=output, icbc=icbc, pde=pde, iterations=300,
        ...
        loss_function="MSE", lr=1e-3, net=net, path='model/1model4.pth', optimizer="Adam", report_interval=10, imsize=64)
