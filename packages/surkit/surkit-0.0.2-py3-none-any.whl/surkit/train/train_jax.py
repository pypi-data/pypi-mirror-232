...
def train(
    ...
    def jax_grad(variable: str):
        if variable not in grad_fn_dict:
            ...
            ind_x = list(geo.keys()).index(right)
            if '_' not in left:
    ...
    def parse_icbc(condition, params_, x_):
        ...
        for when in whens:
            ...
            geo_copy[variable] = jnp.zeros_like(geo_copy[variable]) + eval_with_vars(value, geo, constant, {}, {})
        out_ = fwd(params_, jnp.concatenate(list(geo_copy.values()), axis=1))
        for index, out in enumerate(output):
            out_dic_[out] = jnp.expand_dims(out_[:, index], axis=1)
        for key_ in icbc_intermediate_dict.keys():
            grad_fn, ind = grad_fn_dict[key_]
            if len(geo) > 1:
                icbc_intermediate_dict[key_] = grad_fn(params_, jnp.concatenate(list(geo_copy.values()), axis=1))[:, ind].reshape(-1, 1)
            else:
                icbc_intermediate_dict[key_] = grad_fn(params_, jnp.concatenate(list(geo_copy.values()), axis=1))
        value_target_pairs = []
        for cond in conds:
            value_target_pairs.append(
                parse_pde(cond, geo, constant, out_dic_, icbc_intermediate_dict)
            )
    ...
    def train_step(state_):
        loss_, grads = (jax.value_and_grad(loss_fn))(state_.params, list(geo.values()))
    ...
    for epoch in range(iterations):
        ...
        for key, value in input.items():
            if isinstance(value, dict):
                geo[key] = jnp.array(random_sampler(value["low"], value["high"], value["size"]))
...
def train_bayesian(
    ...
    def loss_fn(params_):
        return elbo(jnp.concatenate(list(geo.values()), axis=1),
