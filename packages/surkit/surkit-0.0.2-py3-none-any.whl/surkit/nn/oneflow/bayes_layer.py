...
class BayesLinear(nn.Module):
    ...
    def __init__(self, in_d, out_d, prior_mean=0., prior_var=1.):
        ...
        self.w_mu = nn.Parameter(flow.zeros(out_d, in_d))
        self.w_rho = nn.Parameter(flow.zeros(out_d, in_d))

        # initialize mu and rho parameters for the layer's bias

        self.b_mu = nn.Parameter(flow.zeros(out_d))
        self.b_rho = nn.Parameter(flow.zeros(out_d))
        ...
        self.prior = Normal(prior_mean, np.sqrt(prior_var))

    def forward(self, x):
        # sample weights
        w_epsilon = Normal(0, 1).sample(self.w_mu.shape).to(self.w_mu.device)
        self.w = self.w_mu + flow.log(1 + flow.exp(self.w_rho)) * w_epsilon

        # sample bias
        b_epsilon = Normal(0, 1).sample(self.b_mu.shape).to(self.b_mu.device)
        self.b = self.b_mu + flow.log(1 + flow.exp(self.b_rho)) * b_epsilon

        # record log prior by evaluating log pdf of prior at sampled weight and bias
        w_log_prior = self.prior.log_prob(self.w)
        b_log_prior = self.prior.log_prob(self.b)
        self.log_prior = flow.sum(w_log_prior) + flow.sum(b_log_prior)
        ...
        self.w_post = Normal(self.w_mu.data, flow.log(1 + flow.exp(self.w_rho)))
        self.b_post = Normal(self.b_mu.data, flow.log(1 + flow.exp(self.b_rho)))
...
class BayesConv2d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size, stride=1,
        ...
        # initialize mu and rho parameters for the weights of the layer
        self.w_mu = nn.Parameter(flow.zeros(out_channels, in_channels, *_pair(kernel_size)))
        self.w_rho = nn.Parameter(flow.zeros(out_channels, in_channels, *_pair(kernel_size)))
        #
        # initialize mu and rho parameters for the layer's bias
        if self.bias:
            self.b_mu = nn.Parameter(flow.zeros(out_channels, in_channels, *_pair(kernel_size)))
            self.b_rho = nn.Parameter(flow.zeros(out_channels, in_channels, *_pair(kernel_size)))
    ...
    def forward(self, x):
        ...
        # record log prior by evaluating log pdf of prior at sampled weight and bias
        w_log_prior = self.prior.log_prob(self.w)
        self.log_prior = flow.sum(w_log_prior)
        if self.bias:
            b_log_prior = self.prior.log_prob(self.b)
            self.log_prior = flow.sum(w_log_prior) + flow.sum(b_log_prior)
