...
if 'SUK_BACKEND' in os.environ:
    backend = os.environ['SUK_BACKEND']
    if backend:
...
if not backend_name:
    ...
    for backend in supported_backends:
        try:
            ...
            continue
    if not backend_name:
        raise ValueError("No supported backends found. Please install one of %s".format(','.join(supported_backends)))
