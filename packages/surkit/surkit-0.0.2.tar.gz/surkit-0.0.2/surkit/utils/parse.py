...
def eval_with_vars(code, geo, constant, outs, dic):
    """
    Run a code string with dictionaries as vars.

    Args:
        code (str): code to be executed
        geo (dict[str, Tensor]): dict with input features' names and their values
        constant (dict[str, int | float]): dict with constants' names and their values
...
def get_grad(variable: str, geo, dic, outs):
    """
    ...
    return bkd.grad(outs[y], geo[x]) if '_' not in y else bkd.grad(dic[y], geo[x])

def get_grad2d(variable: str, geo, dic, outs, imgfilter=None):
...
def equation_to_runnable(equation: str, geo, constant, outs, dic):
    """
    ...
    replacement_pattern = {
        "geo" : lambda variable : "geo['%s']" % variable,
        "constant" : lambda variable : "constant['%s']" % variable,
    ...
    def locate_var_in_dicts(variable: str):
        if variable in geo:
            return replacement_pattern['geo'](variable)
        if variable in constant:
            return replacement_pattern['constant'](variable)
        if variable in outs:
...
def split_pde(equation: str, geo, constant, outs, dic):
    code = equation_to_runnable(equation, geo, constant, outs, dic)
    left, right = code.split('=')
    return [safe_compile(left), safe_compile(right)]

def split_condition(equation: str, geo, constant, outs, dic):
    code = equation_to_runnable(equation, geo, constant, outs, dic)
...
def split_hard_condition(equation: str, geo, constant, outs, dic):
    code = equation_to_runnable(equation, geo, constant, outs, dic)
    left, right = code.split('=')
