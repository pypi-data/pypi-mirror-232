...
def main():
    ...
    y = np.exp(sampler['x'])
    target = {"y": y}
    train(sampler=sampler, output=args.output_name, net=my_net, iterations=args.max_iteration, target=target,
          pde=args.pde, icbc=args.icbc, constant=args.constant, optimizer=args.optimizer, lr=args.lr, device=args.device,
          loss_function=args.loss_function, path=args.save_path, report_interval=args.report_interval)
