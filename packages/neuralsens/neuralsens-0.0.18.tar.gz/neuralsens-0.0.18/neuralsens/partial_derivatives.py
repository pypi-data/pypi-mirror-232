...
def alpha_sens_curves(
    ...
    if kind == 'line':
        texts = []
        fig, ax = plt.subplots(len(raw_sens), 2, sharey=True, gridspec_kw=dict(width_ratios=[max_alpha, 2]))
        ...
        for out, rs in enumerate(raw_sens):
            ...
            for inp, input in enumerate(input_name):
                ...
                ax[1].scatter(max_alpha + 3, max_alpha_mean, color=var_color)
                texts.append(
                    ax[1].text(
            ...
            ax[1].set_xticks(ticks=[max_alpha+3], labels=[r"$\infty$"])
