...
def reconcile_forecasts(yhat: np.ndarray, S: np.ndarray, residuals: np.ndarray=None, method: str='ols') -> np.ndarray:
    ...
    if is_sparse_S:
        Scsc = S.tocsc()
        Jright = Scsc[ms:]
        Jleft = sparse.csc_matrix((n_bottom_timeseries, ms), dtype=np.float32)
        J = sparse.hstack([Jleft, Jright])
        Uleft = sparse.eye(ms, dtype=np.float32)
        Uright = -Scsc[:ms]
        Ut = sparse.hstack([Uleft, Uright])
    else:
    ...
    if method == 'ols':
        ...
        if is_sparse_S:
            Wdiag = np.sum(S, axis=1).A.squeeze()
            W = sparse.diags(Wdiag)
            UtW = Ut @ W
        else:
        ...
        if is_sparse_S:
            Wdiag = np.sum(residuals**2, axis=1).A.squeeze() / residuals.shape[1]
            W = sparse.diags(Wdiag)
            UtW = Ut @ W
        else:
    ...
    if is_sparse_S:
        G = (J - np.linalg.solve(UtW @ Ut.T, UtW @ J.T).T @ Ut)
        G = (J - sparse.linalg.spsolve(UtW @ Ut.T, UtW @ J.T).T @ Ut)
    else:
...
def calc_summing_matrix(df: pd.DataFrame, aggregation_cols: List[str], aggregations: List[List[str]] = None, 
    ...
    for aggregation in aggregations:
        ...
        S_agg = pd.get_dummies(levels[aggregation].astype(str).agg('-'.join, axis=1), sparse=sparse, dtype=np.float32).T    
    ...
    df_S_bottom = pd.get_dummies(bottom_timeseries, sparse=sparse, dtype=np.float32).T 
    ...
    S_top = np.ones((1, n_bottom_timeseries), dtype=np.float32)
