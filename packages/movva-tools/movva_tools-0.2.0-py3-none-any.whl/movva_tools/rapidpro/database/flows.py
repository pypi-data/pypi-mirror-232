...
from rapidpro.databases import Base
from rapidpro.constants import MAX_NAME_LEN, Flow
...
class RapidProFlows(Base):
    ...
    def get_unique_name(cls, org, base_name: str, ignore=None) -> str:
        ...
        while True:
            count_str = f" {count}"
            name = f"{base_name[MAX_NAME_LEN - len(count_str)]}{count_str}" if count > 1 else base_name
    ...
    def clone(self, user, main_flow):
        """
        Returns a clone of this flow
        """
        name = self.get_unique_name(main_flow.org, f"Copy of {main_flow.name}"[: MAX_NAME_LEN].strip())
        copy = RapidProFlows.create(
            self.org,
            user,
            name,
            flow_type=main_flow.flow_type,
            expires_after_minutes=main_flow.expires_after_minutes,
            base_language=main_flow.base_language,
        )
...
class RapidProFlowsRevision(Base):
    ...
    def create(self, flow_name, flow_uuid, user_id):
        ...
        default_flow_revision_definition = {
            "name": flow_name,
            "uuid": flow_uuid,
            "spec_version": "13.1.0",
            "language": "base",
