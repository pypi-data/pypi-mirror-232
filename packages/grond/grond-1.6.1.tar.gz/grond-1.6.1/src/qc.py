...
def plot_color_line(axes, x, y, t, color, tmin, tmax):
    ...
    points = num.array([x, y], dtype=float).T.reshape(-1, 1, 2)
...
def plot_polarizations(
    ...
    if event is None:
        slats = num.array([s.lat for s in stations], dtype=float)
        slons = num.array([s.lon for s in stations], dtype=float)
    ...
    ns, es = num.array(locations, dtype=float).T
