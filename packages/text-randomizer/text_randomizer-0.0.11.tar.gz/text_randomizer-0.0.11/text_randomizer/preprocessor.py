    ...
    def preprocess(self) -> str:
        ...
        for i in range(len(self.__source)):
            if self.__source[i:i + 6] == '%дата+' and self.__source[i + 6] != '%':
                ...
                self.__source = self.__source[:i] + \
                    self.__source[
                        i + 7 + len(add_days_count):
                ] + incremented_date + self.__source[i + 7 + len(add_days_count):]
            ...
            if self.__source[i:i + 6] == '%дата-' and self.__source[i + 6] != '%':
                ...
                self.__source = self.__source[:i] + \
                    self.__source[
                        i + 7 + len(remove_days_count):
                ] + decremented_date + self.__source[i + 7 + len(remove_days_count):]
