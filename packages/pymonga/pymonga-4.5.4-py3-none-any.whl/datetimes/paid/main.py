    ...
    if PAID_BOT.upper() == "YES":
        ...
        if paid_status["is_paid"]:
            ...
            if will_expire < current_date:
                ...
                try:
                    await bot.send_message(m.chat.id, text=EXPIRE_TEXT.formatmention=mention, user_id=user_id, user_name=user_name, paid_duration=paid_duration, paid_on=paid_on, will_expire=will_expire), reply_markup=PAID_BUTTONS) 
                except Exception as e:
                    logger.info(f"⚠️ Error: {e}")
