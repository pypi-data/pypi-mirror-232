...
def print_with_delay(text, delay_per_letter):
    for letter in text:
        ...
        time.sleep(delay_per_letter / 1000)  # Convert milliseconds to seconds
...
"""
clear_terminal()
print_with_delay(tool, 20)  # Adjust the delay per letter (in milliseconds)
print()
...
def calculate_subnet_details(ip_address, subnet_mask):
    ...
    if '/' in subnet_mask:
        ...
        # If no CIDR notation provided, assume it's a full subnet
...
def save_calculation(expression, result):
    ...
    with open("/usr/bin/calculations.txt", "a") as file:
...
def save_calculation8(operation, start, end):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    filename = "/usr/bin/calculations.txt"
...
def view_calculations():
    try:
        with open("/usr/bin/calculations.txt", "r") as file:
