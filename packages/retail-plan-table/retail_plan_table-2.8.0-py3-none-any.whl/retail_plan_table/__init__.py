...
def retail_plan_table(key=None,data=None,shape=None,price_multiple=None,view_changes=None):
    data = data.to_dict(orient='records') 
    return _retail_plan_table(key=key, data=data ,shape=shape,price_multiple=price_multiple,view_changes=view_changes)
...
if not _RELEASE:
    ...
    df = retail_plan_table(data=pd.DataFrame(st.session_state.data),shape= shape,price_multiple=price_multiple,view_changes=st.session_state.check)
    if df!=None :
