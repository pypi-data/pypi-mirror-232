...
def retail_plan_table(key=None,data=None,shape=None,price_multiple=None,):
    data = data.to_dict(orient='records') 
    return _retail_plan_table(key=key, data=data ,shape=shape,price_multiple=price_multiple)
...
if not _RELEASE:
    ...
    df = retail_plan_table(data=pd.DataFrame(data),shape= shape,price_multiple=price_multiple)
    # if df!=None :
