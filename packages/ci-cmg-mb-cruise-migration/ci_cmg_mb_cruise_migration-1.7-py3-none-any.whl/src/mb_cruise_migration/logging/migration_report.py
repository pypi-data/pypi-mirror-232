...
class MigrationReport(object):
    ...
    def migration_problem_surveys_report(self):
        ...
        for survey in self.identify_missing_paged_surveys():
            logger.info(f"\t{survey}")
    ...
    def identify_missing_paged_surveys(self):
        surveys_missing = list(
            set(self.paged_surveys) -
            set([survey.survey_name for survey in self.problem_surveys]) -
            set([survey.survey_name for survey in self.failed_surveys]) -
            set(self.migrated_surveys) -
            set([survey.survey_name for survey in self.skipped_surveys])
        )
    ...
    def add_pages_surveys(self, surveys: [MbSurvey]):
        survey_names = [survey.survey_name for survey in surveys]
        self.paged_surveys.extend(survey_names)
