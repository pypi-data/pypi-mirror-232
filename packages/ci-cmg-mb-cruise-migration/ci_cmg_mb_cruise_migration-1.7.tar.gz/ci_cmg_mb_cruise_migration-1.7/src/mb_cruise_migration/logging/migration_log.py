...
class MigrationLog(object):
    ...
    def log_mb_survey_query(query):
        MigrationLog.log.info(f"{LogConsts.TRACKING} -- Querying MB.SURVEY for next batch of surveys: \n\t\t{query}")
    ...
    def log_paged_surveys(surveys: [MbSurvey]):
        ...
        MigrationLog.log.info(f"{LogConsts.TRACKING} -- Surveys paged: " + ", ".join(survey_names))
    ...
    def log_tracking(cls, message):
        MigrationLog.log.error(f"{LogConsts.TRACKING} -- {message}")
