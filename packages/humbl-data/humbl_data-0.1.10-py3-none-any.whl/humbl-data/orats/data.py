...
import polars as pl
import requests
from humbl-data.constants import IV_ENDPOINTS, ORATS_BASE_URL, ORATS_ENDPOINTS
from humbl-data.credentials import APICredentials
from humbl-data.helpers import MessageHelpers as msg
...
class OratsData:
    ...
    def _get_data(
        ...
        try:
            data = pl.DataFrame(json.loads(r.text)["data"])
    ...
    def _get_data_ivol(
        ...
        if fromdate is not None and todate is not None:
            ...
            assert fromdate < todate, "`fromdate` must be earlier than `todate`"
            data = data.filter(
                (pl.col("tradeDate") >= str(fromdate))
                & (pl.col("tradeDate") <= str(todate))
            )
