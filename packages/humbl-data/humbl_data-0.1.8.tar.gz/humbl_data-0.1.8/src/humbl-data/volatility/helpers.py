...
import polars as pl
from humbl-data.constants import IV_COLUMNS, IV_ENDPOINTS, RV_COLUMNS
from humbl-data.helpers import DataStructureHelpers, MessageHelpers
from humbl-data.plotting import plotly_theme  # noqa: F401
...
def vap_performance_engine(self, **kwargs):
    # Setup --------------------------------------------------------------------
    import numpy as np
    from humbl-data.performance.stats import performance_stats as p_stats
    ...
    trade_data = []
    sell_date = None
    init_investment = kwargs.get("init_investment", 1)
...
def plot_engine(self, plot_method):
...
def _plot_simple_layout(fig, ticker):
    fig.update_layout(
        # template="humbl_dark",
        height=600,
        hovermode="x",
        spikedistance=500,  # Distance from the cursor (in pixels) at which a spike is shown # noqa: E501
        # plot_bgcolor="rgb(25,25,25)",
        title=f"{ticker} | Volatility Adjusted Positioning",
        legend=dict(
            xanchor="center",
            x=0.5,
            y=-0.35,
...
def _plot_simple_layout_trace(fig):
    fig.add_shape(
