...
class KeyStoreConfigDetailsLocal:
    status: str
    last_key_rotation_time: str

    @staticmethod
    def from_dict(data: Dict[str, Any]) -> 'KeyStoreConfigDetailsLocal':
        return KeyStoreConfigDetailsLocal(
            status=data['status'], last_key_rotation_time=data['last_key_rotation_time']
        )
...
class KeyStoreConfig:
    ...
    def from_dict(data: Dict[str, Any]) -> 'KeyStoreConfig':
        ...
        if config_type == 'ENCRYPTION_TYPE_LOCAL':
            return KeyStoreConfig(
                config_type=config_type,
                config_details=KeyStoreConfigDetailsLocal.from_dict(data['value']),
            )
        elif config_type == 'ENCRYPTION_TYPE_KMIP':
            return KeyStoreConfig(
                config_type=config_type,
                config_details=KeyStoreConfigDetailsKmip.from_dict(data['value']),
            )
        else:
...
class KmipKeyStoreConfigPut:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {'type': 'ENCRYPTION_TYPE_KMIP', 'value': dataclass_to_dict_omit_none_fields(self)}


@dataclass
class KeyStoreStatusKmip:
    hostname: str
    key_id: str
    certificate_expiry: str
    key_expiry: str
    ...
    @staticmethod
    def from_dict(data: Dict[str, Any]) -> 'KeyStoreStatusKmip':
        return KeyStoreStatusKmip(
            hostname=data['hostname'],
            key_id=data['key_id'],
            certificate_expiry=data['certificate_expiry'],
            key_expiry=data['key_expiry'],
        )
...
class KeyStoreStatus:
    config_type: str
    config_status: Union[KeyStoreConfigDetailsLocal, KeyStoreStatusKmip]
    ...
    def from_dict(data: Dict[str, Any]) -> 'KeyStoreStatus':
        ...
        if config_type == 'ENCRYPTION_TYPE_LOCAL':
            return KeyStoreStatus(
                config_type=config_type,
                config_status=KeyStoreConfigDetailsLocal.from_dict(data['value']),
            )
        elif config_type == 'ENCRYPTION_TYPE_KMIP':
            return KeyStoreStatus(
                config_type=config_type, config_status=KeyStoreStatusKmip.from_dict(data['value'])
            )
        else:
            raise NotImplementedError(f'Unknown encryption config type: {config_type}')
...
class Encryption:
    ...
    def put_key_store_config(self, config: Optional[KmipKeyStoreConfigPut]) -> None:
        method = 'PUT'
        uri = '/v2/encryption/key-store'
        ...
        if config:
            ...
            self.client.send_request(
                method, uri, body={'type': 'ENCRYPTION_TYPE_LOCAL', 'value': 'null'}
            )
