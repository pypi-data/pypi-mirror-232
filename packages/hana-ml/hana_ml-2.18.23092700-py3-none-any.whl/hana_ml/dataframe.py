...
restricted by the keys of ``to_replace`` and ``value``.
    ...
    def _describe(self, cols=None):
        ...
        final_p_m = self.connection_context.sql("SELECT *, \"count\" - \"valid observations\" AS \"nulls\" FROM ({})".format(final_p.select_statement))
        final_p_m = final_p_m.select(["VARIABLE_NAME"] + sorted(final_p_m.deselect("VARIABLE_NAME").columns))
        unique_cols = []
