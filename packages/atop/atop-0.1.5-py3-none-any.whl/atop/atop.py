...
class Ton_retriever:
    ...
    def __init__(self, _telephone_num, _comprehensive, _tor, _silent, _telegram_pivot, _outputdir=None, telegram_api_hash=None, telegram_api_id=None, telegram_api_telephone=None):
        if _telegram_pivot:
            load_dotenv()
            try:
                ...
                if _telegram_pivot:
                    if self.api_telephone != "" and self.api_hash != "" and self.api_telephone != "":
                        self.telegram_pivot = True
                    elif _telegram_pivot:
                        print(
                            "You have to setup your sockpuppet.. you haven't compiled your env data.. "
                        )
                        exit(0)
                    self.tgchecker = TelegramHelper(self.api_hash,self.api_id,self.api_telephone, self.outputdir)
            except:
                pass
    ...
    def enrich_telegram_asset(self):
        for currentnft in self.nfts["data"]["nftItemsByOwner"]["items"]:
            ...
            if "collection" in currentnft.keys() and currentnft["collection"]:
                if currentnft["collection"]["name"].strip() == "Anonymous Telegram Numbers" and self.telegram_pivot:
                    if self.tgchecker:
                        currentnft["tg-data"] = self.tgchecker.check_telegram_number(currentnft["name"])
                if currentnft["collection"]["name"].strip() == "Telegram Usernames" and self.telegram_pivot:
                    if self.tgchecker:
                        nickname_data = self.tgchecker.check_telegram_nickname(currentnft["name"])
                        if nickname_data:
                            currentnft["tg-data"] = [ currentnft["name"], nickname_data["apidetail"].id,  nickname_data]
                        else:
        ...
        try:
            ...
            for element in obj["nft_items"]:
                ...
                if search_field == self.target:
                    ...
                    if "sale" in element.keys() and element["owner"]["address"] != element["sale"]["owner"]["address"]:
...
def run():
    ...
    try:
        ...
        ton_ret = Ton_retriever(args.target, args.comprehensive, args.tor, args.silent, args.phonepivot, args.picpath)
